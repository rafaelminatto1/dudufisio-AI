...
import { InventoryAlert, InventoryAlertType } from '../../types';
...
type AlertFilter = 'all' | 'critical' | 'high' | 'unread' | 'read';
...
const AlertsTab: React.FC<AlertsTabProps> = ({
  ...
  const getAlertIcon = (type: InventoryAlertType, severity: string) => {
  ...
  const getTypeLabel = (type: InventoryAlertType) => {
    const labels = {
      ...
      <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
        <div className="flex flex-col lg:flex-row gap-4">
          <div className="flex-1 relative">
            ...
            <input
              type="text"
              placeholder="Buscar por item ou mensagem..."
              value={searchQuery}
          ...
          <select
            value={sortBy}
            onChange={(e) => setSortBy(e.target.value as AlertSort)}
            className="px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                  ...
                  <div className="flex items-center gap-2 mt-3">
                    {!alert.isRead && (
                      <button
                        ...
                        className="px-3 py-1 bg-blue-100 text-blue-700 rounded-md text-sm hover:bg-blue-200 flex items-center gap-1"
                      >
                    ...
                    <button
                      ...
                      className="px-3 py-1 bg-slate-100 text-slate-700 rounded-md text-sm hover:bg-slate-200 flex items-center gap-1"
                    >
