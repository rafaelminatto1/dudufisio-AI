...
import { Plus, Edit, Trash2, RotateCcw, Activity, MapPin } from 'lucide-react';
...
interface BodyMapPainProps {
  painPoints: PainPoint[];
  onAddPainPoint: (point: Omit<PainPoint, 'id'>) => void;
  onUpdatePainPoint: (id: string, point: Partial<PainPoint>) => void;
  onDeletePainPoint: (id: string) => void;
}

const BodyMapPain: React.FC<BodyMapPainProps> = ({
  painPoints,
  onAddPainPoint,
  onUpdatePainPoint,
  onDeletePainPoint
  ...
  const handleCanvasClick = (event: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    const existingPoint = painPoints.find(point => 
      point.bodyPart === activeSide &&
  ...
  const handleEditPoint = (point: PainPoint) => {
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-2">
              {currentSidePoints.map((point) => (
                <div key={point.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                  <div className="flex items-center space-x-3">
                    <div
                      className={`w-4 h-4 rounded-full ${
                        point.intensity <= 2 ? 'bg-green-500' :
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => onDeletePainPoint(point.id)}
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Select value={formData.type} onValueChange={(value) => setFormData({ ...formData, type: value as PainPoint['type'] })}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            <div>
              ...
              <Textarea
                ...
                placeholder="Descreva a dor em detalhes"
                rows={3}
              />
            </div>
