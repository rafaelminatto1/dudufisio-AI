...
export interface PersonalizedVideo {
  ...
  savedAt?: string;
  lastModified?: string;
  status: 'generated' | 'saved' | 'archived' | 'deleted';
  isPersonalized: boolean;
  hash: number;
  views: number;
  likes: number;
  downloads: number;
  integratedWithExercises: string[];
  protocols: string[];
  notes?: string;
  metadata: {
    fileSize?: string;
...
export interface VideoSortOptions {
  field: 'exerciseName' | 'generatedAt' | 'views' | 'likes' | 'duration';
  direction: 'asc' | 'desc';
}
...
export interface VideoStats {
  ...
  mostPopular: PersonalizedVideo[];
  recentlyGenerated: PersonalizedVideo[];
}

class PersonalizedVideoCrudService {
  private readonly STORAGE_KEY = 'personalizedVideos';
  private readonly BACKUP_KEY = 'personalizedVideosBackup';
  private readonly STATS_KEY = 'videoStats';

  // CREATE - Criar novo vídeo
  async createVideo(videoData: Omit<PersonalizedVideo, 'id' | 'generatedAt' | 'status' | 'hash' | 'views' | 'likes' | 'downloads' | 'integratedWithExercises' | 'protocols'>): Promise<PersonalizedVideo> {
    try {
      const video: PersonalizedVideo = {
        ...videoData,
        id: this.generateId(),
  ...
  async getVideoById(id: string): Promise<PersonalizedVideo | null> {
    try {
      const videos = await this.getAllVideos();
  ...
  async getAllVideos(): Promise<PersonalizedVideo[]> {
    try {
  ...
  async searchVideos(filters: VideoFilters = {}, sort: VideoSortOptions = { field: 'generatedAt', direction: 'desc' }): Promise<PersonalizedVideo[]> {
    try {
      ...
      if (filters.bodyParts && filters.bodyParts.length > 0) {
        videos = videos.filter(video => 
          filters.bodyParts!.some(part => video.bodyParts.includes(part))
        );
      }

      if (filters.equipment && filters.equipment.length > 0) {
        videos = videos.filter(video => 
          filters.equipment!.some(eq => video.equipment.includes(eq))
        );
      }
      ...
      if (filters.search) {
        ...
        videos = videos.filter(video => 
          video.exerciseName.toLowerCase().includes(searchTerm) ||
  ...
  async updateVideo(id: string, updates: Partial<PersonalizedVideo>): Promise<boolean> {
    try {
      const videos = await this.getAllVideos();
  ...
  async integrateWithExercise(videoId: string, exerciseId: string): Promise<boolean> {
    try {
  ...
  async addToProtocol(videoId: string, protocolId: string): Promise<boolean> {
    try {
      const video = await this.getVideoById(videoId);
      if (!video) return false;
  ...
  async deleteVideo(id: string, permanent: boolean = false): Promise<boolean> {
    try {
      if (permanent) {
        const videos = await this.getAllVideos();
  ...
  async deleteMultipleVideos(ids: string[], permanent: boolean = false): Promise<boolean> {
    try {
      if (permanent) {
        const videos = await this.getAllVideos();
  ...
  async unarchiveVideo(id: string): Promise<boolean> {
    try {
      return await this.updateVideo(id, { status: 'saved' });
    } catch (error) {
  ...
  async bulkUpdate(ids: string[], updates: Partial<PersonalizedVideo>): Promise<boolean> {
    try {
      for (const id of ids) {
  ...
  async duplicateVideo(id: string, newName?: string): Promise<PersonalizedVideo | null> {
    try {
      const originalVideo = await this.getVideoById(id);
      if (!originalVideo) return null;

      const duplicatedVideo: PersonalizedVideo = {
        ...originalVideo,
        id: this.generateId(),
  ...
  async exportVideos(format: 'json' | 'csv' = 'json'): Promise<string> {
    try {
      const videos = await this.getAllVideos();
      ...
      if (format === 'csv') {
        ...
        const csvContent = [
          ...
          ...videos.map(video => 
            headers.map(header => {
              const value = video[header as keyof PersonalizedVideo];
  ...
  async importVideos(data: string, format: 'json' | 'csv' = 'json'): Promise<boolean> {
    try {
      let importedVideos: PersonalizedVideo[];
      ...
      if (format === 'csv') {
        ...
        importedVideos = lines.slice(1).map(line => {
          ...
          return video as PersonalizedVideo;
        });
      } else {
  ...
  private generateHash(exerciseName: string, modality: string, tool: string): number {
  ...
  private async saveVideos(videos: PersonalizedVideo[]): Promise<void> {
