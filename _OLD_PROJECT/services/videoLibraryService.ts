...
import { GeneratedVideo, SPORT_MODALITIES } from './ai/soraService';

export interface VideoLibraryItem extends GeneratedVideo {
  linkedExercises: string[]; // IDs dos exercícios vinculados
  category: string;
  subcategory?: string;
...
export interface SportModalityStats {
  modality: string;
  totalVideos: number;
  totalExercises: number;
  totalViews: number;
  popularVideos: VideoLibraryItem[];
  recentVideos: VideoLibraryItem[];
}
...
export class VideoLibraryService {
  private videos: Map<string, VideoLibraryItem> = new Map();
  private exerciseVideoLinks: Map<string, string[]> = new Map(); // exerciseId -> videoIds[]
  ...
  private initializeMockData() {
    ...
    const mockVideos: Partial<VideoLibraryItem>[] = [
      {
    ...
    mockVideos.forEach(video => {
      const fullVideo: VideoLibraryItem = {
        ...video,
        id: video.id!,
   ...
   */
  async createVideo(video: Omit<VideoLibraryItem, 'id' | 'createdAt' | 'updatedAt' | 'views' | 'likes' | 'downloadCount'>): Promise<VideoLibraryItem> {
    const newVideo: VideoLibraryItem = {
      ...video,
   ...
   */
  async updateVideo(id: string, updates: Partial<VideoLibraryItem>): Promise<VideoLibraryItem | null> {
    ...
    if (updates.linkedExercises) {
      ...
      video.linkedExercises.forEach(exerciseId => {
        ...
        this.exerciseVideoLinks.set(
          exerciseId,
          links.filter(vid => vid !== id)
        );
      });
    ...
    const updatedVideo: VideoLibraryItem = {
      ...video,
      ...updates,
      id: video.id,
      updatedAt: new Date().toISOString(),
   ...
   * Busca vídeo por ID
   */
  async getVideoById(id: string): Promise<VideoLibraryItem | null> {
    const video = this.videos.get(id);
    if (video) {
      // Incrementar views
      video.views++;
  ...
  async listVideos(filters?: {
    ...
    isPublic?: boolean;
    searchTerm?: string;
    sortBy?: 'recent' | 'popular' | 'liked' | 'downloaded';
    limit?: number;
   ...
   */
  async getVideosByModality(modality: string): Promise<VideoLibraryItem[]> {
   ...
   */
  async getVideosByExercise(exerciseId: string): Promise<VideoLibraryItem[]> {
    const videoIds = this.exerciseVideoLinks.get(exerciseId) || [];
    return videoIds.map(id => this.videos.get(id)).filter(Boolean) as VideoLibraryItem[];
  }

  /**
   * Vincula vídeo a exercício
   */
  async linkVideoToExercise(videoId: string, exerciseId: string): Promise<boolean> {
   ...
   */
  async unlinkVideoFromExercise(videoId: string, exerciseId: string): Promise<boolean> {
    const video = this.videos.get(videoId);
    if (!video) return false;
   ...
   */
  async getModalityStats(modality: string): Promise<SportModalityStats> {
   ...
   */
  async getPopularVideos(limit: number = 10): Promise<VideoLibraryItem[]> {
   ...
   */
  async getRecentVideos(limit: number = 10): Promise<VideoLibraryItem[]> {
   ...
   */
  getCategoriesByModality(modality: string): string[] {
