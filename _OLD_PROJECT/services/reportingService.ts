...
export interface ReportTemplate {
  ...
  name: string;
  description: string;
  category: 'financial' | 'clinical' | 'operational' | 'compliance' | 'custom';
...
export interface ReportSection {
  ...
  title: string;
  description?: string;
  type: 'metrics' | 'chart' | 'table' | 'text' | 'image' | 'analysis';
  content: any;
  order: number;
  isVisible: boolean;
}
...
export type VisualizationType =
  | 'line' | 'bar' | 'column' | 'pie' | 'doughnut' | 'area'
  | 'scatter' | 'bubble' | 'heatmap' | 'gauge' | 'funnel'
  | 'waterfall' | 'treemap' | 'sankey' | 'radar';
...
export interface BusinessIntelligenceMetrics {
  ...
  patients: {
    ...
    retention: number;
    satisfaction: number;
    riskSegments: { segment: string; count: number; percentage: number }[];
  };
  operations: {
    capacity: number;
    utilization: number;
    ...
    waitTime: number;
    noShowRate: number;
    staffProductivity: { therapist: string; productivity: number }[];
  };
  clinical: {
    outcomeSuccess: number;
    averageTreatmentTime: number;
    protocolEffectiveness: { protocol: string; successRate: number }[];
    complianceRate: number;
    readmissionRate: number;
  };
  financial: {
    averageRevenuePatientsHour: number;
...
export interface ComplianceReport {
  cfft: {
    registrations: { valid: number; expired: number; pending: number };
    documentation: { compliant: number; issues: number };
    ceeRate: number;
  };
  lgpd: {
    dataProcessing: { lawful: number; violations: number };
    consentManagement: { valid: number; expired: number; withdrawn: number };
    dataRequests: { fulfilled: number; pending: number };
    breaches: { reported: number; resolved: number };
  };
  quality: {
    protocolAdherence: number;
    documentationQuality: number;
    safetyIncidents: { total: number; resolved: number };
    patientComplaints: { total: number; resolved: number };
  };
}
...
export interface PredictiveAnalytics {
  ...
  patientRisk: {
    dropoutPrediction: { patientId: string; risk: number; factors: string[] }[];
    outcomePreiction: { patientId: string; successProbability: number }[];
    noShowPrediction: { appointmentId: string; risk: number }[];
  };
  financialProjection: {
    revenueForcast: { month: string; projected: number; confidence: number }[];
    cashflowPrediction: { week: string; inflow: number; outflow: number }[];
    profitabilityTrends: { period: string; margin: number }[];
  };
  operational: {
    capacityOptimization: { timeSlot: string; recommendedCapacity: number }[];
    staffWorkload: { therapist: string; recommendedHours: number }[];
    resourceUtilization: { resource: string; optimalUsage: number }[];
  };
}
...
class ReportingService {
  ...
  private async generateFinancialReportData(parameters: Record<string, any>): Promise<ReportData> {
    return {
      summary: {
        totalRecords: 1250,
        dateRange: {
  ...
  private async generateClinicalReportData(parameters: Record<string, any>): Promise<ReportData> {
    return {
      summary: {
        totalRecords: 892,
        dateRange: {
  ...
  private async generateOperationalReportData(parameters: Record<string, any>): Promise<ReportData> {
    return {
      summary: {
        totalRecords: 156,
        dateRange: {
  ...
  private async generateComplianceReportData(parameters: Record<string, any>): Promise<ReportData> {
    return {
      summary: {
        totalRecords: 45,
        dateRange: {
  ...
  private async generateDefaultReportData(parameters: Record<string, any>): Promise<ReportData> {
    return {
      summary: {
        totalRecords: 0,
        dateRange: { start: '', end: '' },
...
Gerado pelo sistema MoocaFisio-AI
  ...
  public async getBusinessIntelligenceMetrics(
  ...
  // Report management
  public getGeneratedReports(userId?: string): GeneratedReport[] {
  ...
  public getReport(reportId: string): GeneratedReport | null {
