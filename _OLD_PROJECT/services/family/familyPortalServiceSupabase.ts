...
class FamilyPortalServiceSupabase {
   ...
   */
  async logPortalAccess(familyMemberId: string, action: string): Promise<void> {
    try {
      const { error } = await supabase
   ...
   */
  private mapDatabaseToFamilyMember(data: any): FamilyMember {
    return {
      id: data.id,
