...
// Helper functions
export function getPainIntensityColor(intensity) {
    if (intensity <= 2)
        return '#22c55e'; // Green
    if (intensity <= 5)
...
export function getPainIntensityLabel(intensity) {
    if (intensity === 0)
        return 'Sem dor';
    if (intensity <= 2)
        return 'Dor leve';
...
export function getPainTypeLabel(type) {
    const labels = {
        acute: 'Aguda',
        chronic: 'Crônica',
...
export function getBodySideLabel(side) {
    const labels = {
        left: 'Esquerdo',
        right: 'Direito',
        bilateral: 'Bilateral',
...
export function getBodyRegionsByCategory(category) {
    return Object.values(BODY_REGIONS).filter(region => region.category === category);
}
export function calculatePainTrend(timeline) {
