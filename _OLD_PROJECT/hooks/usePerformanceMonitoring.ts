...
class PerformanceMonitor {
  ...
  private checkThresholds(metric: PerformanceMetrics) {
    ...
    if (metric.renderTime > this.thresholds.renderTime) {
      warnings.push(
        `⚠️ ${metric.componentName}: Render lento (${metric.renderTime.toFixed(2)}ms > ${this.thresholds.renderTime}ms)`
      );
    }

    if (metric.renderCount > this.thresholds.renderCount) {
      warnings.push(
        `⚠️ ${metric.componentName}: Muitos re-renders (${metric.renderCount} > ${this.thresholds.renderCount})`
      );
    }
...
export const usePerformanceMonitoring = (componentName: string) => {
  ...
  const lastMemory = useRef<number | undefined>(undefined);
...
export const useMemoryLeakDetection = (componentName: string) => {
  const initialMemory = useRef<number | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    checkInterval.current = setInterval(() => {
      ...
      if (memoryIncrease > 50 * 1024 * 1024) { // 50MB
        console.error(
          `🚨 Possível memory leak em ${componentName}: +${(memoryIncrease / 1024 / 1024).toFixed(2)}MB`
        );
      }
