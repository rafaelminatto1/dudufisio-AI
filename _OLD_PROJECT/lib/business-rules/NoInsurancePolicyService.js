...
export class NoInsurancePolicyService {
     ...
     */
    static validateNoInsurancePolicy(data, context) {
        const violations = [];
     ...
     */
    static checkForbiddenFields(data) {
        const violations = [];
     ...
     */
    static checkForbiddenKeywords(data) {
        const violations = [];
        if (!data || typeof data !== 'object') {
            return violations;
        }
     ...
     */
    static checkForbiddenPaymentTypes(data) {
        const violations = [];
        if (!data || typeof data !== 'object') {
            return violations;
        }
     ...
     */
    static validateFinancialTransaction(transaction) {
     ...
     */
    static enforceNoInsurancePolicy(data, context) {
     ...
     */
    static containsInsuranceReference(text) {
        if (!text || typeof text !== 'string') {
            return false;
        }
        const lowerText = text.toLowerCase();
        return this.INSURANCE_KEYWORDS.some(keyword => lowerText.includes(keyword.toLowerCase()));
    }
    /**
     * Remove ou substitui referências a convênios em um texto
     */
    static sanitizeInsuranceReferences(text) {
        if (!text || typeof text !== 'string') {
            return text;
        }
        let sanitized = text;
...
NoInsurancePolicyService.INSURANCE_KEYWORDS = [
...
NoInsurancePolicyService.FORBIDDEN_FIELDS = [
 ...
 */
export function NoInsurancePolicy(context = 'unknown') {
