...
export class EmailChannel extends BaseChannel {
    ...
    initializeTransporter() {
        ...
        try {
            ...
            throw new CommunicationError('EMAIL_INIT_FAILED', `Failed to initialize email transporter: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    /**
     * Send email message
     ...
     */
    async prepareEmailMessage(message, content, config) {
        const recipient = message.recipient;
        const emailMessage = {
     ...
     * Convert plain text to basic HTML
     */
    convertTextToHTML(text, message, config) {
        const html = text
            .replace(/\n/g, '<br>')
            .replace(/\t/g, '&nbsp;&nbsp;&nbsp;&nbsp;');
        // Basic HTML template
     ...
     */
    isDangerousAttachment(filename) {
     ...
     * Handle nodemailer-specific errors
     */
    handleNodemailerError(error, messageId) {
        ...
        if (error && typeof error === 'object' && 'code' in error) {
            ...
            switch (nodeError.code) {
                ...
                case 'EMESSAGE':
                    ...
                    break;
                default:
                    communicationError = new CommunicationError('EMAIL_SMTP_ERROR', `SMTP error: ${nodeError.code} - ${nodeError.response || 'Unknown error'}`, nodeError.responseCode ? nodeError.responseCode >= 500 : true);
                    break;
            }
        }
        else {
     ...
     */
    async processBounceWebhook(bounceData) {
        try {
     ...
     */
    async processComplaintWebhook(complaintData) {
        try {
