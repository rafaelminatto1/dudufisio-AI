...
import { ChannelCapability, CommunicationChannel } from '../../../types';
import { BaseChannel } from './BaseChannel';
import { CommunicationError } from '../core/types';
import { ResendService } from '../../integrations/email/ResendService';
/**
 * Resend Email Communication Channel
 */
export class ResendEmailChannel extends BaseChannel {
    ...
    initializeResendService() {
        ...
        try {
            ...
            this.logger.error('Failed to initialize Resend service', error instanceof Error ? error : new Error(String(error)));
            throw error;
        }
    }
    /**
    ...
    async send(message) {
        ...
        try {
            ...
            this.logger.error('Email sending failed', error instanceof Error ? error : new Error(String(error)), {
                messageId: message.id,
                duration
            });
            const commError = error instanceof CommunicationError
                ? error
                : new CommunicationError('EMAIL_SEND_FAILED', error instanceof Error ? error.message : 'Unknown error', true);
            return {
                success: false,
                messageId: message.id,
    ...
    async testConnection() {
        try {
            ...
            this.logger.error('Email service test failed', error instanceof Error ? error : new Error(String(error)));
            return false;
        }
    }
    /**
     * Send welcome email
     */
    async sendWelcomeEmail(recipient, name) {
        if (!recipient.email) {
            return {
                success: false,
        ...
        try {
            ...
            const commError = error instanceof CommunicationError
                ? error
                : new CommunicationError('WELCOME_EMAIL_FAILED', error instanceof Error ? error.message : 'Unknown error', true);
            return {
                success: false,
                messageId: 'welcome-failed',
                channel: CommunicationChannel.Email,
     ...
     */
    async sendAppointmentReminder(recipient, appointmentDate, patientName, therapistName) {
        if (!recipient.email) {
            return {
                success: false,
        ...
        try {
            return await this.resendService.sendAppointmentReminder(recipient.email, appointmentDate, patientName, therapistName);
        }
        catch (error) {
            const commError = error instanceof CommunicationError
                ? error
                : new CommunicationError('REMINDER_EMAIL_FAILED', error instanceof Error ? error.message : 'Unknown error', true);
            return {
                success: false,
                messageId: 'reminder-failed',
                channel: CommunicationChannel.Email,
    ...
    async sendPasswordResetEmail(recipient, resetToken) {
        ...
        try {
            ...
            const commError = error instanceof CommunicationError
                ? error
                : new CommunicationError('PASSWORD_RESET_FAILED', error instanceof Error ? error.message : 'Unknown error', true);
            return {
                success: false,
                messageId: 'reset-failed',
                channel: CommunicationChannel.Email,
    ...
    validateRecipientForChannel(recipient) {
        const errors = [];
        const warnings = [];
        if (!recipient.email) {
