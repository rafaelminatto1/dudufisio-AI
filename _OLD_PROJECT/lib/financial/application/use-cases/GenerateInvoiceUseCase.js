...
export class GenerateInvoiceUseCase {
    constructor(repository, billingService) {
        this.repository = repository;
        this.billingService = billingService;
        this.standardTaxRates = [
    ...
    async generateInvoiceForPackage(packageId, createdBy, dueDate, notes) {
        try {
            // 1. Get package
            ...
            const invoice = await this.billingService.generateInvoiceForPackage(pkg, pkg.getPatientId(), createdBy, dueDate);
            if (notes) {
    ...
    async generateMonthlyInvoices(month, year, createdBy) {
        const results = [];
        try {
            ...
            const transactionsByPatient = new Map();
    ...
    validateTransactionsPatient(transactions, patientId) {
        for (const transaction of transactions) {
            if (transaction.getPatientId() !== patientId) {
                throw new BusinessRuleError('All transactions must belong to the same patient');
            }
        }
    }
    async generateLineItemsFromTransactions(transactions) {
        const lineItems = [];
        for (const transaction of transactions) {
    ...
    generateCustomLineItems(customItems) {
