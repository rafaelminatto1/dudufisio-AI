...
export class BillingService {
    ...
    async calculatePackagePricing(packageType, patientId, customDiscounts = []) {
    ...
    async generateInvoiceForPackage(pkg, patientId, createdBy, dueDate) {
    ...
    async createPaymentPlan(totalAmount, installmentCount, patientId, paymentMethod, createdBy, description, interestRate) {
    ...
    async calculateRefundAmount(packageId, refundDate) {
    ...
    async calculateDiscount(packageType, patientId, customDiscounts) {
        const basePrice = this.getBasePrice(packageType);
    ...
    isDiscountApplicable(rule, packageType, packageCount, isReturningPatient) {
        if (!rule.condition)
            return true;
