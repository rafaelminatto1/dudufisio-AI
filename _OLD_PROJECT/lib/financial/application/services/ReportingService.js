...
export class ReportingService {
    ...
    async generateCashFlowReport(period) {
        const query = `
    ...
    async generateRevenueAnalysis(period) {
    ...
    `;
        ...
        const totalRevenue = packageResults.reduce((sum, row) => sum + parseFloat(row.revenue), 0);
        const byPackageType = packageResults.map((row) => ({
    ...
    `;
        ...
        const byPaymentMethod = methodResults.map((row) => ({
            method: row.method,
            revenue: parseFloat(row.revenue),
    ...
    `;
        ...
        const byMonth = monthlyResults.map((row) => ({
            month: row.month,
            revenue: parseFloat(row.revenue),
        ...
        return {
            ...
            summary: {
                totalRevenue: new Money(totalRevenue),
                totalTransactions: packageResults.reduce((sum, row) => sum + parseInt(row.transaction_count), 0),
                averageMonthlyRevenue: new Money(averageMonthlyRevenue),
                growthRate
            }
        };
    }
    async getTopPatientsByRevenue(limit = 10, period) {
    ...
    `;
        ...
        const totalOverdueAmount = results.reduce((sum, row) => sum + parseFloat(row.net_amount), 0);
        const overdueTransactionCount = results.length;
        const averageDaysOverdue = results.length > 0 ?
            results.reduce((sum, row) => sum + parseInt(row.days_overdue), 0) / results.length : 0;
        // Group by age
        const byAgeGroup = [
    ...
    async generateProfitabilityAnalysis(period) {
        const query = `
      SELECT 
