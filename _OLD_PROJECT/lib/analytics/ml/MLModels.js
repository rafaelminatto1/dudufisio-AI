...
export class MLModels {
    ...
    async predictNoShow(appointmentId, features) {
        try {
    ...
    async predictTreatmentOutcome(patientId, treatmentType) {
        try {
    ...
    async extractNoShowFeatures(appointmentId) {
    ...
    calculateNoShowProbability(features) {
    ...
    async extractOutcomeFeatures(patientId, treatmentType) {
    ...
    categorizeNoShowRisk(probability) {
        if (probability < 0.2)
            return 'low';
        if (probability < 0.4)
            return 'medium';
    ...
    getNoShowRecommendations(probability, factors) {
    ...
    getOutcomeRecommendations(prediction) {
        const recommendations = [];
    ...
    async detectRevenueAnomalies(period) {
        const anomalies = [];
        ...
        const dailyTotals = dailyRevenue.reduce((acc, transaction) => {
    ...
    async detectAppointmentAnomalies(period) {
        const anomalies = [];
        ...
        if (!dailyAppointments)
            return anomalies;
        const dailyCounts = dailyAppointments.reduce((acc, appointment) => {
    ...
    async detectNoShowAnomalies(period) {
        const anomalies = [];
        ...
        if (!appointments)
            return anomalies;
        const dailyStats = appointments.reduce((acc, appointment) => {
            const date = appointment.appointment_date.split('T')[0];
            if (!acc[date])
    ...
    async detectCancellationAnomalies(period) {
        const anomalies = [];
