...
export class DataExtractor {
    ...
    async extractPatients(lastExtraction) {
        try {
    ...
    async extractAppointments(lastExtraction) {
        try {
            let query = this.supabase
                .from('appointments')
                .select(`
    ...
    async extractFinancialTransactions(lastExtraction) {
        try {
            let query = this.supabase
    ...
    async extractTreatmentSessions(lastExtraction) {
        try {
            let query = this.supabase
    ...
    async extractTherapists(lastExtraction) {
        try {
            let query = this.supabase
                .from('therapists')
                .select(`
    ...
    async extractPatientEngagement(lastExtraction) {
        try {
            let query = this.supabase
    ...
    async extractAllTables(config, lastExtractionMap = {}) {
        const results = [];
    ...
    async validateExtractionIntegrity(tableName, expectedCount) {
        try {
