import { DocumentType } from '../../../../types/medical-records';
export class FHIRTransformer {
     ...
     */
    static generateConclusion(content, type) {
    ...
    static transformClinicalDataToFHIR(data) {
        return {
            ...
            observations: data.assessments.map(assessment => this.toFHIRObservation(assessment)),
            diagnosticReports: data.documents.map(document => this.toFHIRDiagnosticReport(document))
        };
    }
}
