...
export class CFMComplianceValidator {
    ...
    async validateClinicalDocument(document) {
        ...
        try {
            ...
            violations.push(new ComplianceViolation('CFM_VALIDATION_ERROR', `Validation error: ${error instanceof Error ? error.message : String(error)}`, 'critical'));
     ...
     */
    async validateTherapistRegistration(therapistId) {
        const violations = [];
        try {
            const therapist = await this.therapistRegistry.getTherapist(therapistId);
            if (!therapist) {
                violations.push(new ComplianceViolation('CFM_001', 'Therapist not found in registry', 'critical', 'createdBy'));
                return violations;
            }
            // Verificar se o terapeuta está ativo
            if (!therapist.isActive) {
                violations.push(new ComplianceViolation('CFM_002', 'Therapist registration is not active', 'critical', 'createdBy'));
            }
            // Verificar se o terapeuta tem especialidade adequada
            if (!therapist.specialties || therapist.specialties.length === 0) {
                violations.push(new ComplianceViolation('CFM_003', 'Therapist must have at least one specialty registered', 'high', 'createdBy'));
            }
            // Verificar validade do registro
            if (therapist.registrationExpiry && therapist.registrationExpiry < new Date()) {
                violations.push(new ComplianceViolation('CFM_004', 'Therapist registration has expired', 'critical', 'createdBy'));
            }
            return violations;
        }
        catch (error) {
            violations.push(new ComplianceViolation('CFM_005', `Error validating therapist registration: ${error instanceof Error ? error.message : String(error)}`, 'critical', 'createdBy'));
            return violations;
        }
    }
    /**
     ...
     */
    async validateDigitalSignature(document) {
        const violations = [];
        // Verificar se documento requer assinatura
        const requiresSignature = document.requiresSignature?.() ?? true;
        if (requiresSignature) {
            const isSigned = document.isSigned?.() ?? false;
            if (!isSigned) {
                violations.push(new ComplianceViolation('CFM_006', 'Clinical document requires digital signature', 'critical', 'signature'));
                return violations;
            }
            // Verificar integridade da assinatura
            const validateIntegrity = document.validateIntegrity?.() ?? true;
            if (!validateIntegrity) {
                violations.push(new ComplianceViolation('CFM_007', 'Document signature integrity validation failed', 'critical', 'signature'));
            }
            // Verificar se assinatura é recente (não mais de 30 dias)
            const signature = document.getSignature?.();
            if (signature) {
                const daysSinceSignature = Math.floor((Date.now() - signature.timestamp.time.getTime()) / (1000 * 60 * 60 * 24));
                if (daysSinceSignature > 30) {
                    violations.push(new ComplianceViolation('CFM_008', 'Document signature is older than 30 days', 'medium', 'signature'));
                }
            }
        }
        return violations;
     ...
     */
    async validateMinimumContent(document) {
        const violations = [];
        const content = document.content;
        const data = content.data;
        // Verificar identificação do paciente
        if (!data.patientId) {
            violations.push(new ComplianceViolation('CFM_009', 'Patient identification is required', 'critical', 'content.patientId'));
        }
        // Verificar data da consulta
        if (!data.consultationDate) {
            violations.push(new ComplianceViolation('CFM_010', 'Consultation date is required', 'critical', 'content.consultationDate'));
        }
        // Verificar anamnese
        if (!data.anamnesis || data.anamnesis.trim().length < 50) {
            violations.push(new ComplianceViolation('CFM_011', 'Anamnesis must be at least 50 characters', 'high', 'content.anamnesis'));
        }
        // Verificar exame físico
        if (!data.physicalExam || data.physicalExam.trim().length < 30) {
            violations.push(new ComplianceViolation('CFM_012', 'Physical exam description must be at least 30 characters', 'high', 'content.physicalExam'));
        }
        // Verificar diagnóstico
        if (!data.diagnosis || data.diagnosis.trim().length < 10) {
            violations.push(new ComplianceViolation('CFM_013', 'Diagnosis is required and must be at least 10 characters', 'critical', 'content.diagnosis'));
        }
        // Verificar conduta
        if (!data.conduct || data.conduct.trim().length < 20) {
            violations.push(new ComplianceViolation('CFM_014', 'Conduct/treatment plan must be at least 20 characters', 'high', 'content.conduct'));
        }
        return violations;
    }
    /**
     ...
     */
    async validateSigningTiming(document) {
        const violations = [];
        if (document.isSigned()) {
            ...
            if (signature) {
                ...
                if (signatureTime < documentTime) {
                    violations.push(new ComplianceViolation('CFM_015', 'Signature timestamp cannot be before document creation', 'critical', 'signature.timestamp'));
                }
                // Verificar se assinatura não é muito posterior à criação (máximo 7 dias)
                const daysDifference = Math.floor((signatureTime.getTime() - documentTime.getTime()) / (1000 * 60 * 60 * 24));
                if (daysDifference > 7) {
                    violations.push(new ComplianceViolation('CFM_016', 'Document must be signed within 7 days of creation', 'high', 'signature.timestamp'));
                }
            }
        }
        return violations;
    ...
    async validateDataRetention(document) {
        ...
        if (!metadata.retentionPolicy) {
            violations.push(new ComplianceViolation('CFM_017', 'Document must have retention policy defined', 'medium', 'metadata.retentionPolicy'));
        }
        ...
        if (this.containsSensitiveData(data)) {
            if (!metadata.encryptionLevel || metadata.encryptionLevel !== 'high') {
                violations.push(new ComplianceViolation('CFM_018', 'Sensitive data must be encrypted with high-level encryption', 'high', 'metadata.encryptionLevel'));
            }
        }
        return violations;
    }
     ...
     */
    async generateComplianceReport(documentId) {
        ...
        return new ComplianceReport(documentId, document.type, validationResult, this.calculateComplianceScore(validationResult.violations), this.generateRecommendations(validationResult.violations), new Date());
    }
    /**
