 ...
 */
import { DocumentType, DomainError } from '../../../types/medical-records';
import { ValidationResult, ValidationViolation as ComplianceViolation } from '../clinical/assessment/ClinicalTemplateEngine';
export class COFFITOValidator {
    constructor(therapistRegistry, documentRepository) {
     ...
     */
    async validateClinicalDocument(document) {
        const violations = [];
        try {
            ...
            violations.push(new ComplianceViolation('COFFITO_VALIDATION_ERROR', `Validation error: ${error instanceof Error ? error.message : String(error)}`, 'critical'));
     ...
     */
    async validateCOFFITORegistration(therapistId) {
        const violations = [];
        try {
            const therapist = await this.therapistRegistry.getTherapist(therapistId);
            if (!therapist) {
                violations.push(new ComplianceViolation('COFFITO_001', 'Therapist not found in COFFITO registry', 'critical', 'createdBy'));
                return violations;
            }
            // Verificar se tem registro COFFITO
            if (!therapist.coffitoRegistration) {
                violations.push(new ComplianceViolation('COFFITO_002', 'COFFITO registration is required for physiotherapy practice', 'critical', 'createdBy'));
            }
            // Verificar se registro está ativo
            if (!therapist.isActive) {
                violations.push(new ComplianceViolation('COFFITO_003', 'COFFITO registration is not active', 'critical', 'createdBy'));
            }
            // Verificar validade do registro
            if (therapist.registrationExpiry && therapist.registrationExpiry < new Date()) {
                violations.push(new ComplianceViolation('COFFITO_004', 'COFFITO registration has expired', 'critical', 'createdBy'));
            }
            // Verificar se tem especialidade registrada
            if (!therapist.specialties || therapist.specialties.length === 0) {
                violations.push(new ComplianceViolation('COFFITO_005', 'Therapist must have at least one COFFITO specialty registered', 'high', 'createdBy'));
            }
            return violations;
        }
        catch (error) {
            violations.push(new ComplianceViolation('COFFITO_006', `Error validating COFFITO registration: ${error instanceof Error ? error.message : String(error)}`, 'critical', 'createdBy'));
            return violations;
        }
    }
    /**
     ...
     */
    async validateSpecialty(document) {
        const violations = [];
        ...
        try {
            ...
            if (therapist && therapist.specialties) {
                const hasSpecialty = therapist.specialties.some(specialty => specialty.toLowerCase() === documentSpecialty.toLowerCase());
                if (!hasSpecialty) {
                    violations.push(new ComplianceViolation('COFFITO_007', `Therapist does not have registered specialty: ${documentSpecialty}`, 'high', 'metadata.specialty'));
                }
            }
            ...
            if (!validSpecialties.includes(documentSpecialty)) {
                violations.push(new ComplianceViolation('COFFITO_008', `Invalid specialty for physiotherapy: ${documentSpecialty}`, 'medium', 'metadata.specialty'));
            }
            return violations;
        }
        catch (error) {
            violations.push(new ComplianceViolation('COFFITO_009', `Error validating specialty: ${error instanceof Error ? error.message : String(error)}`, 'medium', 'metadata.specialty'));
            return violations;
        }
    }
    /**
     ...
     */
    async validatePhysiotherapyContent(document) {
        const violations = [];
        const content = document.content;
        const data = content.data;
        // Verificar avaliação postural
        if (!data.posturalAssessment) {
            violations.push(new ComplianceViolation('COFFITO_010', 'Postural assessment is required for physiotherapy documents', 'high', 'content.posturalAssessment'));
        }
        // Verificar avaliação de amplitude de movimento
        if (!data.rangeOfMotion) {
            violations.push(new ComplianceViolation('COFFITO_011', 'Range of motion assessment is required', 'high', 'content.rangeOfMotion'));
        }
        // Verificar avaliação de força muscular
        if (!data.muscleStrength) {
            violations.push(new ComplianceViolation('COFFITO_012', 'Muscle strength assessment is required', 'high', 'content.muscleStrength'));
        }
        // Verificar testes especiais
        if (!data.specialTests || data.specialTests.length === 0) {
            violations.push(new ComplianceViolation('COFFITO_013', 'At least one special test must be performed', 'medium', 'content.specialTests'));
        }
        // Verificar avaliação funcional
        if (!data.functionalAssessment) {
            violations.push(new ComplianceViolation('COFFITO_014', 'Functional assessment is required', 'high', 'content.functionalAssessment'));
        }
        return violations;
    }
    /**
     ...
     */
    async validateFunctionalAssessment(document) {
        const violations = [];
        const content = document.content;
        const data = content.data;
        if (data.functionalAssessment) {
            ...
            for (const test of requiredTests) {
                if (!assessment[test]) {
                    violations.push(new ComplianceViolation('COFFITO_015', `Functional test '${test}' is required`, 'medium', `content.functionalAssessment.${test}`));
                }
            }
            // Verificar se tem escalas funcionais
            if (!assessment.functionalScales || assessment.functionalScales.length === 0) {
                violations.push(new ComplianceViolation('COFFITO_016', 'At least one functional scale must be used', 'medium', 'content.functionalAssessment.functionalScales'));
            }
            // Verificar limitações funcionais
            if (!assessment.functionalLimitations) {
                violations.push(new ComplianceViolation('COFFITO_017', 'Functional limitations must be documented', 'medium', 'content.functionalAssessment.functionalLimitations'));
            }
        }
        return violations;
    }
     ...
     */
    async validateTreatmentPlan(document) {
        const violations = [];
        const content = document.content;
        const data = content.data;
        if (data.treatmentPlan) {
            ...
            if (!plan.goals || plan.goals.length === 0) {
                violations.push(new ComplianceViolation('COFFITO_018', 'Treatment plan must have at least one goal', 'high', 'content.treatmentPlan.goals'));
            }
            // Verificar intervenções
            if (!plan.interventions || plan.interventions.length === 0) {
                violations.push(new ComplianceViolation('COFFITO_019', 'Treatment plan must specify interventions', 'high', 'content.treatmentPlan.interventions'));
            }
            // Verificar frequência
            if (!plan.frequency) {
                violations.push(new ComplianceViolation('COFFITO_020', 'Treatment frequency must be specified', 'medium', 'content.treatmentPlan.frequency'));
            }
            // Verificar duração
            if (!plan.duration) {
                violations.push(new ComplianceViolation('COFFITO_021', 'Treatment duration must be specified', 'medium', 'content.treatmentPlan.duration'));
            }
            // Verificar contraindicações
            if (!plan.contraindications) {
                violations.push(new ComplianceViolation('COFFITO_022', 'Contraindications must be documented', 'medium', 'content.treatmentPlan.contraindications'));
            }
        }
        return violations;
    }
     ...
     */
    async validateClinicalEvolution(document) {
        const violations = [];
        if (document.type === DocumentType.EVOLUTION) {
            ...
            if (!data.subjectiveAssessment || data.subjectiveAssessment.trim().length < 20) {
                violations.push(new ComplianceViolation('COFFITO_023', 'Subjective assessment must be at least 20 characters', 'medium', 'content.subjectiveAssessment'));
            }
            // Verificar avaliação objetiva
            if (!data.objectiveFindings || data.objectiveFindings.trim().length < 20) {
                violations.push(new ComplianceViolation('COFFITO_024', 'Objective findings must be at least 20 characters', 'medium', 'content.objectiveFindings'));
            }
            // Verificar técnicas aplicadas
            if (!data.techniquesApplied || data.techniquesApplied.length === 0) {
                violations.push(new ComplianceViolation('COFFITO_025', 'Applied techniques must be documented', 'high', 'content.techniquesApplied'));
            }
            // Verificar resposta do paciente
            if (!data.patientResponse || data.patientResponse.trim().length < 10) {
                violations.push(new ComplianceViolation('COFFITO_026', 'Patient response must be documented', 'medium', 'content.patientResponse'));
            }
            // Verificar plano para próxima sessão
            if (!data.nextSessionPlan || data.nextSessionPlan.trim().length < 15) {
                violations.push(new ComplianceViolation('COFFITO_027', 'Next session plan must be specified', 'medium', 'content.nextSessionPlan'));
            }
        }
        return violations;
    }
     ...
     */
    async validateExercisePrescription(document) {
        const violations = [];
        const content = document.content;
        const data = content.data;
            ...
            if (exercises && exercises.length > 0) {
                for (let i = 0; i < exercises.length; i++) {
                    ...
                    if (!exercise.name || exercise.name.trim().length === 0) {
                        violations.push(new ComplianceViolation('COFFITO_028', `Exercise ${i + 1} must have a name`, 'medium', `content.exercises[${i}].name`));
                    }
                    // Verificar descrição
                    if (!exercise.description || exercise.description.trim().length < 10) {
                        violations.push(new ComplianceViolation('COFFITO_029', `Exercise ${i + 1} must have detailed description`, 'medium', `content.exercises[${i}].description`));
                    }
                    // Verificar repetições
                    if (!exercise.repetitions || exercise.repetitions <= 0) {
                        violations.push(new ComplianceViolation('COFFITO_030', `Exercise ${i + 1} must specify repetitions`, 'medium', `content.exercises[${i}].repetitions`));
                    }
                    // Verificar séries
                    if (!exercise.sets || exercise.sets <= 0) {
                        violations.push(new ComplianceViolation('COFFITO_031', `Exercise ${i + 1} must specify sets`, 'medium', `content.exercises[${i}].sets`));
                    }
                    // Verificar instruções
                    if (!exercise.instructions || exercise.instructions.trim().length < 15) {
                        violations.push(new ComplianceViolation('COFFITO_032', `Exercise ${i + 1} must have detailed instructions`, 'medium', `content.exercises[${i}].instructions`));
                    }
                }
            }
        }
     ...
     */
    async generateCOFFITOComplianceReport(documentId) {
        ...
        return new COFFITOComplianceReport(documentId, document.type, validationResult, this.calculateComplianceScore(validationResult.violations), this.generateRecommendations(validationResult.violations), new Date());
    }
    /**
...
export class COFFITOComplianceReport {
    constructor(documentId, documentType, validationResult, complianceScore, recommendations, generatedAt) {
    ...
    get physiotherapySpecificViolations() {
        return this.validationResult.violations.filter(v => v.code.startsWith('COFFITO_') &&
