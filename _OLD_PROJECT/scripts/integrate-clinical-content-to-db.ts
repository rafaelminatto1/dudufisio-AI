...
import {
  ...
  CLINICAL_LIBRARY,
  EDUCATIONAL_CONTENT
} from './populate-clinical-content';
...
export function getContentBySpecialty(specialty: string) {
  return {
    protocols: CLINICAL_PROTOCOLS.filter(p => p.specialty === specialty),
    exercises: EXERCISES_LIBRARY.filter(e => e.specialty.includes(specialty as any)),
...
export function searchByTags(tags: string[]) {
  const results = {
    protocols: [] as typeof CLINICAL_PROTOCOLS,
    exercises: [] as typeof EXERCISES_LIBRARY,
    assessments: [] as typeof SPECIALIZED_ASSESSMENTS,
    materials: [] as typeof CLINICAL_MATERIALS,
    library: [] as typeof CLINICAL_LIBRARY,
    educational: [] as typeof EDUCATIONAL_CONTENT
  };
  
  results.protocols = CLINICAL_PROTOCOLS.filter(p => 
    tags.some(tag => p.tags.includes(tag))
  );
  
  results.exercises = EXERCISES_LIBRARY.filter(e => 
    tags.some(tag => e.tags.includes(tag))
  );
  
  results.assessments = SPECIALIZED_ASSESSMENTS.filter(a => 
    tags.some(tag => a.tags.includes(tag))
  );
  
  results.materials = CLINICAL_MATERIALS.filter(m => 
    tags.some(tag => m.tags.includes(tag))
  );
  
  results.library = CLINICAL_LIBRARY.filter(l => 
    tags.some(tag => l.keywords.includes(tag))
  );
  
  results.educational = EDUCATIONAL_CONTENT.filter(e => 
    tags.some(tag => e.tags.includes(tag))
  );
...
export function getStatistics() {
  return {
    ...
    totalImages: 
      CLINICAL_PROTOCOLS.reduce((sum, p) => sum + p.images.length, 0) +
