...
import {
  ...
  CLINICAL_LIBRARY,
  EDUCATIONAL_CONTENT
} from './generate-assessments-materials';
...
async function generateProtocolImages(protocol: ClinicalProtocol) {
  ...
  imagePromises.push(
    imagenService.generateImageObject('protocol', {
      name: protocol.title,
  ...
  for (const phase of phasesToImage) {
    imagePromises.push(
      imagenService.generateImageObject('educational', {
...
async function generateExerciseImages(exercise: Exercise) {
  ...
  for (const phase of phases) {
    imagePromises.push(
      imagenService.generateImageObject('exercise', {
        name: exercise.name,
...
async function generateAssessmentImages(assessment: SpecializedAssessment) {
...
async function exportToJSON() {
  ...
  await fs.writeFile(
    path.join(outputDir, 'clinical-content-complete.json'),
