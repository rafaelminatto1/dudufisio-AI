...
import { auditService, logExerciseCreate, logExerciseUpdate, logExerciseDelete, logExerciseDuplicate } from '../services/auditService';
const ExerciseContext = createContext(undefined);
// Provider Component
export const ExerciseProvider = ({ children }) => {
    ...
    const getExercise = useCallback(async (id) => {
        setIsLoading(true);
        try {
    ...
    // Busca e Filtros
    const searchExercises = useCallback(async (filters) => {
        setIsLoading(true);
        try {
            ...
            if (filters.equipment && filters.equipment.length > 0) {
                results = results.filter(ex => ex.equipment.some(eq => filters.equipment?.includes(eq)));
            }
            if (filters.targetMuscles && filters.targetMuscles.length > 0) {
                results = results.filter(ex => ex.targetMuscles.some(muscle => filters.targetMuscles?.includes(muscle)));
            }
    ...
    const updateProtocol = useCallback(async (id, protocolData) => {
        setIsLoading(true);
        try {
            await new Promise(resolve => setTimeout(resolve, 400));
    ...
    // Atribuições
    const assignExerciseToPatient = useCallback(async (patientId, exerciseId, data) => {
        setIsLoading(true);
        try {
            await new Promise(resolve => setTimeout(resolve, 400));
    ...
    const getPatientAssignments = useCallback(async (patientId) => {
        setIsLoading(true);
        try {
            await new Promise(resolve => setTimeout(resolve, 200));
    ...
    // Utilitários
    const duplicateExercise = useCallback(async (id) => {
    ...
    const exportExercises = useCallback(async (ids) => {
        try {
    ...
    const importExercises = useCallback(async (data) => {
        setIsLoading(true);
        try {
            const importedExercises = data.exercises || [];
            const newExercises = importedExercises.map((ex) => ({
                ...ex,
                id: uuidv4(),
                createdAt: new Date(),
                updatedAt: new Date()
