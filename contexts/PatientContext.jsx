 ...
 */
function convertFormDataToPatient(formData, existingPatient) {
...
export const PatientProvider = ({ children }) => {
    const [patients, setPatients] = useState(() => loadPatientsFromStorage());
    ...
    const updatePatient = useCallback(async (id, formData) => {
        ...
        try {
            ...
            const updatedPatient = convertFormDataToPatient({ ...existingPatient, ...formData }, existingPatient);
    ...
    const searchPatients = useCallback((query) => {
        ...
        return patients.filter(patient => patient.name.toLowerCase().includes(lowerQuery) ||
     ...
     */
    const filterPatients = useCallback((filters) => {
     ...
     */
    const validateUniqueCPF = useCallback((cpf, excludeId) => {
        return !patients.some(p => p.cpf === cpf && p.id !== excludeId);
    }, [patients]);
    /**
     * Valida se email é único
     */
    const validateUniqueEmail = useCallback((email, excludeId) => {
