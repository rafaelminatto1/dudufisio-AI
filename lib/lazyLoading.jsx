...
export function createLazyComponent(importFn, fallback) {
    ...
    const WrappedComponent = (props) => (<ErrorBoundary onError={(error, errorInfo) => {
 ...
 */
export const CompleteDashboard = createLazyComponent(() => import('../pages/CompleteDashboard'));
export const PatientPortalDashboard = createLazyComponent(() => import('../pages/PatientPortalDashboard'));
export const PartnerPortalDashboard = createLazyComponent(() => import('../pages/PartnerPortalDashboard'));
export const WhatsAppManagementPage = createLazyComponent(() => import('../pages/WhatsAppManagementPage'));
/**
...
export const LazyComponents = {
    ...
    MedicalRecordsSystem: createLazyComponent(() => import('../components/medical-records/MedicalRecordsSystem').then(m => ({ default: m.default || m }))),
...
export function usePreload() {
    ...
    const preload = React.useCallback((importFn, key) => {
...
export class SuspenseWrapper extends React.Component {
    ...
    render() {
        if (this.state.hasError) {
          ...
          <button onClick={() => this.setState({ hasError: false, error: undefined })} className="px-4 py-2 bg-sky-600 text-white rounded-lg hover:bg-sky-700 transition-colors">
            Tentar novamente
          </button>
        </div>);
        }
...
export const preloadUserRoleComponents = (userRole) => {
    setTimeout(() => {
        switch (userRole) {
            case 'Admin':
            case 'Therapist':
