...
async (error) => {
    ...
    if (config.retries < MAX_RETRIES && shouldRetry) {
        ...
        logger.warn(`[API Retry] Tentativa ${config.retries}/${MAX_RETRIES}. Retentando em ${delay}ms.`, { context: 'api.retry', data: { retries: config.retries, delay, url: config.url, method: config.method } });
...
export const checkApiHealth = async () => {
    try {
        ...
        logger.error('Falha no health check da API.', { context: 'api.checkApiHealth', data: { message: error.message } });
 ...
 */
export const getMentoriaSessions = async () => {
    try {
        ...
        logger.error('Erro ao buscar sessões de mentoria.', { context: 'api.getMentoriaSessions', data: { message: error.message } });
