...
import { CommunicationError, TemplateNotFoundError } from '../core/types';
/**
...
export class TemplateManager {
    constructor(config, repository, configProvider, logger, metrics) {
        ...
        this.templateEngine = new TemplateEngine(config.engine, repository, configProvider, logger, metrics);
        this.loadBuiltInTemplates();
    }
    /**
     * Render a template for a specific context
     */
    async renderTemplate(criteria, context, options = {}) {
        const startTime = Date.now();
        try {
            // Find appropriate template
            const template = await this.findTemplate(criteria);
            if (!template) {
                throw new TemplateNotFoundError(`No template found for type: ${criteria.type}, channel: ${criteria.channel}`);
            }
            // Build enhanced context
            const enhancedContext = await this.buildEnhancedContext(context, template, criteria, options);
            ...
            // Render template
            const result = await this.templateEngine.renderTemplate(template, enhancedContext, compilationContext);
     ...
     */
    async renderAppointmentTemplate(type, appointment, patient, channel, options = {}) {
        const templateType = `appointment_${type}`;
        const context = {
            patient: {
                id: patient.id,
                name: patient.name,
     ...
     */
    selectBestTemplate(templates, criteria) {
    ...
    scoreTemplate(template, criteria) {
        let score = 0;
        // Exact type match
        if (template.type === criteria.type)
            score += 100;
        // Locale match
        if (template.locale === criteria.locale)
            score += 50;
        // Category match
        if (template.category === criteria.context)
            score += 30;
        // Channel-specific content exists
        const channelContent = this.getChannelContent(template, criteria.channel);
        if (channelContent && channelContent !== template.body)
            score += 20;
        // Active template
     ...
     */
    getChannelContent(template, channel) {
        switch (channel) {
     ...
     */
    async buildEnhancedContext(baseContext, template, criteria, options) {
     ...
     */
    async renderFallbackTemplate(criteria, context, options) {
...
Para mais informações, responda esta mensagem.`,
        ...
        const enhancedContext = await this.buildEnhancedContext(context, fallbackTemplate, criteria, options);
        const result = await this.templateEngine.renderTemplate(fallbackTemplate, enhancedContext, compilationContext);
     ...
     */
    buildTemplateCacheKey(criteria) {
        return [
            criteria.type,
            criteria.channel,
     ...
     */
    recordTemplateMetrics(template, channel, duration, success) {
    ...
    async registerTemplate(template) {
        try {
            ...
            throw new CommunicationError('TEMPLATE_REGISTRATION_FAILED', `Failed to register template: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    /**
 ...
 */
export const defaultTemplateManagerConfig = {
    engine: {
        cacheEnabled: true,
        cacheSize: 1000,
