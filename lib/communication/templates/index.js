...
export class TemplateValidator {
    ...
    static validateContext(template, context) {
        ...
        const missingVariables = templateVariables.filter(variable => !contextKeys.includes(variable));
        const extraVariables = contextKeys.filter(key => !templateVariables.includes(key));
        return {
     ...
     */
    static extractContextKeys(context, prefix = '') {
        const keys = [];
...
export class TemplatePreview {
     ...
     */
    static generatePreviewContext(templateType) {
        const baseContext = {
            clinic: {
     ...
     */
    static async previewTemplate(template, templateEngine, // Would be properly typed in real implementation
    context) {
        ...
        for (const channel of channels) {
            try {
                const result = await templateEngine.renderTemplate(template, previewContext, {
                    channel,
...
export class TemplateMigration {
     ...
     */
    static migrateTemplate(oldTemplate, targetVersion) {
     ...
     */
    static needsMigration(template, currentVersion) {
...
export class TemplatePerformance {
     ...
     */
    static analyzeComplexity(template) {
        let score = 0;
        const factors = [];
 ...
 */
export function createTemplateContext(type, data) {
    const baseContext = {
        clinic: {
            name: 'DuduFisio - Clínica de Fisioterapia',
            phone: '(11) 1234-5678',
