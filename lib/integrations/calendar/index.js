...
export class CalendarIntegrationManager {
     ...
     */
    async sendCalendarInvite(appointment) {
        const startTime = Date.now();
        try {
            const jobId = await this.queue.addCalendarInvite(appointment);
            this.monitor.recordOperation('system', 'send_invite_queued', true, Date.now() - startTime);
            return jobId;
        }
        catch (error) {
            this.monitor.recordOperation('system', 'send_invite_queued', false, Date.now() - startTime, error instanceof Error ? error.message : 'Unknown error');
            throw error;
        }
    }
    /**
    ...
    async updateCalendarInvite(appointmentId, changes) {
        ...
        try {
            ...
            this.monitor.recordOperation('system', 'update_invite_queued', true, Date.now() - startTime);
            return jobId;
        }
        catch (error) {
            this.monitor.recordOperation('system', 'update_invite_queued', false, Date.now() - startTime, error instanceof Error ? error.message : 'Unknown error');
            throw error;
        }
    }
    /**
    ...
    async cancelCalendarInvite(appointmentId) {
        ...
        try {
            ...
            this.monitor.recordOperation('system', 'cancel_invite_queued', true, Date.now() - startTime);
            return jobId;
        }
        catch (error) {
            this.monitor.recordOperation('system', 'cancel_invite_queued', false, Date.now() - startTime, error instanceof Error ? error.message : 'Unknown error');
            throw error;
        }
    }
    /**
    ...
    async testProvider(provider, config) {
        ...
        try {
            ...
            this.monitor.recordOperation(provider, 'test_connection', result.success, Date.now() - startTime, result.error);
            return result;
        }
        catch (error) {
            this.monitor.recordOperation(provider, 'test_connection', false, Date.now() - startTime, error instanceof Error ? error.message : 'Unknown error');
            throw error;
        }
    }
    /**
 ...
 */
export function appointmentToCalendarEvent(appointment, patient, preferences) {
    return {
