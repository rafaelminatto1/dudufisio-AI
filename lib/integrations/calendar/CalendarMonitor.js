export class CalendarMonitor {
    ...
    recordOperation(provider, operation, success, responseTime, error) {
    ...
    checkAlertConditions(provider, success, error) {
        const providerMetrics = this.metrics.providerMetrics.get(provider);
        if (!providerMetrics)
            return;
    ...
    isConsecutiveFailure(provider, threshold) {
    ...
    async performHealthCheck(provider, testConfig) {
    ...
    logToExternalMonitoring(provider, operation, success, responseTime, error) {
