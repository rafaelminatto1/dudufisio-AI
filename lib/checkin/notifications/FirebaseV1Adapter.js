...
export class FirebaseV1Adapter {
    ...
    async sendMessage(message) {
        try {
            ...
            const response = await fetch(`https://fcm.googleapis.com/v1/projects/${this.config.project_id}/messages:send`, {
                method: 'POST',
                headers: {
     ...
     */
    async sendToMultipleTokens(tokens, messageTemplate) {
        const promises = tokens.map(token => this.sendMessage({ ...messageTemplate, token }));
 ...
 */
export function convertToFirebaseV1Message(token, title, body, data, options) {
