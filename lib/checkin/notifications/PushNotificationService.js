import { createAPNS } from '../adapters/AppleAPNSAdapter';
export class PushNotificationService {
    constructor(fcmConfig, apnsConfig) {
        this.fcmConfig = null;
        this.apnsConfig = null;
        this.apnsAdapter = null;
    ...
    async registerDevice(patientId, token, platform) {
        const deviceToken = {
            token,
            platform,
            userId: patientId,
    ...
    async unregisterDevice(patientId, token) {
        const userTokens = this.deviceTokens.get(patientId);
        if (userTokens) {
    ...
    async sendAppointmentReminder(patientId, appointment, hoursBeforeAppointment = 24) {
        ...
        await this.scheduleNotification(patientId, notification, scheduledTime, `reminder-${appointment.id}-${hoursBeforeAppointment}h`);
    ...
    async sendCheckInConfirmation(patientId, queuePosition, estimatedWaitTime) {
        const notification = {
    ...
    async sendQueueUpdate(patientId, newPosition, newWaitTime) {
    ...
    async sendAppointmentReady(patientId) {
        const notification = {
    ...
    async sendTreatmentPlanUpdate(patientId, exercisesAdded) {
        const notification = {
    ...
    async sendExerciseReminder(patientId, exerciseName) {
        const notification = {
    ...
    async sendProgressCelebration(patientId, milestone) {
        const notification = {
            title: 'Parabéns! 🎉',
    ...
    async sendMessageNotification(patientId, senderName, messagePreview) {
        const notification = {
    ...
    async scheduleNotification(patientId, notification, scheduledTime, notificationId) {
    ...
    async sendImmediate(patientId, notification) {
    ...
    async sendViaFCM(deviceToken, notification) {
        if (!this.fcmConfig) {
    ...
    async sendViaAPNS(deviceToken, notification) {
    ...
    mockNotificationSend(deviceToken, notification) {
    ...
    async getPatientDeviceTokens(patientId) {
    ...
    async getLastQueueUpdate(patientId) {
        // Mock implementation - in production, this would be stored in database/cache
        return null;
    }
    async saveLastQueueUpdate(patientId, update) {
    ...
    async updateDeviceTokenStatus(deviceTokens, results) {
        results.forEach((result, index) => {
    ...
    async scheduleRecurring(schedule) {
        ...
        await this.scheduleNotification(schedule.patientId, schedule.notification, nextTime, `${schedule.id}-recurring-${nextTime.getTime()}`);
    }
    ...
    async getScheduledNotifications(patientId) {
        const notifications = [];
    ...
    async scheduleExerciseReminders(patientId, exerciseName, times, // ["09:00", "15:00", "21:00"]
    endDate) {
        const scheduleIds = [];
        for (const time of times) {
            ...
            const scheduleId = await this.scheduleNotification(patientId, notification, scheduledTime, `exercise-${exerciseName}-${time}-${patientId}`);
