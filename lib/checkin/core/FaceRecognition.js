import { createAWSRekognition } from '../adapters/AWSRekognitionAdapter';
export class EnrollmentResultImpl {
    constructor(success, patientId, qualityScore, enrolledAt, error) {
...
export class RecognitionResultImpl {
    constructor(type, patientId, confidence, matches, error) {
        this.type = type;
        this.patientId = patientId;
...
export class FaceMatchImpl {
    constructor(patientId, confidence) {
        this.patientId = patientId;
        this.confidence = confidence;
    }
}
...
export class FaceRecognitionService {
    ...
    async enrollPatient(patientId, photoData) {
        try {
    ...
    async recognizePatient(photoData) {
        try {
            // Use AWS Rekognition if available
            if (this.useAWS && this.awsRekognition) {
    ...
    async findMatches(unknownEncoding) {
        const matches = [];
        ...
        for (const [patientId, knownEncoding] of this.encodingCache) {
            ...
            if (confidence > 0.6) {
                matches.push(new FaceMatchImpl(patientId, confidence));
            }
        }
    ...
    calculateEuclideanDistance(encoding1, encoding2) {
        let sum = 0;
    ...
    async assessImageQuality(imageData, face) {
        ...
        if (faceRatio < 0.1)
            quality *= 0.7; // Face too small
        else if (faceRatio > 0.5)
            quality *= 1.1; // Good face size
        return Math.min(quality, 1.0);
    }
    async storeFaceEncoding(patientId, encoding) {
    ...
    async searchInDatabase(unknownEncoding) {
        ...
        for (const patientId of mockPatientIds) {
            if (Math.random() > 0.7) { // 30% chance of match
                ...
                mockMatches.push(new FaceMatchImpl(patientId, confidence));
            }
        }
        return mockMatches;
    }
    async preloadEncodings(patientIds) {
    ...
    async loadEncodingFromDatabase(patientId) {
    ...
    async getStats() {
        const stats = {
            provider: this.useAWS ? 'AWS_REKOGNITION' : 'MOCK',
