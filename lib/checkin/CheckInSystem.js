...
export class CheckInSystem {
    constructor(config) {
        ...
        this.notificationService = new PushNotificationService(fcmConfig, config.notifications?.apns);
        ...
        this.checkInEngine = new CheckInEngine(this.faceRecognition, undefined, // Will use default mock services
        undefined, this.queueManager, undefined, undefined);
    ...
    async initialize() {
        ...
        try {
            ...
            await this.analytics.trackError('system_initialization', 'INIT_FAILED', `System initialization failed: ${error instanceof Error ? error.message : String(error)}`, { error: error instanceof Error ? error.toString() : String(error) });
            throw error;
        }
    }
    ...
    async processCheckIn(checkInData) {
        ...
        try {
            ...
            await this.analytics.trackCheckInStarted(sessionId, checkInData.deviceId, undefined, { method: checkInData.photo ? 'facial_recognition' : 'manual_search' });
            let result;
            ...
            await this.analytics.trackCheckInCompleted(sessionId, result.checkIn?.patientId || 'unknown', Date.now() - parseInt(sessionId.split('_')[1]), result.checkIn?.method || 'unknown', result.success);
            ...
            await this.analytics.trackError('checkin_processing', 'CHECKIN_FAILED', `Check-in failed: ${error}`, { sessionId, deviceId: checkInData.deviceId });
            return {
                success: false,
    ...
    async enrollPatientFace(patientId, photoData) {
    ...
    async registerDeviceForNotifications(patientId, token, platform) {
    ...
    // Analytics methods
    async getCheckInMetrics(startDate, endDate) {
    ...
    async getUsagePatterns(startDate, endDate) {
    ...
    async preloadCriticalData() {
        ...
        const criticalPatientIds = ['patient-1', 'patient-2', 'patient-3'].map(id => id);
    ...
    async handleSuccessfulCheckIn(checkIn) {
        // Send check-in confirmation notification
        if (checkIn.queuePosition && checkIn.estimatedWaitTime) {
            await this.notificationService.sendCheckInConfirmation(checkIn.patientId, checkIn.queuePosition, checkIn.estimatedWaitTime);
        }
        // Track in analytics
        await this.analytics.trackEvent({
...
export function createCheckInSystem(config) {
    return new CheckInSystem(config);
}
// Export default configuration
export const defaultCheckInConfig = {
    faceRecognition: {
        apiKey: 'mock-api-key',
