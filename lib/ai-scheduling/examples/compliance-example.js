 ...
 */
import { createAISchedulingService } from '../index';
...
export async function exemploAgendamentoComCompliance() {
    ...
    try {
        ...
        const supervision = await coffitoService.registerSupervision('supervisor_1', 'therapist_1', {
            type: 'clinical',
            level: 'direct',
            frequency: 'weekly',
...
export async function exemploPromptComCompliance() {
    ...
    try {
        ...
        const lgpdService = complianceManager.getLGPDService();
        const hasConsent = await lgpdService.hasValidConsent('patient_123', 'data_processing', 'ai_processing');
        if (!hasConsent) {
...
export async function exemploRelatorioConformidade() {
    ...
    try {
        ...
        const combinedReport = await complianceManager.generateComplianceReport('combined', period, 'system');
...
export async function exemploGestaoAlertas() {
    ...
    try {
        ...
        const resolved = await complianceManager.resolveAlert(alert.id, 'admin', 'Consentimento renovado com sucesso');
        if (resolved) {
