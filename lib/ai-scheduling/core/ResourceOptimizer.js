...
export class ResourceOptimizer {
     ...
     */
    async optimizeResources(request) {
        try {
     ...
     */
    async optimizeMultipleAppointments(requests) {
        try {
     ...
     */
    async getAvailableResources(request, usedResources) {
        const available = this.resources.filter(resource => {
            ...
            if (request.constraints.requiredSkills) {
                const hasRequiredSkills = request.constraints.requiredSkills.every(skill => resource.skills.includes(skill));
                if (!hasRequiredSkills)
                    return false;
            }
            // Verificar equipamentos necessários
            if (request.constraints.requiredEquipment) {
                const hasRequiredEquipment = request.constraints.requiredEquipment.every(equipment => resource.equipment.includes(equipment));
     ...
     * Gerar combinações possíveis de recursos
     */
    generateResourceCombinations(availableResources, request) {
        const combinations = [];
        ...
        for (const therapist of therapists) {
            for (const room of rooms) {
                ...
                if (!this.isCompatible(therapist, room))
                    continue;
                // Encontrar equipamentos necessários
                const requiredEquipment = this.getRequiredEquipment(request);
                const availableEquipment = equipment.filter(eq => requiredEquipment.includes(eq.id));
     ...
     */
    async evaluateCombinations(combinations, request) {
        const evaluations = [];
     ...
     */
    async calculateCombinationMetrics(combination, request) {
     ...
     */
    calculateCombinationScore(metrics, preferences) {
        let score = 0;
     ...
     * Selecionar melhor combinação
     */
    selectBestCombination(evaluations, preferences) {
     ...
     */
    identifyConflicts(resources, request) {
        const conflicts = [];
     ...
     */
    generateRecommendations(resources, conflicts, request) {
     ...
     */
    calculateCostSavings(resources, request) {
     ...
     */
    calculateUtilizationRate(resources) {
        if (resources.length === 0)
            return 0;
    ...
    isResourceAvailable(resource, timeWindow) {
        // Implementar verificação de disponibilidade
        return true;
    }
    isCompatible(therapist, room) {
        // Verificar se terapeuta e sala são compatíveis
        return therapist.location === room.id || room.equipment.some(eq => therapist.equipment.includes(eq));
    }
    getRequiredEquipment(request) {
    ...
    calculateResourceEfficiency(combination, request) {
    ...
    calculateConvenience(combination, request) {
        ...
        if (request.preferences.preferredTherapist) {
            const hasPreferredTherapist = combination.some(r => r.type === 'therapist' && r.id === request.preferences.preferredTherapist);
            if (hasPreferredTherapist)
                convenience += 0.3;
        }
        // Preferência de sala
        if (request.preferences.preferredRoom) {
            const hasPreferredRoom = combination.some(r => r.type === 'room' && r.id === request.preferences.preferredRoom);
            if (hasPreferredRoom)
                convenience += 0.2;
        }
    ...
    checkSchedulingConflicts(resources, request) {
        // Implementar verificação de conflitos de horário
        return [];
    }
    checkCapacityConflicts(resources, request) {
        // Implementar verificação de conflitos de capacidade
        return [];
    }
    checkSkillConflicts(resources, request) {
        // Implementar verificação de conflitos de habilidades
        return [];
    }
    checkEquipmentConflicts(resources, request) {
