...
export class CreatePackageUseCase {
    constructor(repository, billingService, paymentService) {
        ...
        try {
            ...
            await this.repository.withTransaction(async (tx) => {
                ...
                if (command.installments && command.installments > 1) {
                    const installments = this.createInstallmentTransactions(transaction, command.installments, command.createdBy);
    ...
    async validatePatientEligibility(patientId) {
        ...
        if (activePackages.length > 0) {
            ...
            if (activePackage.hasRemainingSessions()) {
                throw new BusinessRuleError(`Patient already has an active package with ${activePackage.getRemainingSessions()} remaining sessions`);
            }
        }
        ...
        if (overdueTransactions.length > 0) {
            const totalOverdue = overdueTransactions.reduce((sum, tx) => sum.add(tx.getAmount()), Money.zero());
            throw new BusinessRuleError(`Patient has overdue payments totaling ${totalOverdue.toString()}. Please settle before purchasing new package.`);
        }
    }
    async calculateCustomPricing(command) {
        ...
        const pricing = await this.billingService.calculatePackagePricing(command.packageType, command.patientId);
    ...
    calculateExpiryDate(packageType) {
    ...
    createInstallmentTransactions(originalTransaction, installmentCount, createdBy) {
        const installments = [];
