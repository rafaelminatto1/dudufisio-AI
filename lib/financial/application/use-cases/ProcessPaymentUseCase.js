...
export class ProcessPaymentUseCase {
    ...
    async execute(command) {
        try {
            ...
            // 5. Process payment
            const paymentResult = await this.paymentService.processPayment(paymentRequest, command.gatewayName);
    ...
    async executeInstallmentPayment(command) {
        try {
            // 1. Validate command
            ...
            if (!paymentPlan.isActive()) {
                throw new BusinessRuleError(`Cannot process payment for ${paymentPlan.getStatus()} payment plan`);
            }
            // 4. Find installment
            ...
            const paymentResult = await this.paymentService.processInstallmentPayment(command.paymentPlanId, command.installmentId, command.gatewayName);
            if (!paymentResult.success) {
                return {
                    success: false,
                    error: paymentResult.error,
    ...
    async retryFailedPayment(transactionId, gatewayName) {
        ...
        if (!transaction.isPending() && !transaction.isOverdue()) {
            throw new BusinessRuleError(`Cannot retry payment for transaction with status: ${transaction.getStatus()}`);
        }
    ...
    async processRefund(transactionId, refundAmount, reason, gatewayName) {
        try {
            ...
            const refundAmountMoney = refundAmount ?
                ...
                undefined;
            const result = await this.paymentService.processRefund(transactionId, refundAmountMoney, reason, gatewayName);
            return {
