...
export class Transaction {
    ...
    static create(data) {
        ...
        const now = new Date();
        return new Transaction(id, data.patientId, data.type, data.amount, data.paymentMethod, data.installments || 1, data.installmentNumber || 1, data.dueDate, data.paidDate || null, data.status || TransactionStatus.PENDING, data.gatewayTransactionId || null, data.gatewayResponse || null, data.description || null, data.metadata || {}, data.fiscalDocumentNumber || null, data.taxAmount || Money.zero(data.amount.toJSON().currency), data.createdBy, data.updatedBy || null, now, now, 1);
    }
    getId() {
        return this.id;
    }
