...
export class InitialAssessment {
    constructor(id, patientId, chiefComplaint, medicalHistory, physicalExam, functionalTests, diagnosis, treatmentPlan, goals, specialty, createdBy, createdAt) {
        this.id = id;
    ...
    static create(data) {
        ...
        return new InitialAssessment(id, data.patientId, data.chiefComplaint, data.medicalHistory, data.physicalExam, data.functionalTests, data.diagnosis, data.treatmentPlan, data.goals, data.specialty, data.createdBy, now);
    }
    /**
    ...
    update(updates, updatedBy) {
        ...
        return new InitialAssessment(this.id, updatedData.patientId, updatedData.chiefComplaint, updatedData.medicalHistory, updatedData.physicalExam, updatedData.functionalTests, updatedData.diagnosis, updatedData.treatmentPlan, updatedData.goals, updatedData.specialty, this.createdBy, this.createdAt);
    }
    /**
    ...
    addFunctionalTest(test) {
        ...
        return new InitialAssessment(this.id, this.patientId, this.chiefComplaint, this.medicalHistory, this.physicalExam, updatedTests, this.diagnosis, this.treatmentPlan, this.goals, this.specialty, this.createdBy, this.createdAt);
    }
    /**
    ...
    addTreatmentGoal(goal) {
        ...
        return new InitialAssessment(this.id, this.patientId, this.chiefComplaint, this.medicalHistory, this.physicalExam, this.functionalTests, this.diagnosis, this.treatmentPlan, updatedGoals, this.specialty, this.createdBy, this.createdAt);
    }
    /**
    ...
    updateDiagnosis(diagnosis) {
        ...
        return new InitialAssessment(this.id, this.patientId, this.chiefComplaint, this.medicalHistory, this.physicalExam, this.functionalTests, diagnosis, this.treatmentPlan, this.goals, this.specialty, this.createdBy, this.createdAt);
    }
    /**
    ...
    updateTreatmentPlan(plan) {
        ...
        return new InitialAssessment(this.id, this.patientId, this.chiefComplaint, this.medicalHistory, this.physicalExam, this.functionalTests, this.diagnosis, plan, this.goals, this.specialty, this.createdBy, this.createdAt);
    }
    /**
    ...
    identifyFunctionalLimitations() {
        ...
        const restrictedROM = this.physicalExam.rangeOfMotion.cervical
            ...
            .filter((joint) => (joint.flexion && joint.flexion < 50) ||
        ...
        const failedTests = this.functionalTests.filter(test => test.result.toLowerCase().includes('limitado') ||
    ...
    validateInvariants() {
        if (!this.id) {
            throw new DomainError('Assessment ID is required', 'INVALID_ID_ERROR', 'id');
        }
        if (!this.patientId) {
            throw new DomainError('Patient ID is required', 'INVALID_PATIENT_ID_ERROR', 'patientId');
        }
        if (!this.chiefComplaint) {
            throw new DomainError('Chief complaint is required', 'INVALID_CHIEF_COMPLAINT_ERROR', 'chiefComplaint');
        }
        if (!this.diagnosis) {
            throw new DomainError('Diagnosis is required', 'INVALID_DIAGNOSIS_ERROR', 'diagnosis');
        }
        if (!this.treatmentPlan) {
            throw new DomainError('Treatment plan is required', 'INVALID_TREATMENT_PLAN_ERROR', 'treatmentPlan');
        }
        if (!this.createdBy) {
     ...
     */
    static validateChiefComplaint(complaint) {
        if (!complaint.description || complaint.description.trim().length === 0) {
            throw new ValidationError('Chief complaint description is required', 'chiefComplaint.description');
        }
        if (complaint.description.length < 10) {
            throw new ValidationError('Chief complaint description must be at least 10 characters', 'chiefComplaint.description');
        }
        if (!complaint.onset) {
            throw new ValidationError('Pain onset date is required', 'chiefComplaint.onset');
        }
        if (!complaint.characteristics) {
            throw new ValidationError('Pain characteristics are required', 'chiefComplaint.characteristics');
        }
    }
    /**
     * Valida exame físico
     */
    static validatePhysicalExam(exam) {
        if (!exam.inspection || exam.inspection.trim().length === 0) {
            throw new ValidationError('Inspection findings are required', 'physicalExam.inspection');
        }
        if (!exam.palpation || exam.palpation.trim().length === 0) {
            throw new ValidationError('Palpation findings are required', 'physicalExam.palpation');
        }
        if (!exam.rangeOfMotion) {
            throw new ValidationError('Range of motion assessment is required', 'physicalExam.rangeOfMotion');
        }
        if (!exam.muscleStrength) {
            throw new ValidationError('Muscle strength assessment is required', 'physicalExam.muscleStrength');
        }
    }
    /**
     * Valida diagnóstico
     */
    static validateDiagnosis(diagnosis) {
        if (!diagnosis.primaryDiagnosis || diagnosis.primaryDiagnosis.trim().length === 0) {
            throw new ValidationError('Primary diagnosis is required', 'diagnosis.primaryDiagnosis');
        }
        if (!diagnosis.severity) {
            throw new ValidationError('Diagnosis severity is required', 'diagnosis.severity');
        }
        if (!['mild', 'moderate', 'severe'].includes(diagnosis.severity)) {
            throw new ValidationError('Invalid diagnosis severity', 'diagnosis.severity');
        }
    }
    /**
     * Valida teste funcional
     */
    validateFunctionalTest(test) {
        if (!test.name || test.name.trim().length === 0) {
            throw new ValidationError('Functional test name is required', 'functionalTest.name');
        }
        if (!test.result || test.result.trim().length === 0) {
            throw new ValidationError('Functional test result is required', 'functionalTest.result');
        }
    }
    /**
     * Valida meta de tratamento
     */
    validateTreatmentGoal(goal) {
        if (!goal.description || goal.description.trim().length === 0) {
            throw new ValidationError('Treatment goal description is required', 'treatmentGoal.description');
        }
        if (!goal.targetDate) {
            throw new ValidationError('Treatment goal target date is required', 'treatmentGoal.targetDate');
        }
        if (goal.targetDate < new Date()) {
            throw new ValidationError('Treatment goal target date cannot be in the past', 'treatmentGoal.targetDate');
        }
        if (!goal.priority) {
            throw new ValidationError('Treatment goal priority is required', 'treatmentGoal.priority');
        }
        if (!['high', 'medium', 'low'].includes(goal.priority)) {
            throw new ValidationError('Invalid treatment goal priority', 'treatmentGoal.priority');
        }
    }
    /**
    ...
    validateTreatmentPlan(plan) {
        if (!plan.goals || plan.goals.length === 0) {
            throw new ValidationError('Treatment plan must have at least one goal', 'treatmentPlan.goals');
        }
        if (!plan.interventions || plan.interventions.length === 0) {
            throw new ValidationError('Treatment plan must have at least one intervention', 'treatmentPlan.interventions');
        }
        if (!plan.frequency || plan.frequency.trim().length === 0) {
            throw new ValidationError('Treatment frequency is required', 'treatmentPlan.frequency');
        }
        if (!plan.duration || plan.duration.trim().length === 0) {
            throw new ValidationError('Treatment duration is required', 'treatmentPlan.duration');
        }
    }
    /**
    ...
    validateUpdatePermissions(updatedBy) {
        if (updatedBy !== this.createdBy) {
            throw new DomainError('Only the creator can update the assessment', 'UPDATE_PERMISSION_ERROR', 'updatedBy');
        }
    }
    /**
    ...
    static fromPersistence(data) {
        return new InitialAssessment(data.id, data.patientId, data.chiefComplaint, data.medicalHistory, data.physicalExam, data.functionalTests, data.diagnosis, data.treatmentPlan, data.goals, data.specialty, data.createdBy, data.createdAt);
    }
    /**
