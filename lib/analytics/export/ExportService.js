...
<body>
    ...
    <div class="section">
      ...
      `;
            ...
            <table>
                ...
                <tbody>
                    ${dataSet.rows.slice(0, 50).map(row => `<tr>${row.map(cell => `<td>${cell}</td>`).join('')}</tr>`).join('')}
                </tbody>
            </table>
    ...
    async compressExport(content, format) {
    ...
    async generateExportSummary(exports) {
        const summary = {
...
${Object.entries(summary.formatDistribution).map(([format, count]) => `- **${format.toUpperCase()}**: ${count} arquivo(s)`).join('\n')}
...
- ${summary.formatDistribution['pdf'] > (summary.totalExports * 0.7) ? 'Alto uso de PDF - considerar otimização' : 'Distribuição de formatos equilibrada'}
    ...
    async scheduleExport(reportId, format, schedule, recipients) {
    ...
    async validateExportPermissions(userId, format) {
    ...
    async getExportHistory(userId, days = 30) {
        // Mock export history
        const history = [
            {
                id: 'export_1',
        ...
        return history.filter(exp => !userId || exp.userId === userId).filter(exp => exp.timestamp.getTime() > Date.now() - (days * 24 * 60 * 60 * 1000));
    }
}
