...
export class DataWarehouse {
    ...
    async insertDateBatch(dates) {
        const values = dates.map(date => `(${date.date_key}, '${date.full_date}', ${date.day_of_week}, '${date.day_name}',
        ${date.day_of_month}, ${date.day_of_year}, ${date.week_of_year},
        ${date.month_number}, '${date.month_name}', ${date.quarter}, ${date.year},
        ${date.is_weekend}, ${date.is_holiday}, ${date.holiday_name ? `'${date.holiday_name}'` : 'NULL'})`).join(',');
        const query = `
