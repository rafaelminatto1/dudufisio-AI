...
export class ReportGenerator {
    ...
    async generateSection(sectionType, dashboardData, period) {
        switch (sectionType) {
...
${dashboardData.alerts.length > 0 ?
            dashboardData.alerts.map((alert) => `- **${alert.title}**: ${alert.description}`).join('\n') :
    ...
    generateFinancialAnalysis(financial) {
        const content = `
...
- ${financial.avgTransactionValue < 100 ? 'Avaliar reajuste de preços ou pacotes de serviços' : 'Ticket médio adequado'}
    ...
    generateOperationalMetrics(operational) {
        const content = `
...
- ${operational.cancellationRate > 10 ? 'Revisar política de cancelamentos' : 'Taxa de cancelamentos sob controle'}
    ...
    generateClinicalOutcomes(clinical) {
        const content = `
## Resultados Clínicos
...
- ${clinical.avgSessionsPerTreatment > 10 ? 'Avaliar eficiência dos tratamentos prolongados' : 'Duração dos tratamentos adequada'}
    ...
    generatePatientInsights(patient) {
        const content = `
...
- ${patient.referralRate < 5 ? 'Criar programa de indicações' : 'Manter programa de indicações ativo'}
    ...
    async generatePredictiveAnalytics(period) {
        try {
...
${anomalies.length > 0 ?
                anomalies.map(anomaly => `**${anomaly.affectedMetric}**: ${anomaly.description} (Desvio: ${anomaly.deviation.toFixed(1)}%)`).join('\n') :
...
${anomalies.length > 0 ?
                anomalies.slice(0, 3).map(anomaly => anomaly.recommendations[0]).join('\n- ') :
...
- Ajustar estratégias baseado nos resultados
    ...
    async generateCharts(dashboardData, period) {
        const charts = [];
        // Revenue trend chart
        const revenueChart = {
            id: 'revenue_trend',
