...
export class ExecutiveDashboard {
    ...
    async calculateOperationalKPIs(period) {
        try {
            // Total Appointments
    ...
    async calculateClinicalKPIs(period) {
        try {
            // Total Treatments
    ...
    async calculatePatientKPIs(period) {
        try {
            // Total Patients
            ...
            const retainedPatients = Array.from(previousActivePatients).filter(p => currentActivePatients.has(p)).length;
            ...
            const avgAgeGroup = Object.entries(ageGroupCounts).reduce((a, b) => ageGroupCounts[a[0]] > ageGroupCounts[b[0]] ? a : b)?.[0] || 'Adulto';
    ...
    async calculateMetricTrend(metric, period) {
        const previousPeriod = this.getPreviousPeriod(period);
        let currentValue = 0;
        let previousValue = 0;
        switch (metric) {
    ...
    async generateBusinessAlerts(financial, operational, clinical, patient) {
        const alerts = [];
        // Revenue alerts
    ...
    async getAppointmentsCountForPeriod(period) {
    ...
    async getAvgSatisfactionForPeriod(period) {
        const { data } = await this.supabase
            .from('fact_clinical_outcomes')
    ...
    async getNoShowRateForPeriod(period) {
        const { data } = await this.supabase
            .from('fact_appointments')
            .select('is_no_show')
            .gte('appointment_date', period.start.toISOString())
    ...
    getTrendDescription(metric, trend, change) {
    ...
    async exportDashboard(dashboard, format) {
        switch (format) {
            case 'json':
