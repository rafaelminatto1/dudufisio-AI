...
export class ChartService {
    ...
    async generateFinancialCharts(period) {
        const charts = [];
        try {
    ...
    async generateOperationalCharts(period) {
        const charts = [];
        try {
    ...
    async generateClinicalCharts(period) {
        const charts = [];
        try {
    ...
    async generatePatientCharts(period) {
        const charts = [];
        try {
    ...
    // Revenue Trend Chart
    async generateRevenueTrendChart(period) {
        ...
        // Group by date
        const dailyRevenue = (data || []).reduce((acc, transaction) => {
    ...
    // Payment Methods Pie Chart
    async generatePaymentMethodsChart(period) {
        const { data } = await this.supabase
            .from('fact_financial_transactions')
    ...
    async generateMonthlyRevenueChart(period) {
    ...
    // Appointments Trend Chart
    async generateAppointmentsTrendChart(period) {
        const { data } = await this.supabase
    ...
    // No-Show Rate Chart
    async generateNoShowRateChart(period) {
        const { data } = await this.supabase
            .from('fact_appointments')
            .select('appointment_date, is_no_show')
            .gte('appointment_date', period.start.toISOString())
            .lte('appointment_date', period.end.toISOString());
        const dailyStats = (data || []).reduce((acc, appointment) => {
            const date = appointment.appointment_date.split('T')[0];
            if (!acc[date])
    ...
    async generatePainReductionChart(period) {
        const { data } = await this.supabase
            ...
            .order('session_date');
        const dailyAverages = (data || []).reduce((acc, session) => {
    ...
    async generateAgeDistributionChart(period) {
        const { data } = await this.supabase
            .from('dim_patients')
            .select('age_group')
            .eq('is_current', true);
    ...
    async generateServiceRevenueChart(period) {
    ...
    async generateTherapistUtilizationChart(period) {
        return {
    ...
    async generateTimeSlotDistributionChart(period) {
        return {
    ...
    async generateWeeklyPatternChart(period) {
        return {
            id: 'weekly_pattern',
            type: 'bar',
    ...
    async generatePatientSatisfactionChart(period) {
        return {
    ...
    async generateTreatmentEffectivenessChart(period) {
        return {
    ...
    async generateSessionDurationChart(period) {
        return {
            id: 'session_duration',
            type: 'bar',
    ...
    async generateAgeGroupSuccessChart(period) {
        return {
    ...
    async generatePatientGrowthChart(period) {
        return {
            id: 'patient_growth',
            type: 'line',
    ...
    async generateRetentionChart(period) {
        return {
            id: 'retention_rate',
            type: 'line',
    ...
    async generateEngagementHeatmap(period) {
