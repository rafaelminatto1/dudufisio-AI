...
import { OutcomeScenario } from '../../types/predictiveAnalyticsTypes';

interface PredictionScenarioCardProps {
  scenario: OutcomeScenario;
  isSelected?: boolean;
  onClick?: () => void;
}

export const PredictionScenarioCard: React.FC<PredictionScenarioCardProps> = ({
  scenario,
  isSelected = false,
  onClick,
}) => {
  const getProbabilityColor = () => {
    if (scenario.probability >= 0.7) return 'text-green-600 bg-green-100';
    if (scenario.probability >= 0.4) return 'text-blue-600 bg-blue-100';
