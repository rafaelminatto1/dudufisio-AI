        ...
        <div className="ml-3 flex-1">
          ...
          <div className="mt-4 flex space-x-3">
            {onRetry && (
              <button
                onClick={onRetry}
                className={`inline-flex items-center px-3 py-2 border border-transparent text-sm font-medium rounded-md text-white ${styles.button} focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500`}
              >
...
/**
 * 🎯 Componente para exibir múltiplos erros
 */
...
export const ErrorList: React.FC<ErrorListProps> = ({
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      {errors.map(({ id, error, context, onRetry, onDismiss }) => (
        <ErrorDisplay
          key={id}
          error={error}
          context={context}
