...
export function useExercises() {
    ...
    const updateExercise = async (id, exerciseData) => {
        try {
            ...
            setState(prev => ({
                ...prev,
                exercises: prev.exercises.map(exercise => exercise.id === id ? updated : exercise),
            }));
    ...
    const searchExercises = (query, filters) => {
        let filtered = state.exercises;
        // Text search
        if (query) {
            const searchQuery = query.toLowerCase();
            filtered = filtered.filter(exercise => exercise.name.toLowerCase().includes(searchQuery) ||
        ...
        if (filters?.muscle_groups && filters.muscle_groups.length > 0) {
            filtered = filtered.filter(exercise => filters.muscle_groups.some(group => exercise.muscle_groups.includes(group)));
        }
        if (filters?.equipment && filters.equipment.length > 0) {
            filtered = filtered.filter(exercise => filters.equipment.some(equip => exercise.equipment.includes(equip)));
        }
        return filtered;
    };
    ...
    return {
        ...
        updateExercise: (exercise) => updateExercise(exercise.id, exercise),
