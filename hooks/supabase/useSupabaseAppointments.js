...
export const useSupabaseAppointments = (filters) => {
    ...
    // Mark as no-show
    const markAsNoShow = useCallback(async (id) => {
        try {
    ...
    const createRecurringAppointments = useCallback(async (baseAppointment, recurrenceType, occurrences) => {
        try {
            const appointments = await appointmentService.createRecurringAppointments(baseAppointment, recurrenceType, occurrences);
            await loadAppointments();
...
export const useSupabaseTimeSlots = (therapistId, date, duration = 60) => {
...
// Hook for week view
export const useSupabaseWeekAppointments = (date, therapistId) => {
    const [appointments, setAppointments] = useState([]);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState(null);
    const loadAppointments = useCallback(async () => {
...
export const useSupabaseTodayAppointments = (therapistId) => {
...
export const useSupabaseUpcomingAppointments = (patientId, limit = 5) => {
    const [appointments, setAppointments] = useState([]);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState(null);
    const loadAppointments = useCallback(async () => {
