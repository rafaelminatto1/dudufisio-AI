...
export const useSupplierPerformanceReport = (supplierId?: string) => {
  const [report, setReport] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
...
export const usePerformanceMetrics = (period?: {
  ...
  const calculateMetrics = useCallback(async (periodStart?: string, periodEnd?: string) => {
    try {
...
export const useScheduledReports = () => {
  ...
  const editReport = useCallback(async (id: string, reportData: any) => {
    try {
...
export const useReportHistory = (limit: number = 50) => {
  ...
  const addHistoryItem = useCallback(async (historyData: any) => {
    try {
...
export const useReportExport = () => {
  ...
  const exportToCSV = useCallback(async (reportData: any[], filename: string): Promise<string> => {
    try {
      setIsExporting(true);
      setError(null);
  ...
  const generateFilename = useCallback((reportType: string, format: string, date?: string): string => {
