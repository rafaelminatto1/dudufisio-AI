...
export const validateExerciseDescription = (description: string): boolean => {
  return description.length >= 10 && description.length <= 2000;
};

export const validateExerciseInstructions = (instructions: string[]): boolean => {
...
export const validateExerciseParameters = (exercise: {
  ...
  if (sets && (sets < 1 || sets > 50)) return false;
  if (reps && (reps < 1 || reps > 1000)) return false;
  if (weight && (weight < 0 || weight > 1000)) return false;
  if (distance && (distance < 0 || distance > 50000)) return false;
...
export type ExerciseProtocolData = z.infer<typeof ExerciseProtocolSchema>;
export type ExerciseAssignmentData = z.infer<typeof ExerciseAssignmentSchema>;
export type ExerciseProgressData = z.infer<typeof ExerciseProgressSchema>;
...
export type ExerciseSearchFiltersData = z.infer<typeof ExerciseSearchFiltersSchema>;
export type ImportOptionsData = z.infer<typeof ImportOptionsSchema>;
export type ExerciseExportDataSchemaType = z.infer<typeof ExerciseExportDataSchema>;
