      ...
      <div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex space-x-2">
            {['light', 'dark', 'system'].map((mode) => (<button key={mode} onClick={() => updateTheme({ mode })} className={`px-3 py-1 text-sm rounded-md border ${theme.mode === mode
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-4 gap-2">
            {Object.keys(designTokens.colors).filter(key => !['neutral', 'semantic'].includes(key)).map((colorKey) => {
            const color = designTokens.colors[colorKey];
            return (<button key={colorKey} onClick={() => updateTheme({ primaryColor: colorKey })} className={`w-10 h-10 rounded-md border-2 ${theme.primaryColor === colorKey ? 'border-gray-900' : 'border-gray-300'}`} style={{ backgroundColor: color[500] }} title={colorKey}/>);
        })}
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex space-x-2">
            {['none', 'sm', 'md', 'lg', 'xl'].map((radius) => (<button key={radius} onClick={() => updateTheme({ borderRadius: radius })} className={`px-3 py-1 text-sm border ${theme.borderRadius === radius
                ? 'bg-primary-500 text-white border-primary-500'
                : 'bg-background border-border hover:bg-muted'}`} style={{ borderRadius: designTokens.borderRadius[radius] }}>
        ...
        <div className="space-y-2">
          ...
          <div className="flex space-x-2">
            {['sm', 'base', 'lg'].map((size) => (<button key={size} onClick={() => updateTheme({ fontSize: size })} className={`px-3 py-1 text-sm rounded-md border ${theme.fontSize === size
                ? 'bg-primary-500 text-white border-primary-500'
                : 'bg-background border-border hover:bg-muted'}`}>
        ...
        {/* Reset Button */}
        <button onClick={resetTheme} className="w-full mt-4 px-3 py-2 text-sm bg-gray-100 hover:bg-gray-200 rounded-md border border-gray-300">
          Restaurar Padrão
        </button>
      </div>
    </div>);
