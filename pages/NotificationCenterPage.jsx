...
const NotificationItem = ({ notification, onMarkAsRead }) => {
    return (<div onClick={() => !notification.isRead && onMarkAsRead(notification.id)} className={`flex items-start p-4 border-b border-slate-200 transition-colors duration-200 ${!notification.isRead ? 'bg-sky-50/50 hover:bg-sky-100 cursor-pointer' : 'bg-white'}`}>
...
const BroadcastForm = ({ onSent }) => {
    ...
    const [targetGroup, setTargetGroup] = useState(Role.Therapist);
    ...
    return (<form onSubmit={handleSubmit} className="p-6 space-y-4">
            <div>
                <label htmlFor="broadcast-message" className="block text-sm font-medium text-slate-700">Mensagem</label>
                <TiptapEditor value={message} onChange={setMessage} minHeight="120px" placeholder="Digite seu comunicado aqui..."/>
            </div>
            <div>
                <label htmlFor="target-group" className="block text-sm font-medium text-slate-700">Destinatários</label>
                <select id="target-group" value={targetGroup} onChange={(e) => setTargetGroup(e.target.value)} className="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-teal-500 focus:ring-teal-500">
            ...
            <div className="text-right">
                <button type="submit" disabled={isSending} className="inline-flex items-center justify-center rounded-lg border border-transparent bg-teal-500 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-teal-600 disabled:bg-teal-300">
...
const NotificationCenterPage = () => {
    ...
    return (<>
            <PageHeader title="Centro de Notificações" subtitle="Acompanhe suas mensagens e envie comunicados para a equipe."/>
            <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
                <div className="border-b border-slate-200">
                    <nav className="flex space-x-4 px-6" aria-label="Tabs">
                        ...
                        {user?.role === Role.Admin && (<button onClick={() => setActiveTab('broadcast')} className={`flex items-center whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'broadcast' ? 'border-teal-500 text-teal-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}>
                ...
                {activeTab === 'my' && (<div>
                        ...
                        {isLoading ? (Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} className="h-20 w-full"/>)) : notifications.length > 0 ? (<div className="divide-y divide-slate-200">
                                {notifications.map(n => <NotificationItem key={n.id} notification={n} onMarkAsRead={markAsRead}/>)}
                            </div>) : (<div className="text-center p-12">
                ...
                {activeTab === 'broadcast' && user?.role === Role.Admin && (<BroadcastForm onSent={refetch}/>)}
            </div>
        </>);
};
