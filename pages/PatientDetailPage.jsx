...
const PatientDetailPage = () => {
    ...
    return (<div className="min-h-screen bg-slate-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <Card className="mb-6">
          ...
          <CardContent>
            {loading ? (<div className="flex items-center justify-center py-8">
              ...
              </div>) : assignedProtocols.length === 0 ? (<div className="text-center py-8">
                ...
                {assignedProtocols.map(protocol => (<div key={protocol.id} className="border border-slate-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        {protocol.objectives.length > 0 && (<div>
                            <h5 className="text-sm font-medium text-slate-700 mb-2">Objetivos:</h5>
                            <ul className="text-sm text-slate-600 space-y-1">
                              {protocol.objectives.slice(0, 3).map((objective, index) => (<li key={index} className="flex items-start gap-2">
                                  <span className="text-blue-500 mt-1">•</span>
                                  <span>{objective}</span>
                                </li>))}
                              {protocol.objectives.length > 3 && (<li className="text-slate-500 text-xs">
                    ...
                    {protocol.tags.length > 0 && (<div className="mt-3 pt-3 border-t border-slate-100">
                        <div className="flex flex-wrap gap-1">
                          {protocol.tags.map(tag => (<span key={tag} className="text-xs bg-slate-100 text-slate-600 px-2 py-1 rounded">
                              #{tag}
                            </span>))}
                        </div>
                      </div>)}
        ...
        {assignedProtocols.length > 0 && (<Card>
            <CardHeader>
