...
const KnowledgeRow = ({ entry, onEdit }) => {
    const typeInfo = {
    ...
    return (<tr className="border-b border-slate-200 hover:bg-slate-50 cursor-pointer" onClick={() => onEdit(entry)}>
            ...
            <td className="p-4 whitespace-nowrap">
                <div className="flex flex-wrap gap-1">
                    {entry.tags.map(tag => (<span key={tag} className="px-2 py-0.5 text-xs bg-slate-200 text-slate-700 rounded-full">{tag}</span>))}
                </div>
            </td>
        </tr>);
};
const KnowledgeBasePage = () => {
    ...
    const [entries, setEntries] = useState(knowledgeService.getAll());
    ...
    return (<>
            <PageHeader title="Base de Conhecimento" subtitle="Gerencie o cérebro da sua clínica: protocolos, exercícios e técnicas.">
            ...
            <KnowledgeContributionModal isOpen={isModalOpen} onClose={handleCloseModal} onSave={handleSave} entryToEdit={entryToEdit}/>

            <div className="bg-white p-6 rounded-2xl shadow-sm">
                <div className="flex items-center justify-between mb-4">
                    <div className="relative w-full max-w-xs">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400"/>
                        <input type="text" placeholder="Buscar por título, conteúdo ou tag..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500"/>
                    </div>
                </div>
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-slate-200">
                        ...
                        <tbody className="bg-white divide-y divide-slate-200">
                            {filteredEntries.map((entry) => (<KnowledgeRow key={entry.id} entry={entry} onEdit={handleOpenModal}/>))}
                        </tbody>
                    </table>
                     {filteredEntries.length === 0 && (<div className="text-center p-10">
