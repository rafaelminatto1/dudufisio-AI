...
const VoucherStorePage = () => {
    ...
    return (<>
            <PageHeader title="Serviços da Parceria" subtitle="Complemente seu tratamento com os planos de acompanhamento da nossa educadora física parceira."/>
            <VoucherPurchaseModal isOpen={!!selectedPlan} onClose={() => setSelectedPlan(null)} onConfirm={handlePurchase} plan={selectedPlan}/>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-stretch">
                {isLoading ? (Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} className="h-96 rounded-2xl"/>)) : (plans.map(plan => (<VoucherPlanCard key={plan.id} plan={plan} onSelect={() => setSelectedPlan(plan)}/>)))}
            </div>
        </>);
};
