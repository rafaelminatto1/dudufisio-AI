...
import { generatePatientProgressSummary } from '../../services/geminiService';
...
const LoadingSkeleton = () => (<div className="bg-white p-8 rounded-2xl shadow-sm animate-pulse">
...
const PatientProgressPage = () => {
    ...
    const [patient, setPatient] = useState(null); // <-- Add state for patient
    ...
    return (<>
            <PageHeader title="Meu Progresso" subtitle="Veja sua jornada de recuperação de forma clara e motivadora."/>
            
            {isLoading ? <LoadingSkeleton /> : (<div className="space-y-8">
                    <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm">
                        {summary ? <MarkdownRenderer content={summary} className="text-md"/> : (<div className="text-center py-12">
                    ...
                    {activeMetrics.length > 0 && notes.length > 1 && (<div>
                             <h2 className="text-2xl font-bold text-slate-800 mb-4 flex items-center">
                                <BarChart className="w-6 h-6 mr-3 text-sky-500"/> Evolução das Métricas
                            </h2>
                             <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                {activeMetrics.map(metric => (<PatientMetricChart key={metric.id} metric={metric} notes={notes}/>))}
                            </div>
                        </div>)}
