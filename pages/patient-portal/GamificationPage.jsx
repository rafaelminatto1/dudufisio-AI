...
const GamificationPage = () => {
    ...
    const { level, points, xpForNextLevel, pointsTowardsLevel, streak, achievements, pointsBreakdown, activeChallenges, completedChallenges, availableRewards, unlockedRewards, leaderboard, nextMilestone, recentActivities, } = progress;
    const unlockedAchievements = achievements.filter(a => a.unlocked);
    const availableRewardsMerged = [...availableRewards, ...unlockedRewards];
    return (<>
            <PageHeader title="Meu Engajamento" subtitle="Sua jornada de recuperação gamificada para te manter no caminho certo!"/>
            
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                <LevelProgress level={level} totalPoints={points} pointsTowardsLevel={pointsTowardsLevel} xpForNextLevel={xpForNextLevel}/>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm flex flex-col justify-center">
                    ...
                    <div className="mt-4">
                        ...
                        <div className="w-full h-3 bg-slate-200 rounded-full">
                            <div className="h-3 bg-teal-500 rounded-full" style={{ width: `${Math.min(100, ((nextMilestone.targetPoints - nextMilestone.pointsRemaining) / nextMilestone.targetPoints) * 100)}%` }}/>
                        </div>
            ...
            {!!activeChallenges.length && (<section className="mb-8">
            ...
            {!!completedChallenges.length && (<section className="mb-8">
                    <h3 className="text-lg font-semibold text-slate-700 mb-3">Desafios concluídos recentemente</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {completedChallenges.map(challenge => (<ChallengeCard key={challenge.id} challenge={challenge}/>))}
                    </div>
                </section>)}

            {!!availableRewardsMerged.length && (<section className="mb-8">
                    <h2 className="text-2xl font-bold text-slate-800 mb-4">Recompensas</h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {availableRewardsMerged.map(reward => (<RewardCard key={reward.id} reward={reward}/>))}
                    </div>
                </section>)}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {achievements.slice(0, 4).map(achievement => (<AchievementCard key={achievement.id} achievement={achievement}/>))}
                    </div>
                </div>
            </div>
