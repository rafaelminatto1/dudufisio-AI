...
const DocumentsPage = () => {
    ...
    return (<>
            <PageHeader title="Meus Documentos" subtitle="Acesse seus atestados, recibos e exames importantes."/>
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-slate-200">
                        ...
                        <tbody className="bg-white divide-y divide-slate-200">
                            {isLoading ? (Array.from({ length: 3 }).map((_, i) => (<tr key={i}><td colSpan={3} className="p-4"><Skeleton className="h-10 w-full"/></td></tr>))) : documents.length > 0 ? (documents.map(doc => (<tr key={doc.id} className="hover:bg-slate-50">
                                        <td className="p-4">
