...
const PainPointHistoryRow = ({ point, onEdit }) => (<div onClick={() => onEdit(point)} className="p-4 bg-white rounded-lg shadow-sm hover:bg-slate-50 cursor-pointer">
...
const PatientPainDiaryPage = () => {
    ...
    const handleMapClick = (x, y, bodyPart) => {
        setPainMapModal({ isOpen: true, pointToEdit: { x, y, bodyPart } });
    };
    const handlePointClick = (point) => {
        setPainMapModal({ isOpen: true, pointToEdit: point });
    };
    const handleSavePainPoint = async (pointData) => {
        if (!patient || !painMapModal.pointToEdit)
            return;
    ...
    const handleDeletePainPoint = async () => {
        if (!patient || !painMapModal.pointToEdit?.id)
            return;
    ...
    return (<>
            <PageHeader title="Diário de Dor" subtitle="Use o mapa corporal para registrar seus pontos de dor e acompanhar sua evolução."/>

            {painMapModal.isOpen && painMapModal.pointToEdit && (<PainPointModal isOpen={painMapModal.isOpen} onClose={() => setPainMapModal({ isOpen: false, pointToEdit: null })} onSave={handleSavePainPoint} onDelete={painMapModal.pointToEdit.id ? handleDeletePainPoint : undefined} point={painMapModal.pointToEdit}/>)}
                ...
                <div className="lg:col-span-2 space-y-6">
                     <div className="bg-white p-6 rounded-2xl shadow-sm">
                        ...
                        <p className="text-sm text-slate-500 mb-4">Clique em uma área para adicionar um ponto de dor, ou em um ponto existente para editar.</p>
                         {isLoading || !patient ? <Skeleton className="h-96 w-full"/> : (<InteractiveBodyMap selectedParts={[]} points={patient.painPoints || []} onSelectPart={() => { }} onMapClick={handleMapClick} onPointClick={handlePointClick}/>)}
                    </div>
                </div>
