...
const MyExercisesPage = () => {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            if (planData?.exercises) {
                const enrichedExercises = await Promise.all(planData.exercises.map(async (prescription) => {
                ...
                setExercises(enrichedExercises.filter((ex) => ex !== null));
            }
            else {
                setExercises([]);
            }
    ...
    const renderContent = () => {
        ...
        if (!plan || exercises.length === 0) {
            return (<div className="col-span-full text-center p-12 bg-white rounded-2xl shadow-sm">
        ...
        return exercises.map(ex => {
            const todaysEvaluation = evaluations.find(e => e.exerciseId === ex.id && e.date.toDateString() === new Date().toDateString());
            return (<ExerciseEvaluationCard key={ex.id} exercise={ex} prescription={ex.prescription} todaysEvaluation={todaysEvaluation} onSave={handleSaveEvaluation}/>);
        });
    };
    return (<>
            <PageHeader title="Meus Exercícios" subtitle="Veja seu plano de exercícios e registre seu feedback após cada sessão."/>
