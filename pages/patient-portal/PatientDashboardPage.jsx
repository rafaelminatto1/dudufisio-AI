...
const QuickActionCard = ({ to, icon, title, subtitle }) => (<ReactRouterDOM.Link to={to} className="bg-white p-6 rounded-2xl shadow-sm hover:shadow-lg hover:bg-slate-50 transition-all duration-300 flex items-center">
...
const NextAppointmentCard = ({ appointment }) => (<div className="bg-white p-6 rounded-2xl shadow-sm">
        ...
        {appointment ? (<div>
            ...
            </div>) : (<p className="text-slate-500">Nenhuma consulta agendada.</p>)}
    </div>);
const TodaysExerciseCard = ({ exercise }) => (<div className="bg-slate-50 p-3 rounded-lg flex items-center gap-4">
...
const PatientDashboardPage = () => {
    ...
    return (<>
            <PageHeader title={`Bem-vindo(a), ${user?.name.split(' ')[0] || user?.name}!`} subtitle="Sua jornada de recuperação começa aqui. O que vamos fazer hoje?"/>
            {isLoading ? (<Skeleton className="h-screen w-full"/>) : (<div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <QuickActionCard to="/portal/my-exercises" icon={<Dumbbell className="w-8 h-8"/>} title="Exercícios de Hoje" subtitle="Comece sua rotina diária"/>
                    <QuickActionCard to="/portal/pain-diary" icon={<NotebookText className="w-8 h-8"/>} title="Registrar Dor" subtitle="Conte-nos como se sente"/>
                </div>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <NextAppointmentCard appointment={nextAppointment}/>
                     <div className="bg-white p-6 rounded-2xl shadow-sm">
                        ...
                        {exercises.length > 0 ? (<div className="space-y-3 max-h-48 overflow-y-auto pr-2">
                               {exercises.slice(0, 3).map(ex => <TodaysExerciseCard key={ex.id} exercise={ex}/>)}
                            </div>) : (<p className="text-slate-500 text-center py-8">Nenhum exercício prescrito para hoje.</p>)}
                    </div>
                </div>
            </div>)}
        </>);
