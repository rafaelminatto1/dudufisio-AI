...
const MyAppointmentsPage = () => {
    ...
    const renderList = (apps) => (<div className="space-y-4">
            {apps.map(app => (<AppointmentCard key={app.id} appointment={app} therapistName={therapists.find(t => t.id === app.therapistId)?.name || 'N/A'}/>))}
        </div>);
    return (<>
            <PageHeader title="Meus Agendamentos" subtitle="Acompanhe suas próximas consultas e seu histórico de atendimentos."/>
            {isLoading ? (<Skeleton className="h-96 w-full rounded-2xl"/>) : appointments.length === 0 ? (<div className="text-center p-12 bg-white rounded-2xl shadow-sm">
