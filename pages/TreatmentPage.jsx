...
import { Plus, Search, Filter, Edit, Eye, Trash2, Clock, User, Calendar, Activity, AlertCircle, CheckCircle2, Pause, Play } from 'lucide-react';
...
const TreatmentPage = () => {
    ...
    return (<PermissionGuard permission="treatments:read">
      {/* Content with nested permission checks */}
    <div className="p-6 max-w-7xl mx-auto">
      <PageHeader title="Tratamentos" subtitle="Gerencie todos os planos de tratamento da clínica"/>

      {/* Header Actions */}
      <div className="flex flex-col sm:flex-row gap-4 mb-6">
        <div className="flex-1 relative">
          <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-4 h-4"/>
          <input type="text" placeholder="Buscar por paciente, condição ou terapeuta..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500"/>
        </div>

        <div className="flex gap-2">
          <button onClick={() => setShowFilters(!showFilters)} className={`flex items-center gap-2 px-4 py-2 border rounded-lg transition-colors ${showFilters
      ...
      {/* Filters */}
      {showFilters && (<div className="bg-white rounded-lg border border-gray-200 p-4 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Status</label>
              <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-sky-500 focus:border-sky-500">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredTreatments.map((treatment) => (<tr key={treatment.id} className="hover:bg-gray-50">
                ...
                treatment.status === 'paused' ? 'Pausado' :
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <div className="flex-1 bg-gray-200 rounded-full h-2 mr-3">
                        <div className="bg-sky-600 h-2 rounded-full" style={{ width: `${treatment.progress}%` }}></div>
                      </div>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    {treatment.nextAppointment ? (<div className="flex items-center gap-1">
            ...
            }} className="text-sky-600 hover:text-sky-900 p-1 rounded" title="Visualizar">
                        <Eye className="w-4 h-4"/>
                      </button>
                      <IfPermission permission="treatments:write">
                        <button className="text-gray-600 hover:text-gray-900 p-1 rounded" title="Editar">
                          <Edit className="w-4 h-4"/>
                        </button>
                      </IfPermission>
                      <IfPermission permission="treatments:delete">
                        <button className="text-red-600 hover:text-red-900 p-1 rounded" title="Excluir">
        ...
        {filteredTreatments.length === 0 && (<div className="text-center py-12">
