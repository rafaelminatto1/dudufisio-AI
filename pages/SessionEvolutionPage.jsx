...
import { generateSessionEvolution } from '../services/geminiService';
...
// 🚀 Componentes de formulário memoizados
const FormInput = memo(({ label, name, value, onChange, placeholder }) => (<div>
...
const FormTextarea = memo(({ label, name, value, onChange, placeholder, rows = 2 }) => (<div className="sm:col-span-2">
        <label htmlFor={name} className="block text-sm font-medium text-slate-700">{label}</label>
        <TiptapEditor value={value} onChange={onChange} placeholder={placeholder} minHeight={`${rows * 20}px`}/>
    </div>));
...
const SessionEvolutionPage = () => {
    ...
    }, [evolutionNote, showToast]);
    // 🚀 Valor computado memoizado
    const isSubmitDisabled = useMemo(() => isLoading || !formData.relato_paciente.trim(), [isLoading, formData.relato_paciente]);
    return (<>
            <PageHeader title="Gerador de Evolução de Sessão" subtitle="Preencha as anotações da sessão para gerar uma nota de evolução em formato SOAP."/>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                <div className="bg-white p-6 rounded-2xl shadow-sm space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select id="patient-select" value={selectedPatientId} onChange={(e) => setSelectedPatientId(e.target.value)} className="w-full p-2 border border-slate-300 rounded-lg bg-white">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                     ...
                     <button onClick={handleSubmit} disabled={isSubmitDisabled} title={isSubmitDisabled && !isLoading ? 'Preencha o Relato do Paciente para gerar a evolução' : undefined} className="w-full inline-flex justify-center items-center px-6 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-teal-500 hover:bg-teal-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-teal-500 disabled:bg-teal-300 disabled:cursor-not-allowed">
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm">
                    ...
                    <div className="bg-slate-50 p-4 rounded-lg min-h-[600px] overflow-y-auto border border-slate-200 prose">
                        ...
                        {!isLoading && !evolutionNote && (<div className="text-center text-slate-500 flex flex-col justify-center items-center h-full">
