...
const GerarLaudoPage = () => {
    ...
    if (isLoading) {
        return (<>
                <PageHeader title="Gerar Laudo Médico" subtitle="Selecione um paciente para gerar um laudo médico com IA."/>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                ...
                <div className="mb-6">
                    <div className="relative">
                        ...
                        <input type="text" placeholder="Buscar paciente por nome ou email..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500"/>
                    </div>
                </div>
                ...
                {filteredPatients.length === 0 ? (<div className="text-center py-12">
                        ...
                        {filteredPatients.map((patient) => (<button key={patient.id} onClick={() => handleSelectPatient(patient.id)} className="w-full flex items-center justify-between p-4 bg-slate-50 hover:bg-sky-50 border border-slate-200 hover:border-sky-300 rounded-lg transition-all duration-200 group">
