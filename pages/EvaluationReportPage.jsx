...
const AccordionSection = memo(({ title, isOpen, onToggle, children }) => (<Card className="mb-4">
        <CardHeader className="cursor-pointer hover:bg-slate-50 transition-colors" onClick={onToggle}>
...
*Laudo gerado automaticamente em ${new Date().toLocaleDateString('pt-BR')}*`;
    ...
    const isSubmitDisabled = useMemo(() => isLoading || !formData.queixa_principal.trim(), [isLoading, formData.queixa_principal]);
                        ...
                        <AccordionSection title="Dados e Anamnese" isOpen={openSections.anamnese} onToggle={() => handleToggleSection('anamnese')}>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <Input id="nome_paciente" name="nome_paciente" value={formData.nome_paciente} onChange={handleInputChange} placeholder="Ex: João da Silva"/>
                                </div>
                                <div>
                                    <Label htmlFor="idade_paciente">Idade</Label>
                                    <Input id="idade_paciente" name="idade_paciente" value={formData.idade_paciente} onChange={handleInputChange} placeholder="Ex: 35"/>
                                </div>
                                <div>
                                    <Label htmlFor="profissao_paciente">Profissão</Label>
                                    <Input id="profissao_paciente" name="profissao_paciente" value={formData.profissao_paciente} onChange={handleInputChange} placeholder="Ex: Engenheiro de Software"/>
                                </div>
                                <div>
                                    <Label htmlFor="escala_dor">Escala de Dor (EVA 0-10)</Label>
                                    <Input id="escala_dor" name="escala_dor" value={formData.escala_dor} onChange={handleInputChange} placeholder="Ex: 8"/>
                                </div>
                                <div className="sm:col-span-2">
                                    <Label htmlFor="queixa_principal">Queixa Principal *</Label>
                                    <Textarea id="queixa_principal" name="queixa_principal" value={formData.queixa_principal} onChange={handleInputChange} placeholder="Dor no ombro direito ao levantar o braço" rows={3}/>
                                </div>
                                <div className="sm:col-span-2">
                                    <Label htmlFor="hda">Histórico da Doença Atual (HDA)</Label>
                                    <Textarea id="hda" name="hda" value={formData.hda} onChange={handleInputChange} placeholder="Paciente relata início da dor há 3 semanas..." rows={3}/>
                                </div>
                                <div className="sm:col-span-2">
                                    <Label htmlFor="hmp">Histórico Médico Pregresso (HMP)</Label>
                                    <Textarea id="hmp" name="hmp" value={formData.hmp} onChange={handleInputChange} placeholder="Hipertensão controlada, cirurgia de apendicite em 2010..." rows={3}/>
                                </div>
                                <div className="sm:col-span-2">
                                    <Label htmlFor="objetivos_paciente">Objetivos do Paciente</Label>
                                    <Textarea id="objetivos_paciente" name="objetivos_paciente" value={formData.objetivos_paciente} onChange={handleInputChange} placeholder="Voltar a jogar tênis sem dor..." rows={3}/>
                                </div>
                            </div>
                        </AccordionSection>
                        ...
                        <AccordionSection title="Exame Físico" isOpen={openSections.exameFisico} onToggle={() => handleToggleSection('exameFisico')}>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div className="sm:col-span-2">
                                    ...
                                    <Textarea id="inspecao_palpacao" name="inspecao_palpacao" value={formData.inspecao_palpacao} onChange={handleInputChange} placeholder="Edema leve, ponto de dor em..." rows={3}/>
                                </div>
                                <div className="sm:col-span-2">
                                    <Label htmlFor="adm">Amplitude de Movimento (ADM)</Label>
                                    <Textarea id="adm" name="adm" value={formData.adm} onChange={handleInputChange} placeholder="Flexão de ombro 120°, abdução 110°..." rows={3}/>
                                </div>
                                <div className="sm:col-span-2">
                                    <Label htmlFor="teste_forca">Teste de Força Muscular (0-5)</Label>
                                    <Textarea id="teste_forca" name="teste_forca" value={formData.teste_forca} onChange={handleInputChange} placeholder="Manguito rotador grau 3, deltoide grau 4..." rows={3}/>
                                </div>
                                <div className="sm:col-span-2">
                                    <Label htmlFor="testes_especiais">Testes Especiais</Label>
                                    <Textarea id="testes_especiais" name="testes_especiais" value={formData.testes_especiais} onChange={handleInputChange} placeholder="Teste de Neer positivo, Jobe negativo..." rows={3}/>
                                </div>
                            </div>
                        </AccordionSection>

                        <Button onClick={handleSubmit} disabled={isSubmitDisabled} className="w-full" size="lg">
                            {isLoading ? (<Loader className="w-5 h-5 mr-3 animate-spin"/>) : (<Sparkles className="w-5 h-5 mr-3"/>)}
                    ...
                    <Card>
                        <CardHeader>
                            <div className="flex justify-between items-center">
                                ...
                                <Button onClick={handleCopy} disabled={!report || copied} variant="outline" size="sm">
                        ...
                        <CardContent>
                            <div className="bg-slate-50 p-4 rounded-lg min-h-[600px] overflow-y-auto border border-slate-200">
                                {isLoading && (<div className="space-y-4 animate-pulse p-2">
                                ...
                                {!isLoading && !report && (<div className="text-center text-slate-500 flex flex-col justify-center items-center h-full">
                                ...
                                {report && (<pre className="whitespace-pre-wrap text-sm text-slate-700 font-mono">
                                        {report}
                                    </pre>)}
                            </div>
                        </CardContent>
