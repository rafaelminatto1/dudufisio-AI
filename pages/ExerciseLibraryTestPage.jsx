    ...
    return (<div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-6xl mx-auto px-4">
        <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
          ...
          <button onClick={runTests} className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">
            🔄 Executar Testes Novamente
          </button>
        </div>

        {testResults.error ? (<div className="bg-red-50 border border-red-200 rounded-lg p-6">
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              ...
              <div className="mt-4">
                ...
                <ul className="text-sm text-gray-600 space-y-1">
                  {testResults.exercises?.sample?.map((e, i) => (<li key={i}>• {e.name} ({e.category}) - {e.specialty}</li>))}
                </ul>
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              ...
              <ul className="text-sm text-gray-600 space-y-1">
                {testResults.categories?.map((cat, i) => (<li key={i}>• {cat}</li>))}
              </ul>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              ...
              <div className="flex flex-wrap gap-2">
                {testResults.bodyParts?.slice(0, 8).map((part, i) => (<span key={i} className="bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded">
                    {part}
                  </span>))}
                {(testResults.bodyParts?.length || 0) > 8 && (<span className="text-xs text-gray-500">
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              ...
              <div className="flex flex-wrap gap-2">
                {testResults.equipment?.map((equip, i) => (<span key={i} className="bg-green-100 text-green-800 text-xs px-2 py-1 rounded">
                    {equip}
                  </span>))}
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              ...
              <ul className="text-sm text-gray-600 space-y-1">
                {testResults.searchResults?.results?.map((result, i) => (<li key={i}>• {result}</li>))}
              </ul>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              ...
              <ul className="text-sm text-gray-600 space-y-1">
                {testResults.filteredResults?.results?.map((result, i) => (<li key={i}>• {result}</li>))}
              </ul>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              ...
              <div className="space-y-3">
                {testResults.recommendations?.sample?.map((rec, i) => (<div key={i} className="bg-yellow-50 p-3 rounded border">
