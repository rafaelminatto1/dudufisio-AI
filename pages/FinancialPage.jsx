...
import { PlusCircle, DollarSign, TrendingUp, TrendingDown, CreditCard, Banknote, Receipt, FileText, Download, AlertTriangle, CheckCircle, RefreshCw, Settings, PieChart as PieChartIcon, BarChart3, Target, ArrowUpRight, ArrowDownRight, Activity, Bell } from 'lucide-react';
import PageHeader from '../components/PageHeader';
import TransactionFormModal from '../components/financial/TransactionFormModal';
import TransactionList from '../components/financial/TransactionList';
import * as financialService from '../services/financialService';
// import { auditHelpers } from '../services/auditService'; // TODO: Implementar auditHelpers
...
const FinancialDashboardPage = () => {
    ...
    const [transactionToEdit, setTransactionToEdit] = useState(undefined);
    ...
    const [cashFlowPredictions, setCashFlowPredictions] = useState([]);
    ...
    useEffect(() => {
        if (data && !isLoading) {
            // TODO: auditHelpers.logFinancialOperation('Current User', 'CREATE_TRANSACTION', 'dashboard', data.kpis.grossRevenue, `Visualizou dashboard financeiro - Período: ${period}`);
        }
    ...
    const loadRecurrentPayments = async () => {
        const payments = [
            {
                id: '1',
    ...
    const handleSaveExpense = async (expense) => {
        try {
            if (expense.id) {
                await financialService.updateExpense(expense);
                // Log de auditoria para atualização
                // TODO: await auditHelpers.logFinancialOperation('Current User', 'UPDATE_TRANSACTION', expense.id, expense.amount || 0, `Transação atualizada: ${expense.description}`);
            }
            else {
                ...
                // TODO: await auditHelpers.logFinancialOperation('Current User', 'CREATE_TRANSACTION', expenseId, expense.amount || 0, `Nova transação: ${expense.description}`);
            }
            refetch();
            handleCloseModal();
        }
    ...
    const handleDeleteExpense = async (id) => {
        try {
            ...
            // TODO: await auditHelpers.logFinancialOperation('Current User', 'DELETE_TRANSACTION', id, expense?.amount || 0, `Transação excluída: ${expense?.description || 'Descrição não disponível'}`);
            refetch();
            handleCloseModal();
        }
        catch (error) {
    ...
    return (<PermissionGuard permission="financial:read">
      <div className="p-6 max-w-7xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <PageHeader title="Gestão Financeira" subtitle="Dashboard financeiro completo com análises avançadas e gestão de pagamentos"/>

          <div className="flex items-center gap-3">
            <select value={period} onChange={(e) => setPeriod(e.target.value)} className="rounded-lg border-gray-300 bg-white shadow-sm focus:ring-sky-500 focus:border-sky-500">
              <option value="this_month">Este Mês</option>
              <option value="last_3_months">Últimos 3 Meses</option>
              <option value="this_year">Este Ano</option>
            </select>
            <button onClick={() => handleOpenModal()} className="flex items-center bg-sky-600 hover:bg-sky-700 text-white font-medium py-2 px-4 rounded-lg shadow-sm transition-colors">
        ...
        <TransactionFormModal isOpen={isModalOpen} onClose={handleCloseModal} onSave={handleSaveExpense} onDelete={handleDeleteExpense} transactionToEdit={transactionToEdit}/>

        {/* Financial Alerts */}
        {financialAlerts.length > 0 && (<div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
            <h3 className="text-lg font-semibold text-gray-900 mb-3 flex items-center gap-2">
              <Bell className="w-5 h-5 text-sky-600"/>
              Alertas Financeiros
            </h3>
            <div className="space-y-3">
              {financialAlerts.map((alert) => (<div key={alert.id} className={`border rounded-lg p-3 ${getSeverityColor(alert.severity)}`}>
                  <div className="flex items-start gap-3">
                    {getAlertIcon(alert.type)}
                    <div className="flex-1">
                      <h4 className="font-medium">{alert.title}</h4>
                      <p className="text-sm mt-1">{alert.message}</p>
                      {alert.actionable && alert.action && (<p className="text-xs mt-2 font-medium">💡 {alert.action}</p>)}
                    </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          ...
          <TabsContent value="overview" className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-200">
                ...
                <div className="p-6">
                  <ResponsiveContainer width="100%" height={300}>
                    <PieChart>
                      <Pie data={data.expenseBreakdown} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={60} outerRadius={100} paddingAngle={5}>
            ...
            <TransactionList transactions={transactions} onAddExpense={() => handleOpenModal()} onEditExpense={handleOpenModal}/>
          </TabsContent>
          ...
          <TabsContent value="payments" className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-200">
                ...
                <div className="p-6">
                  <div className="space-y-4">
                    {paymentMethods.map((method) => (<div key={method.id} className="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
                        <div className="flex items-center gap-3">
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-200">
                ...
                <div className="p-6">
                  <div className="space-y-4">
                    {recurrentPayments.map((payment) => (<div key={payment.id} className="border border-gray-200 rounded-lg p-4">
          ...
          <TabsContent value="goals" className="space-y-6">
            ...
            return (<div key={goal.id} className="border border-gray-200 rounded-lg p-6">
                        <div className="flex items-center justify-between mb-4">
                          ...
                          <span className={`px-3 py-1 rounded-full text-sm font-medium ${goal.priority === 'high' ? 'bg-red-100 text-red-700' :
                    ...
                    `${formatCurrency(goal.current)} / ${formatCurrency(goal.target)}`}
                          ...
                          <div className="w-full bg-gray-200 rounded-full h-3">
                            <div className={`h-3 rounded-full transition-all duration-500 ${progress >= 100 ? 'bg-green-500' :
                    progress >= 75 ? 'bg-blue-500' :
                        progress >= 50 ? 'bg-yellow-500' :
                            'bg-red-500'}`} style={{ width: `${progress}%` }}/>
                          </div>
          ...
          <TabsContent value="predictions" className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {cashFlowPredictions.map((prediction, index) => (<div key={index} className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
