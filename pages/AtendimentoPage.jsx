...
const AtendimentoPage = () => {
    const { appointmentId } = ReactRouterDOM.useParams();
    ...
    const [currentPainPoint, setCurrentPainPoint] = useState(null);
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(async () => {
            ...
            const formattedMetricResults = Object.entries(metricResults)
    ...
    const handleGenerateSuggestion = async () => {
        if ((!subjective.trim() && !objective.trim()) || isAiLoading)
            return;
        setIsAiLoading(true);
        ...
        try {
            ...
            if (assessmentMatch?.[1])
                setAssessment(assessmentMatch[1].trim());
            if (planMatch?.[1])
                setPlanState(planMatch[1].trim());
    ...
    const handleSavePainPoint = () => {
        ...
        if (currentPainPoint.observation.trim() === '') {
            ...
            setPainPoints(prev => {
                ...
                if (existing)
                    return prev.map(p => p.part === currentPainPoint.part ? currentPainPoint : p);
    ...
    const handleDeletePainPoint = (part) => {
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
                    <div className="lg:col-span-1 space-y-6">
                        {treatmentPlan && (<InfoCard title="Plano de Tratamento" icon={<Target />}>
                        ...
                        {previousNote && (<InfoCard title={`Última Evolução (${previousNote.date})`} icon={<FileText />}>
                    ...
                    <div className="lg:col-span-2 bg-white p-6 rounded-2xl shadow-sm space-y-4">
                        ...
                        {activeMetrics.length > 0 && (<div className="p-4 bg-slate-50 rounded-lg">
                                <h3 className="text-sm font-semibold text-teal-700 mb-2">Métricas de Acompanhamento</h3>
                                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                                    {activeMetrics.map(metric => (<div key={metric.id}>
                                            <label className="text-xs font-medium text-slate-600">{metric.name}</label>
                                            <div className="relative">
                                                <input type="number" value={metricResults[metric.id] ?? ''} onChange={e => handleMetricChange(metric.id, e.target.value)} className="mt-1 w-full p-2 pr-10 border border-slate-300 rounded-lg" aria-label={`Valor para ${metric.name}`} placeholder="0"/>
