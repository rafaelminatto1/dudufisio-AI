...
const TeleconsultaPage = () => {
    ...
    const [sessionNote, setSessionNote] = useState({ subjective: '', objective: '', assessment: '', plan: '' });
    ...
    useEffect(() => {
        const saveNote = async () => {
            if (!patient || !appointment || !debouncedNote)
                return;
    ...
    return (<div className="flex h-full bg-slate-900 text-white -m-8">
      {/* Main Content */}
      <main className="flex-1 flex flex-col p-4 overflow-hidden">
        <header className="flex justify-between items-center mb-4">
            <div className="flex items-center gap-4">
                <h1 className="text-xl font-bold truncate">Teleconsulta: {patient.name}</h1>
                <ConnectionStatus isConnected={!!localStream} quality={connectionQuality} bitrate={bitrate} latency={latency}/>
            </div>
            <div className="flex items-center gap-4">
        ...
        <SharedContentDisplay patientStream={isScreenSharing ? screenStream : localStream} therapistStream={localStream} isTherapistCameraOn={isCameraOn} sharedContent={sharedContent} isScreenSharing={isScreenSharing}/>

        <footer className="mt-4">
            <ControlBar isMicOn={isMicOn} isCameraOn={isCameraOn} isScreenSharing={isScreenSharing} onToggleMic={toggleMic} onToggleCamera={toggleCamera} onToggleScreenShare={toggleScreenShare} onEndCall={handleEndCall}/>
        </footer>
      </main>

      {/* Toolbar */}
      <aside className="w-96 bg-slate-800 border-l border-slate-700 flex flex-col">
        <TeleconsultaToolbar patient={patient} onShareContent={setSharedContent} sessionNote={sessionNote} onNoteChange={setSessionNote} isSaving={isSaving}/>
      </aside>
    </div>);
};
