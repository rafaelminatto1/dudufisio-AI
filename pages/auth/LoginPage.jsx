...
import { Stethoscope, Loader, Eye, EyeOff, Mail, Lock, AlertCircle, Github } from 'lucide-react';
      ...
      <div className="w-full max-w-md space-y-6">
        ...
        <Card className="shadow-xl border-0">
          ...
          <CardContent className="space-y-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input id="email" name="email" type="email" value={formData.email} onChange={handleInputChange} placeholder="seu@email.com" className="pl-10" autoComplete="email" data-testid="login-email" required/>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input id="password" name="password" type={showPassword ? 'text' : 'password'} value={formData.password} onChange={handleInputChange} placeholder="••••••••" className="pl-10 pr-10" autoComplete="current-password" data-testid="login-password" required/>
              ...
              <Button type="submit" className="w-full" disabled={isLoading || loading} data-testid="login-submit">
                {isLoading || loading ? (<>
            ...
            {/* Demo Toggle */}
            <Button type="button" variant="ghost" onClick={() => setShowDemo(!showDemo)} className="w-full text-sm">
              {showDemo ? 'Ocultar' : 'Ver'} contas de demonstração
            </Button>

            {/* Demo Accounts */}
            {showDemo && (<div className="space-y-2 p-4 bg-gray-50 rounded-lg">
                <p className="text-sm font-medium text-gray-700 mb-3">
                  Contas de Demonstração:
                </p>
                {demoAccounts.map((account, index) => (<button key={index} onClick={() => handleDemoLogin(account.email)} className="w-full text-left p-3 bg-white rounded border hover:bg-gray-50 transition-colors">
          ...
          <CardFooter className="flex flex-col space-y-2">
            ...
            <div className="text-sm text-center text-gray-600">
              Não tem uma conta?{' '}
              <button className="text-indigo-600 hover:text-indigo-700 font-medium" onClick={() => {
            // Navigate to register page - this will be handled by the parent component
            if (window.__navigateToRegister) {
                window.__navigateToRegister();
            }
        }}>
                Criar conta
              </button>
