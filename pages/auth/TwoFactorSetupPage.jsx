...
import { Shield, Smartphone, Copy, CheckCircle, AlertTriangle, ArrowLeft, Download, Key } from 'lucide-react';
import { useSupabaseAuth } from '../../contexts/SupabaseAuthContext';
import { Button } from '../../components/ui/button';
import { Input } from '../../components/ui/input';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '../../components/ui/card';
import { Alert, AlertDescription } from '../../components/ui/alert';
const TwoFactorSetupPage = ({ onComplete, onBack }) => {
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 flex items-center justify-center p-4">
      <div className="w-full max-w-md space-y-6">
        ...
        {/* Setup Step */}
        {step === 'setup' && (<Card className="shadow-xl border-0">
            <CardHeader>
        ...
        {/* Verify Step */}
        {step === 'verify' && (<Card className="shadow-xl border-0">
            <CardHeader>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input id="verification-code" type="text" value={verificationCode} onChange={(e) => setVerificationCode(e.target.value.replace(/\D/g, '').slice(0, 6))} placeholder="000000" className="text-center text-lg font-mono tracking-widest" maxLength={6}/>
              </div>
              ...
              <div className="flex space-x-3">
                ...
                <Button onClick={handleVerification} disabled={verificationCode.length !== 6 || isLoading} className="flex-1">
        ...
        {/* Complete Step */}
        {step === 'complete' && (<Card className="shadow-xl border-0">
            <CardHeader>
            ...
            <CardContent className="space-y-4">
              {/* Backup Codes */}
              {backupCodes.length > 0 && (<div className="space-y-3">
                  ...
                  <div className="bg-gray-50 p-3 rounded-lg">
                    <div className="grid grid-cols-2 gap-2 text-sm font-mono">
                      {backupCodes.map((code, index) => (<div key={index} className="p-2 bg-white rounded border">
                          {code}
                        </div>))}
                    </div>
                  </div>
        ...
        {/* Back Button */}
        {step === 'setup' && onBack && (<Button type="button" variant="ghost" onClick={onBack} className="w-full">
            <ArrowLeft className="w-4 h-4 mr-2"/>
            Voltar para Login
          </Button>)}
      </div>
