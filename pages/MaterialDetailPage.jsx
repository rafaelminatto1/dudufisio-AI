...
const mapCategoryToType = (categoryName) => {
    if (categoryName.includes('Avaliação'))
        return 'Escala de Avaliação';
    if (categoryName.includes('Protocolo'))
        return 'Protocolo Clínico';
...
const MaterialDetailPage = () => {
    ...
    const renderLoading = () => (<div className="bg-white p-8 rounded-2xl shadow-sm animate-pulse">
    ...
    return (<>
            <PageHeader title={pageTitle} subtitle="Conteúdo detalhado do material da biblioteca clínica.">
                <ReactRouterDOM.Link to="/materials" className="inline-flex items-center rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 shadow-sm hover:bg-slate-50">
                    <ChevronLeft className="-ml-1 mr-2 h-5 w-5"/>
                    Voltar para Biblioteca
                </ReactRouterDOM.Link>
            </PageHeader>
            {isLoading ? renderLoading() : (<div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm">
