...
import { LineChart, Line, BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, Cell, PieChart, Pie, AreaChart, Area, RadialBarChart, RadialBar } from 'recharts';
import PageHeader from '../components/PageHeader';
import { TrendingDown, Activity, Users, Brain, Target, Clock, Stethoscope, BarChart3, Download, RefreshCw, AlertTriangle, CheckCircle2, Star, Eye, Settings, FileText, ArrowUpRight } from 'lucide-react';
...
const ClinicalAnalyticsPage = () => {
    ...
    const [patientSegmentation, setPatientSegmentation] = useState([]);
    ...
    const loadClinicalMetrics = async () => {
        const metrics = [
            {
                id: '1',
    ...
    const loadPatientSegmentation = async () => {
        const segmentation = [
            {
    ...
    return (<PermissionGuard permission="clinical_analytics:read">
      <div className="p-6 max-w-7xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <PageHeader title="Analytics Clínicos" subtitle="Análises avançadas de performance clínica e outcomes de tratamentos"/>

          <div className="flex items-center gap-3">
            <select value={timeRange} onChange={(e) => setTimeRange(e.target.value)} className="rounded-lg border-gray-300 bg-white shadow-sm focus:ring-sky-500 focus:border-sky-500">
        ...
        {/* Clinical Alerts */}
        {clinicalAlerts.length > 0 && (<div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
            <h3 className="text-lg font-semibold text-gray-900 mb-3 flex items-center gap-2">
              <AlertTriangle className="w-5 h-5 text-orange-600"/>
              Alertas Clínicos
            </h3>
            <div className="space-y-3">
              {clinicalAlerts.map((alert) => (<div key={alert.id} className={`border rounded-lg p-3 ${getSeverityColor(alert.severity)}`}>
                  <div className="flex items-start gap-3">
                    {getAlertIcon(alert.type)}
                    <div className="flex-1">
                      <h4 className="font-medium">{alert.title}</h4>
                      <p className="text-sm mt-1">{alert.message}</p>
                      {alert.actionRequired && (<button className="text-xs mt-2 px-2 py-1 bg-white rounded border font-medium hover:bg-gray-50">
                          Ação Necessária
                        </button>)}
                    </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {clinicalMetrics.map((metric) => (<div key={metric.id} className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <div className="flex items-center justify-between mb-4">
                <div className={`p-3 rounded-lg ${metric.category === 'outcome' ? 'bg-green-100' :
              ...
              <div className="flex items-center justify-between mt-3 text-sm">
                <span className={`flex items-center gap-1 ${metric.change > 0 ? 'text-green-600' : metric.change < 0 ? 'text-red-600' : 'text-gray-600'}`}>
                  {metric.change > 0 ? '+' : ''}{metric.change}
                  {metric.unit === '%' ? 'pp' : metric.unit}
                </span>
                {metric.benchmark && (<span className="text-gray-500">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-200">
                ...
                <div className="p-6">
                  <ResponsiveContainer width="100%" height={300}>
                    <PieChart>
                      <Pie data={patientSegmentation} dataKey="percentage" nameKey="segment" cx="50%" cy="50%" outerRadius={100} label={({ name, percentage }) => `${name}: ${percentage}%`}>
                        {patientSegmentation.map((entry, index) => (<Cell key={`cell-${index}`} fill={['#3b82f6', '#10b981', '#f59e0b', '#ef4444'][index]}/>))}
                      </Pie>
                      <Tooltip />
                    </PieChart>
                  </ResponsiveContainer>
            ...
            { name: 'Ruim', value: 5, fill: '#ef4444' }
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-200">
                ...
                <div className="p-6 space-y-4">
                  {clinicalMetrics.slice(0, 4).map((metric) => (<div key={metric.id}>
                      <div className="flex justify-between text-sm mb-1">
                        <span>{metric.name}</span>
                        <span>{metric.value}{metric.unit}</span>
                      </div>
                      <div className="w-full bg-gray-200 rounded-full h-2">
                        <div className={`h-2 rounded-full ${metric.value >= (metric.benchmark || 0) ? 'bg-green-500' : 'bg-yellow-500'}`} style={{ width: `${Math.min((metric.value / (metric.benchmark || 100)) * 100, 100)}%` }}/>
                      </div>
                      {metric.benchmark && (<div className="text-xs text-gray-500 mt-1">
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200">
              ...
              <div className="p-6">
                <div className="space-y-6">
                  {treatmentOutcomes.map((protocol) => (<div key={protocol.protocolId} className="border border-gray-200 rounded-lg p-6">
                      <div className="flex items-center justify-between mb-4">
                        <div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`px-3 py-1 rounded-full text-sm font-medium ${protocol.successRate >= 90 ? 'bg-green-100 text-green-700' :
                    ...
                    protocol.successRate >= 70 ? 'bg-yellow-100 text-yellow-700' :
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          ...
                          <div className="w-full bg-gray-200 rounded-full h-2">
                            <div className="bg-green-500 h-2 rounded-full" style={{ width: `${protocol.successRate}%` }}/>
                          </div>
                        </div>
                        <div>
                          <div className="flex justify-between text-sm mb-1">
                          ...
                          <div className="w-full bg-gray-200 rounded-full h-2">
                            <div className="bg-red-500 h-2 rounded-full" style={{ width: `${protocol.complicationRate * 10}%` }}/>
                          </div>
                        </div>
                      </div>
                    </div>))}
          ...
          <TabsContent value="segmentation" className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {patientSegmentation.map((segment, index) => (<div key={index} className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                  <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="mb-4">
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-3">
                      <div className={`h-3 rounded-full ${segment.averageOutcome >= 90 ? 'bg-green-500' :
                segment.averageOutcome >= 80 ? 'bg-blue-500' :
                    segment.averageOutcome >= 70 ? 'bg-yellow-500' :
                        'bg-red-500'}`} style={{ width: `${segment.averageOutcome}%` }}/>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <ul className="space-y-1">
                      {segment.characteristics.map((char, idx) => (<li key={idx} className="text-sm text-gray-600 flex items-center gap-2">
