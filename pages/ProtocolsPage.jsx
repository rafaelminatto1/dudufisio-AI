...
import { BookOpen, Search, Eye, Edit, Star, TrendingUp, Users, Clock, Award, BarChart3, FileText, X, CheckCircle, AlertCircle, Activity, Zap, Stethoscope, Heart, Brain, Baby, Dumbbell } from 'lucide-react';
const ProtocolsPage = () => {
    // State management
    ...
    const fetchData = useCallback(async () => {
        setIsLoading(true);
        try {
            const [protocolsData, analyticsData, statsData, prescriptionsData, toolsData, metricsData] = await Promise.all([
    ...
    const renderLibrary = () => (<div className="space-y-6">
            {/* Header with stats */}
            {libraryStats && (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <Card>
                ...
                <CardContent className="pt-6">
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                        <div>
                            ...
                            <select id="category" className="w-full mt-1 p-2 border rounded-md" value={protocolFilters.category || ''} onChange={(e) => setProtocolFilters(prev => ({
            ...prev,
            category: e.target.value || undefined
                        ...
                        <div>
                            ...
                            <select id="evidence" className="w-full mt-1 p-2 border rounded-md" value={protocolFilters.evidenceLevel || ''} onChange={(e) => setProtocolFilters(prev => ({
            ...prev,
            evidenceLevel: e.target.value || undefined
                        ...
                        <div>
                            ...
                            <select id="status" className="w-full mt-1 p-2 border rounded-md" value={protocolFilters.isActive?.toString() || ''} onChange={(e) => setProtocolFilters(prev => ({
            ...prev,
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {protocols
            .filter(protocol => {
            if (protocolFilters.category && protocol.category !== protocolFilters.category)
                return false;
            if (protocolFilters.evidenceLevel && protocol.evidenceLevel !== protocolFilters.evidenceLevel)
                return false;
            if (protocolFilters.isActive !== undefined && protocol.isActive !== protocolFilters.isActive)
                return false;
    ...
    const renderEvidence = () => (<div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <Card>
                    ...
                    <CardContent>
                        {libraryStats && (<div className="space-y-3">
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="space-y-3">
                            {libraryStats?.recentlyUpdated.slice(0, 5).map((protocol) => (<div key={protocol.id} className="flex items-center justify-between">
                                    <div>
    ...
    if (isLoading) {
        return (<div className="space-y-6">
                <PageHeader title="Protocolos Clínicos" subtitle="Carregando protocolos..."/>
                <Skeleton className="h-96 w-full"/>
            </div>);
    }
    return (<div className="space-y-6">
            <PageHeader title="Protocolos Clínicos" subtitle="Biblioteca completa de protocolos baseados em evidências para fisioterapia"/>
