...
const ExerciseLibraryPage = () => {
    ...
    const handleSaveGroup = async (newName) => {
        ...
        else if (groupModalState.mode === 'edit' && groupModalState.name)
            await updateCategory(groupModalState.name, newName);
        else if (groupModalState.mode === 'copy' && groupModalState.name)
            await copyCategory(groupModalState.name, newName);
    ...
    return (<>
      <PageHeader title="Biblioteca de Exercícios" subtitle="Gerencie os exercícios utilizados nas prescrições da clínica.">
      ...
      {isLoading ? renderSkeleton() : (<div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                ...
                return (<div key={category} className="bg-white rounded-2xl shadow-sm transition-shadow hover:shadow-md">
                  ...
                  {isOpen && (<div className="p-4 border-t border-slate-200">
                      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                        {categoryExercises.map(ex => (<ExerciseCard key={ex.id} exercise={ex} onEdit={() => handleOpenExerciseModal(ex)} onDelete={() => handleDeleteExercise(ex)} onPlay={() => ex.video_url && setPlayingVideo({ url: ex.video_url, title: ex.name })}/>))}
                      </div>
                    </div>)}
                </div>);
            })}
