...
const MetricDisplay = ({ label, value, icon }) => (<div className="bg-slate-50 p-3 rounded-lg flex items-start border border-slate-200">
...
const RiskBadge = ({ level }) => {
    ...
    return (<div className={`mb-4 p-3 rounded-lg border text-center ${info.color}`}>
...
const RiskAnalysisPage = () => {
    ...
    return (<>
            <PageHeader title="Análise de Risco de Abandono" subtitle="Identifique pacientes em risco e receba sugestões para aumentar a retenção."/>
                    ...
                    <div className="relative" ref={searchRef}>
                        <label htmlFor="patient-search" className="block text-sm font-medium text-slate-700 mb-1">Selecionar Paciente*</label>
                         <div className="relative">
                            <input id="patient-search" type="text" value={searchTerm} onChange={(e) => {
            setSearchTerm(e.target.value);
            if (selectedPatientId)
                setSelectedPatientId('');
            setShowDropdown(true);
        }} onFocus={() => setShowDropdown(true)} placeholder="-- Selecione um paciente --" className="w-full p-2 pl-10 border border-slate-300 rounded-lg bg-white" autoComplete="off"/>
                            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400 pointer-events-none"/>
                        </div>
                        {showDropdown && (<div className="absolute z-10 w-full mt-1 bg-white border border-slate-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                                {filteredPatients.length > 0 ? (filteredPatients.map(p => (<div key={p.id} onClick={() => {
                    setSelectedPatientId(p.id);
                    setSearchTerm(p.name);
                    setShowDropdown(false);
                }} className="px-4 py-2 text-slate-800 hover:bg-sky-100 cursor-pointer">
                                            {p.name}
                                        </div>))) : (<div className="px-4 py-2 text-slate-500">Nenhum paciente encontrado.</div>)}
                            </div>)}
                    </div>

                    {isFetchingData ? <Skeleton className="h-64 w-full"/> : selectedPatientId && (<div className="space-y-4 animate-fade-in-fast">
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm">
                    ...
                    <div className="bg-slate-50 p-6 rounded-lg min-h-[500px] overflow-y-auto border border-slate-200">
                        ...
                        {!isLoading && !analysisResult && (<div className="text-center text-slate-500 flex flex-col justify-center items-center h-full">
