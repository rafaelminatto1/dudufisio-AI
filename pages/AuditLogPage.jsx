...
const getActionBadge = (action) => {
    ...
    if (lowerAction.includes('login_success'))
        return 'bg-green-100 text-green-800';
    if (lowerAction.includes('failed'))
        return 'bg-yellow-100 text-yellow-800';
    if (lowerAction.includes('view'))
        return 'bg-blue-100 text-blue-800';
    if (lowerAction.includes('security'))
        return 'bg-orange-100 text-orange-800';
    return 'bg-slate-100 text-slate-800';
};
// 🚀 Componente LogRow memoizado
const LogRow = memo(({ log }) => (<tr className="border-b border-slate-200">
...
const AuditLogPage = () => {
    ...
    const renderContent = () => {
        if (isLoading) {
            return Array.from({ length: 5 }).map((_, i) => (<tr key={i}><td colSpan={4} className="p-4"><Skeleton className="h-10 w-full"/></td></tr>));
        }
        if (error) {
    ...
    return (<>
            <PageHeader title="Trilha de Auditoria" subtitle="Monitore todas as atividades importantes realizadas no sistema."/>

            <div className="bg-white p-4 rounded-t-2xl shadow-sm border-b border-slate-200">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="relative">
                        <UserIcon className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400"/>
                        <input type="text" name="user" placeholder="Buscar por usuário ou detalhes..." value={filters.user} onChange={handleFilterChange} className="w-full pl-10 pr-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500"/>
                    </div>
                     <div className="relative">
                        <TypeIcon className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400"/>
                        <select name="action" value={filters.action} onChange={handleFilterChange} className="appearance-none w-full pl-10 pr-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 bg-white">
                            {uniqueActions.map(action => (<option key={action} value={action}>{action === 'All' ? 'Todas as Ações' : action}</option>))}
                        </select>
                    </div>
                    <div className="relative">
                        <CalendarIcon className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400"/>
                        <input type="date" name="date" value={filters.date} onChange={handleFilterChange} className="w-full pl-10 pr-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500"/>
                    </div>
                </div>
            </div>
            ...
            <Pagination currentPage={currentPage} totalItems={totalLogs} itemsPerPage={LOGS_PER_PAGE} onPageChange={setCurrentPage}/>
        </>);
};
