...
import { ArrowLeft, Plus, Activity, Brain, Heart, AlertTriangle, TrendingUp, Shield } from 'lucide-react';
import { useGeriatricAssessments, useLatestGeriatricAssessment, useCreateGeriatricAssessment, useFallPreventionPlans } from '../hooks/useGeriatricCare';
import { useRealtimeSubscription } from '../hooks/useRealtimeSubscription';
import { geriatricKeys } from '../hooks/useGeriatricCare';
export const GeriatricAssessmentPage = () => {
    ...
    return (<div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="mb-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => setShowNewAssessment(true)} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
            <Plus size={20}/>
            Nova Avaliação
          </button>
        </div>
      ...
      {/* Quick Stats */}
      {latestAssessment && (<div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
      ...
      <div className="bg-white rounded-lg shadow mb-6">
        ...
        <div className="p-6">
          {assessments && assessments.length > 0 ? (<div className="space-y-4">
              {assessments.map((assessment) => (<div key={assessment.id} className="border rounded-lg p-4 hover:shadow-md transition-shadow">
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <div className="flex gap-2">
                      {assessment.fall_risk_level === 'high_risk' && (<span className="px-3 py-1 bg-red-100 text-red-700 rounded-full text-sm font-medium">
                          Alto Risco
                        </span>)}
                      {assessment.cognitive_status === 'moderate_impairment' && (<span className="px-3 py-1 bg-orange-100 text-orange-700 rounded-full text-sm font-medium">
                          Déficit Cognitivo
                        </span>)}
                    </div>
                  </div>
                  ...
                  {assessment.recommendations && assessment.recommendations.length > 0 && (<div className="mt-3 p-3 bg-blue-50 rounded">
              ...
              <button onClick={() => setShowNewAssessment(true)} className="mt-4 text-blue-600 hover:text-blue-700 font-medium">
      ...
      {fallPlans && fallPlans.length > 0 && (<div className="bg-white rounded-lg shadow">
          <div className="p-6 border-b">
            <h2 className="text-xl font-semibold flex items-center gap-2">
              <Shield size={24}/>
          ...
          <div className="p-6">
            <div className="space-y-3">
              {fallPlans.map((plan) => (<div key={plan.id} className="border rounded-lg p-4">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    {plan.effectiveness_rating && (<div className="text-right">
                  ...
                  <div className="text-sm">
                    ...
                    {plan.environmental_modifications && plan.environmental_modifications.length > 0 && (<div className="mt-2">
                        <strong>Modificações Ambientais:</strong>
                        <ul className="list-disc list-inside ml-2">
                          {plan.environmental_modifications.map((mod, idx) => (<li key={idx}>{mod}</li>))}
                        </ul>
                      </div>)}

                    {plan.assistive_devices && plan.assistive_devices.length > 0 && (<div className="mt-2">
                        <strong>Dispositivos Assistivos:</strong>
                        <ul className="list-disc list-inside ml-2">
                          {plan.assistive_devices.map((device, idx) => (<li key={idx}>{device}</li>))}
                        </ul>
                      </div>)}
                  </div>
                </div>))}
      ...
      {showNewAssessment && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={(e) => {
                ...
                createMutation.mutate({
                    ...
                    assessment_date: formData.get('date'),
                    assessed_by: formData.get('assessor'),
                    morse_score: Number(formData.get('morse')),
                    berg_score: Number(formData.get('berg')),
                    meem_score: Number(formData.get('meem')),
                    overall_assessment: formData.get('assessment'),
                    intervention_plan: formData.get('plan'),
                    recommendations: [],
                });
              ...
              <div>
                ...
                <input type="date" name="date" required defaultValue={new Date().toISOString().split('T')[0]} className="w-full border rounded-lg p-2"/>
              </div>

              <div>
                <label className="block text-sm font-medium mb-1">Avaliado por</label>
                <input type="text" name="assessor" required className="w-full border rounded-lg p-2"/>
              </div>
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  ...
                  <input type="number" name="morse" min="0" max="125" className="w-full border rounded-lg p-2"/>
                </div>
                ...
                <div>
                  ...
                  <input type="number" name="berg" min="0" max="56" className="w-full border rounded-lg p-2"/>
                </div>
                ...
                <div>
                  ...
                  <input type="number" name="meem" min="0" max="30" className="w-full border rounded-lg p-2"/>
                </div>
              </div>

              <div>
                <label className="block text-sm font-medium mb-1">Avaliação Geral</label>
                <textarea name="assessment" required rows={3} className="w-full border rounded-lg p-2" placeholder="Descreva a avaliação geral do paciente..."/>
              </div>

              <div>
                <label className="block text-sm font-medium mb-1">Plano de Intervenção</label>
                <textarea name="plan" required rows={3} className="w-full border rounded-lg p-2" placeholder="Descreva o plano de intervenção..."/>
              </div>

              <div className="flex gap-3 justify-end">
                <button type="button" onClick={() => setShowNewAssessment(false)} className="px-4 py-2 border rounded-lg hover:bg-gray-50">
                  Cancelar
                </button>
                <button type="submit" disabled={createMutation.isPending} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:bg-gray-400">
