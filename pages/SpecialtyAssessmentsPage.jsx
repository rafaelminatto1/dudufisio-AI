...
const SpecialtyAssessmentsPage = () => {
    ...
    const renderContent = () => {
        ...
        if (!selectedPatientId) {
            return (<div className="text-center p-10 bg-white rounded-2xl shadow-sm">
        ...
        return (<SpecialtyAssessmentGallery onSelectSpecialty={(specialty) => setSelectedSpecialty(specialty)}/>);
    };
    ...
    return (<>
            <PageHeader title={pageTitle} subtitle={pageSubtitle}>
                {selectedSpecialty && (<button onClick={handleBack} className="inline-flex items-center rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 shadow-sm hover:bg-slate-50 mr-3">
                        <ChevronLeft className="-ml-1 mr-2 h-5 w-5"/>
                        Voltar para Galeria
                    </button>)}
            </PageHeader>
            
            {!selectedSpecialty && (<div className="mb-6 bg-white p-4 rounded-2xl shadow-sm">
                    <label htmlFor="patient-select" className="block text-sm font-medium text-slate-700 mb-1">Paciente</label>
                    <select id="patient-select" value={selectedPatientId} onChange={(e) => setSelectedPatientId(e.target.value)} className="w-full max-w-sm p-2 border border-slate-300 rounded-lg bg-white" disabled={isLoadingPatients}>
