...
export const FamilyPortalPage = () => {
    ...
    const loadData = async () => {
        try {
            ...
            if (members.length > 0) {
                ...
                const progressReports = await familyPortalServiceSupabase.getProgressReports(patientId, members[0].id);
    ...
    const handleSendMessage = async () => {
        if (!selectedMember || !patientId)
            return;
        const message = prompt('Digite sua mensagem para o terapeuta:');
        if (!message)
            return;
        try {
            await familyPortalServiceSupabase.sendMessageToTherapist(selectedMember.id, patientId, message);
    ...
    return (<div className="min-h-screen bg-gray-50">
      ...
      {/* Content */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {familyMembers.length === 0 ? (<div className="bg-white rounded-lg shadow-md p-12 text-center">
            ...
            <div className="bg-white rounded-lg shadow-md p-6">
              ...
              <div className="space-y-3">
                {familyMembers.map((member) => (<div key={member.id} className={`p-4 rounded-lg border-2 cursor-pointer transition ${selectedMember?.id === member.id
                    ? 'border-purple-500 bg-purple-50'
                    : 'border-gray-200 hover:border-purple-300'}`} onClick={() => setSelectedMember(member)}>
                    <div className="flex items-start justify-between">
                      ...
                      {member.isPrimaryContact && (<span className="text-xs bg-purple-100 text-purple-700 px-2 py-1 rounded">
                          Principal
                        </span>)}
                    </div>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      {member.permissions.canViewMedicalRecords && (<span className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded">
                          <FileText className="w-3 h-3 inline mr-1"/>
                          Registros
                        </span>)}
                      {member.permissions.canScheduleAppointments && (<span className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded">
                          <Calendar className="w-3 h-3 inline mr-1"/>
                          Agendar
                        </span>)}
                      {member.permissions.canMessageTherapist && (<span className="text-xs bg-orange-100 text-orange-700 px-2 py-1 rounded">
            ...
            <div className="lg:col-span-2 bg-white rounded-lg shadow-md p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                {selectedMember?.permissions.canMessageTherapist && (<button onClick={handleSendMessage} className="flex items-center gap-2 px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition">
                    <Mail className="w-4 h-4"/>
                    Mensagem
                  </button>)}
              </div>

              {reports.length > 0 ? (<div className="space-y-4">
                  {reports.map((report) => (<div key={report.reportId} className="border border-gray-200 rounded-lg p-4 hover:border-blue-300 transition">
                      <div className="flex items-start justify-between mb-2">
                        ...
                        <span className={`text-xs px-2 py-1 rounded ${report.painLevelChange < 0 ? 'bg-green-100 text-green-700' :
