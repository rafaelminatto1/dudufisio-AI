...
const FilterCheckbox = memo(({ id, label, checked, onChange }) => (<div className="flex items-center">
        <input id={id} type="checkbox" checked={checked} onChange={(e) => onChange(e.target.checked)} className="h-4 w-4 rounded border-gray-300 text-teal-600 focus:ring-teal-500"/>
        <label htmlFor={id} className="ml-3 text-sm text-slate-600">
            {label}
        </label>
    </div>));
FilterCheckbox.displayName = 'FilterCheckbox';
const EnhancedExerciseLibraryPage = () => {
    ...
    const [groupModalState, setGroupModalState] = useState({ mode: 'create' });
    ...
    const handleOpenGroupModal = (mode, name) => {
        setGroupModalState({ mode, ...(name && { name }) });
        setIsGroupModalOpen(true);
    };
    const handleSaveGroup = async (newName) => {
    ...
    const renderSkeleton = () => (<div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        <Skeleton className="lg:col-span-1 h-96 rounded-2xl"/>
        <div className="lg:col-span-3 space-y-4">
             {Array.from({ length: 3 }).map((_, i) => (<div key={i} className="bg-white p-4 rounded-2xl shadow-sm">
    ...
    return (<>
      <PageHeader title="Biblioteca de Exercícios Integrada" subtitle="Exercícios do sistema + conteúdo clínico especializado com vinculação a protocolos.">
      ...
      <VideoPlayerModal isOpen={!!playingVideo} onClose={() => setPlayingVideo(null)} videoUrl={playingVideo?.url} title={playingVideo?.title}/>

      {loading ? renderSkeleton() : (<div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          <aside className="lg:col-span-1 space-y-6">
            <div className="bg-white p-4 rounded-2xl shadow-sm">
                <h3 className="font-bold text-slate-800 mb-4 flex items-center justify-between">
                    <span className="flex items-center"><Filter className="w-5 h-5 mr-2"/>Filtros</span>
                     {areFiltersActive && (<button onClick={resetFilters} className="text-xs font-semibold text-teal-600 hover:underline flex items-center">
                ...
                <div className="space-y-6">
                    ...
                    <div>
                        ...
                        <select id="specialty-select" value={selectedSpecialty} onChange={(e) => setSelectedSpecialty(e.target.value)} className="w-full mt-1 p-2 border border-slate-300 rounded-lg" title="Filtrar exercícios por especialidade clínica">
                    ...
                    <div>
                        ...
                        <input id="difficulty-range" type="range" min="1" max="5" value={maxDifficulty} onChange={(e) => setMaxDifficulty(Number(e.target.value))} className="w-full mt-2" title={`Filtrar exercícios até nível de dificuldade ${maxDifficulty}`}/>
                    </div>

                    <div className="flex items-center">
                        <input id="showProtocols" type="checkbox" checked={showOnlyWithProtocols} onChange={(e) => setShowOnlyWithProtocols(e.target.checked)} className="h-4 w-4 rounded border-gray-300 text-teal-600 focus:ring-teal-500"/>
          ...
          <main className="lg:col-span-3 space-y-4">
            {categories.map(category => {
                ...
                return (<div key={category} className="bg-white rounded-2xl shadow-sm transition-shadow hover:shadow-md">
                  <div className="group flex items-center p-4 cursor-pointer" onClick={() => toggleCategory(category)}>
                    <h3 className="text-xl font-bold text-slate-800 flex-1">
                      ...
                      {categoryExercises.some(ex => ex.linkedProtocols?.length > 0) && (<span className="ml-2 inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">
                  ...
                  {isOpen && (<div className="p-4 border-t border-slate-200">
                      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                        {categoryExercises.map(ex => (<div key={ex.id} className="relative">
                            <ExerciseCard exercise={ex} onEdit={() => handleOpenExerciseModal(ex)} onDelete={() => handleDeleteExercise(ex)} onPlay={() => ex.media?.videoUrl && setPlayingVideo({ url: ex.media.videoUrl, title: ex.name })}/>
                            {ex.linkedProtocols?.length > 0 && (<div className="absolute top-2 right-2">
                                <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                  <Link className="w-3 h-3 mr-1"/>
                                  {ex.linkedProtocols.length}
                                </span>
                              </div>)}
                          </div>))}
                      </div>
             ...
             {filteredExercises.length === 0 && (<div className="text-center p-10 bg-white rounded-2xl shadow-sm">
