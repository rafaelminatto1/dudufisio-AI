...
const StatCard = React.memo(({ icon: Icon, title, value, change, changeType }) => (<div className="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
    ...
    return (<div className="p-6 space-y-6">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
                ...
                <div className="flex items-center space-x-3 mt-4 lg:mt-0">
                    <select value={timeframe} onChange={(e) => setTimeframe(e.target.value)} className="border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent" aria-label="Selecionar período de tempo">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                ...
                <div className="xl:col-span-2 bg-white rounded-lg shadow-sm border border-slate-200 p-6">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <button className="text-slate-400 hover:text-slate-600" title="Baixar relatório de receita" aria-label="Baixar relatório de receita">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
                    ...
                    <div className="space-y-3">
                        {todayAppointments.slice(0, 4).map((appointment, index) => (<div key={index} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                ...
                <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
                    ...
                    <div className="space-y-3">
                        {recentPatients.map((patient, index) => (<div key={index} className="flex items-center space-x-3 p-3 hover:bg-slate-50 rounded-lg cursor-pointer">
                                ...
                                <div className="flex items-center">
                                    {[...Array(5)].map((_, i) => (<Star key={i} className={`w-4 h-4 ${i < patient.rating ? 'text-yellow-400 fill-current' : 'text-slate-300'}`}/>))}
                                </div>
                            </div>))}
                    </div>
                </div>
...
const LazyElement = (Component) => (<Suspense fallback={<PageLoader />}>
        <Component />
    </Suspense>);
// Alternative wrapper for router components
const RouterElement = (Component) => (<Suspense fallback={<PageLoader />}>
        <Component />
    </Suspense>);
const SessionRoute = ({ mode = 'view' }) => {
    ...
    if (mode === 'form') {
        return (<Suspense fallback={<PageLoader />}>
                <SessionFormPage appointmentId={appointmentId} onClose={() => navigate('/agenda')}/>
            </Suspense>);
    }
    return (<Suspense fallback={<PageLoader />}>
            <SessionPage appointmentId={appointmentId} onClose={() => navigate('/agenda')}/>
        </Suspense>);
};
const CompleteDashboard = ({ user, onLogout }) => {
