...
import { generateHep } from '../services/geminiService';
...
const FormInput = ({ label, name, value, onChange, placeholder }) => (<div>
...
const FormTextarea = ({ label, name, value, onChange, placeholder, rows = 2 }) => (<div className="sm:col-span-2">
        <label htmlFor={name} className="block text-sm font-medium text-slate-700">{label}</label>
        <TiptapEditor value={value} onChange={onChange} placeholder={placeholder} minHeight={`${rows * 20}px`}/>
    </div>);
const HepGeneratorPage = () => {
    ...
    return (<>
            <PageHeader title="Gerador de Plano de Exercícios (HEP)" subtitle="Crie um plano de exercícios domiciliar personalizado e didático com IA."/>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                <div className="bg-white p-6 rounded-2xl shadow-sm space-y-4">
                    ...
                    <FormTextarea label="Observações e Contraindicações" name="observacoes" value={formData.observacoes} onChange={(value) => handleRichTextChange('observacoes', value)} placeholder="Ex: Evitar movimentos bruscos, não prender a respiração"/>
                     ...
                     <button onClick={handleSubmit} disabled={isSubmitDisabled} title={isSubmitDisabled && !isLoading ? 'Preencha o Objetivo e a Lista de Exercícios' : undefined} className="w-full inline-flex justify-center items-center px-6 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-teal-500 hover:bg-teal-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-teal-500 disabled:bg-teal-300 disabled:cursor-not-allowed">
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm">
                    ...
                    <div className="bg-slate-50 p-6 rounded-lg min-h-[600px] overflow-y-auto border border-slate-200">
                        ...
                        {!isLoading && !hep && (<div className="text-center text-slate-500 flex flex-col justify-center items-center h-full">
