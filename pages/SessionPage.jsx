...
import { X, User, Clock, FileText, Plus, History, Activity } from 'lucide-react';
...
const SessionPage = ({ appointmentId, onClose }) => {
    ...
    // 🚀 Handler memoizado
    const handleSaveNote = useCallback(async (newNoteData) => {
        if (!patient)
            return;
        try {
    ...
    // 🚀 Valor memoizado
    const therapist = useMemo(() => therapists.find(t => t.id === appointment?.therapistId), [therapists, appointment?.therapistId]);
    if (isLoading) {
    ...
    const TabButton = memo(({ id, icon: Icon, label }) => (<button onClick={() => setActiveTab(id)} className={`flex items-center px-4 py-2 text-sm font-medium rounded-lg transition-colors ${activeTab === id
            ...
            <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                <div className="bg-white rounded-2xl shadow-2xl w-full max-w-6xl max-h-[95vh] flex flex-col">
                    ...
                    <div className="flex-1 overflow-hidden">
                        {activeTab === 'session' && (<div className="p-6 h-full overflow-y-auto">
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-full">
                                    ...
                                    <div className="space-y-4">
                                        <div className="bg-slate-50 rounded-lg p-4">
                                            ...
                                            <div className="space-y-2 text-sm">
                                                ...
                                                {appointment.sessionNumber && appointment.totalSessions && (<div><strong>Sessão:</strong> {appointment.sessionNumber} de {appointment.totalSessions}</div>)}
                                            </div>
                                        </div>

                                        {/* Condições do Paciente */}
                                        {patient.conditions && patient.conditions.length > 0 && (<div className="bg-blue-50 rounded-lg p-4">
                                                <h3 className="font-semibold text-slate-800 mb-3">Condições/Queixas</h3>
                                                <div className="space-y-2">
                                                    {patient.conditions.map((condition, index) => (<div key={index} className="text-sm bg-white p-2 rounded border-l-4 border-blue-400">
                                                            <strong>{condition.name}</strong>
                                                            {condition.description && (<p className="text-slate-600 mt-1">{condition.description}</p>)}
                                                        </div>))}
                                                </div>
                                            </div>)}

                                        <button onClick={() => setIsSoapModalOpen(true)} className="w-full bg-sky-500 hover:bg-sky-600 text-white font-semibold py-3 px-4 rounded-lg transition-colors flex items-center justify-center">
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        {patientNotes.length > 0 ? (<div className="bg-slate-50 rounded-lg p-4 max-h-96 overflow-y-auto">
                                            ...
                                            </div>) : (<div className="bg-slate-50 rounded-lg p-4 text-center text-slate-500">
                        ...
                        {activeTab === 'history' && (<div className="p-6 h-full overflow-y-auto">
                                <h3 className="font-semibold text-slate-800 mb-4 flex items-center">
                                    <History className="w-5 h-5 mr-2"/>
                                    Histórico de Sessões ({patientNotes.length})
                                </h3>
                                {patientNotes.length > 0 ? (<div className="space-y-4">
                                        {patientNotes.map((note, index) => (<div key={note.id} className="bg-white border border-slate-200 rounded-lg p-4">
                                    ...
                                    </div>) : (<div className="text-center text-slate-500 py-8">
                                        <FileText className="w-12 h-12 mx-auto mb-3 text-slate-300"/>
                                        <p>Nenhuma sessão registrada ainda</p>
                                    </div>)}
                            </div>)}

                        {activeTab === 'patient' && (<div className="p-6 h-full overflow-y-auto">
                                <h3 className="font-semibold text-slate-800 mb-4 flex items-center">
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                    ...
                                    <div className="space-y-4">
                                        {patient.conditions && patient.conditions.length > 0 && (<div className="bg-blue-50 rounded-lg p-4">
                                                ...
                                                <div className="space-y-2">
                                                    {patient.conditions.map((condition, index) => (<div key={index} className="text-sm">
                                                            <strong>{condition.name}</strong>
                                                            {condition.description && (<p className="text-slate-600">{condition.description}</p>)}
                                                        </div>))}
                                                </div>
                                            </div>)}

                                        {patient.surgeries && patient.surgeries.length > 0 && (<div className="bg-red-50 rounded-lg p-4">
                                                <h4 className="font-semibold text-slate-800 mb-3">Histórico Cirúrgico</h4>
                                                <div className="space-y-2">
                                                    {patient.surgeries.map((surgery, index) => (<div key={index} className="text-sm">
