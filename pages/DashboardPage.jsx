...
const DashboardPage = () => {
    ...
    const { data: patients, isLoading: isPatientsLoading, refetch: refetchPatients } = useOptimizedPatients({ ttl: 2 * 60 * 1000 });
    const { data: appointments, isLoading: isAppointmentsLoading, refetch: refetchAppointments } = useOptimizedAppointments({ ttl: 1 * 60 * 1000 });
    ...
    return (<main className="space-y-8" role="main">
            <header>
                <PageHeader title="Dashboard Administrativo" subtitle="Visão 360° do negócio com métricas financeiras, operacionais e clínicas."/>
            </header>
