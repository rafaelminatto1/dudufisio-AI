...
const ClientRow = ({ client }) => {
    ...
    return (<tr className="border-b border-slate-200 hover:bg-slate-50 cursor-pointer" onClick={() => navigate(`/partner/clients/${client.patient.id}`)}>
...
const ClientListPage = () => {
    ...
    const filteredClients = useMemo(() => {
        return clients.filter(client => client.patient.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
            client.patient.email.toLowerCase().includes(searchTerm.toLowerCase()));
    }, [searchTerm, clients]);
    return (<>
            <PageHeader title="Meus Clientes" subtitle="Gerencie os pacientes que adquiriram seus planos de serviço."/>
             <div className="bg-white p-6 rounded-2xl shadow-sm">
                <div className="flex items-center justify-between mb-4">
                    <div className="relative w-full max-w-xs">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400"/>
                        <input type="text" placeholder="Buscar por nome ou email..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-slate-300 rounded-lg"/>
                    </div>
                </div>
                    ...
                    <table className="min-w-full divide-y divide-slate-200">
                        <thead className="bg-slate-50">
                         ...
                         <tbody className="bg-white divide-y divide-slate-200">
                            {isLoading ? (Array.from({ length: 3 }).map((_, i) => (<tr key={i}><td colSpan={4} className="p-4"><Skeleton className="h-12 w-full"/></td></tr>))) : filteredClients.length > 0 ? (filteredClients.map(client => <ClientRow key={client.patient.id} client={client}/>)) : (<tr><td colSpan={4} className="text-center p-10 text-slate-500">Nenhum cliente encontrado.</td></tr>)}
                        </tbody>
                    </table>
                </div>
            </div>
