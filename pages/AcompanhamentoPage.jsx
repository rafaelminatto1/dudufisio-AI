...
const AcompanhamentoPage = () => {
    ...
    const handleLogContact = async (patientId, type) => {
    ...
    const handleAddObservation = (patientId) => {
        navigate(`/patients/${patientId}`);
    };
    const renderSkeletons = () => (<div className="space-y-6">
    ...
    return (<div className="space-y-8">
            <PageHeader title="Acompanhamento de Pacientes" subtitle="Monitore presença, evolução clínica e priorize ações para retenção."/>

            {loading || !alerts || !metrics || !quickActions ? (renderSkeletons()) : (<div className="space-y-8">
                    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                    ...
                    <QuickActionsPanel quickActions={quickActions} onLogContact={handleLogContact} onReschedule={handleReschedule} onAddObservation={handleAddObservation}/>
                </div>)}
        </div>);
};
