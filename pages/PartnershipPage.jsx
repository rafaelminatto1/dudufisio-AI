...
const PartnershipPage = () => {
    ...
    const handleSellVoucher = async (patientId, partnerId, planId) => {
        try {
    ...
    return (<>
            <PageHeader title="Gestão de Parcerias" subtitle="Gerencie seus parceiros e realize a venda de serviços complementares."/>

            <PartnerFormModal isOpen={isPartnerModalOpen} onClose={() => setIsPartnerModalOpen(false)} onSave={handleSavePartner} partnerToEdit={partnerToEdit}/>

            {isLoading ? (<Skeleton className="h-96 w-full"/>) : (<div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                    <div className="lg:col-span-1 space-y-6">
                        <SellVoucherCard patients={patients} partners={partners} plans={sampleVoucherPlans} onSell={handleSellVoucher}/>
                    </div>
                    <div className="lg:col-span-2">
                        <PartnerList partners={partners} onAdd={() => handleOpenPartnerModal()} onEdit={handleOpenPartnerModal}/>
                    </div>
                </div>)}
        </>);
};
