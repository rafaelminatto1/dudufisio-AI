...
import { ArrowLeft, RefreshCw, Download, AlertTriangle, Shield, Activity, TrendingUp } from 'lucide-react';
...
export const RiskStratificationPage = () => {
    ...
    const handleTakeAction = (assessment) => {
    ...
    if (!riskProfile || !patient) {
        ...
        <div className="text-center">
          ...
          <button onClick={() => navigate(-1)} className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">
            Voltar
          </button>
        </div>
      </div>);
    ...
    return (<div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <button onClick={() => navigate(-1)} className="p-2 hover:bg-gray-100 rounded-lg transition" aria-label="Voltar" title="Voltar">
            ...
            <div className="flex items-center gap-3">
              <button onClick={handleRefresh} disabled={refreshing} className="flex items-center gap-2 px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition disabled:opacity-50">
                <RefreshCw className={`w-4 h-4 ${refreshing ? 'animate-spin' : ''}`}/>
                <span className="font-medium">Atualizar</span>
              </button>
              <button onClick={handleExportReport} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <RiskAssessmentDashboard riskProfile={riskProfile} onViewDetails={handleViewDetails} onTakeAction={handleTakeAction}/>
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {riskProfile.assessments.reduce((sum, a) => sum + a.factors.filter(f => f.isModifiable).length, 0)}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {riskProfile.assessments.reduce((sum, a) => sum + a.recommendations.filter(r => !r.completed).length, 0)}
                </p>
              </div>
            </div>
          </div>
      ...
      {/* Detail Modal */}
      {showDetailModal && selectedAssessment && (<RiskDetailModal assessment={selectedAssessment} onClose={() => {
