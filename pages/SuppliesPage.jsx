...
const SuppliesPage = () => {
    ...
    const renderContent = () => {
        switch (viewMode) {
            case 'dashboard':
                return (<SuppliesDashboard onNavigateToSupplies={handleNavigateToSupplies} onNavigateToAlerts={handleNavigateToAlerts} onNavigateToOrders={handleNavigateToOrders}/>);
            case 'list':
                return (<SuppliesList onAddSupply={handleAddSupply} onEditSupply={handleEditSupply} onViewSupply={handleViewSupply} onDeleteSupply={handleDeleteSupply}/>);
            case 'add':
            case 'edit':
                return (<SupplyForm supply={selectedSupply || undefined} onSave={handleSaveSupply} onCancel={handleCancelForm} isLoading={isFormLoading}/>);
            case 'view':
            ...
            <div className="bg-white rounded-lg shadow-sm border p-6">
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex space-x-2">
                  <button onClick={() => setViewMode('edit')} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
                    Editar
                  </button>
                  <button onClick={() => setViewMode('dashboard')} className="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50">
                    Voltar
                  </button>
                </div>
              </div>
              
              {selectedSupply && (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center space-x-4">
              <button onClick={() => setViewMode('dashboard')} className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors ${viewMode === 'dashboard'
            ? 'bg-blue-100 text-blue-700'
            : 'text-gray-600 hover:text-gray-900 hover:bg-gray-100'}`}>
                Dashboard
              </button>
              <button onClick={() => setViewMode('list')} className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors ${viewMode === 'list'
            ? 'bg-blue-100 text-blue-700'
            : 'text-gray-600 hover:text-gray-900 hover:bg-gray-100'}`}>
                Lista de Insumos
              </button>
            ...
            <div className="flex items-center space-x-4">
              <button onClick={handleAddSupply} className="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
