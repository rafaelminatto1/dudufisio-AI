...
const TimeSlotRow = ({ slot, onUpdate, onRemove }) => (<div className="grid grid-cols-4 gap-3 items-center">
        <input type="time" value={slot.startTime} onChange={e => onUpdate('startTime', e.target.value)} className="w-full p-2 border border-slate-300 rounded-lg"/>
        <input type="time" value={slot.endTime} onChange={e => onUpdate('endTime', e.target.value)} className="w-full p-2 border border-slate-300 rounded-lg"/>
        <input type="number" min="0" value={slot.limit} onChange={e => onUpdate('limit', parseInt(e.target.value, 10) || 0)} className="w-full p-2 border border-slate-300 rounded-lg"/>
...
const AgendaSettingsPage = () => {
    const [settings, setSettings] = useState(() => schedulingSettingsService.getSettings());
    const { showToast } = useToast();
    const handleUpdateSlot = (dayType, index, field, value) => {
    ...
    const handleRemoveSlot = (dayType, index) => {
        setSettings(prevSettings => ({
            ...prevSettings,
            limits: {
                ...prevSettings.limits,
    ...
    return (<>
            <PageHeader title="Configurações da Agenda" subtitle="Defina os limites de agendamento e a capacidade da sua clínica por horário.">
            ...
            <div className="space-y-8">
                <div className="bg-white p-6 rounded-2xl shadow-sm">
                    ...
                    <div className="space-y-2">
                        {settings.limits.weekday.map((slot, index) => (<TimeSlotRow key={slot.id} slot={slot} onUpdate={(field, value) => handleUpdateSlot('weekday', index, field, value)} onRemove={() => handleRemoveSlot('weekday', index)}/>))}
                    </div>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm">
                    ...
                    <div className="grid grid-cols-4 gap-3 text-sm font-medium text-slate-600 px-1 mb-2">
                     ...
                     <div className="space-y-2">
                        {settings.limits.saturday.map((slot, index) => (<TimeSlotRow key={slot.id} slot={slot} onUpdate={(field, value) => handleUpdateSlot('saturday', index, field, value)} onRemove={() => handleRemoveSlot('saturday', index)}/>))}
                    </div>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm">
                     ...
                     <div>
                         ...
                         <input id="max-evals" type="number" min="0" value={settings.maxEvaluationsPerSlot} onChange={e => setSettings(s => ({ ...s, maxEvaluationsPerSlot: parseInt(e.target.value) || 0 }))} className="w-full max-w-xs p-2 border border-slate-300 rounded-lg"/>
                     </div>
                </div>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm">
                    ...
                    <div className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                        ...
                        <label className="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" checked={settings.teleconsultaEnabled} onChange={e => setSettings(s => ({ ...s, teleconsultaEnabled: e.target.checked }))} className="sr-only peer"/>
