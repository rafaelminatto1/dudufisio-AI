...
const EventsListPage = () => {
    ...
    const FilterButton = ({ value, label }) => (<button onClick={() => setStatusFilter(value)} className={`px-4 py-2 text-sm font-semibold rounded-full transition-colors ${statusFilter === value ? 'bg-teal-500 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
            {label}
        </button>);
    return (<>
            <PageHeader title="Gestão de Eventos" subtitle="Crie e gerencie corridas, workshops e campanhas de saúde.">
                <button onClick={() => handleOpenModal()} className="inline-flex items-center justify-center rounded-lg border border-transparent bg-teal-500 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-teal-600">
            ...
            <div className="mb-6 bg-white p-4 rounded-2xl shadow-sm flex flex-col sm:flex-row items-center gap-4">
                <div className="relative flex-grow w-full sm:w-auto">
                    ...
                    <input type="text" placeholder="Buscar por nome do evento..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-slate-300 rounded-full"/>
                </div>
            ...
            {isLoading ? (<Skeleton className="h-96 w-full rounded-2xl"/>) : (<div className="space-y-10">
                    <section>
                        <h2 className="text-2xl font-bold text-slate-800 mb-4">Próximos Eventos</h2>
                        {upcomingEvents.length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {upcomingEvents.map(event => (<EventCard key={event.id} event={event} onEdit={() => handleOpenModal(event)}/>))}
                            </div>) : (<div className="text-center p-12 bg-white rounded-2xl shadow-sm">
                     ...
                     <section>
                        ...
                        {pastEvents.length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {pastEvents.map(event => (<EventCard key={event.id} event={event} onEdit={() => handleOpenModal(event)}/>))}
                            </div>) : (<div className="text-center p-12 bg-white rounded-2xl shadow-sm">
