...
const MaterialsPage = () => {
    ...
    if (isLoading) {
        return (<>
        <PageHeader title="Materiais Clínicos" subtitle="Biblioteca de materiais e protocolos clínicos"/>
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[...Array(6)].map((_, i) => (<div key={i} className="bg-white p-6 rounded-xl shadow-sm">
    ...
    return (<>
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex-1">
            <div className="relative">
              ...
              <input type="text" placeholder="Buscar materiais..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500"/>
            </div>
          </div>
          <div className="md:w-64">
            <div className="relative">
              <Filter className="absolute left-3 top-1/2 transform -translate-y-1/2 text-slate-400 w-5 h-5"/>
              <select value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500 appearance-none bg-white" aria-label="Filtrar por categoria">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-slate-900">
                {categories.reduce((total, cat) => total + cat.materials.filter(m => m.type === 'Protocolo Clínico').length, 0)}
              </p>
            </div>
          </div>
        </div>
        <div className="bg-white p-6 rounded-xl shadow-sm">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-slate-900">
                {categories.reduce((total, cat) => total + cat.materials.filter(m => m.type === 'Escala de Avaliação').length, 0)}
              </p>
            </div>
          </div>
        </div>
      ...
      {/* Lista de Materiais */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredMaterials.map((material) => (<Link key={material.id} to={`/material-detail/${material.id}`} className="bg-white p-6 rounded-xl shadow-sm hover:shadow-lg transition-shadow duration-300 group">
      ...
      {filteredMaterials.length === 0 && !isLoading && (<div className="text-center py-12">
