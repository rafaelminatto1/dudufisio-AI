...
import { Users, Calendar, Target, TrendingUp, AlertCircle, CheckCircle2, Activity, Heart } from 'lucide-react';
...
const MetricCard = memo(({ title, value, subtitle, icon: Icon, trend, alert }) => (<div className={`bg-white rounded-xl shadow-sm border p-6 transition-all hover:shadow-md ${alert ? 'border-orange-200 bg-orange-50' : 'border-slate-200'}`}>
      ...
      <div className="flex-1">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {subtitle && (<p className={`text-xs ${alert ? 'text-orange-600' : 'text-slate-500'}`}>{subtitle}</p>)}
          </div>
        </div>
      </div>
...
const PatientProgressCard = memo(({ patient, progress, nextSession, status }) => {
    const statusConfig = {
    ...
    return (<div className="bg-white rounded-lg border border-slate-200 p-4 hover:shadow-sm transition-shadow">
      ...
      <div className="mb-3">
        ...
        <div className="w-full bg-slate-200 rounded-full h-2">
          <div className={`h-2 rounded-full transition-all ${status === 'excellent' ? 'bg-green-500' : status === 'on-track' ? 'bg-sky-500' : 'bg-orange-500'}`} style={{ width: `${progress}%` }}/>
        </div>
      </div>
...
const TherapistDashboard = () => {
    ...
    const patientsWithProgress = useMemo(() => {
        return patients
            ...
            status: ['excellent', 'on-track', 'needs-attention'][Math.floor(Math.random() * 3)]
        }));
    }, [patients]);
    if (isLoading) {
    ...
    return (<div className="space-y-8">
      <PageHeader title="Dashboard Clínico" subtitle="Visão especializada para fisioterapeutas com foco no cuidado e progresso dos pacientes."/>

      {/* Métricas Principais */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <MetricCard title="Consultas Hoje" value={metrics.todayAppointments} icon={Calendar} trend={{ value: 12, positive: true }}/>

        <MetricCard title="Pacientes Ativos" value={metrics.activePatients} subtitle="Em tratamento" icon={Users} trend={{ value: 8, positive: true }}/>

        <MetricCard title="Satisfação (NPS)" value={`${metrics.patientSatisfaction}/10`} subtitle="Baseado em 24 avaliações" icon={Heart} trend={{ value: 3, positive: true }}/>

        <MetricCard title="Alertas Pendentes" value={metrics.criticalAlerts} subtitle="Requerem atenção" icon={AlertCircle} alert={metrics.criticalAlerts > 0}/>
      </div>

      {/* Métricas Secundárias */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <MetricCard title="Tratamentos Concluídos" value={metrics.completedTreatments} subtitle="Últimos 30 dias" icon={CheckCircle2}/>

        <MetricCard title="Tempo Médio Recuperação" value={`${metrics.averageRecoveryTime} sem`} subtitle="Por paciente" icon={TrendingUp}/>

        <MetricCard title="Avaliações Pendentes" value={metrics.pendingEvaluations} subtitle="Para esta semana" icon={Target} alert={metrics.pendingEvaluations > 3}/>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            <TodaysAppointments appointments={appointments.filter(apt => {
            const startTime = apt.startTime instanceof Date ? apt.startTime : new Date(apt.startTime);
            const today = new Date();
            return startTime.toDateString() === today.toDateString();
        })} patients={patients} therapists={therapists}/>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            ...
            <div className="grid gap-4">
              {patientsWithProgress.map(({ patient, progress, nextSession, status }) => (<PatientProgressCard key={patient.id} patient={patient} progress={progress} nextSession={nextSession} status={status}/>))}
            </div>
          </div>
        </div>
