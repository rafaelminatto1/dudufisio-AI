...
import { Home, Calendar, Dumbbell, FileText, TrendingUp, Gift, Award, LogOut, Users } from 'lucide-react';
...
const PatientPortalDashboard = ({ user, onLogout }) => {
    ...
    const NavLink = ({ item }) => (<button onClick={() => {
            setCurrentPage(item.id);
            setIsSidebarOpen(false);
        }} className={`w-full flex items-center p-3 rounded-lg transition-colors duration-200 ${currentPage === item.id
            ? 'bg-teal-50 text-teal-600 font-semibold'
            : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'}`}>
            <item.icon className="w-5 h-5 mr-3"/>
            <span className="text-sm">{item.label}</span>
        </button>);
    const groupItems = (items, group) => items.filter(item => item.group === group);
                ...
                <div className="flex flex-col h-full">
                    ...
                    <div className="flex-1 overflow-y-auto py-4 px-3">
                        ...
                        <div className="space-y-1 mb-6">
                            ...
                            {groupItems(menuItems, 'main').map((item) => (<NavLink key={item.id} item={item}/>))}
                        </div>
                        ...
                        <div className="space-y-1 mb-6">
                            ...
                            {groupItems(menuItems, 'health').map((item) => (<NavLink key={item.id} item={item}/>))}
                        </div>
                        ...
                        <div className="space-y-1 mb-6">
                            ...
                            {groupItems(menuItems, 'store').map((item) => (<NavLink key={item.id} item={item}/>))}
                        </div>
                        ...
                        <div className="space-y-1">
                            ...
                            {groupItems(menuItems, 'fun').map((item) => (<NavLink key={item.id} item={item}/>))}
                        </div>
                    </div>
                    ...
                    <div className="border-t border-slate-200 p-3">
                        ...
                        <button onClick={onLogout} className="w-full flex items-center p-2 text-slate-600 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors">
