...
import { FileText, TrendingUp, BarChart3, Users, Download, RefreshCw, Search, Eye, Trash2, Clock, AlertTriangle, Target, Zap, Brain, Activity, Shield, DollarSign, Settings, FileSpreadsheet, Award, Heart, ArrowUpRight, ArrowDownRight } from 'lucide-react';
import { AreaChart, Area, BarChart, Bar, LineChart as RechartsLineChart, Line, PieChart as RechartsPieChart, Pie, Cell, ResponsiveContainer, XAxis, YAxis, CartesianGrid, Tooltip } from 'recharts';
const AdvancedReportsPage = () => {
    ...
    // Main reporting hook
    const { templates, reports, biMetrics, complianceReport, predictiveAnalytics, isLoading, isGenerating, error, generateReport, deleteReport, exportReport, refreshData } = useReporting();
    // Specialized hooks
    ...
    const handleGenerateReport = useCallback(async (templateId, parameters) => {
        try {
    ...
    const getMetricChangeIcon = (changeType) => {
        switch (changeType) {
            case 'positive':
    ...
    return (<PermissionGuard permission="reports:read">
      <div className="p-6 max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex items-center gap-3 mt-4 lg:mt-0">
            ...
            <button onClick={refreshData} disabled={isLoading} className="flex items-center gap-2 px-4 py-2 bg-sky-600 text-white rounded-lg hover:bg-sky-700 transition-colors disabled:opacity-50">
        ...
        {biMetrics && (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          ...
          <TabsContent value="overview" className="space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200">
              ...
              <div className="p-6">
                <div className="space-y-4">
                  {reports.slice(0, 5).map((report) => (<div key={report.id} className="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
                      <div className="flex items-center gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${report.status === 'completed' ? 'bg-green-100 text-green-700' :
                ...
                report.status === 'generating' ? 'Gerando' : 'Erro'}
                        ...
                        {report.status === 'completed' && (<button onClick={() => handleExportReport(report.id, 'pdf')} className="text-sky-600 hover:text-sky-700">
            ...
            {/* Quick Analytics */}
            {biMetrics && (<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-200">
                  ...
                  <div className="p-6">
                    <ResponsiveContainer width="100%" height={300}>
                      <RechartsPieChart>
                        <Pie data={biMetrics.patients.riskSegments} dataKey="count" nameKey="segment" cx="50%" cy="50%" outerRadius={100} label={({ segment, percentage }) => `${segment}: ${percentage}%`}>
                          {biMetrics.patients.riskSegments.map((entry, index) => (<Cell key={`cell-${index}`} fill={index === 0 ? '#10b981' : index === 1 ? '#f59e0b' : '#ef4444'}/>))}
                        </Pie>
                        <Tooltip />
                      </RechartsPieChart>
                    </ResponsiveContainer>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="relative">
                      ...
                      <input type="text" placeholder="Buscar templates..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-sky-500"/>
                    </div>
                    <select value={filterCategory} onChange={(e) => setFilterCategory(e.target.value)} className="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {filteredTemplates.map((template) => (<div key={template.id} className="border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => setReportModal({ isOpen: true, templateId: template.id })} disabled={isGenerating} className="flex-1 bg-sky-600 text-white px-4 py-2 rounded-lg hover:bg-sky-700 transition-colors disabled:opacity-50 text-sm">
                          Gerar Relatório
                        </button>
          ...
          <TabsContent value="reports" className="space-y-6">
            ...
            const template = templates.find(t => t.id === report.templateId);
            return (<tr key={report.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            {template && (<span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${getCategoryColor(template.category)}`}>
                                {template.category}
                              </span>)}
                          </td>
                    ...
                    minute: '2-digit'
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${report.status === 'completed' ? 'bg-green-100 text-green-800' :
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div className="flex items-center gap-2">
                              {report.status === 'completed' && (<>
                                  <button onClick={() => handleExportReport(report.id, 'pdf')} className="text-sky-600 hover:text-sky-700" title="Baixar PDF">
                                    <Download className="w-4 h-4"/>
                                  </button>
                                  <button onClick={() => handleExportReport(report.id, 'excel')} className="text-green-600 hover:text-green-700" title="Baixar Excel">
                                    <FileSpreadsheet className="w-4 h-4"/>
                                  </button>
                                </>)}
                              <IfPermission permission="reports:delete">
                                <button onClick={() => deleteReport(report.id)} className="text-red-600 hover:text-red-700" title="Deletar">
          ...
          <TabsContent value="compliance" className="space-y-6">
            {complianceReport && (<div className="space-y-6">
                {/* Compliance Overview */}
          ...
          <TabsContent value="predictive" className="space-y-6">
            {predictiveAnalytics && (<div className="space-y-6">
                {/* Demand Forecast */}
                <div className="bg-white rounded-lg shadow-sm border border-gray-200">
                  <div className="p-6 border-b border-gray-200">
                    <h3 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-200">
                  ...
                  <div className="p-6">
                    <div className="space-y-4">
                      {predictiveAnalytics.patientRisk.dropoutPrediction.map((prediction, index) => (<div key={index} className="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
                          <div>
                          ...
                          <div className="text-right">
                            <div className={`text-lg font-bold ${prediction.risk > 0.7 ? 'text-red-600' :
        ...
        {reportModal.isOpen && reportModal.templateId && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-md">
              ...
              <div className="flex gap-3">
                <button onClick={() => handleGenerateReport(reportModal.templateId, {
                dateRange: dateRange,
                includeForecasts: true
            })} disabled={isGenerating} className="flex-1 bg-sky-600 text-white px-4 py-2 rounded-lg hover:bg-sky-700 transition-colors disabled:opacity-50">
                  {isGenerating ? 'Gerando...' : 'Gerar'}
                </button>
                <button onClick={() => setReportModal({ isOpen: false, templateId: null })} className="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
                  Cancelar
                </button>
              </div>
            </div>
