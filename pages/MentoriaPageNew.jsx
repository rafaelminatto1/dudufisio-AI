...
import { Users, BookOpen, Award, TrendingUp, Search, UserPlus, GraduationCap, BarChart3 } from 'lucide-react';
...
const MentoriaPage = () => {
    ...
    const fetchData = useCallback(async () => {
        setIsLoading(true);
        try {
            const [mentoriaData, resourcesData, learningPathsData, certificationsData, competenciesData] = await Promise.all([
    ...
    const renderDashboard = () => (<div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <Card>
                    ...
                    <CardContent>
                        {metrics && (<div className="space-y-4">
                                {metrics.monthlyProgress.map((month, index) => (<div key={index} className="flex items-center justify-between">
                ...
                <Card>
                    ...
                    <CardContent>
                        {metrics && (<div className="space-y-4">
                                {Object.entries(metrics.competencyDistribution).map(([category, levels]) => (<div key={category} className="space-y-2">
                                        ...
                                        <div className="grid grid-cols-4 gap-1">
                                            {Object.entries(levels).map(([level, count]) => (<div key={level} className="text-center">
            ...
            <Card>
                ...
                <CardContent>
                    <div className="space-y-4">
                        {interns.slice(0, 5).map((intern) => (<div key={intern.id} className="flex items-center space-x-4">
                                <img src={intern.avatarUrl} alt={intern.name} className="w-8 h-8 rounded-full"/>
    ...
    const renderInterns = () => (<div className="space-y-6">
            {/* Header with actions */}
            <div className="flex justify-between items-center">
                <div>
                ...
                <CardContent className="pt-6">
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div>
                            ...
                            <select className="w-full mt-1 p-2 border rounded-md" value={internFilter.status || ''} onChange={(e) => setInternFilter(prev => ({
            ...prev,
            status: e.target.value || undefined
        }))}>
                                <option value="">Todos</option>
                                {Object.values(InternStatus).map(status => (<option key={status} value={status}>{status}</option>))}
                            </select>
                        </div>
                        <div>
                            <label className="text-sm font-medium">Supervisor</label>
                            <select className="w-full mt-1 p-2 border rounded-md" value={internFilter.supervisor || ''} onChange={(e) => setInternFilter(prev => ({
            ...prev,
            supervisor: e.target.value || undefined
                        ...
                        <div className="md:col-span-2">
                            ...
                            <div className="relative mt-1">
                                ...
                                <input type="text" placeholder="Buscar por nome ou instituição..." className="w-full pl-10 pr-4 py-2 border rounded-md" value={internFilter.search || ''} onChange={(e) => setInternFilter(prev => ({
            ...prev,
            ...
            {/* Interns Table */}
            <InternsTable interns={interns.filter(intern => {
            if (internFilter.status && intern.status !== internFilter.status)
                return false;
            if (internFilter.supervisor && intern.supervisorName !== internFilter.supervisor)
                return false;
            ...
            return true;
        })} onAdd={handleAddIntern} onEdit={handleEditIntern} onDelete={handleDeleteIntern}/>
        </div>);
    if (isLoading) {
        return (<div className="space-y-6">
                <PageHeader title="Módulo de Mentoria e Ensino" subtitle="Carregando dados de mentoria..."/>
                <Skeleton className="h-96 w-full"/>
            </div>);
    }
    return (<div className="space-y-6">
            <PageHeader title="Módulo de Mentoria e Ensino" subtitle="Centro completo de educação continuada e desenvolvimento profissional"/>
