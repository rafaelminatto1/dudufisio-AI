...
import { Users, Calendar, DollarSign, TrendingUp, Activity, AlertTriangle, CheckCircle, Clock, BarChart3, PieChart, Download, RefreshCw } from 'lucide-react';
...
const AdminDashboardPage = () => {
    ...
    useEffect(() => {
        ...
        const loadMetrics = async () => {
            ...
            const appointmentsToday = appointments.filter(apt => new Date(apt.date).toDateString() === today).length;
            const appointmentsThisWeek = appointments.filter(apt => new Date(apt.date) >= startOfWeek).length;
            const appointmentsThisMonth = appointments.filter(apt => new Date(apt.date) >= startOfMonth).length;
            ...
            const systemHealth = Math.min(100, Math.max(85, 100 - (cancelledSessions * 2) - (pendingTasks * 1)));
            setMetrics({
            ...
            if (patient) {
                activities.push({
                    ...
                    type: apt.status === 'completed' ? 'success' :
                        apt.status === 'cancelled' ? 'warning' : 'info'
                });
            }
        });
    ...
    return (<div className="min-h-screen bg-slate-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8">
          {metricCards.map((card, index) => (<div key={index} className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
              <div className="flex items-center justify-between">
                <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={revenueData}>
                  ...
                  <YAxis yAxisId="revenue" orientation="left" stroke="#64748b" fontSize={12} tickFormatter={(value) => `R$ ${(value / 1000).toFixed(0)}k`}/>
                  <YAxis yAxisId="sessions" orientation="right" stroke="#64748b" fontSize={12}/>
            ...
            name === 'revenue' ? 'Receita' : 'Sessões'
                  ...
                  <Area yAxisId="revenue" type="monotone" dataKey="revenue" stroke="#3b82f6" fill="#3b82f6" fillOpacity={0.1} strokeWidth={2}/>
                  <Area yAxisId="sessions" type="monotone" dataKey="sessions" stroke="#10b981" fill="#10b981" fillOpacity={0.1} strokeWidth={2}/>
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <RechartsPieChart>
                  <Pie data={sessionStatusData} cx="50%" cy="50%" innerRadius={60} outerRadius={100} paddingAngle={5} dataKey="value">
            ...
            <div className="flex justify-center space-x-6 mt-4">
              {sessionStatusData.map((item, index) => (<div key={index} className="flex items-center space-x-2">
                  <div className={`w-3 h-3 rounded-full ${item.name === 'Completadas' ? 'bg-green-500' :
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            ...
            boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)'
                  ...
                  <Line type="monotone" dataKey="count" stroke="#8b5cf6" strokeWidth={3} dot={{ fill: '#8b5cf6', strokeWidth: 2, r: 4 }} activeDot={{ r: 6, stroke: '#8b5cf6', strokeWidth: 2 }}/>
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
          ...
          <div className="space-y-4">
            {recentActivities.map((activity) => (<div key={activity.id} className="flex items-center space-x-4 p-3 rounded-lg hover:bg-slate-50 transition-colors">
