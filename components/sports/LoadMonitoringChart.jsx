...
export const LoadMonitoringChart = ({ data }) => {
    if (data.length === 0) {
    ...
    return (<div className="bg-white rounded-lg shadow-md p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className={`text-2xl font-bold ${isHighRisk ? 'text-red-600' :
      ...
      {/* Alert se necessário */}
      {(isHighRisk || isLowRisk) && (<div className={`mb-4 p-4 rounded-lg border ${isHighRisk ? 'bg-red-50 border-red-200' : 'bg-orange-50 border-orange-200'}`}>
          <div className="flex items-start gap-3">
            <AlertTriangle className={`w-5 h-5 ${isHighRisk ? 'text-red-600' : 'text-orange-600'}`}/>
            <div>
              <h4 className={`font-semibold mb-1 ${isHighRisk ? 'text-red-900' : 'text-orange-900'}`}>
                {isHighRisk ? '⚠️ Risco Aumentado de Lesão' : '⚠️ Carga Insuficiente'}
              </h4>
              <p className={`text-sm ${isHighRisk ? 'text-red-800' : 'text-orange-800'}`}>
                {isHighRisk
              ...
              {lastLoad.recommendations && lastLoad.recommendations.length > 0 && (<ul className="mt-2 space-y-1">
                  {lastLoad.recommendations.map((rec, idx) => (<li key={idx} className="text-sm">• {rec}</li>))}
                </ul>)}
            </div>
          </div>
        </div>)}
      ...
      <div className="h-80">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData}>
            ...
            <Line type="monotone" dataKey="acwr" stroke="#3b82f6" strokeWidth={3} name="ACWR" dot={{ fill: '#3b82f6', r: 4 }}/>
            <Line type="monotone" dataKey="acuteLoad" stroke="#10b981" strokeWidth={2} name="Carga Aguda" strokeDasharray="5 5"/>
            <Line type="monotone" dataKey="chronicLoad" stroke="#8b5cf6" strokeWidth={2} name="Carga Crônica" strokeDasharray="5 5"/>
          </LineChart>
        </ResponsiveContainer>
      </div>
