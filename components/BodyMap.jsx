...
import { History, AlertCircle, Calendar } from 'lucide-react';
const BodyMap = ({ patient, bodyPoints, onAddPoint, onUpdatePoint, onDeletePoint, isEditable = true }) => {
    ...
    return (<div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
            <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center gap-2">
                    <button onClick={() => setShowHistory(!showHistory)} className={`p-2 rounded-lg transition-colors ${showHistory ? 'bg-sky-100 text-sky-600' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`} title="Histórico de evolução">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-1 space-y-4">
                    <div className="flex gap-2">
                        <button onClick={() => setActiveView('front')} className={`flex-1 py-2 px-4 rounded-lg font-medium transition-colors ${activeView === 'front'
            ? 'bg-sky-500 text-white'
            : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                            Frente
                        </button>
                        <button onClick={() => setActiveView('back')} className={`flex-1 py-2 px-4 rounded-lg font-medium transition-colors ${activeView === 'back'
            ? 'bg-sky-500 text-white'
            : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                            Costas
                        </button>
            ...
            { range: '9-10', color: '#ef4444', label: 'Severa' }
        ].map(({ range, color, label }) => (<div key={range} className="flex items-center gap-2">
                    ...
                    {/* Pontos Atuais */}
                    {filteredPoints.length > 0 && (<div className="bg-slate-50 rounded-lg p-4">
                            <h4 className="font-medium text-slate-800 mb-3">
                                Pontos Marcados ({activeView === 'front' ? 'Frente' : 'Costas'})
                            </h4>
                            <div className="space-y-2 max-h-32 overflow-y-auto">
                                {filteredPoints.map((point) => (<div key={point.id} onClick={() => setSelectedPoint(point)} className="flex items-center gap-2 p-2 bg-white rounded cursor-pointer hover:bg-slate-100 transition-colors">
                                        <div className="w-3 h-3 rounded-full" style={{ backgroundColor: getPainColor(point.painLevel) }}/>
                ...
                <div className="lg:col-span-2">
                    <div className="relative bg-slate-50 rounded-lg p-4 flex items-center justify-center min-h-[600px]">
                        <svg ref={svgRef} viewBox="0 0 300 600" className="w-full h-full max-w-sm cursor-pointer" onClick={handleSvgClick}>
                            {/* Corpo humano simplificado */}
                            {activeView === 'front' ? (<g fill="none" stroke="#94a3b8" strokeWidth="2">
                                    {/* Cabeça */}
                                ...
                                </g>) : (<g fill="none" stroke="#94a3b8" strokeWidth="2">
                                    {/* Cabeça (costas) */}
                                    <circle cx="150" cy="60" r="40" fill="#f1f5f9"/>
                            ...
                            {/* Pontos de dor */}
                            {filteredPoints.map((point) => (<circle key={point.id} cx={(point.coordinates.x / 100) * 300} cy={(point.coordinates.y / 100) * 600} r="8" fill={getPainColor(point.painLevel)} stroke="white" strokeWidth="2" className="cursor-pointer hover:r-10 transition-all" onClick={(e) => handlePointClick(point, e)} style={{
            ...
            {showHistory && (<div className="mt-6 border-t pt-6">
                ...
                .map(([date, points]) => (<div key={date} className="bg-slate-50 rounded-lg p-4">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                        {points.map((point) => (<div key={point.id} className="flex items-center gap-2 text-sm">
                                                <div className="w-3 h-3 rounded-full" style={{ backgroundColor: getPainColor(point.painLevel) }}/>
