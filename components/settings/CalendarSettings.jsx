...
import { Calendar, Clock, CheckCircle, XCircle, AlertTriangle, Loader2, TestTube, BarChart3, Trash2 } from 'lucide-react';
export const CalendarSettings = ({ patientId, isAdmin = false }) => {
    ...
    const testProvider = async (providerName) => {
        ...
        try {
            ...
            setProviderStatuses(prev => prev.map(provider => provider.name === providerName
                ? {
                    ...provider,
    ...
    return (<div className="space-y-6">
          ...
          <CardContent className="space-y-6">
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                ...
                <Select value={preferences.preferredProvider} onValueChange={(value) => setPreferences({ ...preferences, preferredProvider: value })}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <Switch checked={preferences.enableReminders} onCheckedChange={(checked) => setPreferences({ ...preferences, enableReminders: checked })}/>
              </div>

              {preferences.enableReminders && (<div>
                  <Label>Horários dos Lembretes (minutos antes)</Label>
                  <div className="flex gap-2 mt-2">
                    {preferences.reminderTimes.map((time, index) => (<Badge key={index} variant="secondary">
              ...
              <div className="flex items-center justify-between">
                ...
                <Switch checked={preferences.autoAcceptInvites} onCheckedChange={(checked) => setPreferences({ ...preferences, autoAcceptInvites: checked })}/>
              </div>
            </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            {providerStatuses.map((provider) => (<div key={provider.name} className="border rounded-lg p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <Button size="sm" variant="outline" onClick={() => testProvider(provider.name)} disabled={testingProvider === provider.name}>
                    {testingProvider === provider.name ? (<Loader2 className="h-4 w-4 animate-spin"/>) : ('Testar')}
                  </Button>
                </div>

                <div className="text-sm text-gray-600 space-y-1">
                  <p>Recursos: {provider.features.join(', ')}</p>
                  {provider.lastTest && (<p>Último teste: {provider.lastTest.toLocaleString('pt-BR')}</p>)}
                  {provider.testResult?.responseTime && (<p>Tempo de resposta: {provider.testResult.responseTime}ms</p>)}
                </div>

                {provider.testResult?.error && (<Alert className="mt-3">
      ...
      {/* Integration History */}
      {patientId && integrations.length > 0 && (<Card>
          <CardHeader>
            <CardTitle className="flex items-center justify-between">
              ...
              <Button size="sm" variant="outline" onClick={clearIntegrationHistory}>
          ...
          <CardContent>
            <div className="space-y-3">
              {integrations.slice(0, 10).map((integration) => (<div key={integration.id} className="flex items-center justify-between border-b pb-2">
                  <div>
                  ...
                  <Badge variant={integration.status === 'sent' ? 'default' :
      ...
      {isAdmin && metrics && (<Card>
          ...
          <CardContent>
            ...
            <div className="space-y-3">
              ...
              {Object.entries(metrics.providerPerformance).map(([provider, stats]) => (<div key={provider} className="flex items-center justify-between border rounded p-3">
