...
const StockMovementModal = ({ isOpen, onClose, onSave, item, movementType }) => {
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md" onClick={e => e.stopPropagation()}>
                ...
                <main className="p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <input type="number" value={quantity} onChange={(e) => setQuantity(Math.max(1, parseInt(e.target.value, 10)))} min="1" className="mt-1 w-full p-2 border border-slate-300 rounded-lg" autoFocus/>
                    </div>
                     <div>
                        <label className="text-sm font-medium text-slate-700">Motivo (Opcional)</label>
                        <input type="text" value={reason} onChange={(e) => setReason(e.target.value)} className="mt-1 w-full p-2 border border-slate-300 rounded-lg" placeholder={isOut ? 'Ex: Uso com paciente' : 'Ex: Recebimento de pedido'}/>
                    </div>
                </main>
                <footer className="flex justify-end items-center p-4 border-t border-slate-200 bg-slate-50">
                    ...
                    <button onClick={handleSubmit} disabled={isSaving} className={`px-4 py-2 text-sm font-medium text-white rounded-lg flex items-center disabled:opacity-70 ${buttonClass}`}>
