...
const ItemCard = ({ item, suppliers, categories, onAddStock, onRemoveStock, onEdit }) => {
    const category = categories.find(c => c.id === item.categoryId);
    const supplier = suppliers.find(s => s.id === item.supplierId);
    return (<div className="bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col h-full group">
