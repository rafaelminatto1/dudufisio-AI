...
import { Lightbulb, TrendingUp, AlertCircle, Zap, Eye, Clock, Award, Target, Sparkles } from 'lucide-react';
...
const ProtocolRecommendationsPanel = ({ patientId, patientAge, diagnosis, onPrescribeProtocol }) => {
    const [recommendations, setRecommendations] = useState([]);
    const [assessmentBasedRecommendations, setAssessmentBasedRecommendations] = useState([]);
    ...
    const handlePrescribe = (protocolId) => {
    ...
    const uniqueRecommendations = allRecommendations.filter((protocol, index, self) => index === self.findIndex(p => p.id === protocol.id));
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                {assessmentBasedRecommendations.length > 0 && (<span className="flex items-center mt-1">
        ...
        <CardContent>
          <div className="space-y-4">
            {uniqueRecommendations.map((protocol) => {
            const isFromAssessment = assessmentBasedRecommendations.some(p => p.id === protocol.id);
            return (<div key={protocol.id} className="border rounded-lg p-4 hover:bg-slate-50 transition-colors">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        <h4 className="font-semibold text-slate-800">{protocol.name}</h4>
                        {isFromAssessment && (<Badge className="bg-blue-100 text-blue-800 text-xs">
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        {protocol.specialty && (<Badge variant="outline" className="text-xs">
                            {protocol.specialty}
                          </Badge>)}
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3 text-xs text-muted-foreground">
                    ...
                    {protocol.successRate && (<div className="flex items-center text-green-600">
                        <TrendingUp className="w-3 h-3 mr-1"/>
                        <span>{protocol.successRate}% sucesso</span>
                      </div>)}
                    {protocol.timesUsed > 0 && (<div className="flex items-center">
                        <Target className="w-3 h-3 mr-1"/>
                        <span>{protocol.timesUsed} usos</span>
                      </div>)}
                    {protocol.linkedExercises && protocol.linkedExercises.length > 0 && (<div className="flex items-center text-blue-600">
                  ...
                  <div className="flex justify-between items-center pt-3 border-t">
                    ...
                    <Button size="sm" onClick={() => handlePrescribe(protocol.id)} className="bg-blue-600 hover:bg-blue-700">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between p-6 border-b">
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  {selectedProtocol.specialty && (<Badge variant="outline">
            ...
            <div className="flex-1 overflow-y-auto p-6">
              <div className="space-y-6">
                ...
                {selectedProtocol.inclusionCriteria.length > 0 && (<div>
                    <h3 className="text-lg font-semibold mb-2">Critérios de Inclusão</h3>
                    <ul className="list-disc list-inside space-y-1">
                      {selectedProtocol.inclusionCriteria.map((criteria, index) => (<li key={index} className="text-sm text-muted-foreground">{criteria}</li>))}
                    </ul>
                  </div>)}

                {selectedProtocol.treatmentPlan.length > 0 && (<div>
