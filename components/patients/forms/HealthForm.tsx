...
export const HealthForm: React.FC<HealthFormProps> = ({ form }) => {
  return (
    <div className="space-y-6">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Textarea 
                    placeholder="Liste as cirurgias realizadas (ex: Apendicectomia em 2010)"
                    {...field}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Textarea 
                    placeholder="Liste as doenças crônicas (ex: Diabetes, Hipertensão)"
                    {...field}
                    value={Array.isArray(field.value) ? field.value.join(', ') : field.value || ''}
                    onChange={(e) => field.onChange(e.target.value.split(',').map(item => item.trim()))}
                  />
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Textarea 
                    placeholder="Doenças na família (ex: Diabetes na família materna)"
                    {...field}
                    value={Array.isArray(field.value) ? field.value.join(', ') : field.value || ''}
                    onChange={(e) => field.onChange(e.target.value.split(',').map(item => item.trim()))}
                  />
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <FormField
            control={form.control}
            name="medicalHistory.currentSymptoms"
            render={({ field }) => (
              <FormItem>
                <FormLabel>Sintomas Principais</FormLabel>
                <FormControl>
                  <Textarea 
                    placeholder="Descreva os sintomas atuais (ex: Dor lombar, Rigidez matinal)"
                    {...field}
                    value={Array.isArray(field.value) ? field.value.join(', ') : field.value || ''}
                    onChange={(e) => field.onChange(e.target.value.split(',').map(item => item.trim()))}
                  />
          ...
          <FormField
            control={form.control}
            name="medicalHistory.painLocation"
            render={({ field }) => (
              <FormItem>
          ...
          <FormField
            control={form.control}
            name="medicalHistory.painDuration"
            render={({ field }) => (
              <FormItem>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <FormField
            control={form.control}
            name="medicalHistory.exerciseFrequency"
            render={({ field }) => (
              <FormItem>
