...
import { withMemoization } from '../lib/memoization';
import { NavLink, Link, useNavigate } from 'react-router-dom';
import { LayoutGrid, Users, Calendar, Stethoscope, ChevronLeft, ChevronRight, BarChart3, ShieldCheck, Cog, Library, AreaChart, LogOut, FilePlus, FileClock, Dumbbell, AlertTriangle, Mail, BrainCircuit, ClipboardList, PieChart, DollarSign, SlidersHorizontal, Bell, MessageSquare, Handshake, Package, Ticket, Activity, Users2, BookMarked, FileText, TrendingUp, Monitor, HardDrive, CreditCard, Eye, FileCheck, Search, Target, FileSpreadsheet, Zap, Globe, Archive, Film } from 'lucide-react';
import { useApp } from "../contexts/AppContext";
import { useNotifications } from '../hooks/useNotifications';
import { Role } from '../types';
import SidebarSearch from './SidebarSearch';
import NotificationBell from './NotificationBell';
const NavLinkComponent = withMemoization(({ to, icon: Icon, label, isCollapsed, badgeCount }) => (<NavLink to={to} className={({ isActive }) => `flex items-center p-1.5 rounded-lg transition-colors duration-200 ${isActive
        ? 'bg-sky-50 text-sky-600 font-semibold'
        : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'} ${isCollapsed ? 'justify-center' : ''}`} title={isCollapsed ? label : undefined}>
        <div className="relative flex items-center w-full min-w-0">
            <Icon className={`w-4 h-4 shrink-0 ${isCollapsed ? '' : 'mr-2'}`}/>
            {!isCollapsed && (<span className="text-sm leading-tight break-words hyphens-auto flex-1 min-w-0">
                {label}
              </span>)}
            
            {!isCollapsed && badgeCount && badgeCount > 0 ? (<span className="ml-2 flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-xs font-medium text-white shrink-0">
                    {badgeCount > 9 ? '9+' : badgeCount}
                </span>) : null}

             {isCollapsed && badgeCount && badgeCount > 0 ? (<span className="absolute -top-1 -right-1 h-2 w-2 rounded-full bg-red-500 ring-2 ring-white"></span>) : null}
        </div>
    </NavLink>), {
    areEqual: (prev, next) => prev.to === next.to &&
        prev.label === next.label &&
        prev.isCollapsed === next.isCollapsed &&
        prev.badgeCount === next.badgeCount,
    displayName: 'NavLinkComponent'
});
const NavGroup = withMemoization(({ title, isCollapsed, children }) => (<div>
        {!isCollapsed && (<h3 className="px-2 pt-3 pb-1 text-xs font-semibold uppercase text-slate-400 tracking-wider">
                {title}
            </h3>)}
    ...
    areEqual: (prev, next) => prev.title === next.title &&
        prev.isCollapsed === next.isCollapsed,
...
const getFilteredNavigation = (userRole, unreadCount) => {
        ...
        case Role.Admin:
            ...
            return {
                ...
                clinicalNav: [
                    ...
                    { to: '/free-video-generator', icon: Film, label: 'Gerador de Vídeos' },
...
const Sidebar = () => {
    ...
    const navigation = useMemo(() => user ? getFilteredNavigation(user.role, unreadCount) : {
        mainNav: [],
        clinicalNav: [],
        aiToolsNav: [],
        managementNav: [],
    ...
    const getItemGroup = useCallback((to) => {
        if (navigation.mainNav.some((item) => item.to === to))
            return 'Principal';
        if (navigation.clinicalNav?.some((item) => item.to === to))
            return 'Clínico';
        if (navigation.analyticsNav?.some((item) => item.to === to))
            return 'Analytics & BI';
        if (navigation.aiToolsNav.some((item) => item.to === to))
            return 'Ferramentas IA';
        if (navigation.managementNav.some((item) => item.to === to))
            return 'Gestão';
        if (navigation.systemNav?.some((item) => item.to === to))
            return 'Sistema';
        return 'Outros';
    }, [navigation]);
    // Função para buscar itens da navegação - memoizada
    const searchNavigationItems = useCallback((query) => {
        if (!query.trim())
            return [];
        const allItems = [
            ...navigation.mainNav,
        ...
        return allItems.filter(item => item.label.toLowerCase().includes(query.toLowerCase())).map(item => ({
            ...item,
    ...
    const handleResultClick = useCallback((to) => {
        navigate(to);
    }, [navigate]);
    return (<aside className={`transition-all duration-300 ease-in-out bg-white border-r border-slate-200 flex flex-col ${isCollapsed ? 'w-14' : 'w-56'}`} data-testid="sidebar" role="navigation" aria-label="Menu principal">
      <div className="flex items-center p-3 border-b border-slate-200 h-14 shrink-0">
        ...
        <button onClick={() => setIsCollapsed(!isCollapsed)} className={`p-1.5 rounded-full text-slate-500 hover:bg-slate-100 transition-colors ${isCollapsed ? 'mx-auto' : 'ml-auto'}`} aria-label={isCollapsed ? "Expandir menu" : "Recolher menu"} title={isCollapsed ? "Expandir menu" : "Recolher menu"}>
          {isCollapsed ? <ChevronRight className="w-5 h-5"/> : <ChevronLeft className="w-5 h-5"/>}
        </button>
      </div>

      {/* Search Component */}
      <SidebarSearch isCollapsed={isCollapsed} onSearch={handleSearch} onClear={handleClearSearch} searchResults={searchNavigationItems(searchQuery)} onResultClick={handleResultClick}/>
      
      <nav className="flex-1 px-2 py-3 space-y-2 overflow-y-auto">
        {navigation.mainNav.length > 0 && (<NavGroup title="Principal" isCollapsed={isCollapsed}>
            {navigation.mainNav.map(item => <NavLinkComponent key={item.to} {...item} isCollapsed={isCollapsed}/>)}
          </NavGroup>)}
        {navigation.clinicalNav && navigation.clinicalNav.length > 0 && (<NavGroup title="Clínico" isCollapsed={isCollapsed}>
            {navigation.clinicalNav.map(item => <NavLinkComponent key={item.to} {...item} isCollapsed={isCollapsed}/>)}
          </NavGroup>)}
        {navigation.analyticsNav && navigation.analyticsNav.length > 0 && (<NavGroup title="Analytics & BI" isCollapsed={isCollapsed}>
            {navigation.analyticsNav.map(item => <NavLinkComponent key={item.to} {...item} isCollapsed={isCollapsed}/>)}
          </NavGroup>)}
        {navigation.aiToolsNav.length > 0 && (<NavGroup title="Ferramentas IA" isCollapsed={isCollapsed}>
            {navigation.aiToolsNav.map(item => <NavLinkComponent key={item.to} {...item} isCollapsed={isCollapsed}/>)}
          </NavGroup>)}
        {navigation.managementNav.length > 0 && (<NavGroup title="Gestão" isCollapsed={isCollapsed}>
            {navigation.managementNav.map(item => <NavLinkComponent key={item.to} {...item} isCollapsed={isCollapsed}/>)}
          </NavGroup>)}
        {navigation.systemNav && navigation.systemNav.length > 0 && (<NavGroup title="Sistema" isCollapsed={isCollapsed}>
            {navigation.systemNav.map(item => <NavLinkComponent key={item.to} {...item} isCollapsed={isCollapsed}/>)}
          </NavGroup>)}
      </nav>

      {user && (<div className="p-3 border-t border-slate-200 shrink-0">
            <div className="p-2 rounded-lg bg-gradient-to-r from-slate-50 to-sky-50 border border-slate-200">
                ...
                <Link to="/settings" title="Ver perfil e configurações" className="flex items-center w-full mb-2">
                    <div className="relative">
                        ...
                        <div className={`absolute -bottom-1 -right-1 w-3 h-3 rounded-full border-2 border-white ${user.role === Role.Admin ? 'bg-red-500' :
                user.role === Role.Therapist ? 'bg-blue-500' :
                    user.role === Role.EducadorFisico ? 'bg-orange-500' :
                        user.role === Role.Patient ? 'bg-green-500' :
                            user.role === Role.EducadorFisico ? 'bg-orange-500' :
                                'bg-gray-500'}`} title={`Perfil: ${user.role}`}/>
                    </div>
                    {!isCollapsed && (<div className="ml-3 text-left flex-1 overflow-hidden min-w-0">
                            <p className="text-sm font-semibold text-slate-700 truncate">{user.name}</p>
                            <div className="flex items-center gap-1">
                                <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${user.role === Role.Admin ? 'bg-red-100 text-red-700' :
                ...
                {!isCollapsed && (<div className="flex items-center justify-between space-x-1">
                ...
                {isCollapsed && (<div className="flex items-center justify-center space-x-1">
                        <NotificationBell unreadCount={unreadCount} isCollapsed={isCollapsed}/>
