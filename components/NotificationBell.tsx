...
const NotificationBell: React.FC<NotificationBellProps> = ({ unreadCount, isCollapsed }) => {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="flex items-center space-x-1 p-2 rounded-lg hover:bg-slate-100 transition-colors relative"
        title="Notificações"
      >
        <Bell className="w-4 h-4 text-slate-600" />
        <span className="text-xs font-medium text-slate-700 truncate">Notificações</span>
        {unreadCount > 0 && (
          <span className="bg-red-500 text-white text-xs rounded-full w-4 h-4 flex items-center justify-center text-xs shrink-0">
            {unreadCount > 9 ? '9+' : unreadCount}
          </span>
        )}
      </button>
