...
import { Zap, Plus, Edit2, Trash2, BarChart3, MessageSquare, Calendar, Bell, Play, } from 'lucide-react';
import { supabase } from '@/lib/supabase';
export const WhatsAppAutomationDashboard = ({ clinicId }) => {
      ...
      <Card>
              ...
              <DialogContent className="max-w-2xl">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <Input value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="Ex: Mensagem de Boas-Vindas"/>
                  </div>

                  <div>
                    <label className="text-sm font-medium">Descrição</label>
                    <Input value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} placeholder="Descreva o que esta automação faz"/>
                  </div>

                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="text-sm font-medium">Tipo de Gatilho</label>
                      <select value={formData.trigger_type} onChange={(e) => setFormData({ ...formData, trigger_type: e.target.value })} className="w-full px-3 py-2 border rounded-md">
                    ...
                    <div>
                      ...
                      <Input value={formData.trigger_value} onChange={(e) => setFormData({ ...formData, trigger_value: e.target.value })} placeholder={formData.trigger_type === 'keyword' ? 'Ex: oi|olá|ola' :
            formData.trigger_type === 'time_based' ? 'Ex: 09:00' :
                'Ex: appointment_confirmed'}/>
                  ...
                  <div>
                    ...
                    <select value={formData.action_type} onChange={(e) => setFormData({ ...formData, action_type: e.target.value })} className="w-full px-3 py-2 border rounded-md">
                  ...
                  {formData.action_type === 'send_message' && (<div>
                      <label className="text-sm font-medium">Mensagem</label>
                      <textarea value={formData.action_data.message} onChange={(e) => setFormData({
                ...formData,
                action_data: { message: e.target.value }
            })} className="w-full min-h-[150px] p-3 border rounded-md" placeholder="Digite a mensagem que será enviada automaticamente..."/>
                  ...
                  <div>
                    ...
                    <Input type="number" value={formData.priority} onChange={(e) => setFormData({ ...formData, priority: parseInt(e.target.value) || 0 })} placeholder="0"/>
                    <p className="text-xs text-muted-foreground mt-1">
        ...
        <CardContent>
          {loading ? (<div className="text-center py-8 text-muted-foreground">
            ...
            </div>) : automations.length === 0 ? (<div className="text-center py-8 text-muted-foreground">
              Nenhuma automação configurada
            </div>) : (<div className="space-y-4">
              {automations.map((automation) => (<div key={automation.id} className="p-4 border rounded-lg hover:bg-accent/50 transition-colors">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      {automation.description && (<p className="text-sm text-muted-foreground mb-2">
