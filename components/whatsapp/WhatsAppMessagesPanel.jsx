...
import { MessageSquare, Send, CheckCheck, Clock, AlertCircle, Search, Download, RefreshCw, } from 'lucide-react';
import { supabase } from '@/lib/supabase';
import { getMetaWhatsAppService } from '@/services/whatsapp/MetaWhatsAppService';
export const WhatsAppMessagesPanel = ({ clinicId }) => {
    ...
    const filterMessages = () => {
        ...
        if (searchTerm) {
            filtered = filtered.filter(m => m.phone.includes(searchTerm) ||
    ...
    const sendMessage = async () => {
        if (!selectedPhone || !messageContent)
            return;
        setSending(true);
        try {
    ...
    const getStatusBadge = (status) => {
        ...
        return (<Badge className={colors[status] || colors.pending}>
        {status}
      </Badge>);
    };
              ...
              <div className="flex gap-2">
                <div className="flex-1">
                  <div className="relative">
                    ...
                    <Input placeholder="Buscar por telefone, nome ou conteúdo..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-8"/>
                  </div>
                </div>
                <select value={selectedDirection} onChange={(e) => setSelectedDirection(e.target.value)} className="px-3 py-2 border rounded-md">
              ...
              <ScrollArea className="h-[600px]">
                <div className="space-y-2">
                  {loading ? (<div className="text-center py-8 text-muted-foreground">
                    ...
                    </div>) : (filteredMessages.map((message) => (<div key={message.id} className={`p-4 border rounded-lg ${message.direction === 'inbound'
            ...
            <TabsContent value="send" className="space-y-4">
              <div className="space-y-4">
                <div>
                  ...
                  <Input placeholder="5511999999999" value={selectedPhone} onChange={(e) => setSelectedPhone(e.target.value)}/>
                ...
                <div>
                  ...
                  <textarea className="w-full min-h-[200px] p-3 border rounded-md" placeholder="Digite sua mensagem..." value={messageContent} onChange={(e) => setMessageContent(e.target.value)}/>
                  <p className="text-xs text-muted-foreground mt-1">
                    {messageContent.length} caracteres
                  </p>
                </div>

                <Button onClick={sendMessage} disabled={sending || !selectedPhone || !messageContent} className="w-full">
                  {sending ? (<>
