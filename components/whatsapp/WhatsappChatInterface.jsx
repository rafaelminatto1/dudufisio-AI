...
const WhatsappChatInterface = () => {
    ...
    const addBotResponse = (text) => {
        setIsBotTyping(true);
        setTimeout(() => {
            const botMessage = { id: Date.now(), text, sender: 'bot' };
    ...
    const handleUserInput = async (text) => {
        ...
        if (mockPatient.id && mockPatient.name && mockPatient.phone) {
            whatsappService.sendMessage(mockPatient, text, 'chat');
        }
        // --- Simple Bot Logic ---
            ...
            <div className="w-full max-w-lg mx-auto bg-white border border-slate-200 rounded-2xl shadow-lg flex flex-col">
                ...
                <div className="flex-1 p-4 h-96 overflow-y-auto bg-slate-50">
                    <div className="space-y-4">
                        {messages.map(msg => (<div key={msg.id} className={`flex items-end gap-2 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                        ...
                        {isBotTyping && (<div className="flex items-end gap-2 justify-start">
                ...
                <div className="p-3 bg-slate-100 rounded-b-2xl border-t">
                    <div className="flex items-center space-x-2">
                        <input type="text" value={input} onChange={e => setInput(e.target.value)} onKeyPress={e => e.key === 'Enter' && input.trim() && handleUserInput(input)} placeholder="Digite sua mensagem..." className="flex-1 w-full px-4 py-2 bg-white border border-slate-300 rounded-full focus:outline-none focus:ring-2 focus:ring-teal-500"/>
                        <button onClick={() => input.trim() && handleUserInput(input)} disabled={!input.trim()} className="p-3 bg-teal-500 text-white rounded-full hover:bg-teal-600 disabled:bg-teal-300 transition-colors">
