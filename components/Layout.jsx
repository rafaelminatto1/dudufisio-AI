...
import { LayoutGrid, Users, Calendar, Stethoscope, BarChart3, LogOut, Bell, Menu, X, Activity, DollarSign, ClipboardList } from 'lucide-react';
...
const Layout = ({ user, onLogout, children }) => {
    ...
    const NavItem = memo(({ item }) => (<NavLink to={item.path} onClick={() => setIsMobileMenuOpen(false)} end={item.path === '/dashboard'} className={({ isActive }) => {
            const active = isActive || isItemActive(item.path);
            return `w-full flex items-center p-2.5 rounded-lg transition-colors duration-200 ${active
                ? 'bg-sky-50 text-sky-600 font-semibold'
                : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'} ${isCollapsed ? 'justify-center' : ''}`;
    ...
    const groupItems = useCallback((items, group) => items.filter(item => item.group === group), []);
            ...
            {isMobileMenuOpen && (<div className="fixed inset-0 z-50 lg:hidden">
                    ...
                    <div className="fixed left-0 top-0 bottom-0 w-64 bg-white">
                        <div className="flex items-center justify-between p-4 border-b border-slate-200">
                            ...
                            <button onClick={() => setIsMobileMenuOpen(false)} className="p-1.5 rounded-lg hover:bg-slate-100 transition-colors" aria-label="Fechar menu">
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                ...
                <div className="lg:hidden bg-white border-b border-slate-200 px-4 py-3">
                    <div className="flex items-center justify-between">
                        <button onClick={() => setIsMobileMenuOpen(true)} className="p-2 rounded-lg hover:bg-slate-100 transition-colors" aria-label="Abrir menu de navegação">
                        ...
                        <div className="flex items-center space-x-2">
                            <button className="p-2 rounded-lg hover:bg-slate-100 transition-colors" aria-label="Notificações">
                                <Bell className="w-5 h-5 text-slate-600"/>
                            </button>
                            <button onClick={onLogout} className="p-2 rounded-lg hover:bg-slate-100 transition-colors" aria-label="Sair do sistema">
                ...
                {/* Content */}
                <main id="main-content" className="flex-1 overflow-y-auto bg-slate-50" data-testid="main-content" role="main" aria-label="Conteúdo principal">
                    <div className="p-6">
                        <Breadcrumbs />
                    {children}
                    </div>
