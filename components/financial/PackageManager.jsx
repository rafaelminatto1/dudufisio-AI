...
import { Search, Filter, Plus, Eye, Package, Activity, AlertTriangle, CheckCircle, XCircle, Clock, TrendingUp, Download } from 'lucide-react';
...
export function PackageManager() {
    ...
    const handleConsumeSession = async (packageId) => {
        try {
    ...
    const getStatusBadge = (status) => {
        const statusConfig = {
            active: { label: 'Ativo', variant: 'default', icon: CheckCircle, color: 'bg-green-100 text-green-800' },
            expired: { label: 'Expirado', variant: 'destructive', icon: XCircle, color: 'bg-red-100 text-red-800' },
            cancelled: { label: 'Cancelado', variant: 'outline', icon: XCircle, color: 'bg-gray-100 text-gray-800' },
            suspended: { label: 'Suspenso', variant: 'secondary', icon: Clock, color: 'bg-yellow-100 text-yellow-800' }
        };
    ...
    return (<div className="space-y-6">
      ...
      {/* Stats Cards */}
      {stats && (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
          <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            <div className="relative">
              ...
              <Input placeholder="Buscar por paciente..." value={filters.search} onChange={(e) => setFilters(prev => ({ ...prev, search: e.target.value }))} className="pl-10"/>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (Array.from({ length: 6 }).map((_, i) => (<Card key={i} className="animate-pulse">
            ...
            </Card>))) : (filteredPackages.map((pkg) => {
            const healthStatus = getHealthStatus(pkg);
            return (<Card key={pkg.id} className="hover:shadow-lg transition-shadow">
                <CardContent className="p-6">
                  <div className="space-y-4">
                    {/* Header */}
                    ...
                    {/* Health Status */}
                    {healthStatus && (<div className={`flex items-center gap-2 text-sm ${healthStatus.color}`}>
                    ...
                    setShowDetails(true);
                      ...
                      {pkg.status === 'active' && pkg.remainingSessions > 0 && (<Button variant="default" size="sm" onClick={() => handleConsumeSession(pkg.id)}>
      ...
      {filteredPackages.length === 0 && !loading && (<Card>
      ...
      <Dialog open={showDetails} onOpenChange={setShowDetails}>
        <DialogContent className="max-w-2xl">
          ...
          {selectedPackage && (<div className="space-y-6">
              ...
              {/* Actions */}
              <div className="flex gap-2 pt-4 border-t">
                {selectedPackage.status === 'active' && selectedPackage.remainingSessions > 0 && (<Button onClick={() => handleConsumeSession(selectedPackage.id)}>
                    <Activity className="w-4 h-4 mr-2"/>
                    Consumir Sessão
                  </Button>)}
                
                {selectedPackage.status === 'active' && (<Button variant="outline" onClick={() => handleStatusUpdate(selectedPackage.id, 'suspended')}>
                    <Clock className="w-4 h-4 mr-2"/>
                    Suspender
                  </Button>)}
                
                {selectedPackage.status === 'suspended' && (<Button variant="outline" onClick={() => handleStatusUpdate(selectedPackage.id, 'active')}>
