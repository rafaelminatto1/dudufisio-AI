...
const TransactionRow = ({ transaction, onEdit }) => {
    ...
    return (<tr className="border-b border-slate-200 hover:bg-slate-50" onClick={!isRevenue ? onEdit : undefined} style={{ cursor: isRevenue ? 'default' : 'pointer' }}>
            <td className="p-3">
...
const TransactionList = ({ transactions, onAddExpense, onEditExpense }) => {
            ...
            <div className="overflow-y-auto max-h-96">
                <table className="min-w-full">
                    ...
                    <tbody className="divide-y divide-slate-200">
                        {transactions.length > 0 ? (transactions.map(txn => (<TransactionRow key={txn.id} transaction={txn} onEdit={() => onEditExpense(txn)}/>))) : (<tr><td colSpan={3} className="text-center p-10 text-slate-500">Nenhum lançamento no período.</td></tr>)}
                    </tbody>
                </table>
            </div>
        </div>);
