...
import { Search, Filter, Plus, Eye, Download, Send, FileText, Calendar, AlertTriangle, CheckCircle, XCircle, Mail, RefreshCw, Edit } from 'lucide-react';
...
export function InvoiceManager() {
    ...
    const getStatusBadge = (status, daysOverdue) => {
        const statusConfig = {
            ...
            issued: { label: 'Emitida', variant: 'default', icon: FileText },
            paid: { label: 'Paga', variant: 'default', icon: CheckCircle },
            overdue: { label: `Vencida (${daysOverdue}d)`, variant: 'destructive', icon: AlertTriangle },
    ...
    return (<div className="space-y-6">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            <div className="relative">
              ...
              <Input placeholder="Buscar por paciente ou número..." value={filters.search} onChange={(e) => setFilters(prev => ({ ...prev, search: e.target.value }))} className="pl-10"/>
            </div>
            ...
            <Button variant={filters.overdueOnly ? "default" : "outline"} onClick={() => setFilters(prev => ({ ...prev, overdueOnly: !prev.overdueOnly }))}>
      ...
      <Card>
        ...
        <CardContent>
          {loading ? (<div className="flex items-center justify-center h-32">
                ...
                <tbody>
                  {filteredInvoices.map((invoice) => (<tr key={invoice.id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="p-3">
                        ...
                        {invoice.notes && (<div className="text-sm text-gray-500 truncate max-w-xs">
                            {invoice.notes}
                          </div>)}
                      </td>
                      <td className="p-3">
                        <div className="font-bold">{formatCurrency(invoice.totalAmount)}</div>
                        {invoice.discountAmount > 0 && (<div className="text-sm text-gray-500">
                      ...
                      <td className="p-3">
                        <div className="flex items-center gap-1 text-sm">
                          ...
                          {invoice.daysUntilDue < 0 && (<span className="text-red-500 ml-1">
                    ...
                    setShowDetails(true);
                          ...
                          {getInvoiceActions(invoice).slice(0, 2).map((action, index) => (<Button key={index} variant={action.variant} size="sm" onClick={action.onClick}>
              ...
              {filteredInvoices.length === 0 && (<div className="text-center py-8 text-gray-500">
      ...
      <Dialog open={showDetails} onOpenChange={setShowDetails}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          {selectedInvoice && (<Tabs defaultValue="details" className="w-full">
              ...
              <TabsContent value="details" className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      {selectedInvoice.daysUntilDue < 0 && (<p className="text-sm text-red-500">
                ...
                <div className="bg-gray-50 p-4 rounded-lg space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    {selectedInvoice.discountAmount > 0 && (<div className="flex justify-between text-green-600">
                        <span>Desconto:</span>
                        <span className="font-medium">-{formatCurrency(selectedInvoice.discountAmount)}</span>
                      </div>)}
                    {selectedInvoice.totalTax > 0 && (<div className="flex justify-between">
                        <span>Impostos:</span>
                ...
                <div className="space-y-3">
                  ...
                  <div className="border rounded-lg overflow-hidden">
                    <table className="w-full">
                      ...
                      <tbody>
                        {selectedInvoice.lineItems.map((item, index) => (<tr key={item.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                            <td className="p-3">
                              <div className="font-medium">{item.description}</div>
                              {item.metadata && (<div className="text-sm text-gray-500">
                                  {Object.entries(item.metadata).map(([key, value]) => (<span key={key} className="mr-2">
                                      {key}: {String(value)}
                                    </span>))}
                                </div>)}
                            </td>
              ...
              <TabsContent value="actions" className="space-y-4">
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {getInvoiceActions(selectedInvoice).map((action, index) => (<Button key={index} variant={action.variant} onClick={action.onClick} className="justify-start">
                  ...
                  <div className="border-t pt-4">
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      {selectedInvoice.status !== 'draft' && (<div className="flex items-center gap-2">
                          <div className="w-2 h-2 bg-green-500 rounded-full"></div>
                          <span>Fatura emitida</span>
                        </div>)}
                      {selectedInvoice.status === 'paid' && (<div className="flex items-center gap-2">
                          <div className="w-2 h-2 bg-green-500 rounded-full"></div>
