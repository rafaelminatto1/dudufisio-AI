...
import { Search, Filter, Plus, Eye, DollarSign, Calendar, CreditCard, RefreshCw, Download, MoreHorizontal, CheckCircle, XCircle, Clock, AlertTriangle } from 'lucide-react';
    ...
    return (<div className="space-y-6">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            <div className="relative">
              ...
              <Input placeholder="Buscar por paciente ou descrição..." value={filters.search} onChange={(e) => setFilters(prev => ({ ...prev, search: e.target.value }))} className="pl-10"/>
            </div>
        ...
        <CardContent>
          {loading ? (<div className="flex items-center justify-center h-32">
              ...
              <table className="w-full">
                ...
                <tbody>
                  {filteredTransactions.map((transaction) => (<tr key={transaction.id} className="border-b hover:bg-gray-50">
                      <td className="p-3">
                        <div>
                      ...
                      <td className="p-3">
                        ...
                        {transaction.installments > 1 && (<div className="text-xs text-gray-500 mt-1">
                            {transaction.installmentNumber}/{transaction.installments}
                          </div>)}
                      </td>
                      <td className="p-3">
                        <div className="font-bold">{formatCurrency(transaction.amount)}</div>
                        {transaction.taxAmount > 0 && (<div className="text-sm text-gray-500">
                      ...
                      <td className="p-3">
                        {transaction.paidDate ? (<div className="flex items-center gap-1 text-sm text-green-600">
                            <CheckCircle className="w-4 h-4"/>
                            {formatDate(transaction.paidDate)}
                          </div>) : (<span className="text-sm text-gray-400">Não pago</span>)}
                      </td>
                      <td className="p-3">
                    ...
                    setShowDetails(true);
                          ...
                          {transaction.status === 'pending' && (<Button variant="default" size="sm" onClick={() => handleProcessPayment(transaction.id)}>
      ...
      <Dialog open={showDetails} onOpenChange={setShowDetails}>
        <DialogContent className="max-w-2xl">
          ...
          {selectedTransaction && (<Tabs defaultValue="details" className="w-full">
              ...
              <TabsContent value="history" className="space-y-4">
                <div className="space-y-3">
                  ...
                  {selectedTransaction.paidDate && (<div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
