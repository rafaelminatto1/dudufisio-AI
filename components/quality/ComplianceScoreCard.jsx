...
export const ComplianceScoreCard = ({ standard, complianceRate, passedChecks, failedChecks, totalChecks, }) => {
    ...
    return (<div className={`rounded-lg border-2 p-6 transition ${getStatusColor()}`}>
      ...
      <div className="mb-4">
        ...
        <div className="w-full bg-gray-200 rounded-full h-3">
          <div className={`h-3 rounded-full transition-all ${complianceRate >= 95 ? 'bg-green-600' :
            complianceRate >= 80 ? 'bg-blue-600' :
                complianceRate >= 60 ? 'bg-orange-600' :
                    'bg-red-600'}`} style={{ width: `${complianceRate}%` }}/>
        </div>
      </div>
