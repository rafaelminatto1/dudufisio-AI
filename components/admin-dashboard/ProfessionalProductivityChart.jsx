...
const ProfessionalProductivityChart = ({ data, isLoading = false }) => {
    if (isLoading) {
    ...
    // Top performer
    const topPerformer = data.reduce((max, curr) => curr.revenue > max.revenue ? curr : max, data[0]);
      ...
      <CardContent>
        <ResponsiveContainer width="100%" height={320}>
          <BarChart data={data} margin={{ top: 10, right: 10, left: 10, bottom: 50 }}>
            ...
            <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fill: '#64748b', fontSize: 11 }} angle={-45} textAnchor="end" height={80}/>
            <YAxis axisLine={false} tickLine={false} tick={{ fill: '#64748b', fontSize: 12 }} tickFormatter={(value) => `${value}`}/>
            <Tooltip content={<CustomTooltip />}/>
            
            <Bar dataKey="sessions" radius={[4, 4, 0, 0]}>
              {data.map((entry, index) => (<Cell key={`cell-${index}`} fill={getBarColor(entry.revenue)}/>))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
        ...
        <div className="mt-6">
          ...
          <div className="space-y-2">
            ...
            return (<div key={professional.name} className="flex items-center justify-between p-2 bg-gray-50 rounded-lg">
