...
import { DollarSign, Clock, FileText, Target, AlertTriangle, Eye, CheckCircle, XCircle } from 'lucide-react';
const AdminAlerts = ({ alerts, onViewDetails, onMarkResolved, isLoading = false }) => {
    ...
    return (<div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {highPriorityAlerts.length > 0 && (<Badge className="bg-red-100 text-red-800">
              {highPriorityAlerts.length} crítico{highPriorityAlerts.length > 1 ? 's' : ''}
            </Badge>)}
          {mediumPriorityAlerts.length > 0 && (<Badge className="bg-yellow-100 text-yellow-800">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {alerts.map((alert) => (<Alert key={alert.id} className={`${getSeverityColor(alert.severity)} border hover:shadow-md transition-shadow`}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between mb-2">
                  ...
                  <div className="flex items-center gap-1 ml-2">
                    {alert.count && (<Badge variant="secondary" className="text-xs">
                        {alert.count}
                      </Badge>)}
                ...
                {alert.dueDate && (<div className="text-xs text-gray-500 mb-3">
                    Vencimento: {alert.dueDate}
                  </div>)}

                <div className="flex items-center gap-2">
                  {onViewDetails && (<Button size="sm" variant="outline" onClick={() => onViewDetails(alert.id)} className="text-xs h-7 px-2">
                      <Eye className="w-3 h-3 mr-1"/>
                      Ver
                    </Button>)}
                  {onMarkResolved && alert.actionRequired !== false && (<Button size="sm" onClick={() => onMarkResolved(alert.id)} className="text-xs h-7 px-2" variant={alert.severity === 'high' ? 'default' : 'secondary'}>
