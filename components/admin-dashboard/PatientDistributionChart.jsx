...
const PatientDistributionChart = ({ data, isLoading = false }) => {
    if (isLoading) {
    ...
    const CustomLabel = ({ cx, cy, midAngle, innerRadius, outerRadius, percent }) => {
        ...
        if (percent < 0.05)
            return null; // Não mostrar label se muito pequeno
      ...
      <CardContent>
        <div className="h-80">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie data={dataWithPercentage} cx="50%" cy="50%" labelLine={false} label={CustomLabel} outerRadius={100} fill="#8884d8" dataKey="value">
                {dataWithPercentage.map((entry, index) => (<Cell key={`cell-${index}`} fill={entry.color}/>))}
              </Pie>
        ...
        <div className="mt-4 space-y-2">
          {dataWithPercentage.map((item, index) => (<div key={index} className="flex items-center justify-between p-2 bg-gray-50 rounded-lg">
        ...
        <div className="mt-4 p-3 bg-yellow-50 rounded-lg">
          ...
          <ul className="text-xs text-yellow-700 space-y-1">
            ...
            (dataWithPercentage.find(item => item.name === 'Ativos')?.value || 0) * 0.2 && (<li>• Alta taxa de pacientes inativos - considere campanha de reativação</li>)}
            {(dataWithPercentage.find(item => item.name === 'Em Avaliação')?.value || 0) >
            total * 0.1 && (<li>• Muitos pacientes em avaliação - acelere processo de triagem</li>)}
