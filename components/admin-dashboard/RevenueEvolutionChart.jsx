...
const RevenueEvolutionChart = ({ data, isLoading = false }) => {
    if (isLoading) {
    ...
    const CustomTooltip = ({ active, payload, label }) => {
        if (active && payload && payload.length) {
          ...
          {payload.map((entry, index) => (<div key={index} className="flex items-center gap-2 mb-1">
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            ...
            <div className={`text-lg font-bold ${parseFloat(goalAchievement) >= 100 ? 'text-green-600' :
      ...
      <CardContent>
        <ResponsiveContainer width="100%" height={320}>
          <ComposedChart data={data} margin={{ top: 10, right: 10, left: 10, bottom: 10 }}>
            ...
            <Line type="monotone" dataKey="goal" stroke="#ef4444" strokeWidth={2} strokeDasharray="8 8" dot={{ fill: '#ef4444', strokeWidth: 2, r: 4 }} name="goal"/>
          </ComposedChart>
        </ResponsiveContainer>
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-purple-50 p-3 rounded-lg">
            ...
            <div className="text-lg font-bold text-purple-600">
              {data.length >= 2 ? (((data[data.length - 1].revenue - data[0].revenue) / data[0].revenue * 100).toFixed(1)) : '0'}%
            </div>
