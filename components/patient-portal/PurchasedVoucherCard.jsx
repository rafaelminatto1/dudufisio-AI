...
const PurchasedVoucherCard = ({ voucher }) => {
    const { plan, remainingCredits, status, expiryDate } = voucher;
    const progressPercentage = (remainingCredits / plan.credits) * 100;
    return (<div className="bg-white p-6 rounded-2xl shadow-sm border-l-4 border-teal-500">
            <div className="flex justify-between items-start">
                <div>
                    <h3 className="text-xl font-bold text-slate-800">{plan.name}</h3>
                    <p className="text-sm text-slate-500">{plan.description}</p>
                </div>
                <span className={`px-3 py-1 text-xs font-semibold rounded-full capitalize ${status === 'activated' ? 'bg-green-100 text-green-800' : 'bg-slate-100 text-slate-700'}`}>
