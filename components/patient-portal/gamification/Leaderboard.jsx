...
const Leaderboard = ({ entries, highlightPatientId }) => {
    if (!entries.length)
        return null;
    return (<div className="bg-white p-6 rounded-2xl shadow-sm">
      ...
      <div className="space-y-3">
        {entries.map((entry) => {
            ...
            return (<div key={entry.patientId} className={`flex items-center justify-between border rounded-xl px-3 py-2 text-sm ${isHighlighted ? 'border-teal-300 bg-teal-50' : 'border-slate-200 bg-slate-50'}`}>
