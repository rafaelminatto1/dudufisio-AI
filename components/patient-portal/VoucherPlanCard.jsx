...
const VoucherPlanCard = ({ plan, onSelect }) => {
    return (<div className={`rounded-2xl border-2 p-6 flex flex-col h-full transition-all duration-300 ${plan.popular ? 'border-teal-500 shadow-lg' : 'border-slate-200 bg-white'}`}>
             {plan.popular && (<div className="flex justify-center mb-4">
            ...
            <button onClick={() => onSelect(plan)} className={`w-full py-3 px-6 rounded-lg font-semibold text-center transition-colors ${plan.popular ? 'bg-teal-500 text-white hover:bg-teal-600' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}>
                Adquirir Plano
            </button>
        </div>);
};
