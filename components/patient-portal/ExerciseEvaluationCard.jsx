...
const ExerciseEvaluationCard = ({ exercise, prescription, todaysEvaluation, onSave }) => {
    const [rating, setRating] = useState(todaysEvaluation?.rating);
    const [painLevel, setPainLevel] = useState(todaysEvaluation?.painLevel);
    ...
    return (<div className="bg-white rounded-2xl shadow-sm flex flex-col overflow-hidden">
            ...
            <div className="p-4 bg-slate-50 border-t">
                ...
                {todaysEvaluation ? (<div className="p-4 bg-green-50 text-green-800 border-l-4 border-green-400 rounded-r-lg text-sm text-center font-semibold flex items-center justify-center">
                ...
                <div className="space-y-4 mt-4">
                    <div>
                        ...
                        <div className="flex justify-around">
                            {ratingOptions.map(opt => (<button key={opt.value} onClick={() => setRating(opt.value)} className={`flex flex-col items-center p-2 rounded-lg w-20 transition-all ${rating === opt.value ? 'bg-teal-100 ring-2 ring-teal-500' : 'hover:bg-slate-100'}`}>
                                    {opt.icon}
                    ...
                    <div>
                        ...
                        <textarea value={comments} onChange={(e) => setComments(e.target.value)} rows={2} className="mt-1 w-full p-2 border border-slate-300 rounded-lg" placeholder="Ex: Senti um estalo no joelho, doeu mais na última série..."/>
                    </div>
                    <button onClick={handleSave} disabled={isSaving || rating === undefined || painLevel === undefined} className="w-full inline-flex items-center justify-center rounded-lg border border-transparent bg-teal-500 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-teal-600 disabled:bg-teal-300">
