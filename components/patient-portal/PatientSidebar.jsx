...
const NavLinkComponent = ({ to, icon: Icon, label, badgeCount }) => (<ReactRouterDOM.NavLink to={to} className={({ isActive }) => `flex items-center p-3 rounded-lg transition-colors duration-200 ${isActive
        ? 'bg-teal-50 text-teal-600 font-semibold'
        : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'}`}>
        <Icon className="w-5 h-5 mr-4"/>
        <span className="flex-1">{label}</span>
        {badgeCount && badgeCount > 0 ? (<span className="ml-auto flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-xs font-medium text-white">
...
const PatientSidebar = () => {
    ...
    return (<div className="w-64 bg-white border-r border-slate-200 flex flex-col">
      ...
      <nav className="flex-1 px-3 py-4 space-y-1">
        {navItems.map(({ to, icon, label, badgeCount }) => (<NavLinkComponent key={to} to={to} icon={icon} label={label} {...(typeof badgeCount === 'number' && { badgeCount })}/>))}
      </nav>
      {user && (<div className="p-3 border-t border-slate-200">
