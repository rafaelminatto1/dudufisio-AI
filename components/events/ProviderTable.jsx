...
const ProviderTable = ({ providers }) => {
    ...
    const handlePay = async (providerId) => {
        setProcessingId(providerId);
        try {
    ...
    return (<div>
            <div className="relative mb-4 max-w-sm">
                ...
                <input type="text" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder="Buscar fisioterapeuta..." className="w-full pl-10 pr-4 py-2 border border-slate-300 rounded-lg"/>
            </div>
             <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-slate-200">
                    ...
                    <tbody className="bg-white divide-y divide-slate-200">
                        {filteredProviders.map(provider => (<tr key={provider.id}>
                                ...
                                <td className="p-3 text-center space-x-2">
                                     {processingId === provider.id ? (<Loader className="w-4 h-4 animate-spin mx-auto text-slate-400"/>) : (<>
                                            {provider.status === ProviderStatus.Applied && (<button onClick={() => handleConfirm(provider.id)} className="p-1.5 text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-full" title="Confirmar Presença">
                                                    <Check className="w-4 h-4"/>
                                                </button>)}
                                            {provider.status === ProviderStatus.Confirmed && (<button onClick={() => handlePay(provider.id)} className="p-1.5 text-green-600 bg-green-50 hover:bg-green-100 rounded-full" title="Registrar Pagamento">
             ...
             {filteredProviders.length === 0 && (<p className="text-center py-8 text-sm text-slate-500">Nenhum fisioterapeuta encontrado.</p>)}
        </div>);
};
