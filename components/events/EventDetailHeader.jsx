...
const EventDetailHeader = ({ event, onEdit, onCheckIn }) => {
    const { text, bg, dot } = statusStyles[event.status];
    const formattedDate = event.startDate.toLocaleString('pt-BR', {
        dateStyle: 'full',
        timeStyle: 'short'
    });
    return (<div className="bg-white rounded-2xl shadow-sm overflow-hidden mb-8">
            <div className="relative h-48 md:h-64 bg-slate-200">
                <img src={event.bannerUrl || 'https://images.unsplash.com/photo-1511795409834-ef04bbd61622?q=80&w=1470'} alt={`Banner for ${event.name}`} className="w-full h-full object-cover"/>
