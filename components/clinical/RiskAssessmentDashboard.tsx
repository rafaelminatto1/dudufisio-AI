...
interface Props {
  ...
  onTakeAction: (assessment: RiskAssessment) => void;
}

export const RiskAssessmentDashboard: React.FC<Props> = ({
  riskProfile,
  onViewDetails,
  onTakeAction
}) => {
  ...
  const getRiskTypeName = (type: RiskType): string => {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-md p-4">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {Array.from(new Set(riskProfile.assessments.map(a => a.riskType))).map(type => (
            <button
              key={type}
        ...
        {filteredAssessments.map(assessment => (
          ...
          >
              ...
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div
                  className={`h-2 rounded-full transition-all ${
                    assessment.score >= 75 ? 'bg-red-600' :
                    assessment.score >= 50 ? 'bg-orange-600' :
                    assessment.score >= 25 ? 'bg-yellow-600' : 'bg-green-600'
                  }`}
            ...
            <div className="flex gap-2">
              ...
              {(assessment.riskLevel === RiskLevel.High || assessment.riskLevel === RiskLevel.Critical) && (
                <button
                  onClick={() => onTakeAction(assessment)}
