...
const Breadcrumbs = () => {
    ...
    const routeLabels = {
        ...
        '/patient-progress': 'Meu Progresso',
        '/pain-diary': 'Diário de Dor',
        '/documents': 'Documentos',
        '/gamification': 'Gamificação',
        '/voucher-store': 'Loja de Vouchers',
        '/educator-dashboard': 'Dashboard Educador',
        '/client-list': 'Lista de Clientes',
        '/partner-exercises': 'Exercícios Parceiro',
        '/ai-tools/consolidated': 'Ferramentas de IA',
        '/reports/consolidated': 'Relatórios Consolidados',
        '/reports/evaluation': 'Relatórios de Avaliação',
    };
    ...
    return (<nav className="flex items-center space-x-1 text-sm text-slate-500 mb-4" aria-label="Breadcrumb">
      <Link to="/dashboard" className="flex items-center hover:text-slate-700 transition-colors" aria-label="Ir para página inicial">
        <Home className="w-4 h-4"/>
      </Link>
      
      {breadcrumbs.map((item, index) => (<React.Fragment key={index}>
          <ChevronRight className="w-4 h-4 text-slate-400"/>
          {item.href ? (<Link to={item.href} className="hover:text-slate-700 transition-colors">
              {item.label}
            </Link>) : (<span className="text-slate-900 font-medium">
              {item.label}
            </span>)}
        </React.Fragment>))}
    </nav>);
