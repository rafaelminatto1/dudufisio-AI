...
import { Plus, Search, Filter, Package, Calendar, Edit, Trash2, Eye } from 'lucide-react';
const SuppliesList = ({ onAddSupply, onEditSupply, onViewSupply, onDeleteSupply }) => {
    ...
    const handleStockFilter = (lowStock) => {
        setFilters(prev => ({
            ...prev,
            lowStock: lowStock || undefined
        }));
    };
    const handleExpiringFilter = (expiring) => {
        setFilters(prev => ({
            ...prev,
    ...
    const getExpirationStatus = (supply) => {
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <button onClick={onAddSupply} className="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          ...
          <div className="flex-1">
            <div className="relative">
              ...
              <input type="text" placeholder="Buscar insumos..." value={searchTerm} onChange={(e) => handleSearch(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"/>
            </div>
          </div>

          {/* Filtros Avançados */}
          <div className="flex items-center gap-2">
            <button onClick={() => setShowFilters(!showFilters)} className="inline-flex items-center px-3 py-2 border border-gray-300 rounded-lg hover:bg-gray-50">
        ...
        {showFilters && (<div className="mt-4 pt-4 border-t border-gray-200">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <select onChange={(e) => handleCategoryFilter(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
              ...
              <div>
                ...
                <select onChange={(e) => handleStockFilter(e.target.value === 'low')} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
              ...
              <div>
                ...
                <select onChange={(e) => handleExpiringFilter(e.target.value === 'expiring')} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                  <option value="">Todos</option>
      ...
      <div className="bg-white rounded-lg border overflow-hidden">
        {supplies.length === 0 ? (<div className="text-center py-12">
            ...
            {!searchTerm && !filters.category && !filters.lowStock && !filters.expiring && (<button onClick={onAddSupply} className="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
                <Plus className="h-5 w-5 mr-2"/>
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200">
                ...
                return (<tr key={supply.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4">
                        <div>
                          <div className="text-sm font-medium text-gray-900">{supply.name}</div>
                          {supply.description && (<div className="text-sm text-gray-500">{supply.description}</div>)}
                          {supply.brand && (<div className="text-xs text-gray-400">Marca: {supply.brand}</div>)}
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <div className="space-y-1">
                          ...
                          {expStatus && (<span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${expStatus.color}`}>
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end space-x-2">
                          <button onClick={() => onViewSupply?.(supply)} className="text-gray-400 hover:text-gray-600" title="Visualizar">
                            <Eye className="h-4 w-4"/>
                          </button>
                          
                          <button onClick={() => onEditSupply?.(supply)} className="text-gray-400 hover:text-blue-600" title="Editar">
                            <Edit className="h-4 w-4"/>
                          </button>
                          
                          <button onClick={() => handleDeleteSupply(supply)} className="text-gray-400 hover:text-red-600" title="Excluir">
      ...
      {/* Resumo */}
      {supplies.length > 0 && (<div className="bg-gray-50 px-6 py-3 rounded-lg">
