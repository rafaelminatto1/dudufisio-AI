...
import { Package, AlertTriangle, Calendar, DollarSign, TrendingUp, Clock, ShoppingCart, AlertCircle } from 'lucide-react';
const SuppliesDashboard = ({ onNavigateToSupplies, onNavigateToAlerts, onNavigateToOrders }) => {
    ...
    return (<div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white p-6 rounded-lg shadow-sm border">
          ...
          <button onClick={onNavigateToSupplies} className="mt-4 text-sm text-blue-600 hover:text-blue-700 font-medium">
            Ver todos →
          </button>
        </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border">
          ...
          {dashboardData.lowStockCount > 0 && (<button onClick={onNavigateToAlerts} className="mt-4 text-sm text-orange-600 hover:text-orange-700 font-medium">
              Ver alertas →
            </button>)}
        </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border">
          ...
          {dashboardData.expiringCount > 0 && (<button onClick={onNavigateToAlerts} className="mt-4 text-sm text-red-600 hover:text-red-700 font-medium">
              Ver alertas →
            </button>)}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border">
          ...
          {dashboardData.topConsumedSupplies.length > 0 ? (<div className="space-y-3">
              {dashboardData.topConsumedSupplies.map((item, index) => (<div key={item.supplyId} className="flex items-center justify-between">
                  <div className="flex items-center">
            ...
            </div>) : (<p className="text-gray-500 text-sm">Nenhum consumo registrado no período</p>)}
        </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border">
          ...
          {dashboardData.recentMovements.length > 0 ? (<div className="space-y-3">
              {dashboardData.recentMovements.slice(0, 5).map((movement) => {
                const isEntrada = MovementTypeUtils.isEntrada(movement.movementType);
                return (<div key={movement.id} className="flex items-center justify-between">
                    <div className="flex items-center">
                      <div className={`w-2 h-2 rounded-full mr-3 ${isEntrada ? 'bg-green-500' : 'bg-red-500'}`}/>
                      <div>
                    ...
                    <div className="text-right">
                      <p className={`text-sm font-medium ${isEntrada ? 'text-green-600' : 'text-red-600'}`}>
            ...
            </div>) : (<p className="text-gray-500 text-sm">Nenhuma movimentação recente</p>)}
        </div>
      </div>

      {/* Alertas Críticos */}
      {dashboardData.alerts.length > 0 && (<div className="bg-white p-6 rounded-lg shadow-sm border">
          <div className="flex items-center justify-between mb-4">
          ...
          <div className="space-y-3">
            {dashboardData.alerts.slice(0, 5).map((alert) => (<div key={alert.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div className="flex items-center">
          ...
          {dashboardData.alerts.length > 5 && (<button onClick={onNavigateToAlerts} className="mt-4 text-sm text-blue-600 hover:text-blue-700 font-medium">
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <button onClick={onNavigateToSupplies} className="flex items-center justify-center p-4 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
            <Package className="h-6 w-6 text-blue-600 mr-2"/>
            <span className="text-sm font-medium text-gray-900">Gerenciar Insumos</span>
          </button>
          
          <button onClick={onNavigateToOrders} className="flex items-center justify-center p-4 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
          ...
          <button onClick={onNavigateToAlerts} className="flex items-center justify-center p-4 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
