...
const PatientTooltip = ({ appointment, x, y }) => {
    return (<div style={{ top: y, left: x, transform: 'translate(10px, -100%)' }} className="absolute z-20 w-64 p-3 bg-slate-800 text-white text-sm rounded-lg shadow-xl pointer-events-none animate-fade-in-fast">
      <h4 className="font-bold mb-2">{appointment.patientName}</h4>
      {appointment.sessionNumber && appointment.totalSessions && (<p className="text-xs text-slate-300 mb-2">
      ...
      {appointment.patientMedicalAlerts && (<div className="flex items-start p-2 bg-amber-500/20 text-amber-200 rounded-md">
