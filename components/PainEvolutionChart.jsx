...
const PainEvolutionChart = ({ bodyPoints, className = '' }) => {
    ...
    const latestTrend = useMemo(() => {
        ...
        if (last.avgPain > previous.avgPain)
            return 'up';
        if (last.avgPain < previous.avgPain)
            return 'down';
        return 'stable';
    ...
    const CustomTooltip = ({ active, payload, label }) => {
        if (active && payload?.length) {
            return (<div className="bg-white p-3 border border-slate-200 rounded-lg shadow-lg">
    ...
    if (chartData.length === 0) {
        return (<div className={`bg-white rounded-2xl shadow-sm border border-slate-200 p-6 ${className}`}>
    ...
    return (<div className={`bg-white rounded-2xl shadow-sm border border-slate-200 p-6 ${className}`}>
            <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center gap-2">
                    {latestTrend === 'up' && (<div className="flex items-center gap-1 text-red-600">
                            <TrendingUp className="w-4 h-4"/>
                            <span className="text-sm font-medium">Aumentando</span>
                        </div>)}
                    {latestTrend === 'down' && (<div className="flex items-center gap-1 text-green-600">
                            <TrendingDown className="w-4 h-4"/>
                            <span className="text-sm font-medium">Diminuindo</span>
                        </div>)}
                    {latestTrend === 'stable' && (<div className="flex items-center gap-1 text-slate-600">
            ...
            <div className="h-64 mb-6">
                <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={chartData}>
                        ...
                        <XAxis dataKey="date" tick={{ fontSize: 12, fill: '#64748b' }} tickLine={{ stroke: '#cbd5e1' }} axisLine={{ stroke: '#cbd5e1' }}/>

                        <YAxis domain={[0, 10]} tick={{ fontSize: 12, fill: '#64748b' }} tickLine={{ stroke: '#cbd5e1' }} axisLine={{ stroke: '#cbd5e1' }} label={{ value: 'Nível de Dor', angle: -90, position: 'insideLeft', style: { textAnchor: 'middle', fontSize: 12, fill: '#64748b' } }}/>

                        <Tooltip content={<CustomTooltip />}/>

                        {/* Área de máximo/mínimo */}
                        <Area dataKey="maxPain" stroke="none" fill="url(#painGradient)" fillOpacity={0.2}/>

                        {/* Linha de dor média */}
                        <Line type="monotone" dataKey="avgPain" stroke="#3b82f6" strokeWidth={3} dot={{ fill: '#3b82f6', strokeWidth: 2, r: 4 }} activeDot={{ r: 6, stroke: '#3b82f6', strokeWidth: 2, fill: '#ffffff' }}/>

                        {/* Linha de dor máxima */}
                        <Line type="monotone" dataKey="maxPain" stroke="#ef4444" strokeWidth={2} strokeDasharray="5 5" dot={{ fill: '#ef4444', strokeWidth: 2, r: 3 }}/>

                        {/* Linha de dor mínima */}
                        <Line type="monotone" dataKey="minPain" stroke="#22c55e" strokeWidth={2} strokeDasharray="5 5" dot={{ fill: '#22c55e', strokeWidth: 2, r: 3 }}/>
                    </AreaChart>
                </ResponsiveContainer>
            </div>
