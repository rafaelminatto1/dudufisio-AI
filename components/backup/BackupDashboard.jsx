...
import { HardDrive, Clock, Settings, Shield, Download, Upload, AlertTriangle, CheckCircle2, RotateCcw, Calendar, Database, Zap, Archive, Trash2, Eye, TrendingUp, Server, Cloud, RefreshCw } from 'lucide-react';
import { backupService } from '../../services/backup/backupService';
const BackupDashboard = ({ onBackupCreated, onConfigChanged }) => {
    ...
    return (<div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button onClick={handleTestSystem} disabled={isTestingSystem} className="flex items-center gap-2 px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors disabled:opacity-50">
              {isTestingSystem ? (<RefreshCw className="w-4 h-4 animate-spin"/>) : (<Zap className="w-4 h-4"/>)}
              Testar Sistema
            </button>

            <button onClick={() => setShowCreateBackup(true)} className="flex items-center gap-2 px-4 py-2 bg-sky-600 text-white rounded-lg hover:bg-sky-700 transition-colors">
      ...
      {currentBackup && (<div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        ].map(tab => (<button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-2 py-4 px-1 border-b-2 font-medium text-sm transition-colors ${activeTab === tab.id
        ...
        <div className="p-6">
          ...
          {activeTab === 'overview' && (<div className="space-y-6">
              {/* Storage Usage */}
              {stats?.storageUsage && (<div>
                  <h3 className="text-lg font-semibold text-gray-900 mb-4">Uso de Armazenamento</h3>
                  <div className="space-y-4">
                    {stats.storageUsage.map((usage, index) => (<div key={index} className="bg-gray-50 rounded-lg p-4">
                        ...
                        <div className="w-full bg-gray-200 rounded-full h-2">
                          <div className="bg-sky-600 h-2 rounded-full" style={{ width: `${Math.min(usage.percentage, 100)}%` }}/>
                        </div>
                      </div>))}
                  </div>
                </div>)}
              ...
              <div>
                ...
                <div className="space-y-3">
                  {history.slice(0, 5).map((backup) => (<div key={backup.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                      <div className="flex items-center gap-3">
                      ...
                      <div className="text-right">
                        ...
                        {backup.duration && (<p className="text-xs text-gray-500">{formatDuration(backup.duration)}</p>)}
                      </div>
                    </div>))}
                </div>
              </div>
          ...
          {activeTab === 'history' && (<div>
              <div className="flex items-center justify-between mb-6">
                ...
                <button onClick={loadDashboardData} className="flex items-center gap-2 px-3 py-2 text-sm border border-gray-300 rounded-lg hover:bg-gray-50">
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {history.map((backup) => (<tr key={backup.id} className="hover:bg-gray-50">
                ...
                }} disabled={backup.status !== 'completed'} className="text-sky-600 hover:text-sky-900 disabled:text-gray-400 disabled:cursor-not-allowed" title="Restaurar">
                              <Download className="w-4 h-4"/>
                            </button>
                            <button className="text-gray-600 hover:text-gray-900" title="Detalhes">
          ...
          {/* Settings Tab */}
          {activeTab === 'settings' && config && (<div className="space-y-6">
              ...
              <div>
                ...
                <div className="space-y-3">
                  {config.destinations.map((destination, index) => (<div key={destination.id} className="bg-gray-50 rounded-lg p-4">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          {destination.type === 'local' ? (<HardDrive className="w-5 h-5 text-gray-600"/>) : destination.type === 'supabase' ? (<Cloud className="w-5 h-5 text-blue-600"/>) : (<Server className="w-5 h-5 text-purple-600"/>)}
                          <div>
          ...
          {activeTab === 'restore' && (<div className="space-y-6">
              ...
              <div>
                ...
                .slice(0, 10)
                .map((backup) => (<div key={backup.id} className="bg-gray-50 rounded-lg p-4">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-4">
                            <div className="p-2 bg-white rounded">
                              {backup.type === 'full' ? (<Database className="w-5 h-5 text-blue-600"/>) : (<Archive className="w-5 h-5 text-green-600"/>)}
                            </div>
                            <div>
                              <p className="font-medium text-gray-900">
                                {backup.type === 'full' ? 'Backup Completo' : 'Backup Incremental'}
      ...
      {/* Modal Create Backup */}
      {showCreateBackup && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-4">
            <div className="p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Criar Novo Backup</h3>

              <div className="space-y-4">
                <button onClick={() => handleCreateBackup('incremental')} className="w-full p-4 border-2 border-gray-200 rounded-lg hover:border-sky-300 hover:bg-sky-50 transition-colors text-left">
                  <div className="flex items-center gap-3">
                    <Archive className="w-6 h-6 text-green-600"/>
                    <div>
                ...
                <button onClick={() => handleCreateBackup('full')} className="w-full p-4 border-2 border-gray-200 rounded-lg hover:border-sky-300 hover:bg-sky-50 transition-colors text-left">
                  <div className="flex items-center gap-3">
                    <Database className="w-6 h-6 text-blue-600"/>
                    <div>
              ...
              <div className="flex gap-3 mt-6">
                <button onClick={() => setShowCreateBackup(false)} className="flex-1 px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50">
                  Cancelar
                </button>
              </div>
            </div>
      ...
      {showRestoreModal && selectedBackup && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            ...
            <div className="p-6">
              ...
              <div className="space-y-4">
                ...
                <div className="text-sm text-gray-600">
                  ...
                  {selectedBackup.duration && (<p>Duração do backup: {formatDuration(selectedBackup.duration)}</p>)}
                </div>
              </div>

              <div className="flex gap-3 mt-6">
                <button onClick={() => setShowRestoreModal(false)} className="flex-1 px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50">
                  Cancelar
                </button>
