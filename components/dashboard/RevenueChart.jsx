...
const RevenueChart = ({ appointments }) => {
    const revenueData = useMemo(() => {
        ...
        const completedAppointments = appointments.filter(app => app.status === AppointmentStatus.Completed && app.startTime >= thirtyDaysAgo);
        for (let i = 0; i <= 30; i++) {
            const date = new Date();
    ...
    return (<div className="bg-white p-6 rounded-2xl shadow-sm hover:shadow-lg transition-shadow duration-300 h-80">
            <h3 className="text-lg font-semibold text-slate-800 mb-4 flex items-center"><Activity className="w-5 h-5 mr-2 text-sky-500"/> Faturamento dos Últimos 30 Dias</h3>
             <ResponsiveContainer width="100%" height="100%">
                <LineChart data={revenueData} margin={{ top: 5, right: 20, left: -10, bottom: -15 }}>
                    ...
                    <Tooltip contentStyle={{ backgroundColor: 'white', border: '1px solid #e2e8f0', borderRadius: '0.5rem' }} labelStyle={{ color: '#1e293b', fontWeight: 'bold' }} formatter={(value) => [`R$${value.toFixed(2)}`, 'Faturamento']}/>
