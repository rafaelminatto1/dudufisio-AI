...
const RecentActivity = () => {
    ...
    return (<div className="bg-white p-6 rounded-2xl shadow-sm flex flex-col h-full max-h-96">
            ...
            {isLoading ? (<div className="space-y-3">
                    ...
                    <Skeleton className="h-12 w-full rounded-lg"/>
                    <Skeleton className="h-12 w-full rounded-lg"/>
                </div>) : activities.length > 0 ? (<div className="space-y-2 overflow-y-auto flex-1 -mr-2 pr-2">
                    {activities.map(act => (<div key={act.id} onClick={() => navigate(`/patients/${act.patientId}`)} className="p-2.5 rounded-lg flex items-center gap-3 hover:bg-slate-50 cursor-pointer transition-colors">
