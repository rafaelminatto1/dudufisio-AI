    ...
    return (<div className="bg-white p-6 rounded-2xl shadow-sm flex flex-col h-full max-h-96">
            <div className="flex justify-between items-center mb-4">
             ...
             {isLoading ? (<div className="space-y-3">
                    ...
                    <Skeleton className="h-10 w-full rounded-lg"/>
                    <Skeleton className="h-10 w-full rounded-lg"/>
                </div>) : tasks.length > 0 ? (<div className="space-y-2 overflow-y-auto flex-1 -mr-2 pr-2">
                    {tasks.map(task => (<div key={task.id} onClick={() => navigate('/tasks')} className="p-2.5 rounded-lg flex items-center justify-between hover:bg-slate-50 cursor-pointer transition-colors">
