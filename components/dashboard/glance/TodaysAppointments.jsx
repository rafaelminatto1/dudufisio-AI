...
const TodaysAppointments = ({ appointments }) => {
    ...
    return (<div className="bg-white p-6 rounded-2xl shadow-sm flex flex-col h-full max-h-96">
            <h3 className="text-lg font-semibold text-slate-800 mb-4 flex items-center">
                <Calendar className="w-5 h-5 mr-2 text-sky-500"/>
                Consultas de Hoje
            </h3>
            {todaysScheduledAppointments.length > 0 ? (<div className="space-y-3 overflow-y-auto flex-1 -mr-2 pr-2">
                    {todaysScheduledAppointments.map(app => (<div key={app.id} onClick={() => handleAppointmentClick(app)} className="p-3 rounded-lg flex items-center gap-3 hover:bg-slate-50 cursor-pointer transition-colors">
                ...
                </div>) : (<div className="flex-1 flex flex-col items-center justify-center text-center text-slate-500">
