...
const TeamProductivityChart = ({ appointments, therapists }) => {
    const productivityData = useMemo(() => {
        const today = new Date();
        const startOfMonth = new Date(today.getFullYear(), today.getMonth(), 1);
        const appointmentsThisMonth = appointments.filter(app => app.status === AppointmentStatus.Completed && app.startTime >= startOfMonth);
    ...
    return (<div className="h-64 mt-4">
             <ResponsiveContainer width="100%" height="100%">
                <BarChart data={productivityData} layout="vertical" margin={{ top: 5, right: 10, left: 10, bottom: 5 }}>
                    ...
                    <Bar dataKey="atendimentos" barSize={20}>
                        {productivityData.map((entry, index) => (<Cell key={`productivity-${index}-${entry.name}`} fill={entry.color}/>))}
                    </Bar>
                </BarChart>
            </ResponsiveContainer>
        </div>);
