...
import { Plus, Edit3, Trash2, Copy, Eye, Save, X, Search, MessageSquare, Mail, Smartphone, Bell, Code } from 'lucide-react';
...
export const TemplateManager = ({ className = '' }) => {
    ...
    const handleSaveTemplate = async () => {
        try {
            ...
            const response = await fetch(`/api/communication/templates${editingTemplate ? `/${editingTemplate.id}` : ''}`, {
    ...
    return (<div className={`space-y-6 ${className}`}>
      ...
      <div className="flex flex-col lg:flex-row space-y-4 lg:space-y-0 lg:space-x-4">
        <div className="flex-1">
          <div className="relative">
            ...
            <input type="text" placeholder="Buscar templates..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500"/>
          </div>
        </div>

        <select value={selectedType} onChange={(e) => setSelectedType(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500">
          <option value="all">Todos os tipos</option>
          {templateTypes.map(type => (<option key={type.value} value={type.value}>{type.label}</option>))}
        </select>

        <select value={selectedChannel} onChange={(e) => setSelectedChannel(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500">
      ...
      {/* Templates Grid */}
      {!isCreating ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredTemplates.map(template => (<TemplateCard key={template.id} template={template} onEdit={handleEditTemplate} onDelete={handleDeleteTemplate} onDuplicate={handleDuplicateTemplate} onPreview={setPreviewTemplate}/>))}
        </div>) : (
        /* Template Form */
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={formData.name} onChange={(e) => setFormData(prev => ({ ...prev, name: e.target.value }))} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500" placeholder="Ex: Lembrete de Consulta"/>
                </div>
                ...
                <div>
                  ...
                  <select value={formData.type} onChange={(e) => setFormData(prev => ({ ...prev, type: e.target.value }))} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500">
                    {templateTypes.map(type => (<option key={type.value} value={type.value}>{type.label}</option>))}
                  </select>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-3">
                  {Object.entries(channelIcons).map(([channel, Icon]) => (<label key={channel} className="flex items-center space-x-2 cursor-pointer">
              ...
              {/* Subject (for email) */}
              {formData.channels.includes(CommunicationChannel.Email) && (<div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Assunto (Email)
                  </label>
                  <input type="text" value={formData.subject || ''} onChange={(e) => setFormData(prev => ({ ...prev, subject: e.target.value }))} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500" placeholder="Assunto do email"/>
                </div>)}
              ...
              <div>
                ...
                <textarea id="template-content" value={formData.content} onChange={(e) => setFormData(prev => ({ ...prev, content: e.target.value }))} rows={8} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500" placeholder="Digite o conteúdo da mensagem. Use {{variavel}} para inserir variáveis dinâmicas."/>
              </div>

              {/* Variables Used */}
              {formData.variables.length > 0 && (<div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Variáveis Utilizadas
                  </label>
                  <div className="flex flex-wrap gap-2">
                    {formData.variables.map(variable => (<span key={variable} className="px-2 py-1 bg-purple-100 text-purple-800 text-sm rounded-lg">
                        {variable}
                      </span>))}
                  </div>
                </div>)}
              ...
              <div className="flex space-x-4 pt-4 border-t">
                <button onClick={handleSaveTemplate} className="flex items-center space-x-2 px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700">
          ...
          <div className="bg-white rounded-lg shadow-sm border p-6">
            ...
            <div className="space-y-3">
              {availableVariables.map(variable => (<div key={variable.name} className="p-3 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer" onClick={() => insertVariable(variable.name)}>
    ...
    return (<div className="bg-white rounded-lg shadow-sm border p-6 hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1">
          <div className="flex items-center space-x-2 mb-2">
            ...
            {!template.isActive && (<span className="px-2 py-1 bg-gray-100 text-gray-600 text-xs rounded-full">
                Inativo
              </span>)}
          </div>
      ...
      <div className="flex items-center space-x-2 mb-4">
        {channels.map(channel => {
            ...
            return Icon ? (<div key={channel} className="flex items-center space-x-1 px-2 py-1 bg-gray-100 rounded-lg">
      ...
      {/* Variables */}
      {template.variables.length > 0 && (<div className="mb-4">
          <p className="text-xs text-gray-500 mb-2">Variáveis: {template.variables.length}</p>
          <div className="flex flex-wrap gap-1">
            {template.variables.slice(0, 3).map(variable => (<span key={variable} className="px-1 py-0.5 bg-purple-100 text-purple-800 text-xs rounded">
                {variable}
              </span>))}
            {template.variables.length > 3 && (<span className="text-xs text-gray-500">
      ...
      <div className="flex items-center justify-between pt-4 border-t">
        <div className="flex items-center space-x-2">
          <button onClick={() => onPreview(template)} className="p-2 text-gray-600 hover:text-purple-600">
            <Eye className="h-4 w-4"/>
          </button>
          <button onClick={() => onEdit(template)} className="p-2 text-gray-600 hover:text-purple-600">
            <Edit3 className="h-4 w-4"/>
          </button>
          <button onClick={() => onDuplicate(template)} className="p-2 text-gray-600 hover:text-purple-600">
            <Copy className="h-4 w-4"/>
          </button>
        </div>
        <button onClick={() => onDelete(template.id)} className="p-2 text-gray-600 hover:text-red-600">
    ...
    return (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full m-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
          ...
          {/* Variables */}
          {template.variables.length > 0 && (<div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Variáveis de Exemplo
              </label>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {template.variables.map(variable => (<div key={variable} className="flex justify-between items-center p-2 bg-purple-50 rounded">
        ...
        <div className="p-6 border-t">
          <button onClick={onClose} className="w-full px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700">
            Fechar Preview
          </button>
        </div>
      </div>
