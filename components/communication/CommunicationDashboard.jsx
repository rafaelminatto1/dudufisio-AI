...
import { MessageSquare, Mail, Smartphone, Bell, TrendingUp, TrendingDown, Users, Clock, AlertTriangle, CheckCircle, XCircle, Download, RefreshCw } from 'lucide-react';
...
export const CommunicationDashboard = ({ className = '' }) => {
    ...
    const handleChannelToggle = (channel) => {
        setSelectedChannels(prev => prev.includes(channel)
    ...
    if (error) {
        ...
        <div className="flex flex-col items-center space-y-4 text-center">
          ...
          <button onClick={loadDashboardData} className="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700">
            Tentar novamente
          </button>
        </div>
      </div>);
    ...
    return (<div className={`space-y-6 ${className}`}>
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center space-y-4 lg:space-y-0">
        ...
        <div className="flex flex-wrap items-center space-x-4">
          {/* Time Range Selector */}
          <select value={selectedTimeRange} onChange={(e) => setSelectedTimeRange(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500">
          ...
          <label className="flex items-center space-x-2">
            <input type="checkbox" checked={autoRefresh} onChange={(e) => setAutoRefresh(e.target.checked)} className="rounded border-gray-300 text-purple-600 focus:ring-purple-500"/>
            <span className="text-sm text-gray-700">Atualização automática</span>
          </label>

          {/* Refresh Button */}
          <button onClick={loadDashboardData} disabled={loading} className="p-2 text-gray-600 hover:text-purple-600 disabled:opacity-50">
      ...
      <div className="flex flex-wrap items-center space-x-4 p-4 bg-gray-50 rounded-lg">
        ...
        {Object.entries(channelIcons).map(([channel, Icon]) => (<label key={channel} className="flex items-center space-x-2 cursor-pointer">
            <input type="checkbox" checked={selectedChannels.includes(channel)} onChange={() => handleChannelToggle(channel)} className="rounded border-gray-300 text-purple-600 focus:ring-purple-500"/>
      ...
      {dashboardData.alerts && dashboardData.alerts.length > 0 && (<div className="space-y-2">
          {dashboardData.alerts.map((alert, index) => (<div key={index} className={`p-4 rounded-lg border-l-4 ${alert.severity >= 8 ? 'bg-red-50 border-red-400' :
                    alert.severity >= 5 ? 'bg-yellow-50 border-yellow-400' :
                        'bg-blue-50 border-blue-400'}`}>
              <div className="flex items-center space-x-2">
                <AlertTriangle className={`h-5 w-5 ${alert.severity >= 8 ? 'text-red-600' :
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <MetricCard title="Mensagens Enviadas" value={formatNumber(dashboardData.overview.totalSent)} change={dashboardData.overview.sentGrowth || 0} icon={MessageSquare} color="purple"/>
        <MetricCard title="Taxa de Entrega" value={formatPercentage(dashboardData.overview.deliveryRate)} change={0} icon={CheckCircle} color="green"/>
        <MetricCard title="Taxa de Engajamento" value={formatPercentage(dashboardData.overview.engagementRate)} change={0} icon={Users} color="blue"/>
        <MetricCard title="Custo Total" value={`R$ ${dashboardData.overview.totalCost.toFixed(2)}`} change={0} icon={TrendingUp} color="orange"/>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie data={Object.entries(dashboardData.messageTypePerformance).map(([type, metrics]) => ({
            type,
            ...
            failed: metrics.failed || 0
                ...
                {Object.entries(dashboardData.messageTypePerformance).map((_, index) => (<Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]}/>))}
              </Pie>
              <Tooltip />
              <Legend />
            </PieChart>
        ...
        }).map(([status, count]) => (<div key={status} className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            return (<tr key={index} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${activity.status === 'delivered' ? 'bg-green-100 text-green-800' :
