...
const PatientInfoPanel = ({ patient }) => {
    ...
    return (<div className="space-y-4 text-sm text-slate-300 p-4">
            <div className="p-3 bg-slate-700/50 rounded-lg">
                ...
                <p><strong>Queixa:</strong> {patient.conditions?.[0]?.name || 'N/A'}</p>
                 {patient.medicalAlerts && (<div className="mt-2 p-2 bg-amber-500/20 text-amber-200 rounded flex items-start">
                         <AlertTriangle className="w-4 h-4 mr-2 mt-0.5 shrink-0"/>
                        <p className="text-xs">{patient.medicalAlerts}</p>
                    </div>)}
            </div>
            
             {plan && (<div className="p-3 bg-slate-700/50 rounded-lg">
            ...
            {lastNote && (<div className="p-3 bg-slate-700/50 rounded-lg">
                    <h3 className="font-bold flex items-center mb-2 text-slate-100">
