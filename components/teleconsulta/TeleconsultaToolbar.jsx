...
const TabButton = ({ icon: Icon, label, isActive, onClick }) => (<button onClick={onClick} className={`flex-1 flex flex-col items-center justify-center p-3 text-xs font-medium border-b-2 transition-colors ${isActive ? 'border-teal-400 text-teal-300 bg-slate-700/50' : 'border-transparent text-slate-400 hover:bg-slate-700'}`} title={label}>
        <Icon className="w-5 h-5 mb-1"/>
        <span className="truncate">{label}</span>
    </button>);
const TeleconsultaToolbar = ({ patient, onShareContent, sessionNote, onNoteChange, isSaving }) => {
    const [activeTab, setActiveTab] = useState('info');
    const handleShareExercise = (exercise) => {
        onShareContent({ type: 'exercise', data: exercise });
    };
    const handleSharePainMap = (points) => {
    ...
    return (<div className="flex flex-col h-full">
            ...
            <footer className="p-2 border-t border-slate-700 bg-slate-900 text-center text-xs text-slate-400">
                {isSaving ? 'Salvando anotações...' : (<span className="flex items-center justify-center text-green-400">
