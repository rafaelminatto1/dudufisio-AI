...
const PatientMetrics = ({ patient, appointments, className = '' }) => {
    ...
    const MetricCard = ({ icon: Icon, title, value, subtitle, color }) => (<div className="bg-white rounded-lg p-4 border border-slate-200">
    ...
    return (<div className={`space-y-6 ${className}`}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <MetricCard icon={Activity} title="Sessões Realizadas" value={stats.totalSessions} subtitle="Total de atendimentos" color="bg-blue-500"/>
        <MetricCard icon={DollarSign} title="Sessões Pagas" value={stats.paidSessions} subtitle={`R$ ${stats.paidValue.toFixed(2)}`} color="bg-green-500"/>
        <MetricCard icon={Calendar} title="Duração do Tratamento" value={stats.treatmentDuration > 0 ? `${stats.treatmentDuration} dias` : 'N/A'} subtitle={stats.treatmentDuration > 365 ? `${Math.floor(stats.treatmentDuration / 365)} ano(s)` : ''} color="bg-purple-500"/>
        <MetricCard icon={TrendingUp} title="Valor Total" value={`R$ ${stats.totalValue.toFixed(2)}`} subtitle={stats.pendingSessions > 0 ? `${stats.pendingSessions} pendente(s)` : 'Tudo pago'} color="bg-orange-500"/>
      </div>
      ...
      <div className="bg-white rounded-lg p-6 border border-slate-200">
        ...
        <div className="space-y-4">
          <ProgressBar label="Sessões Pagas" current={stats.paidSessions} total={stats.totalSessions} color="bg-green-500"/>
          <ProgressBar label="Valor Recebido" current={stats.paidValue} total={stats.totalValue} color="bg-blue-500"/>
        </div>
      </div>

      {/* Resumo Financeiro */}
      {stats.totalValue > 0 && (<div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-6 border border-blue-200">
