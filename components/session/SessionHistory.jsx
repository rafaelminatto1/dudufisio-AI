...
const SessionHistory = ({ patientNotes, onRepeatConduct, className = '' }) => {
    ...
    const getSessionStatusColor = (sessionNumber, totalSessions) => {
        if (!totalSessions)
    ...
    const SessionCard = ({ note, index }) => {
        const isLatest = index === 0;
        return (<div className={`bg-white border rounded-lg overflow-hidden transition-all duration-200 ${isLatest ? 'border-blue-200 shadow-md' : 'border-slate-200 hover:shadow-sm'}`}>
        {/* Header */}
        <div className={`p-4 ${isLatest ? 'bg-blue-50' : 'bg-slate-50'}`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${isLatest ? 'bg-blue-500 text-white' : 'bg-slate-500 text-white'}`}>
            ...
            <div className="flex items-center space-x-2">
              {onRepeatConduct && (<button onClick={() => onRepeatConduct(note)} className="flex items-center px-3 py-1 text-xs bg-green-100 text-green-700 rounded-lg hover:bg-green-200 transition-colors">
                  <Copy className="w-3 h-3 mr-1"/>
                  Repetir
                </button>)}
              <button onClick={() => navigate(`/session-view/${note.id}`)} className="flex items-center px-3 py-1 text-xs bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200 transition-colors">
    ...
    const SummaryCard = () => {
        ...
        const treatmentDuration = firstSession && latestSession
            ...
            : 0;
        return (<div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg p-4 mb-6">
    ...
    return (<div className={`space-y-4 ${className}`}>
      ...
      <div className="space-y-3">
        ...
        {patientNotes.map((note, index) => (<SessionCard key={note.id} note={note} index={index}/>))}
      </div>
    </div>);
};
