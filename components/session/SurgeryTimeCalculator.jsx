...
const SurgeryTimeCalculator = ({ surgeryDate, className = '' }) => {
    const calculateTimeSinceSurgery = (date) => {
        ...
        if (years > 0) {
            ...
            if (months > 0)
                displayText += ` e ${months} mês${months > 1 ? 'es' : ''}`;
        }
        else if (months > 0) {
            displayText += `${months} mês${months > 1 ? 'es' : ''}`;
            if (weeks > 0)
                displayText += ` e ${weeks} semana${weeks > 1 ? 's' : ''}`;
        }
        else if (weeks > 0) {
            displayText += `${weeks} semana${weeks > 1 ? 's' : ''}`;
            if (remainingDays > 0)
                displayText += ` e ${remainingDays} dia${remainingDays > 1 ? 's' : ''}`;
        }
        else {
