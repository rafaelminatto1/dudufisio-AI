...
import { Bell, Mail, MessageSquare, Smartphone, Clock, Save, X, AlertTriangle } from 'lucide-react';
const NotificationSettings = ({ userId, onClose }) => {
    ...
    const getSettingValue = (type, channel, field) => {
        ...
        return setting?.[field] ?? (field === 'isEnabled' ? true : 'immediate');
    };
    const updateSetting = (type, channel, field, value) => {
        const key = `${type}_${channel}_${field}`;
        ...
        try {
            ...
            notificationTypes.forEach(type => {
                channels.forEach(channel => {
                    ...
                    const existingSetting = settings.find(s => s.notificationType === type.key && s.channel === channel.key);
                    if (existingSetting) {
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        {onClose && (<button onClick={onClose} className="text-gray-400 hover:text-gray-600">
            <X className="h-6 w-6"/>
          </button>)}
      </div>
      ...
      <div className="space-y-6">
        {notificationTypes.map(type => (<div key={type.key} className="bg-gray-50 rounded-lg p-4">
            ...
            <div className="space-y-3">
              {channels.map(channel => (<div key={channel.key} className="flex items-center justify-between p-3 bg-white rounded-lg border">
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <label className="flex items-center">
                      <input type="checkbox" checked={getSettingValue(type.key, channel.key, 'isEnabled')} onChange={(e) => updateSetting(type.key, channel.key, 'isEnabled', e.target.checked)} className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"/>
                    ...
                    {/* Frequência */}
                    {getSettingValue(type.key, channel.key, 'isEnabled') && (<select value={getSettingValue(type.key, channel.key, 'frequency')} onChange={(e) => updateSetting(type.key, channel.key, 'frequency', e.target.value)} className="text-sm border border-gray-300 rounded px-2 py-1 focus:ring-2 focus:ring-blue-500">
                        {frequencies.map(freq => (<option key={freq.key} value={freq.key}>
                            {freq.label}
                          </option>))}
                      </select>)}
                  </div>
      ...
      <div className="flex justify-end space-x-3 pt-4 border-t border-gray-200">
        ...
        <button onClick={handleSave} disabled={isSaving} className="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
          {isSaving ? (<>
