...
import { Calendar, TrendingUp, TrendingDown, Activity, BarChart3, PieChartIcon, Filter } from 'lucide-react';
 ...
 */
const BodyMapTimeline = ({ points, selectedDate, onDateChange, analytics, className = '' }) => {
    const [activeView, setActiveView] = useState('evolution');
    const [timeRange, setTimeRange] = useState(30);
    const [showFilters, setShowFilters] = useState(false);
    const [selectedPainTypes, setSelectedPainTypes] = useState(['acute', 'chronic', 'intermittent', 'constant']);
    /**
     ...
     */
    const togglePainType = useCallback((painType) => {
        setSelectedPainTypes(prev => prev.includes(painType)
    ...
    const CustomTooltip = ({ active, payload, label }) => {
        if (active && payload?.length) {
            return (<div className="bg-white p-4 border border-slate-200 rounded-lg shadow-lg">
    ...
    const renderChart = () => {
        switch (activeView) {
            ...
            <AreaChart data={evolutionData}>
              ...
              <Area type="monotone" dataKey="avgPain" stroke="#3b82f6" fill="url(#painGradient)" strokeWidth={2}/>
              <Line type="monotone" dataKey="maxPain" stroke="#ef4444" strokeWidth={2} strokeDasharray="5 5" dot={{ fill: '#ef4444', r: 3 }}/>
            </AreaChart>
          </ResponsiveContainer>);
            case 'distribution':
                return (<ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie data={distributionData} cx="50%" cy="50%" outerRadius={80} dataKey="count" label={({ region, count }) => `${region}: ${count}`}>
    ...
    return (<div className={`bg-white rounded-2xl border border-slate-200 p-6 space-y-6 ${className}`}>
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          {/* Time Range Selector */}
          <select value={timeRange} onChange={(e) => setTimeRange(Number(e.target.value))} className="px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            {TIME_RANGES.map(range => (<option key={range.days} value={range.days}>
                {range.label}
              </option>))}
          </select>

          {/* Filter Toggle */}
          <button onClick={() => setShowFilters(!showFilters)} className={`px-3 py-2 rounded-lg text-sm transition-colors ${showFilters
      ...
      {/* Filters */}
      <AnimatePresence>
        {showFilters && (<motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="border border-slate-200 rounded-lg p-4 bg-slate-50">
            ...
            <div className="space-y-3">
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {Object.entries(PAIN_TYPE_COLORS).map(([type, color]) => (<label key={type} className={`flex items-center gap-2 px-3 py-1 rounded-full text-sm cursor-pointer transition-all ${selectedPainTypes.includes(type)
                    ? 'bg-white border-2'
                    : 'bg-white border border-slate-300 opacity-60'}`} style={{
                    borderColor: selectedPainTypes.includes(type) ? color : undefined
                }}>
                      <input type="checkbox" checked={selectedPainTypes.includes(type)} onChange={() => togglePainType(type)} className="sr-only"/>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-slate-50 rounded-lg p-4 text-center">
          <div className="flex items-center justify-center gap-2 mb-2">
            {trend.direction === 'up' ? (<TrendingUp className="w-5 h-5 text-red-500"/>) : trend.direction === 'down' ? (<TrendingDown className="w-5 h-5 text-green-500"/>) : (<Activity className="w-5 h-5 text-slate-500"/>)}
            <span className={`font-semibold ${trend.direction === 'up' ? 'text-red-600' :
      ...
      <div className="flex flex-wrap gap-2">
        ...
        ].map(({ key, label, icon: Icon }) => (<button key={key} onClick={() => setActiveView(key)} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeView === key
                ? 'bg-blue-100 text-blue-700'
      ...
      {/* Chart */}
      <motion.div key={activeView} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="bg-white rounded-lg border border-slate-200 p-4">
        {renderChart()}
      </motion.div>
    </div>);
};
