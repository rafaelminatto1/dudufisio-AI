 ...
 */
const BodyMapContainer = ({ patient, sessionId, className = '', readOnly = false, showTimeline = true, showAnalytics = true }) => {
    // Professional state management with useBodyMapPro hook
    const { state, actions: { addPoint, updatePoint, deletePoint, selectPoint, clearSelection, switchSide, setTimelineDate, refreshData }, analytics, isLoading, error } = useBodyMapPro(patient.id, sessionId);
     ...
     */
    const handleSVGClick = useCallback((coordinates, region) => {
        if (readOnly)
            return;
     ...
     */
    const handleSavePoint = useCallback(async (pointData) => {
        try {
    ...
    if (error) {
        ...
        <button onClick={refreshData} className="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors">
          Tentar Novamente
        </button>
      </div>);
    }
    return (<div className={`space-y-6 ${className}`}>
      ...
      {showAnalytics && (<motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-2 md:grid-cols-4 gap-4">
      ...
      <div className="flex justify-center">
        <div className="bg-white rounded-xl border border-slate-200 p-1 flex">
          <button onClick={() => switchSide('front')} className={`px-6 py-2 rounded-lg transition-all ${state.activeSide === 'front'
            ? 'bg-blue-500 text-white shadow-sm'
            : 'text-slate-600 hover:text-slate-800'}`}>
            Vista Frontal
          </button>
          <button onClick={() => switchSide('back')} className={`px-6 py-2 rounded-lg transition-all ${state.activeSide === 'back'
            ? 'bg-blue-500 text-white shadow-sm'
            : 'text-slate-600 hover:text-slate-800'}`}>
            Vista das Costas
          </button>
      ...
      {/* Main Body Map SVG */}
      <motion.div key={state.activeSide} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.3 }} className="bg-white rounded-2xl border border-slate-200 p-6">
        <BodyMapSVG side={state.activeSide} points={state.points} onPointClick={handlePointClick} onSVGClick={handleSVGClick} selectedPointId={state.selectedPoint?.id} isLoading={isLoading} readOnly={readOnly}/>
      </motion.div>
      ...
      {/* Timeline Component */}
      {showTimeline && (<BodyMapTimeline points={state.points} selectedDate={state.timelineDate} onDateChange={setTimelineDate} analytics={analytics}/>)}

      {/* Point Modal */}
      <AnimatePresence>
        {showModal && (<PainPointModal point={state.selectedPoint} isOpen={showModal} onClose={() => {
                setShowModal(false);
                setNewPointCoordinates(null);
                clearSelection();
            }} onSave={handleSavePoint} onDelete={state.selectedPoint ? () => handleDeletePoint(state.selectedPoint.id) : undefined} coordinates={newPointCoordinates} bodyRegion="other" // TODO: Auto-detect region from coordinates
        />)}
      </AnimatePresence>
    </div>);
};
