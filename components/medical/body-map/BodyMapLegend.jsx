 ...
 */
const BodyMapLegend = ({ expanded = false, className = '', showHelp = true }) => {
    ...
    return (<div className={`bg-white rounded-xl border border-slate-200 ${className}`}>
      ...
      <div className="flex items-center justify-between p-4 border-b border-slate-200">
        ...
        <div className="flex items-center gap-2">
          {showHelp && (<button onClick={() => setShowGuide(!showGuide)} className={`p-2 rounded-lg transition-colors ${showGuide
                ? 'bg-blue-100 text-blue-600'
                : 'text-slate-400 hover:text-slate-600 hover:bg-slate-100'}`} aria-label="Mostrar/ocultar guia de uso">
              <HelpCircle className="w-4 h-4"/>
            </button>)}

          <button onClick={() => setIsExpanded(!isExpanded)} className={`p-2 rounded-lg transition-colors ${isExpanded
            ? 'bg-blue-100 text-blue-600'
            : 'text-slate-400 hover:text-slate-600 hover:bg-slate-100'}`} aria-label="Expandir/recolher legenda">
      ...
      {/* Usage Guide */}
      <AnimatePresence>
        {showGuide && (<motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="border-b border-slate-200">
            <div className="p-4 bg-blue-50">
              ...
              <div className="space-y-2">
                {INTERACTION_GUIDE.map((guide, index) => (<div key={index} className="flex items-start gap-3 text-sm">
      ...
      <div className="p-4 space-y-4">
        ...
        <div>
          ...
          <div className="space-y-2">
            {PAIN_LEVELS.map((level, index) => (<div key={index} className="flex items-center gap-3">
                <div className="w-4 h-4 rounded-full border border-white shadow-sm flex-shrink-0" style={{ backgroundColor: level.color }}/>
        ...
        {/* Pain Types */}
        {isExpanded && (<motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="border-t border-slate-200 pt-4">
            <h5 className="font-medium text-slate-700 mb-3">Tipos de Dor</h5>
            <div className="grid grid-cols-1 gap-2">
              {PAIN_TYPES.map((type, index) => (<div key={index} className="flex items-start gap-3 p-2 bg-slate-50 rounded-lg">
        ...
        <div className="border-t border-slate-200 pt-4 text-center">
          <button onClick={() => setIsExpanded(!isExpanded)} className="text-xs text-slate-500 hover:text-slate-700 transition-colors">
