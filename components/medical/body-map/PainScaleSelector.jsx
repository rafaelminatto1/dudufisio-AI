...
const getPainColor = (level) => {
    if (level === 0)
        return '#e2e8f0'; // Gray for no pain
    if (level <= 3)
        return '#22c55e'; // Green
    if (level <= 6)
 ...
 */
const PainScaleSelector = ({ value, onChange, showValue = true, disabled = false, size = 'medium', showLabels = true, className = '' }) => {
     ...
     */
    const handleKeyDown = (event, level) => {
        if (disabled)
            return;
        switch (event.key) {
            case 'Enter':
    ...
    return (<div className={`space-y-4 ${className}`}>
      ...
      {showValue && (<div className="text-center">
          <motion.div key={value} initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className={`font-bold ${sizeClasses.value}`} style={{ color: getPainColor(value) }}>
            {value}/10
          </motion.div>
          {showLabels && (<motion.p key={value} initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className={`text-gray-600 font-medium ${sizeClasses.label} mt-1`}>
              {PAIN_DESCRIPTIONS[value]}
            </motion.p>)}
        </div>)}
      ...
      <div className={`flex justify-center ${sizeClasses.container}`}>
        {Array.from({ length: 11 }, (_, index) => {
            ...
            return (<motion.button key={index} type="button" onClick={() => handleSelect(index)} onKeyDown={(e) => handleKeyDown(e, index)} disabled={disabled} className={`
                ${sizeClasses.button}
              ...
              `} style={{
                ...
                }} whileHover={!disabled ? { scale: isSelected ? 1.2 : 1.1 } : {}} whileTap={!disabled ? { scale: 0.95 } : {}} animate={{
                    scale: isSelected ? 1.2 : 1,
                    backgroundColor: color
                }} aria-label={`Nível de dor ${index}: ${PAIN_DESCRIPTIONS[index]}`} aria-pressed={isSelected} role="radio" tabIndex={isSelected ? 0 : -1}>
              {index}
            </motion.button>);
        })}
      </div>
