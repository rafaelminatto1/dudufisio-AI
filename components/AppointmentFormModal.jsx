...
const AppointmentFormModal = ({ isOpen, onClose, onSave, onDelete: _onDelete, appointment: _appointment, appointmentToEdit, initialData, patients = [], therapists = [], allAppointments = [] }) => {
    ...
    const handleSaveClick = async () => {
        ...
        const baseAppointment = {
            ...
            patientAvatarUrl: selectedPatient.avatarUrl || `https://i.pravatar.cc/150?u=${selectedPatient.id}`,
    ...
    const applyTemplate = async () => {
        ...
        try {
            ...
            const result = await recurrenceTemplateService.applyTemplate(selectedTemplateId, slotDate, baseAppointment, allAppointments);
      ...
      <div ref={modalRef} className="bg-white rounded-lg shadow-xl w-full max-w-lg mx-4 max-h-[90vh] flex flex-col">
        ...
        <div className="p-4 space-y-4 overflow-y-auto">
          <div>
            ...
            <PatientSearchInput onSelectPatient={setSelectedPatient} selectedPatient={selectedPatient}/>
          </div>
          
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">Fisioterapeuta</label>
            <select value={therapistId} onChange={(e) => setTherapistId(e.target.value)} className="w-full px-3 py-2 border border-slate-300 rounded-md focus:ring-sky-500 focus:border-sky-500 text-sm">
          ...
          <div>
            ...
            <select value={appointmentType} onChange={(e) => setAppointmentType(e.target.value)} className="w-full px-3 py-2 border border-slate-300 rounded-md focus:ring-sky-500 focus:border-sky-500 text-sm">
          ...
          <div>
            ...
            <div className="flex gap-2">
              {[30, 45, 60].map(min => (<button key={min} onClick={() => setDuration(min)} className={`px-4 py-2 rounded-md border transition text-sm ${duration === min
          ...
          <div>
            ...
            <div className="flex gap-2">
              <select value={selectedTemplateId} onChange={(e) => setSelectedTemplateId(e.target.value || undefined)} className="flex-1 px-3 py-2 border border-slate-300 rounded-md focus:ring-sky-500 focus:border-sky-500 text-sm">
                <option value="">Selecione um template</option>
                {templates.map(template => (<option key={template.id} value={template.id}>{template.title}</option>))}
              </select>
              <button type="button" onClick={applyTemplate} className="px-3 py-2 bg-emerald-500 text-white rounded-md text-sm hover:bg-emerald-600">
                Aplicar
              </button>
            </div>
          ...
          {availableBlocks.length > 0 && (<div className="bg-amber-50 border border-amber-200 text-amber-800 rounded-md p-3 text-xs">
              <strong>Bloqueios próximos:</strong>
              <ul className="list-disc list-inside mt-1 space-y-1">
                {availableBlocks.slice(0, 2).map(block => (<li key={block.id}>{block.reason || 'Bloqueio'} em {format(block.startTime, "dd/MM HH:mm")}</li>))}
                {availableBlocks.length > 2 && <li>+ {availableBlocks.length - 2} bloqueio(s)</li>}
              </ul>
            </div>)}

          {waitlistEntries.length > 0 && (<div className="bg-slate-100 border border-slate-200 text-slate-700 rounded-md p-3 text-xs">
              <strong>Fila de espera prioritária:</strong>
              <ul className="list-disc list-inside mt-1 space-y-1">
                {waitlistEntries.slice(0, 3).map(entry => (<li key={entry.id}>Paciente {entry.patientId} — urgência {entry.urgency}</li>))}
              </ul>
            </div>)}

          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">Observações</label>
            <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={3} className="w-full px-3 py-2 border border-slate-300 rounded-md focus:ring-sky-500 focus:border-sky-500 text-sm" placeholder="Observações sobre o atendimento..."/>
          </div>
        </div>
        ...
        <div className="flex items-center justify-end gap-3 px-4 py-3 bg-slate-50 rounded-b-lg border-t">
          ...
          <button onClick={handleSaveClick} disabled={!selectedPatient || isSaving} className="px-4 py-2 bg-sky-500 text-white rounded-md hover:bg-sky-600 disabled:opacity-50 disabled:cursor-not-allowed transition flex items-center text-sm">
