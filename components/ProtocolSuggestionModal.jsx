...
const ProtocolSuggestionModal = ({ isOpen, onClose, patient, onApply }) => {
    ...
    const handleApplyProtocol = async () => {
        if (!selectedProtocol || !protocolContent)
            return;
        setIsApplying(true);
        try {
            ...
            const updates = {
                ...
                exercises: (parsedData.exercises || []), // Cast for service
            };
    ...
    const renderSuggestions = () => (<>
            ...
            {isLoading ? (<div className="space-y-2"><Skeleton className="h-12 w-full"/><Skeleton className="h-12 w-full"/></div>) : suggestions.length > 0 ? (<ul className="space-y-2">
                    {suggestions.map(proto => (<li key={proto.id} onClick={() => handleSelectProtocol(proto)} className="p-3 border rounded-lg hover:bg-sky-50 hover:border-sky-300 cursor-pointer">
                            <h4 className="font-semibold text-slate-800">{proto.name}</h4>
                            <p className="text-xs text-slate-500">{proto.description}</p>
                        </li>))}
                </ul>) : (<p className="text-center text-slate-500 py-6">Nenhum protocolo sugerido encontrado para este diagnóstico.</p>)}
        </>);
    const renderProtocolDetail = () => (<div>
            ...
            {isLoading ? <Skeleton className="h-64 w-full"/> : (<div className="bg-slate-50 p-4 rounded-lg max-h-96 overflow-y-auto border">
    ...
    return (<div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={onClose}>
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                ...
                {selectedProtocol && (<footer className="flex justify-end items-center p-4 border-t border-slate-200 bg-slate-50">
