...
import { format, differenceInDays, differenceInWeeks, differenceInMonths, differenceInYears, isBefore } from 'date-fns';
import { ptBR } from 'date-fns/locale';
const PatientGoalsManager = ({ goals, onAddGoal, onUpdateGoal, onDeleteGoal }) => {
    ...
    const handleUpdateGoal = () => {
        if (!editingGoal || !formData.title || !formData.targetDate)
            return;
    ...
    const getTimeToTarget = (targetDate) => {
    ...
    return (<Card>
            ...
            <DialogContent className="max-w-md">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Input id="title" value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} placeholder="Ex: Correr maratona"/>
                </div>
                <div>
                  <Label htmlFor="description">Descrição</Label>
                  <Textarea id="description" value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} placeholder="Detalhes do objetivo" rows={3}/>
                </div>
                <div>
                ...
                <div>
                  ...
                  <Input id="targetValue" value={formData.targetValue} onChange={(e) => setFormData({ ...formData, targetValue: e.target.value })} placeholder="Ex: pace 4:30/km"/>
                </div>
                ...
                <div>
                  ...
                  <Input id="currentProgress" type="number" min="0" max="100" value={formData.currentProgress} onChange={(e) => setFormData({ ...formData, currentProgress: e.target.value })} placeholder="0"/>
                </div>
      ...
      <CardContent>
        {goals.length === 0 ? (<div className="text-center py-8 text-slate-500">
            ...
            {activeGoals.length > 0 && (<div>
                ...
                <div className="space-y-4">
                  {activeGoals.map((goal) => {
                    ...
                    return (<div key={goal.id} className="border rounded-lg p-4 space-y-3">
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            ...
                            {goal.targetValue && (<p className="text-sm text-slate-600 mt-1">
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <Button variant="ghost" size="sm" onClick={() => onDeleteGoal(goal.id)} className="text-red-600 hover:text-red-700">
            ...
            {completedGoals.length > 0 && (<div>
                ...
                <div className="space-y-3">
                  {completedGoals.map((goal) => (<div key={goal.id} className="border rounded-lg p-4 bg-green-50">
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <h5 className="font-semibold text-slate-900">{goal.title}</h5>
                          {goal.description && (<p className="text-sm text-slate-600 mt-1">{goal.description}</p>)}
                          {goal.completedAt && (<p className="text-sm text-green-600 mt-1">
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input id="edit-title" value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} placeholder="Ex: Correr maratona"/>
            </div>
            <div>
              <Label htmlFor="edit-description">Descrição</Label>
              <Textarea id="edit-description" value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} placeholder="Detalhes do objetivo" rows={3}/>
            </div>
            <div>
              <Label htmlFor="edit-targetDate">Data Alvo *</Label>
              <Input id="edit-targetDate" type="date" value={formData.targetDate} onChange={(e) => setFormData({ ...formData, targetDate: e.target.value })}/>
            </div>
            <div>
              <Label htmlFor="edit-targetValue">Valor Alvo</Label>
              <Input id="edit-targetValue" value={formData.targetValue} onChange={(e) => setFormData({ ...formData, targetValue: e.target.value })} placeholder="Ex: pace 4:30/km"/>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
            ...
            <div>
              ...
              <Input id="edit-currentProgress" type="number" min="0" max="100" value={formData.currentProgress} onChange={(e) => setFormData({ ...formData, currentProgress: e.target.value })} placeholder="0"/>
            </div>
            <div className="flex justify-end space-x-2">
