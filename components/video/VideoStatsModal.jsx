...
import { BarChart3, TrendingUp, Eye, Heart, Video, Calendar, Users, Clock, Award, Target, X } from 'lucide-react';
import { Dialog, DialogContent, DialogHeader, DialogTitle, } from '../ui/dialog';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '../ui/card';
import { Badge } from '../ui/badge';
import { Progress } from '../ui/progress';
import { personalizedVideoCrudService } from '../../services/personalizedVideoCrudService';
const VideoStatsModal = ({ isOpen, onClose }) => {
      ...
      <DialogContent className="max-w-6xl max-h-[80vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <Card>
            ...
            return (<div key={modality} className="space-y-1">
          ...
          <Card>
            ...
            const percentage = stats.total > 0 ? (count / stats.total) * 100 : 0;
            return (<div key={tool} className="space-y-1">
                      <div className="flex items-center justify-between text-sm">
          ...
          <Card>
            ...
            return (<div key={status} className="space-y-1">
                    ...
                    background: `linear-gradient(to right, ${statusColors[status] || 'bg-gray-500'} ${percentage}%, #e5e7eb ${percentage}%)`
                }}/>
                    </div>);
        })}
              </div>
          ...
          <Card>
            ...
            return (<div key={difficulty} className="space-y-1">
                      <div className="flex items-center justify-between text-sm">
                        <span className="capitalize">{difficulty}</span>
                        <span className="font-medium">{count}</span>
                      </div>
                      <Progress value={percentage} className="h-2" style={{
                    background: `linear-gradient(to right, ${difficultyColors[difficulty] || 'bg-gray-500'} ${percentage}%, #e5e7eb ${percentage}%)`
                }}/>
                    </div>);
        })}
              </div>
          ...
          <Card className="col-span-full md:col-span-2">
            ...
            <CardContent>
              <div className="space-y-3">
                {stats.mostPopular.map((video, index) => (<div key={video.id} className="flex items-center space-x-3 p-3 border rounded-lg">
                    <div className="flex-shrink-0">
                      <Badge variant="outline" className="text-xs">
                        #{index + 1}
                      </Badge>
                    </div>
                    <img src={video.thumbnailUrl} alt={video.exerciseName} className="w-12 h-8 object-cover rounded"/>
          ...
          <Card className="col-span-full md:col-span-1">
            ...
            <CardContent>
              <div className="space-y-3">
                {stats.recentlyGenerated.map((video, index) => (<div key={video.id} className="flex items-center space-x-3">
                    <img src={video.thumbnailUrl} alt={video.exerciseName} className="w-8 h-6 object-cover rounded"/>
                    <div className="flex-1 min-w-0">
