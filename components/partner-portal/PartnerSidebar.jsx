...
const PartnerSidebar = () => {
    ...
    return (<div className="w-64 bg-white border-r border-slate-200 flex flex-col">
      ...
      <nav className="flex-1 px-3 py-4 space-y-1">
        {navItems.map((item) => (<ReactRouterDOM.NavLink key={item.to} to={item.to} className={({ isActive }) => `flex items-center p-3 rounded-lg transition-colors duration-200 ${isActive
