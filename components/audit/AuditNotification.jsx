...
const AuditNotification = ({ maxNotifications = 3, autoHideDelay = 5000, showSecurityOnly = false }) => {
    ...
    useEffect(() => {
        const handleNewLog = (log) => {
            ...
            if (autoHideDelay > 0) {
                setTimeout(() => {
                    setNotifications(prev => prev.map(n => n.id === log.id ? { ...n, isVisible: false } : n));
    ...
    return (<div className="fixed top-4 right-4 z-50 space-y-2 max-w-sm">
          ...
          <div className="flex items-start space-x-2">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => dismissNotification(notification.id)} className="ml-2 flex-shrink-0 text-gray-400 hover:text-gray-600">
                  <X className="w-3 h-3"/>
                </button>
              </div>
