...
const TaskCard = ({ task, therapist, onClick, onDragStart }) => {
    const { icon, color } = priorityInfo[task.priority];
    const dueDate = new Date(task.dueDate);
    const isOverdue = dueDate < new Date() && task.status !== 'Concluído';
    return (<div onClick={onClick} draggable="true" onDragStart={(e) => onDragStart(e, task.id)} className="bg-white p-3 rounded-lg shadow-sm hover:shadow-md border border-slate-200 cursor-grab active:cursor-grabbing transition-shadow duration-200">
            ...
            <div className="flex justify-between items-center">
                ...
                {therapist && (<img src={therapist.avatarUrl} alt={therapist.name} title={`Responsável: ${therapist.name}`} className="w-7 h-7 rounded-full ring-2 ring-white"/>)}
            </div>
        </div>);
};
