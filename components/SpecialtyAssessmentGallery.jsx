...
const SpecialtyAssessmentGallery = ({ onSelectSpecialty }) => {
    ...
    return (<div className="p-4 bg-white rounded-2xl shadow-sm">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
        {specialties?.map(specialty => (<div key={specialty.id} onClick={() => onSelectSpecialty(specialty)} className="group relative rounded-lg overflow-hidden cursor-pointer shadow-lg transform hover:scale-105 transition-transform duration-300 h-48" role="button" tabIndex={0} aria-label={`Iniciar avaliação de ${specialty.name}`} onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && onSelectSpecialty(specialty)}>
