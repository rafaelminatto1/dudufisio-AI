...
const NewSoapNoteModal = ({ isOpen, onClose, onSave, noteToDuplicate }) => {
...
---`;
        try {
            ...
            const content = response.response || response.content || 'Resposta não disponível';
    ...
    return (<div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div ref={modalRef} className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <main className="flex-1 overflow-y-auto p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="flex flex-col">
                ...
                <div className="flex-1 bg-slate-50 rounded-lg p-2 flex items-center justify-center">
                 ...
                 {selectedParts.length > 0 && (<div className="mt-2">
                        <h5 className="text-xs font-bold text-slate-500 uppercase mb-2">Áreas Selecionadas</h5>
                        <div className="flex flex-wrap gap-2">
                            {selectedParts.map(part => (<span key={part} className="px-2 py-1 text-xs bg-red-100 text-red-800 rounded-full">{part}</span>))}
                        </div>
                    </div>)}
            </div>
        </main>
        ...
        <footer className="flex justify-between items-center p-4 border-t border-slate-200">
           <button onClick={handleGenerateSuggestion} disabled={isAiLoading || (!subjective.trim() && !objective.trim())} className="px-4 py-2 text-sm font-medium text-sky-600 bg-sky-50 border border-sky-200 rounded-lg hover:bg-sky-100 flex items-center disabled:bg-slate-100 disabled:text-slate-400 disabled:cursor-not-allowed transition-colors" title="Usa os campos Subjetivo e Objetivo para gerar sugestões de Avaliação e Plano">
              {isAiLoading ? (<>
