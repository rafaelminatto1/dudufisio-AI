...
const StepIndicator = ({ step, setStep, currentStep }) => {
    const isCompleted = currentStep > step;
    const isActive = currentStep === step;
    return (<button type="button" onClick={() => isCompleted && setStep(step)} className="flex items-center space-x-2">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold transition-colors ${isCompleted ? 'bg-sky-500 text-white' : isActive ? 'bg-sky-100 text-sky-700 border-2 border-sky-500' : 'bg-slate-200 text-slate-500'}`}>
                {isCompleted ? '✓' : step}
            </div>
        </button>);
};
export default function PatientForm({ isOpen, onClose }) {
