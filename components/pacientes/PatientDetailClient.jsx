...
const TabButton = ({ label, icon: Icon, isActive, onClick, }) => (<button onClick={onClick} className={`flex items-center px-4 py-2 text-sm font-medium rounded-md transition-colors ${isActive ? 'bg-sky-100 text-sky-700' : 'text-slate-600 hover:bg-slate-100'}`}>
    <Icon className="w-4 h-4 mr-2"/>
    {label}
  </button>);
const InfoPill = ({ label, value }) => (<div>
...
export default function PatientDetailClient({ patient }) {
    ...
    return (<div className="space-y-6">
      <NewSoapNoteModal isOpen={isSoapModalOpen} onClose={() => setIsSoapModalOpen(false)} patientId={patient.id}/>

      <div className="bg-white p-2 rounded-lg shadow-sm">
        <div className="flex items-center space-x-2">
            {tabs.map(tab => (<TabButton key={tab.id} label={tab.label} icon={tab.icon} isActive={activeTab === tab.id} onClick={() => setActiveTab(tab.id)}/>))}
        </div>
      </div>

      <div className="bg-white p-6 rounded-lg shadow-sm min-h-[400px]">
        {activeTab === 'dados' && (<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {activeTab === 'prontuario' && (<div>
                <h3 className="text-lg font-semibold mb-2">Plano de Tratamento</h3>
                {patient.treatmentPlan ? (<p className="text-sm text-slate-600">{patient.treatmentPlan.treatmentGoals}</p>) : (<p className="text-sm text-slate-500">Nenhum plano de tratamento ativo.</p>)}
             </div>)}
