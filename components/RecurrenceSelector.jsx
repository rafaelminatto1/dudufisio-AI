...
const RecurrenceSelector = ({ recurrenceRule, onChange }) => {
    ...
    return (<div className="p-3 bg-slate-50 rounded-lg">
            <div className="flex items-center justify-between">
                ...
                <input type="checkbox" id="recurring-toggle" checked={isRecurring} onChange={(e) => handleToggleRecurring(e.target.checked)} className="h-4 w-4 rounded border-gray-300 text-sky-600 focus:ring-sky-500"/>
            </div>
            {isRecurring && recurrenceRule && (<div className="mt-4 space-y-3 animate-fade-in-fast">
                    <div>
                         ...
                         <div className="mt-2 flex items-center justify-between space-x-1">
                            {weekDays.map(day => (<button key={day.value} type="button" onClick={() => handleDayToggle(day.value)} className={`w-8 h-8 flex items-center justify-center rounded-full text-sm font-bold transition-colors
                     ...
                     <div>
                        ...
                        <input id="until-date" type="date" value={recurrenceRule.until} onChange={handleUntilChange} className="mt-1 w-full p-2 border border-slate-300 rounded-lg"/>
                    </div>
                </div>)}
            <style>{`
