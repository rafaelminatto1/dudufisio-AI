...
import { AlertCircle, CheckCircle, Shield, FileText, Activity, Target, User, Heart, Brain, Bone, 
// Lungs, // Substituído por MdLungs
Baby, Zap, Stethoscope, Download, Edit, Clock, AlertTriangle, 
// PainChart, // Substituído por MdAssessment
BarChart3, 
// Warning, // Substituído por MdWarning
Info, Share, Archive, History, Settings } from 'lucide-react';
import { MdHealthAndSafety, MdPrint, MdDraw, MdWarning } from 'react-icons/md';
export function DocumentViewer({ document, onEdit, onDelete, onSign, onExport, onPrint, onShare, onArchive, onViewHistory, onViewCompliance, onViewSignature, onViewAuditTrail, isLoading = false, canEdit = false, canDelete = false, canSign = false, canExport = false, canPrint = false, canShare = false, canArchive = false }) {
    ...
    return (<div className="max-w-7xl mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-3">
          {/* Status Badges */}
          {document.signature?.isSigned && (<Badge className="bg-green-100 text-green-800">
              <Shield className="h-3 w-3 mr-1"/>
              Assinado
            </Badge>)}
          
          {document.compliance && (<Badge className={getComplianceBadge(document.compliance.cfm.score)}>
              <Shield className="h-3 w-3 mr-1"/>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2">
          <Button variant="outline" size="sm" onClick={() => onViewHistory?.(document.id)} className="flex items-center space-x-2">
            <History className="h-4 w-4"/>
            <span>Histórico</span>
          </Button>
          
          <Button variant="outline" size="sm" onClick={() => setShowComplianceDetails(true)} className="flex items-center space-x-2">
            <Shield className="h-4 w-4"/>
            <span>Compliance</span>
          </Button>
          
          {document.signature?.isSigned && (<Button variant="outline" size="sm" onClick={() => setShowSignatureDetails(true)} className="flex items-center space-x-2">
              <MdDraw className="h-4 w-4"/>
              <span>Assinatura</span>
            </Button>)}
          
          <Button variant="outline" size="sm" onClick={() => setShowAuditTrail(true)} className="flex items-center space-x-2">
        ...
        <div className="flex items-center space-x-2">
          {canEdit && (<Button variant="outline" size="sm" onClick={() => onEdit?.(document.id)} className="flex items-center space-x-2">
              <Edit className="h-4 w-4"/>
              <span>Editar</span>
            </Button>)}
          
          {canSign && !document.signature?.isSigned && (<Button size="sm" onClick={() => onSign?.(document.id)} className="flex items-center space-x-2">
              <MdDraw className="h-4 w-4"/>
              <span>Assinar</span>
            </Button>)}
          
          {canExport && (<Button variant="outline" size="sm" onClick={() => onExport?.(document.id, 'pdf')} className="flex items-center space-x-2">
              <Download className="h-4 w-4"/>
              <span>Exportar</span>
            </Button>)}
          
          {canPrint && (<Button variant="outline" size="sm" onClick={() => onPrint?.(document.id)} className="flex items-center space-x-2">
              <MdPrint className="h-4 w-4"/>
              <span>Imprimir</span>
            </Button>)}
          
          {canShare && (<Button variant="outline" size="sm" onClick={() => onShare?.(document.id)} className="flex items-center space-x-2">
              <Share className="h-4 w-4"/>
              <span>Compartilhar</span>
            </Button>)}
          
          {canArchive && (<Button variant="outline" size="sm" onClick={() => onArchive?.(document.id)} className="flex items-center space-x-2">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  {document.metadata.updatedBy && (<div>
                  ...
                  {document.metadata.updatedAt && (<div>
        ...
        <TabsContent value="compliance" className="space-y-6">
          {document.compliance && (<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {/* CFM Compliance */}
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center space-x-2">
                ...
                return (<div key={index} className="flex items-center space-x-2 text-sm">
        ...
        <TabsContent value="audit" className="space-y-6">
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                {document.auditTrail?.map((entry, index) => (<div key={index} className="flex items-start space-x-4 p-4 border rounded-lg">
                    ...
                    <div className="flex-1">
                      ...
                      {entry.details && (<p className="text-sm text-gray-500 mt-1">
...
function renderInitialAssessment(content) {
...
function renderSessionEvolution(content) {
    return (<div className="space-y-6">
      <div>
      ...
      <div>
        ...
        <ul className="list-disc list-inside text-gray-700">
          {content.techniquesApplied?.map((technique, index) => (<li key={index}>{technique.name}</li>))}
        </ul>
      </div>
...
function renderTreatmentPlan(content) {
    return (<div className="space-y-6">
      <div>
        <h3 className="text-lg font-semibold mb-2">Metas do Tratamento</h3>
        <ul className="list-disc list-inside text-gray-700">
          {content.goals?.map((goal, index) => (<li key={index}>{goal.description}</li>))}
        </ul>
      </div>
      ...
      <div>
        ...
        <ul className="list-disc list-inside text-gray-700">
          {content.interventions?.map((intervention, index) => (<li key={index}>{intervention.description}</li>))}
        </ul>
      </div>
    </div>);
}
function renderDischargeSummary(content) {
    return (<div className="space-y-6">
      <div>
...
function renderReferralLetter(content) {
    return (<div className="space-y-6">
      <div>
...
function renderProgressReport(content) {
    return (<div className="space-y-6">
      <div>
