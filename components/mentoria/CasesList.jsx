...
const CasesList = ({ cases, onAdd, onView, onEdit }) => {
    const areaColorMap = {
    ...
    return (<div className="bg-white p-6 rounded-2xl shadow-sm">
            ...
            <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
                {cases.map((clinicalCase) => (<div key={clinicalCase.id} className="p-3 rounded-lg border border-slate-200 hover:bg-slate-50 group">
                 ...
                 {cases.length === 0 && (<p className="text-center py-8 text-sm text-slate-500">Nenhum caso clínico cadastrado.</p>)}
            </div>
        </div>);
};
