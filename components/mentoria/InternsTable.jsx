...
import { InternStatusColorMap } from '../../types';
import { Plus } from 'lucide-react';
const InternsTable = ({ interns, onAdd, onEdit }) => {
    ...
    return (<div className="bg-white p-6 rounded-2xl shadow-sm">
            <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={onAdd} className="inline-flex items-center justify-center rounded-lg border border-transparent bg-sky-500 px-3 py-1.5 text-sm font-medium text-white shadow-sm hover:bg-sky-600">
            ...
            <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-slate-200">
                    ...
                    <tbody className="bg-white divide-y divide-slate-200">
                        {interns.map((intern) => (<tr key={intern.id} onClick={() => onEdit(intern)} className="hover:bg-slate-50 cursor-pointer">
                 ...
                 {interns.length === 0 && (<p className="text-center py-8 text-sm text-slate-500">Nenhum estagiário cadastrado.</p>)}
            </div>
        </div>);
};
