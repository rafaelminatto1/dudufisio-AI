...
const ListView = ({ appointments, therapists, onAppointmentClick }) => {
    ...
    return (<div className="flex-1 flex flex-col space-y-4">
      {/* Filters */}
      <Card>
        ...
        {showFilters && (<CardContent className="pt-0">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label className="text-sm font-medium text-slate-700 block mb-2">Status</label>
                <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="w-full p-2 border border-slate-300 rounded-lg text-sm">
              ...
              <div>
                ...
                <select value={filterTherapist} onChange={(e) => setFilterTherapist(e.target.value)} className="w-full p-2 border border-slate-300 rounded-lg text-sm">
              ...
              <div>
                ...
                <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="w-full p-2 border border-slate-300 rounded-lg text-sm">
      ...
      <div className="flex-1 space-y-3 overflow-auto">
        {filteredAndSortedAppointments.length === 0 ? (<Card>
            <CardContent className="p-8 text-center">
              <p className="text-slate-500">Nenhum agendamento encontrado com os filtros selecionados.</p>
            </CardContent>
          </Card>) : (filteredAndSortedAppointments.map((appointment) => (<Card key={appointment.id} className="cursor-pointer hover:shadow-md transition-shadow" onClick={() => onAppointmentClick(appointment)}>
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-sm text-slate-600">
                      ...
                      {appointment.value && (<div className="flex items-center gap-2">
                          <DollarSign size={16}/>
                    ...
                    <div className="mt-2 text-sm text-slate-600">
                      ...
                      {appointment.notes && (<div className="mt-1">
                          <strong>Observações:</strong> {appointment.notes}
                        </div>)}
                    </div>
                  </div>

                  <div className={cn("w-3 h-3 rounded-full flex-shrink-0 mt-2", `bg-${appointment.therapistColor}-500`)}/>
                </div>
              </CardContent>
            </Card>)))}
      </div>
