...
const AppointmentContextMenu = ({ appointment, position, onClose, onEdit, onDelete, onDuplicate, onStatusChange, onPaymentToggle, onCall }) => {
    ...
    const menuItems = [
        ...
        { type: 'separator' },
        {
            ...
            action: () => onStatusChange(appointment.status === AppointmentStatus.Completed
    ...
    return (<div ref={menuRef} className="fixed z-50 bg-white border border-gray-200 rounded-lg shadow-lg py-1 min-w-48" style={{
      ...
      {menuItems.map((item, index) => {
            ...
            return (<button key={index} onClick={() => {
                    item.action();
                    onClose();
                }} className={cn('w-full flex items-center px-3 py-2 text-sm transition-colors', item.color)}>
