...
const ImprovedAppointmentCard = ({ appointment, startHour, pixelsPerMinute, isBeingDragged, onClick, onDragStart, onDragEnd }) => {
    ...
    return (<div onClick={(e) => { e.stopPropagation(); onClick(); }} draggable="true" onDragStart={onDragStart} onDragEnd={onDragEnd} className={cn("absolute left-1 right-1 p-2 rounded-lg text-white text-xs cursor-pointer transition-all overflow-hidden flex flex-col border-l-4 shadow-sm", getAppointmentStyle(appointment.therapistColor), isBeingDragged && 'opacity-50 ring-2 ring-sky-400 scale-105')} style={{ top: `${top}px`, height: `${height}px`, zIndex: 20 }}>
      <div className="flex-grow min-h-0 flex flex-col">
        ...
        {height <= 60 && height > 30 && (<div className="text-xs opacity-80">
            {format(appointment.startTime, 'HH:mm')}
          </div>)}
      </div>
    </div>);
};
const WeeklyView = ({ currentDate, appointments, therapists, onSlotClick, onAppointmentClick, onDragStart, onDragEnd, onDragOver, onDrop, draggedAppointmentId }) => {
    ...
    return (<div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="mb-4">
        <div className="grid grid-cols-6 gap-2">
          {weekDays.map(day => {
            ...
            return (<Card key={day.toISOString()} className={cn(isToday(day) && "border-sky-300 bg-sky-50")}>
                <CardHeader className="pb-2">
                  <CardTitle className={cn("text-center text-sm font-semibold", isToday(day) ? "text-sky-600" : "text-slate-700")}>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-16 flex-shrink-0 border-r border-slate-200 bg-slate-50/30">
          ...
          {timeSlots.map(time => (<div key={time} className="h-12 text-right pr-2 text-xs text-slate-600 font-medium flex items-start pt-1 border-b border-slate-100">
        ...
        <div className="flex-1 grid grid-cols-6 gap-px bg-slate-100/50 overflow-auto">
          {weekDays.map((day) => (<div key={day.toISOString()} className="bg-white shadow-sm">
              {/* Therapist header */}
              <div className="h-12 border-b border-slate-200 bg-slate-50/50 px-2 py-1">
                <div className="grid grid-cols-3 gap-1 h-full">
                  {therapists.slice(0, 3).map((therapist) => (<div key={therapist.id} className={cn("text-xs font-medium text-center rounded px-1 py-1 truncate text-white", `bg-${therapist.color}-500`)} title={therapist.name}>
              ...
              <div className="relative" style={{ height: `${(END_HOUR - START_HOUR) * 60 * PIXELS_PER_MINUTE}px` }}>
                {timeSlots.map((time, index) => (<div key={time} className="absolute left-0 right-0 border-t border-slate-100 hover:bg-blue-50/50 transition-colors cursor-pointer group hover:shadow-sm" style={{
                ...
                }} onDragOver={onDragOver} onDrop={(e) => onDrop(e, day, therapists[0]?.id || '')}/>))}

                {/* Appointments for this day */}
                {appointments
                .filter(app => isSameDay(app.startTime, day))
                .map(app => (<ImprovedAppointmentCard key={app.id} appointment={app} startHour={START_HOUR} pixelsPerMinute={PIXELS_PER_MINUTE} isBeingDragged={draggedAppointmentId === app.id} onClick={() => onAppointmentClick(app)} onDragStart={(e) => onDragStart(e, app)} onDragEnd={onDragEnd}/>))}
