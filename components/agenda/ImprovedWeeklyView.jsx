...
const CurrentTimeIndicator = () => {
    ...
    return (<div className="absolute left-0 right-0 z-10 pointer-events-none" style={{ top: `${top}px` }}>
...
const groupOverlappingAppointments = (appointments) => {
    ...
    Object.entries(groupedByTherapist).forEach(([therapistId, therapistApps], therapistIndex) => {
        therapistApps.forEach((app, index) => {
            const overlapping = therapistApps.filter(otherApp => otherApp.id !== app.id &&
...
const MultiTherapistAppointmentCard = ({ appointments, startHour, pixelsPerMinute, isBeingDragged, onClick, onRightClick, onDragStart, onDragEnd, width, leftOffset }) => {
    ...
    const getAppointmentStyle = (color, status) => {
        const baseStyles = {
        ...
        const colorStyle = baseStyles[color] || 'bg-slate-600 hover:bg-slate-700 shadow-md';
        const statusStyle = getStatusStyle(status);
        return statusStyle || colorStyle;
    };
    return (<AppointmentTooltip appointment={appointment}>
      <div onClick={(e) => { e.stopPropagation(); onClick(appointment); }} onContextMenu={(e) => { e.preventDefault(); e.stopPropagation(); onRightClick(appointment, e); }} draggable="true" onDragStart={(e) => onDragStart(e, appointment)} onDragEnd={onDragEnd} className={cn("absolute p-1 rounded-md text-white cursor-pointer transition-all overflow-hidden flex flex-col border-l-2 hover:shadow-lg hover:scale-[1.02] font-semibold", getAppointmentStyle(appointment.therapistColor, appointment.status), isBeingDragged && 'opacity-50 ring-2 ring-blue-400 scale-105')} data-testid="appointment-block" style={{
            top: `${top}px`,
            height: `${height}px`,
            width: `${width}%`,
            left: `${leftOffset}%`,
        ...
        <div className="flex-grow min-h-0 flex flex-col justify-between">
          <div className="flex items-center justify-between">
            ...
            {appointment.paymentStatus === 'paid' && (<div className="w-2 h-2 bg-green-300 rounded-full ml-1 flex-shrink-0"></div>)}
          </div>
...
const ImprovedWeeklyView = ({ currentDate, appointments, therapists, onSlotClick, onAppointmentClick, onDragStart, onDragEnd, onDragOver, onDrop, draggedAppointmentId, onEdit, onDelete, onStatusChange, onPaymentStatusChange }) => {
    ...
    const handleRightClick = (appointment, e) => {
        setContextMenu({
            appointment,
    ...
    return (<div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="mb-3">
        <div className="grid grid-cols-6 gap-0.5 bg-slate-200 ml-2">
          {weekDays.map(day => {
            ...
            return (<Card key={day.toISOString()} className={cn("p-2 transition-colors", isToday(day) ? "border-sky-300 bg-sky-50" : "border-slate-200 bg-white")} data-testid="day-header">
                <div className="text-center">
                  <div className={cn("text-sm font-bold", isToday(day) ? "text-sky-700" : "text-slate-800")}>
                    {format(day, 'd')}/{format(day, 'EEE', { locale: ptBR }).slice(0, 3)}
                  </div>
                  {dayAppointments.length > 0 && (<Badge variant={isToday(day) ? "default" : "secondary"} className="mt-1 text-xs h-4 px-1.5">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-20 flex-shrink-0 border-r border-slate-200 bg-slate-50/30 ml-2">
          ...
          {timeSlots.map(time => (<div key={time} className="h-8 text-right pr-4 text-xs text-slate-600 font-medium flex items-center border-b border-slate-100">
        ...
        <div className="flex-1 grid grid-cols-6 gap-0.5 bg-slate-100/50 overflow-auto scroll-smooth ml-2" data-testid="calendar-grid">
          {weekDays.map((day) => {
            ...
            return (<div key={day.toISOString()} className="bg-white relative shadow-sm">
                {/* Time lines for each hour - Subtle and professional */}
                {timeSlots.map(time => (<div key={time} className={cn("absolute w-full border-b transition-colors", time.endsWith('00') ? "border-slate-200" : "border-slate-100")} style={{ top: `${(parseInt(time.split(':')[0]) - START_HOUR) * 60 * PIXELS_PER_MINUTE}px`, height: '1px' }}></div>))}
                ...
                <div className="relative" style={{ height: `${(END_HOUR - START_HOUR) * 60 * PIXELS_PER_MINUTE}px` }}>
                  ...
                  <div className="absolute inset-0 grid grid-cols-3">
                    {therapists.slice(0, 3).map((therapist) => (<div key={therapist.id} className="border-r border-slate-200 last:border-r-0 hover:bg-slate-50/50 transition-colors" onDragOver={onDragOver} onDrop={(e) => onDrop(e, day, therapist.id)}>
                        {timeSlots.map((time) => (<div key={time} className={cn("border-t border-slate-100 hover:bg-blue-50/50 transition-colors cursor-pointer group", time.endsWith('00') ? "border-slate-200" : "border-slate-100", "hover:shadow-sm")} style={{
                  ...
                  {/* Appointments */}
                  {groupedAppointments.map((group) => (group.appointments[0] && <MultiTherapistAppointmentCard key={group.appointments[0].id} appointments={group.appointments} startHour={START_HOUR} pixelsPerMinute={PIXELS_PER_MINUTE} isBeingDragged={draggedAppointmentId === group.appointments[0].id} onClick={onAppointmentClick} onRightClick={handleRightClick} onDragStart={onDragStart} onDragEnd={onDragEnd} width={group.width} leftOffset={group.leftOffset} therapistId={group.therapistId}/>))}
      ...
      {/* Context Menu */}
      {contextMenu && (<AppointmentContextMenu appointment={contextMenu.appointment} position={contextMenu.position} onClose={() => setContextMenu(null)} onEdit={() => {
                if (onEdit)
                    onEdit(contextMenu.appointment);
            }} onDelete={() => {
                if (onDelete)
                    onDelete(contextMenu.appointment.id);
            }} onDuplicate={() => handleDuplicate(contextMenu.appointment)} onStatusChange={(status) => {
                if (onStatusChange)
                    onStatusChange(contextMenu.appointment, status);
            }} onPaymentToggle={() => {
