...
import { X, Calendar, Clock, User, FileText, Check, ChevronRight, Stethoscope, Activity, Heart, Brain } from 'lucide-react';
...
export default function BookingModal({ slot, onClose, onSuccess }) {
    ...
    return (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden">
        <div className="bg-gradient-to-r from-sky-500 to-sky-600 p-6 text-white">
          ...
          <div className="flex items-center gap-2 mt-6">
            {[1, 2, 3].map(i => (<React.Fragment key={i}>
                <div className={cn("flex items-center justify-center w-8 h-8 rounded-full transition-all", step >= i
                ? "bg-white text-sky-600 font-bold"
                : "bg-white/20 text-sky-100")}>
                  {step > i ? <Check className="w-4 h-4"/> : i}
                </div>
                {i < 3 && (<div className={cn("flex-1 h-1 rounded-full transition-all", step > i ? "bg-white" : "bg-white/20")}/>)}
              </React.Fragment>))}
          </div>
        </div>
        ...
        <div className="p-6">
          <AnimatePresence mode="wait">
            {step === 1 && (<motion.div key="step1" initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -20, opacity: 0 }} className="space-y-6">
                <div>
            ...
            {step === 2 && (<motion.div key="step2" initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -20, opacity: 0 }} className="space-y-6">
                <div>
                  <label className="flex items-center gap-2 text-lg font-semibold text-gray-900 mb-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {appointmentTypes.map(type => (<motion.button key={type.value} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => setAppointmentType(type.value)} className={cn("relative p-4 rounded-xl border-2 transition-all text-left", appointmentType === type.value ? "border-sky-500 bg-sky-50" : "border-gray-200 hover:border-gray-300")}>
            ...
            {step === 3 && (<motion.div key="step3" initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -20, opacity: 0 }} className="space-y-6">
                <div>
                  <label className="flex items-center gap-2 text-lg font-semibold text-gray-900 mb-4">
            ...
            {step === 4 && (<motion.div key="step4" initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex flex-col items-center justify-center py-12">
        ...
        {step < 4 && (<div className="flex items-center justify-between p-6 bg-gray-50 border-t border-gray-200">
