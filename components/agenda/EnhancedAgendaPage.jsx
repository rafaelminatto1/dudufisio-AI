...
export default function EnhancedAgendaPage() {
    ...
    const filteredAppointments = useMemo(() => {
        ...
        if (searchQuery) {
            ...
            filtered = filtered.filter(app => app.patientName.toLowerCase().includes(query) ||
    ...
    const handleSlotClick = (date, time, therapistId) => {
    ...
    const handleDrop = async (e, targetDate, _therapistId) => {
        e.preventDefault();
    ...
    const renderView = () => {
        ...
        if (filteredAppointments.length === 0) {
            return (<AgendaEmptyState viewType={currentView} onAddAppointment={handleAddAppointment} onClearFilters={() => {
                    setSearchQuery('');
                    setFilters({});
                }} hasFilters={!!searchQuery || Object.keys(filters).length > 0} date={currentDate}/>);
        }
        switch (currentView) {
            case 'daily':
                return (<DailyView selectedDate={currentDate} appointments={filteredAppointments} therapists={therapists} onSlotClick={handleSlotClick} onAppointmentClick={handleAppointmentClick} onDragStart={handleDragStart} onDragEnd={handleDragEnd} onDragOver={handleDragOver} onDrop={handleDrop} draggedAppointmentId={draggedAppointmentId}/>);
            case 'weekly':
                return (<ImprovedWeeklyView currentDate={currentDate} appointments={filteredAppointments} therapists={therapists} onSlotClick={handleSlotClick} onAppointmentClick={handleAppointmentClick} onDragStart={handleDragStart} onDragEnd={handleDragEnd} onDragOver={handleDragOver} onDrop={handleDrop} draggedAppointmentId={draggedAppointmentId}/>);
            case 'monthly':
                return (<MonthlyView currentDate={currentDate} appointments={filteredAppointments} therapists={therapists} onDateClick={(date) => {
                        setCurrentDate(date);
    ...
    return (<div className="flex flex-col h-full bg-background">
      {/* Enhanced Header */}
      <EnhancedAgendaHeader currentDate={currentDate} viewType={currentView} onDateChange={setCurrentDate} onViewChange={setCurrentView} onAddAppointment={user?.role !== Role.Patient ? handleAddAppointment : undefined} onSearch={handleSearch} onFilter={handleFilter} appointmentCount={stats.appointmentCount} patientCount={stats.patientCount} totalValue={stats.totalValue} therapists={therapists.map(t => ({
            id: t.id,
            name: t.name,
            color: t.color
        }))}/>
      ...
      {/* Modals */}
      {selectedAppointment && (<AppointmentDetailModal appointment={selectedAppointment} isOpen={!!selectedAppointment} onClose={() => setSelectedAppointment(null)} onEdit={(appointment) => {
                setAppointmentToEdit(appointment);
                setSelectedAppointment(null);
            }}/>)}

      {appointmentToEdit && (<AppointmentFormModal appointment={appointmentToEdit} isOpen={!!appointmentToEdit} onClose={() => setAppointmentToEdit(null)} onSave={async () => {
                await refetch();
                setAppointmentToEdit(null);
                return true;
            }}/>)}

      {isFormOpen && (<AppointmentFormModal isOpen={isFormOpen} onClose={() => setIsFormOpen(false)} initialData={initialFormData} onSave={async () => {
                await refetch();
                setIsFormOpen(false);
                return true;
            }} onDelete={async () => {
                ...
                setIsFormOpen(false);
                return true;
            }} patients={patients} therapists={therapists} allAppointments={appointments}/>)}
    </div>);
}
