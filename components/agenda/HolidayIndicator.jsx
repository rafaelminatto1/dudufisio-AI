...
const HolidayIndicator = ({ date, className = '' }) => {
    const dateString = date.toISOString().split('T')[0];
    const holiday = HOLIDAYS_2025.find(h => h.date === dateString);
    if (!holiday)
        return null;
    return (<Badge variant="destructive" className={`text-xs h-3 px-1 mt-0.5 ${className}`} title={holiday.name} data-testid="holiday-indicator">
      🎉
    </Badge>);
};
