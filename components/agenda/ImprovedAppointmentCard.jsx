...
const ImprovedAppointmentCard = ({ appointment, startHour, pixelsPerMinute, isBeingDragged = false, onClick, onDragStart, onDragEnd, viewMode = 'compact' }) => {
    ...
    if (viewMode === 'list') {
        return (<Card className={cn("w-full cursor-pointer transition-all hover:shadow-md", isBeingDragged && 'opacity-50 ring-2 ring-sky-400', (isCompleted || isCancelled) && 'opacity-60 hover:opacity-100')} onClick={(e) => { e.stopPropagation(); onClick(); }} draggable="true" onDragStart={onDragStart} onDragEnd={onDragEnd}>
        <CardContent className="p-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              ...
              {appointment.seriesId && (<Repeat className="w-4 h-4 text-muted-foreground"/>)}
            </div>
          </div>
        </CardContent>
      </Card>);
    ...
    if (viewMode === 'detailed') {
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <div onClick={(e) => { e.stopPropagation(); onClick(); }} draggable="true" onDragStart={onDragStart} onDragEnd={onDragEnd} className={cn("absolute left-1 right-1 rounded-lg text-white z-10 cursor-pointer transition-all overflow-hidden border-l-4 shadow-sm", style, (isCompleted || isCancelled) && 'opacity-60 hover:opacity-100', isBeingDragged && 'opacity-50 ring-2 ring-sky-400')} style={{ top: `${top}px`, height: `${height}px`, minHeight: '60px' }}>
              <div className="p-3 h-full flex flex-col">
                <div className="flex items-start justify-between mb-2">
                  ...
                  <Badge variant={getStatusBadgeVariant(appointment.status)} className="text-xs ml-2 flex-shrink-0">
                    {getStatusText(appointment.status)}
                  </Badge>
                </div>
                ...
                <div className="flex items-center justify-between text-xs opacity-90 mt-auto">
                  ...
                  {appointment.seriesId && (<Repeat className="w-3 h-3"/>)}
                </div>
              </div>
            </div>
          </TooltipTrigger>
    ...
    return (<TooltipProvider>
      <Tooltip>
        <TooltipTrigger asChild>
          <div onClick={(e) => { e.stopPropagation(); onClick(); }} draggable="true" onDragStart={onDragStart} onDragEnd={onDragEnd} className={cn("absolute left-1 right-1 p-2 rounded-lg text-white z-10 cursor-pointer transition-all overflow-hidden flex flex-col group border-l-4 shadow-md font-semibold", style, (isCompleted || isCancelled) && 'bg-gray-600 hover:bg-gray-700', isBeingDragged && 'opacity-50 ring-2 ring-sky-400')} style={{ top: `${top}px`, height: `${height}px`, minHeight: '32px' }}>
