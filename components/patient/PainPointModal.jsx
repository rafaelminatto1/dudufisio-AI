...
const painTypes = ['aguda', 'cansaço', 'formigamento', 'latejante', 'queimação'];
const PainPointModal = ({ isOpen, onClose, onSave, onDelete, point }) => {
    const [intensity, setIntensity] = useState(point.intensity || 0);
    const [type, setType] = useState(point.type || 'aguda');
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md" onClick={e => e.stopPropagation()}>
                ...
                <main className="p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <select value={type} onChange={e => setType(e.target.value)} className="mt-1 w-full p-2 border border-slate-300 rounded-lg bg-white">
                     ...
                     <div>
                        ...
                        <textarea value={description} onChange={e => setDescription(e.target.value)} rows={3} className="mt-1 w-full p-2 border border-slate-300 rounded-lg" placeholder="Ex: Dor pontual ao realizar rotação externa..."/>
                    </div>
                </main>
                <footer className="flex justify-between items-center p-4 border-t border-slate-200 bg-slate-50">
                    {onDelete && (<button onClick={onDelete} className="px-4 py-2 text-sm font-medium text-red-600 hover:bg-red-50 rounded-lg flex items-center">
