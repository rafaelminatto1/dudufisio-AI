...
const AttendanceChart = ({ attendanceSeries, patients }) => {
    const patientIdsWithData = Object.keys(attendanceSeries);
    const [selectedPatientId, setSelectedPatientId] = useState(patientIdsWithData[0]);
    useEffect(() => {
    ...
    return (<Card>
            <CardHeader className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                ...
                <div className="w-full md:w-64">
                    <Select value={selectedPatientId} onValueChange={setSelectedPatientId}>
                        ...
                        <SelectContent>
                            {patientOptions.map(option => (<SelectItem key={option.value} value={option.value}>
                                    {option.label}
                                </SelectItem>))}
                        </SelectContent>
                    </Select>
            ...
            <CardContent>
                {chartData.length === 0 ? (<div className="h-64 flex items-center justify-center text-sm text-slate-500">
                        {!selectedPatientId
                ...
                : 'Ainda não há dados suficientes de presença para esse paciente.'}
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                            <LineChart data={chartData} margin={{ top: 20, right: 20, bottom: 10, left: 0 }}>
                                ...
                                <YAxis domain={[0, 1]} ticks={[0, 0.5, 1]} tickFormatter={value => valueToLabel(value)} fontSize={12}/>
                                <Tooltip formatter={(value) => valueToLabel(value)} labelFormatter={(label) => `Data: ${label}`}/>
