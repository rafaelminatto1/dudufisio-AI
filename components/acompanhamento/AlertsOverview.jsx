...
const AlertsOverview = ({ alerts, metrics }) => {
    ...
    return (<Card>
            <CardHeader>
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        {metricCards.map(card => (<div key={card.title} className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
            ...
            <CardContent className="space-y-4">
                {summary.map(level => (<div key={level.key} className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
                        <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1">
                                ...
                                {level.examples.length > 0 && (<div className="mt-3">
                                        <p className="text-sm text-slate-500 uppercase tracking-wide font-semibold">Exemplos</p>
                                        <ul className="mt-2 space-y-1 text-sm text-slate-600">
                                            {level.examples.map(name => (<li key={name} className="flex items-center gap-2">
                                                    <span className="w-1.5 h-1.5 bg-slate-300 rounded-full"/>
                                                    {name}
                                                </li>))}
                                            {alerts[level.key].length > level.examples.length && (<li className="text-slate-400 text-xs">
