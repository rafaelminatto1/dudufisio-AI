...
const TimelineBoard = ({ timeline }) => {
    return (<Card>
            <CardHeader>
                <CardTitle className="text-lg font-semibold text-slate-800">Linha do Tempo de Visitas</CardTitle>
            </CardHeader>
            <CardContent>
                {timeline.length === 0 ? (<div className="text-sm text-slate-500 text-center py-16">
                        Ainda não há pacientes com histórico suficiente para exibir.
                    </div>) : (<div className="space-y-4">
                        {timeline.map(entry => {
                const status = statusInfo[entry.status];
                return (<div key={entry.patientId} className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                                    <div className="flex-1">
                                        ...
                                        <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-slate-600">
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span>
                                                    Próxima visita:{' '}
                                                    {entry.nextVisit ? (<span className="text-slate-700">{entry.nextVisit}</span>) : (<span className="text-red-500">Sem agendamento</span>)}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
