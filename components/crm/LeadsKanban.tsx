...
const COLUMNS: Array<{ status: LeadStatus; title: string; color: string }> = [
...
export const LeadsKanban: React.FC<LeadsKanbanProps> = ({ clinicId, onLeadClick }) => {
        ...
        {COLUMNS.map((col) => (
          <div key={col.status} className="flex-shrink-0 w-80">
            <div className="bg-white dark:bg-gray-800 rounded-lg p-4 animate-pulse">
              ...
              {[1, 2, 3].map((i) => (
                <div key={i} className="h-32 bg-gray-100 dark:bg-gray-700 rounded mb-3"></div>
              ))}
            </div>
          </div>
        ))}
