...
const ExerciseFormModal = ({ isOpen, onClose, onSave, exerciseToEdit, defaultCategory, allCategories, saveAsSuggestion = false }) => {
    const [formData, setFormData] = useState(initialFormData);
    const [isSaving, setIsSaving] = useState(false);
    const { showToast } = useToast();
    const { user } = useApp();
    const modalRef = useRef(null);
    const joinArray = (arr) => arr ? arr.join(', ') : '';
            ...
            <div ref={modalRef} className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                ...
                <main className="flex-1 overflow-y-auto p-6 space-y-4">
                    ...
                    <div>
                        ...
                        {formData.instructions.map((inst, index) => (<div key={index} className="flex items-center gap-2 mt-1">
