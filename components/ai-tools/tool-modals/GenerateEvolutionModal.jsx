...
import { FileClock, Loader2, Download, Copy, Check, Clock, TrendingUp, Brain } from 'lucide-react';
import { useToast } from '@/contexts/ToastContext';
const GenerateEvolutionModal = ({ isOpen, onClose, patientData }) => {
        ...
        <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input id="sessionNumber" type="number" value={formData.sessionNumber} onChange={(e) => handleInputChange('sessionNumber', e.target.value)} placeholder="Ex: 5"/>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Select value={formData.painScale} onValueChange={(value) => handleInputChange('painScale', value)}>
                  ...
                  <SelectContent>
                    {Array.from({ length: 11 }, (_, i) => (<SelectItem key={i} value={i.toString()}>
              ...
              <div className="space-y-2">
                ...
                <Textarea id="patientReport" value={formData.patientReport} onChange={(e) => handleInputChange('patientReport', e.target.value)} placeholder="Como o paciente se sente hoje? Que mudanças observou?" rows={3}/>
              </div>

              <div className="space-y-2">
                <Label htmlFor="objectiveData">Dados Objetivos</Label>
                <Textarea id="objectiveData" value={formData.objectiveData} onChange={(e) => handleInputChange('objectiveData', e.target.value)} placeholder="Resultados de testes, medidas, avaliações físicas..." rows={3}/>
              </div>

              <div className="space-y-2">
                <Label htmlFor="interventions">Intervenções Realizadas</Label>
                <Textarea id="interventions" value={formData.interventions} onChange={(e) => handleInputChange('interventions', e.target.value)} placeholder="Técnicas aplicadas, exercícios realizados, modalidades utilizadas..." rows={3}/>
              </div>

              <div className="space-y-2">
                <Label htmlFor="physioAnalysis">Análise do Fisioterapeuta</Label>
                <Textarea id="physioAnalysis" value={formData.physioAnalysis} onChange={(e) => handleInputChange('physioAnalysis', e.target.value)} placeholder="Sua análise profissional sobre a evolução do paciente..." rows={3}/>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Textarea id="nextSteps" value={formData.nextSteps} onChange={(e) => handleInputChange('nextSteps', e.target.value)} placeholder="O que será feito na próxima sessão? Ajustes no tratamento?" rows={2}/>
              </div>
              ...
              <div className="flex justify-end gap-2 pt-4">
                ...
                <Button onClick={generateEvolution} disabled={!isFormValid || isGenerating} className="min-w-[140px]">
                  {isGenerating ? (<>
                ...
                <CardHeader>
                  <CardTitle className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button variant="outline" size="sm" onClick={downloadEvolution} className="flex items-center gap-2">
