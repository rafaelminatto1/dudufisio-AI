...
import { Dumbbell, Loader2, Download, Copy, Check, Clock, Plus, Trash2, Brain } from 'lucide-react';
import { useToast } from '@/contexts/ToastContext';
const GenerateHEPModal = ({ isOpen, onClose, patientData }) => {
    const { showToast } = useToast();
    const [isGenerating, setIsGenerating] = useState(false);
    const [generatedHEP, setGeneratedHEP] = useState('');
        ...
        <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input id="patientName" value={formData.patientName} onChange={(e) => setFormData(prev => ({ ...prev, patientName: e.target.value }))} placeholder="Nome completo do paciente"/>
                </div>
                <div className="space-y-2">
                  <Label htmlFor="diagnosis">Diagnóstico *</Label>
                  <Input id="diagnosis" value={formData.diagnosis} onChange={(e) => setFormData(prev => ({ ...prev, diagnosis: e.target.value }))} placeholder="Ex: Lombalgia, Cervicalgia, etc."/>
                </div>
              </div>

              <div className="space-y-2">
                <Label htmlFor="hepGoal">Objetivo do HEP</Label>
                <Textarea id="hepGoal" value={formData.hepGoal} onChange={(e) => setFormData(prev => ({ ...prev, hepGoal: e.target.value }))} placeholder="Qual o objetivo principal do plano de exercícios?" rows={2}/>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <Button type="button" variant="outline" size="sm" onClick={addExercise} className="flex items-center gap-2">
                    <Plus className="w-4 h-4"/>
                    Adicionar Exercício
                  </Button>
                </div>

                {formData.exercises.map((exercise, index) => (<Card key={exercise.id} className="p-4">
                    <div className="flex items-center justify-between mb-4">
                      <h4 className="font-medium">Exercício {index + 1}</h4>
                      <Button type="button" variant="outline" size="sm" onClick={() => removeExercise(exercise.id)} className="text-red-500 hover:text-red-700">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        <Input value={exercise.name} onChange={(e) => updateExercise(exercise.id, 'name', e.target.value)} placeholder="Ex: Flexão de Joelho"/>
                      </div>
                      <div className="space-y-2">
                        <Label>Descrição</Label>
                        <Input value={exercise.description} onChange={(e) => updateExercise(exercise.id, 'description', e.target.value)} placeholder="Como realizar o exercício"/>
                      </div>
                      <div className="space-y-2">
                        <Label>Séries</Label>
                        <Input value={exercise.sets} onChange={(e) => updateExercise(exercise.id, 'sets', e.target.value)} placeholder="Ex: 3"/>
                      </div>
                      <div className="space-y-2">
                        <Label>Repetições</Label>
                        <Input value={exercise.repetitions} onChange={(e) => updateExercise(exercise.id, 'repetitions', e.target.value)} placeholder="Ex: 10-15"/>
                      </div>
                      <div className="space-y-2">
                      ...
                      <div className="space-y-2">
                        <Label>Duração</Label>
                        <Input value={exercise.duration} onChange={(e) => updateExercise(exercise.id, 'duration', e.target.value)} placeholder="Ex: 5-10 min"/>
                      </div>
                    </div>
                  </Card>))}
              </div>
              ...
              <div className="space-y-2">
                ...
                <Textarea id="generalInstructions" value={formData.generalInstructions} onChange={(e) => setFormData(prev => ({ ...prev, generalInstructions: e.target.value }))} placeholder="Orientações gerais para o paciente..." rows={3}/>
              </div>

              <div className="space-y-2">
                <Label htmlFor="precautions">Precauções</Label>
                <Textarea id="precautions" value={formData.precautions} onChange={(e) => setFormData(prev => ({ ...prev, precautions: e.target.value }))} placeholder="Cuidados e precauções especiais..." rows={2}/>
              </div>

              <div className="space-y-2">
                <Label htmlFor="progression">Critérios de Progressão</Label>
                <Textarea id="progression" value={formData.progression} onChange={(e) => setFormData(prev => ({ ...prev, progression: e.target.value }))} placeholder="Como e quando progredir os exercícios..." rows={2}/>
              </div>
              ...
              <div className="flex justify-end gap-2 pt-4">
                ...
                <Button onClick={generateHEP} disabled={!isFormValid || isGenerating} className="min-w-[140px]">
                  {isGenerating ? (<>
                ...
                <CardHeader>
                  <CardTitle className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button variant="outline" size="sm" onClick={downloadHEP} className="flex items-center gap-2">
