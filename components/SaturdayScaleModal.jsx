...
const SaturdayScaleModal = ({ isOpen, onClose, appointments }) => {
    const saturdaySchedule = useMemo(() => {
        ...
        const sortedSchedule = Object.fromEntries(Object.entries(schedule).sort(([timeA], [timeB]) => timeA.localeCompare(timeB)));
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                <header className="flex items-center justify-between p-5 border-b border-slate-200">
                    <div>
                        ...
                        {saturdaySchedule.date && (<p className="text-sm text-slate-500">{saturdaySchedule.date.toLocaleDateString('pt-BR', { weekday: 'long', day: '2-digit', month: 'long' })}</p>)}
                    </div>
                    <button onClick={onClose} className="p-2 rounded-full hover:bg-slate-100"><X className="w-5 h-5 text-slate-600"/></button>
                </header>
                <main className="flex-1 overflow-y-auto p-6">
                    {scheduleKeys.length > 0 ? (<ul className="space-y-4">
                            {scheduleKeys.map(time => (<li key={time}>
                                    <div className="px-2 py-1 bg-slate-100 rounded-md font-bold text-slate-700 text-sm mb-2">{time}</div>
                                    <ul className="space-y-1 pl-4">
                                        {saturdaySchedule.schedule[time]?.map((patientName, index) => (<li key={`${time}-${index}-${patientName}`} className="text-slate-600 list-disc list-inside">{patientName}</li>))}
                                    </ul>
                                </li>))}
                        </ul>) : (<div className="text-center text-slate-500 py-10">
