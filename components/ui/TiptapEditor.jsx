...
import { Bold, Italic, Underline as UnderlineIcon, Strikethrough, List, ListOrdered, Quote, Undo, Redo, AlignLeft, AlignCenter, AlignRight, AlignJustify, Link as LinkIcon, Image as ImageIcon, Table as TableIcon, Type } from 'lucide-react';
const TiptapEditor = ({ value, onChange, placeholder = "Digite seu texto aqui...", disabled = false, minHeight = "200px", showToolbar = true }) => {
    ...
    const ToolbarButton = ({ onClick, isActive = false, disabled: buttonDisabled = false, children, title }) => (<button type="button" onClick={onClick} disabled={buttonDisabled || disabled} title={title} className={`p-2 rounded hover:bg-slate-200 transition-colors ${isActive ? 'bg-slate-300 text-slate-800' : 'text-slate-600'} ${buttonDisabled || disabled ? 'opacity-50 cursor-not-allowed' : ''}`}>
      {children}
    </button>);
    const Separator = () => <div className="w-px h-6 bg-slate-300 mx-1"/>;
    return (<div className="border border-slate-300 rounded-lg overflow-hidden">
      {showToolbar && (<div className="flex flex-wrap items-center gap-1 p-2 bg-slate-50 border-b border-slate-300">
          {/* Text Formatting */}
          ...
          <ToolbarButton onClick={() => {
            ...
            }} isActive={editor.isActive('link')} title="Inserir link">
      ...
      <EditorContent editor={editor} className={`${disabled ? 'pointer-events-none' : ''}`}/>
      
      {placeholder && !editor.getText() && (<div className="absolute top-4 left-4 text-slate-400 pointer-events-none">
          {placeholder}
        </div>)}
    </div>);
};
