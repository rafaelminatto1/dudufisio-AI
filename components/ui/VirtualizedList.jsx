...
export const useVirtualizedList = (options) => {
    ...
    const visibleEndIndex = Math.min(items.length - 1, Math.ceil((scrollTop + containerHeight) / itemHeight) + overscan);
    ...
    const scrollToIndex = useCallback((index) => {
        const element = document.querySelector('[data-virtualized-container]');
        if (element) {
...
const VirtualizedList = (props) => {
    const { items, itemHeight, containerHeight, renderItem, keyExtractor = (_, index) => index, overscan = 5, onLoadMore, hasMore = false, isLoading = false, loadingComponent, emptyComponent, className = '' } = props;
    const containerRef = useRef(null);
    const { visibleItems, totalHeight, scrollToIndex, scrollToTop, scrollToBottom } = useVirtualizedList({
        items,
        itemHeight,
        containerHeight,
        overscan,
    ...
    const defaultLoadingComponent = (<div className="flex items-center justify-center p-4">
      <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-sky-500"></div>
      <span className="ml-2 text-sm text-slate-600">Carregando mais itens...</span>
    </div>);
    // Componente de lista vazia padrão
    const defaultEmptyComponent = (<div className="flex flex-col items-center justify-center p-8 text-center">
    ...
    return (<div className={`relative ${className}`}>
      <div ref={containerRef} data-virtualized-container className="overflow-auto" style={{ height: containerHeight }} onScroll={handleScroll}>
        <div style={{ height: totalHeight, position: 'relative' }}>
          {visibleItems.map(({ item, index, key, style }) => (<div key={key} style={style}>
      ...
      <div className="flex justify-center space-x-2 mt-4">
        <button onClick={scrollToTop} className="px-3 py-1 text-xs bg-sky-500 text-white rounded hover:bg-sky-600 transition-colors" disabled={items.length === 0}>
          ↑ Topo
        </button>
        <button onClick={scrollToBottom} className="px-3 py-1 text-xs bg-sky-500 text-white rounded hover:bg-sky-600 transition-colors" disabled={items.length === 0}>
          ↓ Final
        </button>
      </div>
    </div>);
...
export const useOptimizedList = (items, options = {}) => {
    ...
    const [sortFn, setSortFn] = useState(initialSort || null);
    const [filterFn, setFilterFn] = useState(initialFilter || (() => true));
    ...
    const filteredAndSortedItems = useMemo(() => {
        ...
        if (debouncedSearchQuery) {
            ...
            result = result.filter(item => JSON.stringify(item).toLowerCase().includes(query));
        }
        // Aplicar ordenação
        if (sortFn) {
