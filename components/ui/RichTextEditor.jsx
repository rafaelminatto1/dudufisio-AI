...
const RichTextEditor = ({ value, onChange, rows = 3, placeholder, disabled = false }) => {
    ...
    return (<div className="flex flex-col">
      ...
      <textarea ref={textareaRef} value={value} onChange={(e) => onChange(e.target.value)} rows={rows} placeholder={placeholder} className={`w-full p-2 border border-slate-300 rounded-b-lg focus:outline-none focus:ring-1 focus:ring-sky-500 text-sm resize-y ${disabled ? 'bg-slate-100 cursor-not-allowed' : ''}`} aria-label="Editor de Texto do Relatório" disabled={disabled}/>
    </div>);
};
