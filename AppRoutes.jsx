import React, { lazy, useState, useEffect, Component, memo, useMemo, useCallback } from 'react';
import { CompleteDashboard, PatientPortalDashboard, PartnerPortalDashboard, preloadCriticalComponents, preloadUserRoleComponents } from './lib/lazyLoading';
...
class AppErrorBoundary extends Component {
    ...
    render() {
        if (this.state.hasError) {
            return (<div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-red-50 to-orange-50">
          <div className="max-w-md w-full bg-white rounded-lg shadow-xl p-8 text-center">
            ...
            <div className="space-y-2">
              <button onClick={() => window.location.reload()} className="w-full px-4 py-2 bg-sky-600 text-white rounded-lg hover:bg-sky-700 transition-colors font-semibold">
                🔄 Recarregar Página
              </button>
...
const AppContent = memo(() => {
    ...
    const LoadingScreen = useMemo(() => (<div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50">
            <div className="text-center">
                <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-indigo-600 mx-auto mb-4"></div>
                <p className="text-gray-600">Carregando...</p>
            </div>
        </div>), []);
    const DashboardLoadingScreen = useMemo(() => (<div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50">
            <div className="text-center">
                <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-indigo-600 mx-auto mb-4"></div>
    ...
    // 2FA Setup flow
    if (show2FASetup) {
        return (<TwoFactorSetupPage onComplete={() => setShow2FASetup(false)} onBack={() => setShow2FASetup(false)}/>);
    }
    ...
    if (isAuthenticated && user) {
        return (<React.Suspense fallback={DashboardLoadingScreen}>
                {renderDashboard()}
            </React.Suspense>);
    }
                ...
                <DebugProvider>
                    <SupabaseAuthProvider>
                        <AppProvider>
                            <PatientProvider>
                                <ExerciseProvider>
                                    <PerformanceProfiler id="AppRoutes" onRender={(id, phase, actualDuration) => {
