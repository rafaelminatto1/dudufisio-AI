...
class MultiCloudBackupService {
    ...
    async initializeEncryption() {
        if (!this.config.encryption.enabled)
            return;
        try {
            const keyMaterial = await crypto.subtle.importKey('raw', new TextEncoder().encode(import.meta.env.BACKUP_ENCRYPTION_KEY || 'default-key-change-in-production'), { name: 'PBKDF2' }, false, ['deriveBits', 'deriveKey']);
    ...
    async createBackup(type, source, providerId) {
        const job = {
    ...
    async scheduleBackup(schedule, type, source, providerId) {
    ...
    async restoreFromBackup(request) {
        try {
    ...
    async encryptData(data) {
        ...
        const encryptedData = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, this.encryptionKey, data);
    ...
    async decryptData(data) {
        ...
        return await crypto.subtle.decrypt({ name: 'AES-GCM', iv }, this.encryptionKey, encryptedData);
    }
    ...
    async uploadToProvider(providerId, fileName, data) {
    ...
    async uploadToSupabase(fileName, data) {
    ...
    async downloadFromProvider(backupId) {
    ...
    async updateBackupJob(job) {
        await this.supabase.from('backup_jobs').upsert(job);
    }
    async updateRestoreRequest(request) {
    ...
    async performDryRunRestore(request) {
    ...
    async updateRetentionPolicy(retention) {
