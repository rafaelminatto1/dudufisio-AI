...
function convertClinicalProtocolToSystemFormat(clinicalProtocol) {
    return {
        ...
        treatmentPlan: clinicalProtocol.phases.map((phase) => ({
            name: phase.name,
            ...
            exerciseProgram: phase.exercises?.map((ex) => ({
        ...
        phases: clinicalProtocol.phases.map((phase) => phase.name),
        dischargeCriteria: clinicalProtocol.dischargeCriteria || [],
        successRate: 85, // Default success rate for clinical protocols
        timesUsed: 0,
        averageOutcomes: {},
        estimatedDuration: {
            min: Math.min(...clinicalProtocol.phases.map((p) => p.durationWeeks || 2)),
            max: Math.max(...clinicalProtocol.phases.map((p) => p.durationWeeks || 8)),
            unit: 'weeks'
        },
        tags: clinicalProtocol.tags || [],
        references: clinicalProtocol.references?.map((ref) => ({
...
export class IntegratedProtocolsService {
    ...
    async getAllProtocols(filters) {
        ...
        if (filters?.specialty) {
            protocols = protocols.filter(p => p.specialty === filters.specialty);
        }
        if (filters?.searchTerm) {
            const searchLower = filters.searchTerm.toLowerCase();
            protocols = protocols.filter(p => p.name.toLowerCase().includes(searchLower) ||
    ...
    async getProtocolsBySpecialty(specialty) {
        return this.getAllProtocols({ specialty, includeSystem: false });
    }
    // Obter protocolos clínicos com exercícios vinculados
    async getClinicalProtocolsWithExercises() {
        return this.clinicalProtocols.map(protocol => ({
            ...protocol,
            linkedExercises: exerciseProtocolService.getExercisesForProtocol(protocol.clinicalId)
        }));
    }
    ...
    async getIntegratedStats() {
        ...
        const specialties = {
            esportiva: clinicalProtocols.filter(p => p.specialty === 'esportiva').length,
            'pos-operatoria': clinicalProtocols.filter(p => p.specialty === 'pos-operatoria').length,
            geriatrica: clinicalProtocols.filter(p => p.specialty === 'geriatrica').length
        };
        ...
        return {
            ...
            categories,
            evidenceLevels,
            protocolsWithExercises: clinicalProtocols.filter(p => exerciseProtocolService.getExercisesForProtocol(p.clinicalId).length > 0).length
        };
    }
    // Prescrever protocolo clínico
    async prescribeClinicalProtocol(protocolId, patientId, prescribedBy) {
    ...
    async getProtocolRecommendations(diagnosis, patientAge) {
        const allProtocols = await this.getAllProtocols();
        ...
        if (patientAge !== undefined) {
            if (patientAge >= 65) {
                ...
                recommendations = recommendations.sort((a, b) => {
                    const aGeriatric = a.specialty === 'geriatrica' ? 1 : 0;
                    const bGeriatric = b.specialty === 'geriatrica' ? 1 : 0;
...
export const getIntegratedProtocols = (filters) => integratedProtocolsService.getAllProtocols(filters);
export const getClinicalProtocolsWithExercises = () => integratedProtocolsService.getClinicalProtocolsWithExercises();
export const getIntegratedStats = () => integratedProtocolsService.getIntegratedStats();
export const prescribeClinicalProtocol = (protocolId, patientId, prescribedBy) => integratedProtocolsService.prescribeClinicalProtocol(protocolId, patientId, prescribedBy);
export const getProtocolRecommendations = (diagnosis, patientAge) => integratedProtocolsService.getProtocolRecommendations(diagnosis, patientAge);
