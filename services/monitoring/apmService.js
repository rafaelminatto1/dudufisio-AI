...
class APMService {
    ...
    setupInteractionTracking() {
        ...
        document.addEventListener('click', (event) => {
            ...
            this.interactions++;
            this.addBreadcrumb('user', `Clicked ${this.getElementSelector(event.target)}`, 'info');
        });
    ...
    setupUnloadTracking() {
        ...
        if ('sendBeacon' in navigator) {
            window.addEventListener('beforeunload', () => {
                ...
                navigator.sendBeacon('/api/apm/session-end', JSON.stringify(sessionData));
            });
        }
    }
    ...
    async trackConversion(event, value, metadata) {
    ...
    shouldTriggerAlert(alert, metric) {
        if (!alert.enabled || alert.status === 'muted')
            return false;
        ...
        if (!metricName.includes(alert.metric.toLowerCase()))
            return false;
    ...
    async sendAlertNotification(channel, alert, event) {
        switch (channel) {
            case 'email':
    ...
    endTransaction(transactionId, name) {
        performance.mark(`transaction-${transactionId}-end`);
        performance.measure(`transaction-${name}`, `transaction-${transactionId}-start`, `transaction-${transactionId}-end`);
    ...
    getErrorTrend(errors, timeRangeHours) {
        const bucketSize = timeRangeHours <= 24 ? 60 : 60 * 24; // 1 hour or 1 day buckets
        const buckets = new Map();
        errors.forEach(error => {
            const bucketKey = new Date(Math.floor(error.timestamp.getTime() / (bucketSize * 60 * 1000)) * bucketSize * 60 * 1000).toISOString();
    ...
    getBrowserDistribution(sessions) {
    ...
    getDeviceDistribution(sessions) {
