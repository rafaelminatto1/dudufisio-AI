...
class PatientService {
    ...
    async getPatientStatistics(patientId) {
        try {
            const [appointmentsResult, sessionsResult, painPointsResult, paymentsResult,] = await Promise.all([
                // Total appointments
                supabase
                    .from('appointments')
            ...
            if (painPointsResult.error)
                throw painPointsResult.error;
    ...
    async getPatientsByTherapist(therapistId) {
        try {
            ...
            if (error)
                throw error;
            const patientIds = [...new Set((data ?? []).map((appointment) => appointment.patient_id).filter((id) => id !== null))];
