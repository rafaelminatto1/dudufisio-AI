...
import { mockAchievements, mockGamificationOverview, } from '../data/mockData';
...
const evaluateAchievement = (achievement, data) => {
...
const updateChallenges = (challenges, stats) => challenges.map((challenge) => {
...
const refreshRewards = (rewards, totalPoints) => rewards.map((reward) => ({
    ...reward,
    unlocked: reward.unlocked || totalPoints >= reward.pointsRequired,
}));
const updateLeaderboard = (leaderboard, patientId, totalPoints, level, streak) => {
    const updated = leaderboard.map((entry) => entry.patientId === patientId
...
export const getGamificationProgress = async (patientId) => {
    ...
    const achievements = mockAchievements.map((achievement) => evaluateAchievement(achievement, {
        streak,
    ...
    const leaderboard = updateLeaderboard(base.leaderboard, patientId, totalPoints, level, streak);
    const nextMilestone = {
        ...base.nextMilestone,
