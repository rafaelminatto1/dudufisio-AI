...
export class SpecialtiesService {
    ...
    delete(id) {
        ...
        if (!specialty || !specialty.isCustom)
            return false; // Não pode deletar especialidades padrão
        const index = this.specialties.findIndex(s => s.id === id);
        if (index === -1)
            return false;
    ...
    search(query) {
        ...
        return this.specialties.filter(s => s.displayName.toLowerCase().includes(lowerQuery) ||
...
export class ContentTypesService {
    ...
    delete(id) {
        ...
        if (!contentType || !contentType.isCustom)
            return false; // Não pode deletar tipos padrão
        const index = this.contentTypes.findIndex(ct => ct.id === id);
        if (index === -1)
            return false;
    ...
    search(query) {
        const lowerQuery = query.toLowerCase();
        return this.contentTypes.filter(ct => ct.displayName.toLowerCase().includes(lowerQuery) ||
