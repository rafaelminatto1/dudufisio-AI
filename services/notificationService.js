...
import { Role, AppointmentStatus } from '../types';
...
class EnhancedNotificationService {
    ...
    async sendPushNotification(userId, config) {
    ...
    sendInAppNotification(userId, config) {
        const notification = {
    ...
    async sendTemplatedNotification(templateId, userId, variables, channels) {
            ...
            try {
                switch (channel) {
                    case 'push':
                        ...
                        if (pushSuccess)
                            results.sentChannels.push('push');
                        break;
                    case 'in_app':
    ...
    scheduleNotification(templateId, userId, variables, scheduleTime) {
...
export const generateRemindersIfNeeded = async (user) => {
    if (!user)
        return;
    ...
    if (user.role === Role.Therapist || user.role === Role.Admin) {
        ...
        const upcomingAppointments = mockAppointments.filter(app => {
            ...
            return (app.therapistId === user.id &&
        ...
        upcomingAppointments.forEach(app => {
            const reminderExists = notifications.some(n => n.userId === user.id &&
...
export const sendBroadcast = async (message, targetGroup) => {
    await delay(500);
...
export const notifySchedulingAlert = async (alert) => {
    const messageMap = {
    ...
    if (alert.alertType === 'patient_no_show_warning') {
        await sendEmail('recepcao@fisioflow.com', 'Paciente com múltiplas faltas', JSON.stringify(alert.payload, null, 2));
    }
