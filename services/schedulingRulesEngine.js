 ...
 */
export async function validateAppointment(newAppointment, patientAppointments) {
    const result = {
        warnings: [],
        suggestions: [],
    };
    ...
    const futureAppointments = patientAppointments.filter((app) => app.startTime >= new Date(new Date().setHours(0, 0, 0, 0)));
    if (futureAppointments.length > 0) {
        ...
        result.warnings.push(`Paciente já possui uma sessão futura agendada para ${formattedDate} às ${formattedTime}.`);
    }
    ...
    if (sessionNum &&
        ...
        result.suggestions.push('Esta é a última sessão do pacote. Lembre-se de discutir a renovação do tratamento com o paciente.');
    }
    // --- RULE 3: First Appointment Suggestion ---
    if (patientAppointments.length === 0) {
        result.suggestions.push('Primeiro agendamento do paciente. Recomenda-se definir o tipo como "Avaliação".');
    }
    // --- RULE 4: Pending Payment Warning ---
    const hasPendingPayments = patientAppointments.some((app) => app.paymentStatus === 'pending');
    if (hasPendingPayments) {
        result.warnings.push('Lembrete: O paciente possui pagamentos pendentes. Verifique a seção financeira.');
    }
    return result;
}
