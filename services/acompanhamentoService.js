...
const hasFutureAppointment = (patientId, allAppointments) => {
    const now = new Date();
    return allAppointments.some(app => app.patientId === patientId &&
        app.startTime > now &&
...
const getPlanSessions = (plan) => {
    if (!plan)
        return { totalSessions: 0, sessionsPerWeek: 0 };
    const totalSessions = plan.durationWeeks * plan.frequencyPerWeek;
    return { totalSessions, sessionsPerWeek: plan.frequencyPerWeek };
};
export const getCategorizedPatients = async (allPatients, allAppointments) => {
    const categorized = {
        abandonment: [],
        highRisk: [],
        attention: [],
    ...
    const patientIdsInAlerts = new Set();
    const treatmentPlans = new Map();
...
export const getIntelligentAlerts = async (patients = mockPatients, appointments) => {
...
const calculateAdherenceRate = (patientAppointments, totalSessions) => {
    if (totalSessions === 0)
        return 100;
...
export const getDashboardMetrics = async (patients = mockPatients, appointments) => {
...
export const getPatientAttendanceSeries = (appointments) => {
    const series = {};
...
export const getPatientTimelines = (patients = mockPatients, appointments) => {
    return patients
...
export const getQuickActionsData = async (patients = mockPatients, appointments) => {
