...
class PredictiveAnalyticsServiceSupabase {
     ...
     */
    calculateSuccessRate(treatments) {
        if (treatments.length === 0)
            return 0.75; // Baseline
        const successful = treatments.filter(t => (t.success_rate || 0) > 0.7 ||
            (t.outcome_score || 0) > 70).length;
        return successful / treatments.length;
    }
    /**
     * Estima número de sessões necessárias
     */
    estimateRequiredSessions(patient, treatmentType) {
        ...
        if (age > 65)
            sessions += 4;
        if (patient.medical_history?.length > 3)
            sessions += 2;
        return sessions;
    }
    /**
     ...
     */
    generateRecommendations(features, successRate) {
     ...
     */
    generateScenarios(baselineSuccess, baselineSessions) {
        return [
            {
