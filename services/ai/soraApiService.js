...
export class SoraApiService {
    constructor() {
        this.apiEndpoint = 'https://api.openai.com/v1/videos/generations';
     ...
     */
    async listQueuedVideos() {
        return Array.from(this.videoQueue.values()).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    }
    /**
     ...
     */
    async optimizePromptForSora(userPrompt, context) {
...
`;
     ...
     */
    async generateVideoWithOptimization(userPrompt, context) {
        // Otimizar prompt
...
<svg width="1920" height="1080" xmlns="http://www.w3.org/2000/svg">
  ...
  >${prompt.substring(0, 60)}...</text>
     ...
     */
    createThumbnailPlaceholder(prompt) {
        const svg = `
