...
import type { DetectedIntent, DetectedSentiment } from '@/types/crm';
...
`;
   ...
   */
  private async extractIntent(message: string): Promise<DetectedIntent> {
   ...
   */
  private getSuggestedActions(intent: DetectedIntent, entities: Record<string, any>): string[] {
   ...
   */
  private calculateConfidence(intent: DetectedIntent): number {
    // Intenções simples têm alta confiança
    const highConfidenceIntents: DetectedIntent[] = [
      'schedule',
      'reschedule',
      'cancel',
      'info_price',
   ...
   */
  private getHistory(leadId: string): Message[] {
    return this.conversationHistory.get(leadId) || [];
  }

  private addToHistory(leadId: string, message: Message): void {
    const history = this.getHistory(leadId);
  ...
  private clearHistory(leadId: string): void {
...
export const getConversationalAgent = (): ConversationalAgent => {
  if (!agentInstance) {
