...
export class SoraService {
     ...
     */
    async optimizeVideoPrompt(userPrompt, modality, exerciseType, additionalContext) {
        const modalityInfo = SPORT_MODALITIES[modality];
...
`;
     ...
     */
    async generateExerciseVideoPrompt(exerciseName, modality, difficulty, duration = 10) {
        const modalityInfo = SPORT_MODALITIES[modality];
        const basePrompt = `
...
`;
     ...
     */
    async generateMartialArtsTechniquePrompt(technique, modality, position, demonstration = 'pair') {
        const modalityInfo = SPORT_MODALITIES[modality];
        const basePrompt = `
...
`;
     ...
     */
    async generateWorkoutSeriesPrompt(exercises, modality, duration = 20) {
        const modalityInfo = SPORT_MODALITIES[modality];
        const basePrompt = `
...
`;
     ...
     */
    async generateVideoObject(type, params, options = {}) {
        let optimizedPrompt;
        // Gerar prompt otimizado baseado no tipo
        switch (type) {
            case 'exercise':
                optimizedPrompt = await this.generateExerciseVideoPrompt(params.name, params.modality, params.difficulty, params.duration || 10);
                break;
            case 'technique':
                optimizedPrompt = await this.generateMartialArtsTechniquePrompt(params.technique, params.modality, params.position, params.demonstration);
                break;
            case 'series':
                optimizedPrompt = await this.generateWorkoutSeriesPrompt(params.exercises, params.modality, params.duration || 20);
                break;
            case 'demonstration':
                optimizedPrompt = await this.optimizeVideoPrompt(params.prompt, params.modality, params.exerciseType, params.additionalContext);
                break;
            default:
...
<svg width="1920" height="1080" xmlns="http://www.w3.org/2000/svg">
  ...
  >Gerando com Sora 2...</text>
     ...
     */
    createThumbnailPlaceholder(text) {
        const svg = `
  ...
  >🎬 ${text}</text>
    ...
    async generateBatch(requests) {
        ...
        for (const request of requests) {
            const video = await this.generateVideoObject(request.type, request.params, request.options);
            videos.push(video);
        }
        return videos;
    }
