...
export interface VideoGenerationGuide {
  tool: string;
...
export class FreeVideoGeneratorService {
  ...
  async generateVideoGuide(
    exerciseName: string,
    modality: string,
    tool: keyof typeof FREE_VIDEO_TOOLS,
    additionalContext?: string
  ): Promise<VideoGenerationGuide> {
   ...
   */
  private async optimizePromptForTool(
    exerciseName: string,
    modality: string,
    tool: keyof typeof FREE_VIDEO_TOOLS,
    additionalContext?: string
...
`;
   ...
   */
  async generateMultiToolPrompts(
    exerciseName: string,
    modality: string,
    additionalContext?: string
  ): Promise<{ [key: string]: VideoGenerationGuide }> {
    const guides: { [key: string]: VideoGenerationGuide } = {};

    for (const toolKey of Object.keys(FREE_VIDEO_TOOLS) as Array<keyof typeof FREE_VIDEO_TOOLS>) {
   ...
   */
  getToolInfo(tool: keyof typeof FREE_VIDEO_TOOLS) {
