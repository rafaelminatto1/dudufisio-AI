...
export class SmartScheduler {
     ...
     */
    async suggestAppointmentSlots(leadId, preferences, limit = 3) {
        try {
            ...
            const availableSlots = await this.getAvailableSlots(preferences.serviceType, preferences.dateRange || {
                from: new Date(),
            ...
            const rankedSlots = await this.rankSlots(availableSlots, lead, urgency, preferences);
...
`;
     ...
     */
    async getAvailableSlots(serviceType, dateRange) {
        try {
     ...
     */
    async rankSlots(slots, lead, urgency, preferences) {
        const rankedSlots = slots.map((slot) => {
            let score = slot.score;
            const reasons = [];
            // Fator 1: Urgência (quanto maior, mais cedo melhor)
            const daysFromNow = Math.floor((slot.date.getTime() - Date.now()) / (1000 * 60 * 60 * 24));
     ...
     */
    async autoSchedule(leadId, slot, clinicId) {
        try {
            // 1. Criar agendamento
            const { data: appointment, error } = await supabase
                ...
                scheduled_at: new Date(slot.date.toDateString() + ' ' + slot.time).toISOString(),
            ...
            try {
                ...
                if (whatsapp.isConfigured()) {
                    const lead = await this.getLeadData(leadId);
                    if (lead?.phone) {
                        await whatsapp.sendTemplateMessage(lead.phone, 'confirmacao_agendamento', [
     ...
     */
    async suggestReschedule(appointmentId, reason) {
        try {
            ...
            return this.suggestAppointmentSlots(appointment.patient_id, preferences, 5 // Top 5 opções
            );
        }
        catch (error) {
...
// Singleton
let smartSchedulerInstance = null;
export const getSmartScheduler = () => {
