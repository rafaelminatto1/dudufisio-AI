...
export class ExerciseProtocolService {
    ...
    shouldLink(protocol, exercise) {
        ...
        const commonTags = protocolTags.filter((tag) => exerciseTags.includes(tag));
        ...
        return keywords.some(keyword => protocolText.includes(keyword) && exerciseText.includes(keyword));
    }
    // Obter fase relevante do protocolo para o exercício
    getRelevantPhase(protocol, exercise) {
    ...
    addLink(link) {
        ...
        const exists = this.links.some(l => l.exerciseId === link.exerciseId && l.protocolId === link.protocolId);
        if (!exists) {
            this.links.push(link);
        }
    }
    // Remover link
    removeLink(exerciseId, protocolId) {
        this.links = this.links.filter(l => !(l.exerciseId === exerciseId && l.protocolId === protocolId));
    }
    // Obter protocolos vinculados a um exercício
    getProtocolsForExercise(exerciseId) {
    ...
    getExercisesForProtocol(protocolId) {
    ...
    getLinkStatistics() {
        ...
        return {
            ...
            protocolsWithExercises: protocols.filter(p => this.links.some(l => l.protocolId === p.id)).length,
            exercisesWithProtocols: exercises.filter(e => this.links.some(l => l.exerciseId === e.id)).length,
    ...
    searchLinks(criteria) {
        return this.links.filter(link => {
            if (criteria.exerciseId && link.exerciseId !== criteria.exerciseId)
                return false;
            if (criteria.protocolId && link.protocolId !== criteria.protocolId)
                return false;
            if (criteria.phase && link.phase !== criteria.phase)
                return false;
            if (criteria.specialty) {
                const exercise = getExercises().find(e => e.id === link.exerciseId);
                if (exercise?.specialty !== criteria.specialty)
                    return false;
            }
            return true;
        });
    ...
    getRecommendations() {
        ...
        protocols.forEach(protocol => {
            exercises.forEach(exercise => {
                ...
                const alreadyLinked = this.links.some(l => l.exerciseId === exercise.id && l.protocolId === protocol.id);
                if (alreadyLinked)
                    return;
                let reason = '';
                let confidence = 0;
                ...
                // Tags em comum = média confiança
                    ...
                    // Palavras-chave em comum = baixa confiança
                        ...
                        const matchingKeywords = keywords.filter(keyword => protocolText.includes(keyword) && exerciseText.includes(keyword));
...
export const getExerciseProtocolLinks = (exerciseId) => exerciseProtocolService.getProtocolsForExercise(exerciseId);
export const getProtocolExerciseLinks = (protocolId) => exerciseProtocolService.getExercisesForProtocol(protocolId);
export const getAllProtocolExerciseLinks = () => exerciseProtocolService.getAllProtocolsWithExercises();
export const getLinkStatistics = () => exerciseProtocolService.getLinkStatistics();
export const getLinkRecommendations = () => exerciseProtocolService.getRecommendations();
