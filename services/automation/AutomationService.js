...
export class AutomationService {
     ...
     */
    static async processAutomationTriggers(clinicId) {
        try {
     ...
     */
    static async processCampaignTrigger(campaign) {
        try {
     ...
     */
    static async triggerNoResponse24h(campaign) {
     ...
     */
    static async triggerAppointmentScheduled(campaign) {
     ...
     */
    static async triggerAppointmentCompleted(campaign) {
        // Similar ao anterior, mas para status 'completed'
    }
    /**
     * Trigger: Lead criado
     */
    static async triggerLeadCreated(campaign) {
        const oneHourAgo = new Date();
        oneHourAgo.setHours(oneHourAgo.getHours() - 1);
        const { data: leads } = await supabase
            .from('leads')
     ...
     */
    static async processScheduledActions() {
        try {
     ...
     */
    static async executeNextStep(campaignLead) {
        try {
            ...
            if (whatsapp.isConfigured() && lead.phone) {
                await whatsapp.sendTemplateMessage(lead.phone, nextStep.template_name, nextStep.variables.map((v) => this.resolveVariable(v, lead)), campaign.clinic_id);
                // Atualizar status
                await supabase
                    .from('campaign_leads')
                    .update({
     ...
     */
    static resolveVariable(variable, lead) {
        const mapping = {
     ...
     */
    static calculateNextActionTime(delayMinutes) {
        const next = new Date();
     ...
     */
    static async createDefaultCampaigns(clinicId) {
     ...
     */
    static async listCampaigns(clinicId) {
