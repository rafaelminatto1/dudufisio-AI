...
class QualityAssuranceServiceSupabase {
     ...
     */
    async getAuditLogs(startDate, endDate, action) {
        try {
            let query = supabase
                .from('audit_trail')
                .select(`
    ...
    async getQualityMetrics(startDate, endDate) {
        try {
            ...
            if (outcomes && outcomes.length > 0) {
                const avgSatisfaction = outcomes.reduce((sum, o) => sum + (o.patient_satisfaction || 0), 0) / outcomes.length;
                metrics.push({
    ...
    async getComplianceReport(startDate, endDate) {
        try {
            ...
            const complianceByType = validations.reduce((acc, v) => {
                ...
                if (v.is_valid)
                    acc[v.validation_type].compliant++;
                return acc;
            }, {});
    ...
    generateCorrectiveActions(violations) {
        if (!violations || violations.length === 0)
            return [];
        return violations.map(v => `Corrigir: ${JSON.stringify(v)}`);
    }
}
