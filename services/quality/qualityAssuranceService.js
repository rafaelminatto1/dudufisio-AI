 ...
 */
import { QualityMetricType, ComplianceFramework, ComplianceStatus, NonComplianceSeverity } from '../../types/qualityAssuranceTypes';
class QualityAssuranceService {
    /**
     * Obtém dashboard completo de garantia de qualidade
     */
    async getQualityAssuranceDashboard(period) {
        const [qualityMetrics, clinicalIndicators, complianceOverview, complianceRequirements, recentAudits, upcomingAudits, correctiveActions, improvementPlans, trends, riskAreas, recommendations, alerts] = await Promise.all([
        ...
        const summary = this.calculateSummary(qualityMetrics, complianceRequirements, correctiveActions, upcomingAudits);
        return {
            period,
            summary,
            qualityMetrics,
     ...
     */
    calculateSummary(metrics, requirements, actions, audits) {
        const overallQualityScore = metrics.reduce((sum, m) => {
            const score = (m.currentValue / m.targetValue) * 100;
            return sum + Math.min(100, score);
        }, 0) / metrics.length;
        const compliantRequirements = requirements.filter(r => r.status === ComplianceStatus.Compliant).length;
        const complianceRate = (compliantRequirements / requirements.length) * 100;
        const activeNonCompliances = requirements.filter(r => r.status === ComplianceStatus.NonCompliant).length;
        const criticalIssues = requirements.reduce((sum, r) => {
            return sum + (r.gaps?.filter(g => g.severity === 'critical').length || 0);
        }, 0);
        const openCorrectiveActions = actions.filter(a => a.status !== 'closed').length;
        return {
            overallQualityScore,
            complianceRate,
     ...
     */
    async getClinicalQualityIndicators(period) {
        return [
            {
                id: 'indicator-1',
     ...
     */
    async getQualityImprovementPlans() {
     ...
     */
    async getQualityBenchmarks(metrics) {
    ...
    // Helper methods
    generateHistoricalData(period, min, max) {
        const data = [];
    ...
    generateTimeSeriesWithTarget(period, min, max, target) {
