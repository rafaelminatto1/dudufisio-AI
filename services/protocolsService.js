...
export const getProtocols = async (filters) => {
    ...
    if (filters?.searchTerm) {
        ...
        filteredProtocols = filteredProtocols.filter(p => p.name.toLowerCase().includes(searchLower) ||
...
export const saveProtocol = async (protocolData) => {
    await delay(500);
    if (protocolData.id) {
...
export const duplicateProtocol = async (id, newName) => {
    await delay(400);
    ...
    return protocols.filter(p => {
        ...
        for (const [condition, keywords] of Object.entries(conditionMappings)) {
            if (lowerDiagnosis.includes(condition)) {
                return keywords.some(keyword => p.name.toLowerCase().includes(keyword) ||
...
const generateFallbackProtocolContent = (protocol) => {
...
// ============================================================================
export const prescribeProtocol = async (protocolId, patientId, prescribedBy, customizations, additionalNotes) => {
    await delay(400);
...
const calculateEstimatedEndDate = (protocol) => {
    const startDate = new Date();
    const estimatedDays = protocol.estimatedDuration.max * (protocol.estimatedDuration.unit === 'weeks' ? 7 :
...
export const updateProtocolPrescription = async (prescriptionId, updates) => {
    await delay(400);
...
export const addAssessmentResult = async (prescriptionId, assessment) => {
    await delay(300);
...
export const recordProtocolOutcome = async (prescriptionId, outcome) => {
    await delay(300);
    const prescription = protocolPrescriptions.find(pp => pp.id === prescriptionId);
    if (!prescription) {
        throw new Error("Protocol prescription not found");
...
export const addProtocolModification = async (prescriptionId, modification) => {
    await delay(300);
    const prescription = protocolPrescriptions.find(pp => pp.id === prescriptionId);
    if (!prescription) {
        throw new Error("Protocol prescription not found");
...
// PROTOCOL ANALYTICS
// ============================================================================
export const getProtocolAnalytics = async (protocolId) => {
    await delay(400);
    if (protocolId) {
...
export const getProtocolLibraryStats = async () => {
    await delay(300);
...
export const generateProtocolReport = async (protocolId, dateRange) => {
    await delay(500);
    const protocol = protocols.find(p => p.id === protocolId);
    if (!protocol) {
        throw new Error("Protocol not found");
    ...
    const successfulCompletions = prescriptions.filter(pp => pp.status === 'completed' &&
        pp.outcomes.some(outcome => outcome.clinicallySignificant)).length;
...
// ============================================================================
export const getProtocolReferences = async (protocolId) => {
    await delay(200);
    if (protocolId) {
        const protocol = protocols.find(p => p.id === protocolId);
...
export const addReferenceToProtocol = async (protocolId, reference) => {
    await delay(300);
    const protocol = protocols.find(p => p.id === protocolId);
    if (!protocol) {
        throw new Error("Protocol not found");
...
// ============================================================================
export const submitProtocolForReview = async (protocolId) => {
    await delay(300);
    const protocol = protocols.find(p => p.id === protocolId);
    if (!protocol) {
        throw new Error("Protocol not found");
...
export const approveProtocol = async (protocolId, reviewedBy) => {
    await delay(300);
    const protocol = protocols.find(p => p.id === protocolId);
    if (!protocol) {
        throw new Error("Protocol not found");
...
export const rejectProtocol = async (protocolId, reason, reviewedBy) => {
    await delay(300);
    const protocol = protocols.find(p => p.id === protocolId);
    if (!protocol) {
        throw new Error("Protocol not found");
