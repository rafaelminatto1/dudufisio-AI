...
export class SimplePDFService {
    static async generateAndDownloadPDF(title, content, filename) {
        try {
...
MÉTRICAS:
${reportData.data?.summary?.metrics?.map((m) => `- ${m.name}: ${m.value} (${m.trend})`).join('\n') || 'Métricas não disponíveis'}

ANÁLISES:
${reportData.data?.analysis?.map((a) => `- ${a.title}: ${a.description}`).join('\n') || 'Análises não disponíveis'}
    `;
    ...
    static async generateClinicalReportPDF(reportData) {
