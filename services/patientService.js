...
export const quickAddPatient = async (name) => {
    if (isSupabaseEnabled) {
        const quickPatient = {
            name: name.trim(),
            cpf: '',
...
export const getPatients = async ({ limit = 15, cursor, searchTerm, statusFilter, startDate, endDate, therapistId }) => {
    ...
    if (searchTerm) {
        ...
        filteredPatients = filteredPatients.filter(patient => patient.name.toLowerCase().includes(lowerSearchTerm) ||
    ...
    if (statusFilter && statusFilter !== 'All') {
        ...
        const internalStatus = filterMap[statusFilter] || statusFilter;
    ...
    const patientSummaries = patientSlice.map(p => ({
        id: p.id,
        name: p.name,
        email: p.email,
        phone: p.phone,
...
export const addAttachment = async (patientId, file) => {
    if (isSupabaseEnabled) {
...
const mapPatientToSummary = (patient) => ({
    id: patient.id,
    name: patient.name,
    email: patient.email,
    phone: patient.phone,
...
const filterPatientsInMemory = (patients, filters) => {
    const { searchTerm, statusFilter, startDate, endDate } = filters;
    let filtered = [...patients];
    if (searchTerm) {
        const lower = searchTerm.toLowerCase();
        filtered = filtered.filter(patient => patient.name.toLowerCase().includes(lower) ||
...
const paginatePatients = (patients, { limit = 15, cursor }) => {
    let startIndex = 0;
    if (cursor) {
...
export const addCommunicationLog = async (patientId, log) => {
    await delay(200);
    const patient = db.getPatientById(patientId);
    if (!patient) {
        throw new Error('Paciente não encontrado.');
...
export const savePainPoints = async (patientId, painPoints) => {
    await delay(200);
    const patient = db.getPatientById(patientId);
    if (!patient) {
        throw new Error('Paciente não encontrado.');
