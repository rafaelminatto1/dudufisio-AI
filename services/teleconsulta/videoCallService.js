...
class VideoCallService {
     ...
     * 🔗 CONEXÕES WEBRTC
     */
    async createPeerConnection(sessionId, participantId) {
    ...
    async startConnectionStats(sessionId, participantId, peerConnection) {
    ...
    async processRecording(sessionId, recordingId, blob) {
        try {
            const session = this.activeSessions.get(sessionId);
            if (!session || !session.recording) {
                return;
     ...
     */
    async toggleAudio(sessionId, userId) {
        try {
            if (!this.localStream) {
                return false;
            }
    ...
    async toggleVideo(sessionId, userId) {
        try {
            if (!this.localStream) {
                return false;
            }
