...
export class PDFService {
  ...
  public async generatePDF(content: PDFContent, filename?: string): Promise<Blob> {
    try {
  ...
  public async downloadPDF(content: PDFContent, filename: string): Promise<void> {
    try {
  ...
  public createReportPDF(report: any): PDFContent {
    return {
      ...
      sections: [
        ...
        {
          title: 'Métricas',
          content: report.data?.summary?.metrics?.map((m: any) => `${m.name}: ${m.value} (${m.trend})`) || ['Métricas não disponíveis']
        },
        {
          title: 'Análises',
  ...
  public createClinicalReportPDF(report: any): PDFContent {
    const sections = [
      {
