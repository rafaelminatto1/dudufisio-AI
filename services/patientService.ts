...
// ============================================================================
// FUNÇÕES DE PATOLOGIA PRINCIPAL (BODY MAP)
// ============================================================================

/**
 * Define a patologia principal do paciente
 */
export const setMainPathology = async (
    patientId: string,
    data: {
    ...
    if (isSupabaseEnabled) {
        const { data: updated, error } = await supabasePatientService.supabase
            .from('patients')
            .update({
                main_pathology: data.mainPathology,
                main_pathology_region: data.mainPathologyRegion,
                main_pathology_since: data.mainPathologySince?.toISOString().split('T')[0],
            })
            .eq('id', patientId)
            .select()
            .single();
    ...
    const updatedPatient = {
        ...
        main_pathology: data.mainPathology,
        main_pathology_region: data.mainPathologyRegion,
        main_pathology_since: data.mainPathologySince,
    };
...
/**
 * Atualiza a patologia principal do paciente
 */
export const updateMainPathology = async (
    patientId: string,
    data: Partial<{
        mainPathology: string;
        mainPathologyRegion: string;
    ...
    if (isSupabaseEnabled) {
        ...
        if (data.mainPathology !== undefined) updateData.main_pathology = data.mainPathology;
        if (data.mainPathologyRegion !== undefined) updateData.main_pathology_region = data.mainPathologyRegion;
        if (data.mainPathologySince !== undefined) {
            updateData.main_pathology_since = data.mainPathologySince.toISOString().split('T')[0];
        }
...
/**
 * Recupera a patologia principal do paciente
 */
export const getMainPathology = async (
    patientId: string
): Promise<{
    mainPathology?: string;
    mainPathologyRegion?: string;
    ...
    if (isSupabaseEnabled) {
        const { data, error } = await supabasePatientService.supabase
            .from('patients')
            .select('main_pathology, main_pathology_region, main_pathology_since')
            .eq('id', patientId)
            .single();
        ...
        return {
            mainPathology: data.main_pathology,
            mainPathologyRegion: data.main_pathology_region,
            mainPathologySince: data.main_pathology_since ? new Date(data.main_pathology_since) : undefined,
        };
    }
    ...
    return {
        mainPathology: patient.main_pathology,
        mainPathologyRegion: patient.main_pathology_region,
        mainPathologySince: patient.main_pathology_since,
    };
};