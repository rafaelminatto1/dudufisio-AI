...
class RiskStratificationServiceSupabase {
   ...
   */
  async saveRiskAssessment(assessment: Omit<RiskAssessment, 'id'>): Promise<RiskAssessment> {
   ...
   */
  private async saveRiskFactors(assessmentId: string, factors: RiskFactor[]): Promise<void> {
   ...
   */
  async getPatientRiskProfile(patientId: string): Promise<RiskProfile | null> {
    const { data, error } = await supabase
      .from('risk_profiles')
      .select('*')
      .eq('patient_id', patientId)
    ...
    return {
      ...
      assessments,
      overallRiskLevel: data.overall_risk_level as RiskLevel,
      highestRisks: data.highest_risks as RiskType[],
   ...
   */
  async upsertRiskProfile(profile: Omit<RiskProfile, 'assessments'>): Promise<void> {
    const { error } = await supabase
      .from('risk_profiles')
      .upsert({
   ...
   */
  async createRiskAlert(alert: Omit<RiskAlert, 'id'>): Promise<RiskAlert> {
    const { data, error } = await supabase
      .from('risk_alerts')
      .insert({
   ...
   */
  async getActiveAlerts(patientId?: string): Promise<RiskAlert[]> {
    let query = supabase
      .from('risk_alerts')
      .select('*')
      .eq('resolved', false)
   ...
   */
  async getHighRiskPatients(riskLevel?: RiskLevel): Promise<any[]> {
    let query = supabase
      .from('risk_profiles')
      .select(`
        *,
   ...
   */
  async getRiskStatistics(startDate?: Date, endDate?: Date): Promise<any> {
    let query = supabase
      .from('risk_assessments')
  ...
  private mapDatabaseToAssessment(data: any): RiskAssessment {
    return {
      ...
      trend: data.trend,
      notes: data.notes,
      factors: data.risk_factors?.map((f: any) => this.mapDatabaseToFactor(f)) || [],
      recommendations: data.risk_recommendations?.map((r: any) => this.mapDatabaseToRecommendation(r)) || [],
    };
  }
   ...
   */
  private mapDatabaseToFactor(data: any): RiskFactor {
    return {
      id: data.id,
      name: data.name,
   ...
   */
  private mapDatabaseToRecommendation(data: any): RiskRecommendation {
    return {
      id: data.id,
   ...
   */
  private mapDatabaseToAlert(data: any): RiskAlert {
    return {
      id: data.id,
      patientId: data.patient_id,
      patientName: data.patient_name,
