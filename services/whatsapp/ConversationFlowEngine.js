...
export class ConversationFlowEngine {
     ...
     */
    async processMessage(lead, message) {
...
Qual seu convênio?`,
     ...
     */
    async handleFirstTimeFlow(lead, message, context) {
...
Digite o número da opção.`;
        ...
        if (['1', '2', '3', '4'].includes(message.trim())) {
            ...
            const interest = interests[message.trim()];
...
Enquanto isso, quer saber mais sobre nossos serviços ou horários disponíveis?`;
     ...
     */
    async handleExistingLeadFlow(lead, message, context) {
        const messageLower = message.toLowerCase();
        // Intenção de agendar
...
Digite o número da opção.`;
     ...
     */
    getServiceIntroduction(service) {
        const intros = {
