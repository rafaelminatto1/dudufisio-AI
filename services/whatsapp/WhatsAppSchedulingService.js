...
export class WhatsAppSchedulingService {
     ...
     */
    async startSchedulingProcess(phone, patientName, clinicId) {
        try {
            // Buscar horários disponíveis para os próximos 7 dias
            const availableSlots = await this.getAvailableSlots(clinicId, 7);
            if (availableSlots.length === 0) {
                await this.whatsappService.sendTextMessage(phone, '😔 Desculpe, não temos horários disponíveis nos próximos 7 dias.\n\n' +
                    'Por favor, entre em contato pelo telefone (11) 5874-9885 para verificar outras opções.', clinicId);
                return;
            }
     ...
     */
    async getAvailableSlots(clinicId, daysAhead) {
        try {
            ...
            for (let i = 1; i <= daysAhead; i++) {
                ...
                for (const time of standardHours) {
                    ...
                    const isBooked = appointments?.some(apt => apt.date === dateStr && apt.time === time);
                    if (!isBooked) {
                        availableSlots.push({
                            date: dateStr,
                            time,
     ...
     */
    formatAvailableSlotsMessage(patientName, slots) {
     ...
     */
    async processSlotSelection(phone, selection, clinicId) {
        try {
            const slotNumber = parseInt(selection);
            if (isNaN(slotNumber) || slotNumber < 1 || slotNumber > 5) {
                await this.whatsappService.sendTextMessage(phone, '❌ Opção inválida. Por favor, responda com um número de 1 a 5.', clinicId);
                return;
            }
            ...
            if (!selectedSlot) {
                await this.whatsappService.sendTextMessage(phone, '❌ Desculpe, esse horário não está mais disponível.\n\n' +
                    'Digite *AGENDAR* para ver novos horários disponíveis.', clinicId);
                return;
            }
            ...
            await this.whatsappService.sendTextMessage(phone, `✅ *Agendamento Solicitado!*\n\n` +
                `📅 Data: ${formattedDate}\n` +
     ...
     */
    async findOrCreatePatient(phone, clinicId) {
        try {
     ...
     */
    async notifyStaff(phone, slot, clinicId) {
        try {
     ...
     */
    async confirmAppointment(appointmentId, therapistId, clinicId) {
        try {
        ...
        `)
            ...
            if (appointment.patient?.phone) {
                await this.whatsappService.sendAppointmentNotification(appointment.patient.phone, {
     ...
     */
    async cancelAppointment(appointmentId, reason, clinicId) {
        try {
        ...
        `)
            ...
            // Notificar paciente
            if (appointment.patient?.phone) {
                await this.whatsappService.sendTextMessage(appointment.patient.phone, `❌ *Agendamento Cancelado*\n\n` +
                    ...
                    `Desculpe o transtorno!`, clinicId);
            }
...
// Singleton instance
let schedulingServiceInstance = null;
export const getWhatsAppSchedulingService = () => {
