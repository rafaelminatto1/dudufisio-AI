...
export class MetaWhatsAppService {
     ...
     */
    async sendTextMessage(to, message, clinicId) {
     ...
     */
    async sendTemplateMessage(to, templateName, components, clinicId, language = 'pt_BR') {
        if (!this.isConfigured()) {
            throw new Error('Meta WhatsApp Service não configurado');
        }
        try {
     ...
     */
    async processIncomingMessage(message, metadata, clinicId) {
        try {
            ...
            try {
                ...
                const autoResponse = await automation.processKeywordAutomation(messageBody, from, clinicId);
                if (autoResponse) {
            ...
            try {
                ...
                await this.sendTextMessage(from, 'Olá! Recebemos sua mensagem e em breve retornaremos o contato. 😊\n\n' +
                    'Digite *AJUDA* para ver o menu de opções.\n\n' +
                    'Horário de atendimento: Segunda a Sexta, 8h às 18h.', clinicId);
            }
        }
        catch (error) {
     ...
     */
    async sendAppointmentNotification(to, appointmentData, clinicId) {
     ...
     */
    async sendAppointmentReminder(to, appointmentData, clinicId) {
     ...
     */
    async sendConfirmationRequest(to, appointmentData, clinicId) {
...
// Singleton instance
let metaWhatsAppServiceInstance = null;
export const getMetaWhatsAppService = () => {
