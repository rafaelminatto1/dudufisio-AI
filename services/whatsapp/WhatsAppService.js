...
export class WhatsAppService {
     ...
     */
    async sendMessage(to, message, clinicId) {
     ...
     */
    async sendTemplateMessage(to, templateName, variables, clinicId, language = 'pt_BR') {
        if (!this.isConfigured()) {
            throw new Error('WhatsApp Service não está configurado');
        }
        try {
     ...
     */
    async processIncomingMessage(webhook, clinicId) {
        try {
            ...
            try {
                ...
                await this.sendMessage(from, 'Olá! Recebemos sua mensagem e em breve retornaremos o contato. 😊', clinicId);
            }
        }
        catch (error) {
     ...
     */
    async scheduleMessage(to, message, scheduledAt, clinicId) {
