...
import { TransactionType, AppointmentStatus } from '../types';
...
const getExpensesFromStorage = () => {
    try {
        ...
        if (stored) {
            ...
            return parsed.map((exp) => ({ ...exp, date: new Date(exp.date) }));
        }
    }
    catch (error) {
...
export const getTransactions = async (period) => {
    ...
    const revenueTransactions = mockAppointments
...
export const getEducatorFinancials = async (educatorId) => {
    await delay(500);
    ...
    const summary = transactionsThisMonth.reduce((acc, txn) => {
    ...
    const allTransactions = allVouchers.map(voucher => {
        const patient = mockPatients.find(p => p.id === voucher.patientId);
        const grossAmount = voucher.plan.price;
