...
export class MetricsService {
     ...
     */
    static async getDashboardMetrics(clinicId) {
        ...
        allLeads.forEach((lead) => {
            ...
            // Por status
            metrics.leads_by_status[lead.status]++;
            ...
            if (lead.source in metrics.leads_by_source) {
                metrics.leads_by_source[lead.source]++;
            }
        });
        // Taxa de conversão
        ...
        try {
            ...
            metrics.avg_response_time_minutes = await InteractionService.calculateAverageResponseTime(clinicId, monthAgo.toISOString(), now.toISOString());
        }
        catch (err) {
     ...
     */
    static async getConversionFunnel(clinicId, dateFrom, dateTo) {
        let query = supabase
            .from('leads')
            .select('status')
            .eq('clinic_id', clinicId)
     ...
     */
    static async getSourcePerformance(clinicId, dateFrom, dateTo) {
        let query = supabase
            .from('leads')
     ...
     */
    static async getLeadsByDay(clinicId, days = 30) {
     ...
     */
    static async getAgentStats(clinicId, dateFrom, dateTo) {
        let query = supabase
            .from('leads')
     ...
     */
    static async getRevenueMetrics(clinicId, dateFrom, dateTo) {
        let query = supabase
            .from('leads')
        ...
        data.forEach((lead) => {
            ...
            if (lead.status === 'convertido') {
                ...
                metrics.by_source[lead.source] =
                    (metrics.by_source[lead.source] || 0) + value;
            }
