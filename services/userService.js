...
class UserService {
    ...
    async getTherapistAvailability(therapistId, date) {
        try {
            ...
            if (!therapist?.profile_settings || !therapist.profile_settings?.working_hours) {
                return [];
            }
    ...
    calculateAvailableSlots(workingHours, unavailableSlots, date) {
        ...
        for (let hour = startHour; hour < endHour; hour++) {
            ...
            const isUnavailable = unavailableSlots.some(slot => {
                ...
                return ((currentSlotStart >= unavailableStart && currentSlotStart < unavailableEnd) ||
