...
export const deleteAppointmentSeries = async (seriesId, fromDate) => {
    await delay(400);
    db.deleteAppointmentSeries(seriesId, fromDate);
    eventService.emit('appointments:changed');
};
export const listRecurrenceTemplates = async () => {
    await delay(200);
...
export const listWaitlistEntries = async (status = 'waiting') => {
    await delay(200);
