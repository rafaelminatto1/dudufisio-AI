...
export class GamificationService {
     ...
     */
    static async getPatientLevel(patientId) {
        try {
            const { data, error } = await supabase
     ...
     */
    static async getPatientAchievements(patientId) {
        try {
     ...
     */
    static async getAvailableRewards(clinicId) {
        try {
            const { data, error } = await supabase
     ...
     * Resgatar recompensa
     */
    static async redeemReward(patientId, rewardId) {
        try {
            const clinicId = await this.getPatientClinicId(patientId);
            // 1. Verificar saldo
     ...
     */
    static async checkAchievements(patientId) {
        try {
            const clinicId = await this.getPatientClinicId(patientId);
            ...
            for (const achievement of achievements || []) {
                ...
                const shouldUnlock = await this.checkAchievementConditions(patientId, achievement);
                if (shouldUnlock) {
     ...
     */
    static async checkAchievementConditions(patientId, achievement) {
        try {
     ...
     */
    static async unlockAchievement(patientId, achievementId, clinicId) {
        try {
     ...
     */
    static async getPatientClinicId(patientId) {
     ...
     */
    static async onAppointmentCompleted(appointmentId) {
    ...
    static async onExerciseCompleted(patientId, exerciseId) {
        await this.addPoints({
            patient_id: patientId,
    ...
    static async onReviewSubmitted(patientId) {
        await this.addPoints({
            patient_id: patientId,
    ...
    static async onReferralScheduled(referrerId, referredId) {
        await this.addPoints({
 ...
 */
GamificationService.POINTS_RULES = {
    attendance: {
        firstAppointment: 50,
