...
import { RegistrationStatus, ProviderStatus } from '../types';
...
class EventService {
    constructor() {
        ...
        this.eventsData = [...mockEvents];
        this.registrationsData = [...mockEventRegistrations];
    }
    ...
    async saveEvent(eventData, organizerId) {
        await delay(400);
        if (eventData.id) {
    ...
    async checkInParticipant(registrationId, method, checkedInBy) {
        await delay(500);
    ...
    async confirmProvider(providerId) {
        await delay(500);
